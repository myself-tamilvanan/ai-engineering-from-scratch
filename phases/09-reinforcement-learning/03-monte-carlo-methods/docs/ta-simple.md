...
Dynamic programming is elegant, but it assumes you can query `P(s' | s, a)` for every state and action. Almost nothing in the real world works that way. A robot cannot analytically compute the distribution over camera pixels after a joint torque. A pricing algorithm (அல்கோரிதம் — வழிமுறை) cannot integrate over every possible customer reaction. An LLM cannot enumerate all possible continuations after a token.
...
The shift from DP to MC is philosophically important: we move from *known model + exact backup* to *sampled rollouts + averaged return*. The variance jumps, but the applicability explodes. Every RL algorithm (அல்கோரிதம் — வழிமுறை) after this lesson — TD, Q-learning, REINFORCE, PPO, GRPO — is a Monte Carlo estimator at heart, sometimes with bootstrapping layered on top.
...
**Monte Carlo Control.** Evaluate → improve → evaluate, just like policy iteration (மறுயுதல் — மீண்டும் செய்தல்), but evaluation is sampling-based:
...
| Teaching RL | Simplest algorithm (அல்கோரிதம் — வழிமுறை) that actually works — strip bootstrapping to see the core. |

Modern deep-RL algorithm (அல்கோரிதம் — வழிமுறை)s (PPO, SAC) interpolate between pure MC (full returns) and pure TD (one-step bootstrap) via `n`-step returns or GAE. Both endpoints are instances of the same estimator.
...
---
name: mc-evaluator
description: Evaluate a policy via Monte Carlo rollouts and produce a convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) report with DP-comparison if available.
version: 1.0.0
phase: 9
lesson: 3
...
1. **Easy.** Implement first-visit MC evaluation of the uniform-random policy on 4×4 GridWorld. Run 10,000 episodes. Plot `V(0,0)` as a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) of episode count against the DP answer.
2. **Medium.** Implement ε-greedy MC control with `ε ∈ {0.01, 0.1, 0.3}`. Compare mean return after 20,000 episodes. What does the curve look like? Where does the bias-variance choice live?
