...
# Policy gradient (சாய்வு — மாற்றத்தின் दिशा) — REINFORCE from Scratch

> Stop estimating value. control with settings the policy directly, compute the gradient (சாய்வு — மாற்றத்தின் दिशा) of expected return, step uphill. Williams (1992) wrote it in one theorem. It is why PPO, GRPO, and every LLM RL loop exist.

**Type:** Build
**Languages:** Python
**Prerequisites:** Phase 3 · 03 (backpropagation (பின்னாக பரப்புதல் — தவறிலிருந்து கற்றுக்கொள்ளுதல்)), Phase 9 · 03 (Monte Carlo), Phase 9 · 04 (TD Learning)
**Time:** ~75 minutes

## The Problem

Q-learning and DQN control with settings the *value* function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). You pick actions by `argmax Q`. That is fine for discrete actions and discrete states. It breaks when actions are continuous (which `argmax` over a 10-dimension (பரிமாணம் — கோணம்)al torque?) or when you want a stochastic policy (`argmax` is deterministic by construction).

Policy gradient (சாய்வு — மாற்றத்தின் दिशा)s control with settings the *policy* instead. `π_θ(a | s)` is a neural net that outputs a distribution over actions. Sample from it to act. Compute the gradient of expected return with respect to `θ`. Step uphill. No `argmax`. No Bellman recursion. Just gradient ascent on `J(θ) = E_{π_θ}[G]`.

The REINFORCE theorem (Williams 1992) tells you this gradient (சாய்வு — மாற்றத்தின் दिशा) is computable: `∇J(θ) = E_π[ G · ∇_θ log π_θ(a | s) ]`. Run an episode. Compute the return. Multiply by `∇ log π_θ(a | s)` at every step. Average. Gradient-ascent. Done.

Every LLM-RL algorithm (அல்கோரிதம் — வழிமுறை) in 2026 — PPO, DPO, GRPO — is a refinement of REINFORCE. Understanding it in your fingers is the prerequisite for the rest of this phase, and for Phase 10 · 07 (RLHF how to build) and Phase 10 · 08 (DPO).

## The Concept

![Policy gradient (சாய்வு — மாற்றத்தின் दिशा): softmax policy, log-π gradient, return-weighted update](../assets/policy-gradient.svg)

**The policy gradient (சாய்வு — மாற்றத்தின் दिशा) theorem.** For any policy `π_θ` parameter (பாரமீட்டர் — கட்டுப்பாடு)ized by `θ`:
...
**Softmax policy parameter (பாரமீட்டர் — கட்டுப்பாடு)ization.** For discrete actions, the standard choice:

`π_θ(a | s) = exp(f_θ(s, a)) / Σ_{a'} exp(f_θ(s, a'))`

where `f_θ` is any neural net that outputs a score per action. The gradient (சாய்வு — மாற்றத்தின் दिशा) has a clean form:
...
```figure
policy-gradient (சாய்வு — மாற்றத்தின் दिशा)-landscape
```
...
```python
def policy_logits(theta, state_feature (பண்பு — தகவல் நெடுவரிசை)s):
...
Use a linear policy (one weight vector (வெக்டர் — எண் பட்டியல்) per action) for a tabular env. For Atari, swap in a CNN and keep the softmax head.
...
The gradient (சாய்வு — மாற்றத்தின் दिशा) `∇ log π(a|s) = e_a - π(·|s)` (onehot of `a` minus probabilities) is the heart of softmax policy gradients. Burn it into muscle memory.
...
- **Exploding gradient (சாய்வு — மாற்றத்தின் दिशा)s.** Returns is huge. Always normalize `G` to `~N(0, 1)` across the batch (பத்தி — தொகுப்பு) before multiplying by `∇ log π`.
...
In 2026, REINFORCE is rarely run directly but its gradient (சாய்வு — மாற்றத்தின் दिशा) formula is everywhere:
...
Save as `outputs/skill-policy-gradient (சாய்வு — மாற்றத்தின் दिशा)-trainer.md`:

```markdown
---
name: policy-gradient (சாய்வு — மாற்றத்தின் दिशा)-trainer
...
1. Policy head. Softmax (discrete) or Gaussian (continuous) with parameter (பாரமீட்டர் — கட்டுப்பாடு) counts.
2. Baseline. None (vanilla), running mean, learned `V̂(s)`, or A2C critic.
3. Variance controls. Reward-to-go on by default, return normalization, gradient (சாய்வு — மாற்றத்தின் दिशा) clip value.
4. Entropy bonus. Coefficient β and decay schedule.
5. batch (பத்தி — தொகுப்பு) size. Episodes per update. on-policy data freshness contract.
...
2. **Medium.** Add a running-mean baseline. Train again. Compare sample speed and variance to the vanilla run. By how much does the baseline reduce steps to convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்)?
...
|------|-----------------|-----------------------|
| Policy gradient (சாய்வு — மாற்றத்தின் दिशा) | "Train the policy directly" | `∇J(θ) = E[G · ∇ log π_θ(a\|s)]`. derived from the log-derivative trick. |
| REINFORCE | "The original PG algorithm (அல்கோரிதம் — வழிமுறை)" | Williams (1992). Monte Carlo returns multiplied by log-policy gradient. |
| Log-derivative trick | "Score function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) estimator" | `∇P(τ;θ) = P(τ;θ) · ∇ log P(τ;θ)`. makes gradients of expectations tractable. |
...
- [Williams (1992). Simple Statistical gradient (சாய்வு — மாற்றத்தின் दिशा)-Following algorithm (அல்கோரிதம் — வழிமுறை)s for Connectionist Reinforcement Learning](https://link.springer.com/article/10.1007/BF00992696) — the original REINFORCE paper.
- [Sutton et al. (2000). Policy Gradient Methods for Reinforcement Learning with function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) Approximation](https://papers.nips.cc/paper_files/paper/1999/hash/464d828b85b0bed98e80ade0a5c43b0f-Abstract.html) — the modern policy-gradient theorem with function approximation.
