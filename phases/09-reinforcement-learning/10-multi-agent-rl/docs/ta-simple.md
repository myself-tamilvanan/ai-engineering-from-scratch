...
This breaks tabular convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) proofs (Q-learning's guarantee assumes a stationary environment). It breaks naive deep RL too: agents chase each other in loops, never converge to a stable policy. You need multi-agent-specific techniques: centralized training / decentralized execution, counterfactual baselines, league play, self-play.
...
- **MAPPO** / **IPPO** with shared critic (Yu et al. 2022): PPO with a centralized value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). Dominant in 2026 for cooperative MARL.
...
**Communication.** Allow agents to send learned messages `m_i` to each other. Works in cooperative settings. Foerster et al. (2016) showed that differentiable inter-agent communication is trained end-to-end. Today's LLM-based multi-agent systems (Phase 16) essentially communicate in natural language.
...
- **League cycles.** Pure self-play can get stuck in a dominance cycle. Fix: league play with many different opponents.
- **Sample explosion.** `n` agents × state space × joint actions. Approximate with function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) approximation. factored action spaces (one policy output head per agent).
...
In 2026, MARL's biggest growth area is LLM-based: swarms of language-model agents negotiating, debating, building software. The RL shows up as preference optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) on *trajectory-level* outputs, not token-level (Phase 16 · 03).
...
1. Regime classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்). Cooperative / adversarial / general-sum. Justify.
2. algorithm (அல்கோரிதம் — வழிமுறை). IPPO / MAPPO / QMIX / self-play / league. Reason tied to coupling tightness and reward structure.
...
3. **Hard.** Implement a centralized critic for MAPPO-style training and compare convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) speed to independent PPO on the coordination task.
...
| MAPPO | "Multi-agent PPO" | PPO with a centralized value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) conditioned on global state. |
...
- [Foerster et al. (2017). Counterfactual Multi-Agent Policy gradient (சாய்வு — மாற்றத்தின் दिशा)s (COMA)](https://arxiv.org/abs/1705.08926) — counterfactual baselines for credit assignment.
- [Rashid et al. (2018). QMIX: Monotonic Value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) Factorisation](https://arxiv.org/abs/1803.11485) — value decomposition with monotonicity.
...
- [Zhang, Yang & Başar (2021). Multi-Agent Reinforcement Learning: A Selective Overview](https://arxiv.org/abs/1911.10635) — survey covering cooperative, competitive, and mixed MARL with convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) results.
