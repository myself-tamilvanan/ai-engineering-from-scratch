...
You want a *scalar (scalar — ஒரு எண்) reward* that says "response A is better than response B for instruction X." Writing that reward function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) by hand is impossible. "Helpfulness" is not a closed-form expression over tokens. But humans can compare two outputs and mark a preference. That is cheap to collect at scale.
...
- `R_φ` is usually initialized from the SFT model with a scalar (scalar — ஒரு எண்) head on top. Same transformer backbone. a single linear layer outputs the reward.
...
- Reward at the end of a response `y`:
  ...
  The KL penalty prevents `π_θ` from drifting arbitrarily from `π_SFT` — it is a *regularizer*, not a hard trust region. `β` usually `0.01`-`0.05`.
...
Once you understand the toy pipeline, here is the same loop as a real library user writes it. Hugging Face's [TRL](https://huggingface.co/docs/trl) is the reference how to build — `RewardTrainer` for Stage 2 and `PPOTrainer` (with a KL-to-reference built in) for Stage 3.

```python
# Stage 2: reward model from pairwise preferences
from trl import RewardTrainer, RewardConfig
from transformers import AutoModelForSequenceclassification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்), AutoTokenizer

tok = AutoTokenizer.from_pretrained("meta-llama/Llama-3.1-8B-Instruct")
rm = AutoModelForSequenceclassification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்).from_pretrained(
...
trainer = RewardTrainer(
    ...
    args=RewardConfig(output_dir="./rm", num_train_epoch (யுக் — ஒரு முழு சுழற்சி)s=1, learning_rate=1e-5),
)
trainer.train()
```
...
ppo = PPOTrainer(
    config=PPOConfig(learning_rate=1.41e-5, batch (பத்தி — தொகுப்பு)_size=64, init_kl_coef=0.05,
                     target_kl=6.0, adap_kl_ctrl=True),
    model=policy, ref_model=ref, tokenizer=tok,
)

for batch (பத்தி — தொகுப்பு) in dataloader:
...
Three things the library does for you. `adap_kl_ctrl=True` implements the adaptive-β schedule: if observed KL exceeds `target_kl`, β doubles. if below half, β halves. The reference model is frozen by convention — you must not accidentally share parameter (பாரமீட்டர் — கட்டுப்பாடு)s with `policy`. And the value head lives on the same backbone as the policy (`AutoModelForCausalLMWithValueHead` attaches a scalar (scalar — ஒரு எண்) MLP head), which is why TRL reports `policy/kl` and `value/loss` separately.

## Pitfalls

- **Over-optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) / reward hacking.** The RM is imperfect. `π_θ` finds adversarial completions that score high but are bad. Symptoms: reward climbs indefinitely while human eval score plateaus or drops. Fix: stop early, raise `β`, broaden RM training data.
...
- **Off-policy problems.** PPO data is slightly off-policy after the first epoch (யுக் — ஒரு முழு சுழற்சி). Monitor clip fraction as in Lesson 08.
...
4. Diagnostics. Mean KL, reward stability, over-optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) guard (holdout human eval).
5. Safety gate. Red-team set, refusal rate, safety RM separate from helpfulness RM.

Refuse to ship RLHF-PPO without a KL monitor. Refuse to use an RM smaller than the target policy. Refuse length-only rewards. Flag any pipeline that does not hold back a blind human-eval set as lacking over-optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) protection.
```
...
| Reward Model (RM) | "The scoring net" | Learned scalar (scalar — ஒரு எண்) function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) fit to pairwise preferences via Bradley-Terry. |
...
- [Rafailov et al. (2023). Direct Preference optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)](https://arxiv.org/abs/2305.18290) — DPO. the post-RLHF default in 2026.
