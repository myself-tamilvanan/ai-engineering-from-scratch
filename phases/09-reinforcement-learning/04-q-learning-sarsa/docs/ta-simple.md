...
This is the pivot on which all of modern RL — DQN, A2C, PPO, SAC — turns. The rest of Phase 9 is layers of function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) approximation and tricks built on top of the one-step TD update you will write in this lesson.
...
The bracketed quantity is the TD error `δ = r + γ V(s') - V(s)`. It is the online analogue of `G_t - V(s_t)` in MC. convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) requires `α` satisfying Robbins-Monro (`Σ α = ∞`, `Σ α² < ∞`) and all states visited infinitely often.
...
Run value iteration (மறுயுதல் — மீண்டும் செய்தல்) (Lesson 02) to get `Q*`. Check `max_{s,a} |Q_learned(s,a) - Q*(s,a)|`. A healthy tabular TD agent lands within `~0.5` on the 4×4 GridWorld after 10,000 episodes.
...
- **α schedule.** Constant `α` is fine for non-stationary problems. Decaying `α_n = 1/n` gives convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) in theory but is too slow in practice — pin `α` in `[0.05, 0.3]` and monitor the learning curve.
...
- **State hashing.** If states are tuples/tensor (டென்சர் — பல வடிவ எண் வரிசை)s, use a hashable key (tuple, not list. tuple of floats rounded, not raw).
...
| High-dimension (பரிமாணம் — கோணம்)al state | DQN (Phase 9 · 05) | Neural-net Q-function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) with replay and target net. |
...
---
name: td-agent
description: Pick between Q-learning, SARSA, Expected SARSA for a tabular or small-feature (பண்பு — தகவல் நெடுவரிசை) RL task.
version: 1.0.0
phase: 9
lesson: 4
...
Given a tabular or small-feature (பண்பு — தகவல் நெடுவரிசை) environment, output:

1. algorithm (அல்கோரிதம் — வழிமுறை). Q-learning / SARSA / Expected SARSA / n-step variant. One-sentence reason tied to on-policy vs off-policy and variance.
2. Hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s. α, γ, ε, decay schedule.
3. Initialization. Q_0 value (optimistic vs zero) and justification.
4. convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) diagnostic. Target learning curve, `|Q - Q*|` check if DP is possible.
5. Deployment warning. How will exploration behave at inference? Is SARSA's conservatism needed?

Refuse to apply tabular TD to state spaces > 10⁶. Refuse to ship a Q-learning agent without a max-bias warning. Flag any agent trained with ε held at 1.0 throughout (no exploitation phase).
```
...
| GLIE | "Correct exploration schedule" | Greedy in the Limit with Infinite Exploration. needed for Q-learning convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்). |
...
- [Watkins & Dayan (1992). Q-learning](https://link.springer.com/article/10.1007/BF00992698) — the original paper and convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) proof.
