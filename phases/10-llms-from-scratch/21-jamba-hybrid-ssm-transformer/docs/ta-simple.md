...
At each step the state evolves via a linear dynamics `A`, takes input `B x_t`, and emits output `C h_t`. `A, B, C` is learned. Note the critical property: computing `y_t` needs only `h_{t-1}` and `x_t`, not any earlier `x`. Memory is constant. Inference is O(1) per token.

The trick for modeling quality is the structure of `A`. S4 (Gu 2021) used a highly structured matrix (மேட்ரிக்ஸ் — எண் மேஜை) that could be evaluated efficiently as a long convolution during training. Mamba (Gu, Dao 2023) replaced the fixed `A, B, C` with data-dependent ones (the "selective" part). Mamba-2 (2024) further simplified the structure. Mamba-3 (2026) re-adds complexity in specific places.
...
AI21 ran ablations: what ratio of attention-to-Mamba gives the best perplexity-per-parameter (பாரமீட்டர் — கட்டுப்பாடு) AND in-context recall on their long-context evals?
...
The simple idea: the Transformer layers handle exact recall and state tracking. The Mamba layers handle the cheap bulk of processing.
...
- SSM state: `28 * hidden * state_size` per token prefix, but this is a fixed-size per layer, not scaling with sequence length. Typical Mamba state is 16 per feature (பண்பு — தகவல் நெடுவரிசை), hidden 4096: `28 * 4096 * 16 * 2 = 3.7 MB` total.
...
1. **Exponential-trapezoidal discretization.** Replaces the Euler-method discretization in Mamba-2 with a more expressive recurrence. Convolution-like operation applied on the state-input within the core recurrence, quite than as an outer convolution on `x_t`.

2. **Complex-valued state update.** Previous Mambas reduced the state matrix (மேட்ரிக்ஸ் — எண் மேஜை) from complex (S4) to real diagonal (Mamba) to scaled identity (Mamba-2). Mamba-3 re-adds complex values — equivalent to a data-dependent rotary embedding (embedding — பொருளை எண்களாக மாற்றுதல்) on the state. This restores state-tracking capabilities that previous real-valued simplifications cost.

3. **Multi-input multi-output (MIMO) projections.** Instead of per-feature (பண்பு — தகவல் நெடுவரிசை) scalar (scalar — ஒரு எண்) projections, use matrix (மேட்ரிக்ஸ் — எண் மேஜை)-valued projections. Improves modeling power and inference-time hardware utilization without increasing decode latency.

At 1.5B parameter (பாரமீட்டர் — கட்டுப்பாடு)s, Mamba-3 improves average downstream accuracy by 0.6 points over Gated DeltaNet. the MIMO variant adds 1.2 more for a total 1.8-point gain. At the same state size, Mamba-3 matches Mamba-2 with half the state.
...
- Context is short (under 16k). The SSM extra work is wasted. pure Transformer is fine.
...
- You are scaling to trillion-parameter (பாரமீட்டர் — கட்டுப்பாடு) frontier models. Pure-Transformer + MLA + MoE (DeepSeek-V3 style) is currently winning the capability race.
...
3. Read Section 3 of the Jamba paper (arXiv:2403.19887). Explain why AI21 uses Mamba-1 quite than Mamba-2 despite Mamba-2 being faster. Hint: the hybrid ablation section documents this.

4. Compute the parameter (பாரமீட்டர் — கட்டுப்பாடு) extra work of MoE-every-other-layer in Jamba 1.5 Large (398B total, 94B active). Compare the active ratio to DeepSeek-V3 (37B/671B) and explain why Jamba's architecture pushes the active ratio higher.

5. Read Section 3 of the Mamba-3 paper (arXiv:2603.15569). Explain in three sentences why a complex-valued state update is equivalent to a data-dependent rotary embedding (embedding — பொருளை எண்களாக மாற்றுதல்). Tie the answer to Phase 7 · Lesson 04's RoPE derivation.
...
| Selective SSM | "Mamba's trick" | Data-dependent A, B, C parameter (பாரமீட்டர் — கட்டுப்பாடு)s that give the model gating-like selectivity at linear time |
...
| MIMO | "Multi-input multi-output" | Mamba-3 innovation using matrix (மேட்ரிக்ஸ் — எண் மேஜை)-valued projections instead of scalar (scalar — ஒரு எண்) per-feature (பண்பு — தகவல் நெடுவரிசை) |
