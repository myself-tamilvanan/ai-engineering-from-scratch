...
- Compute parameter (பாரமீட்டர் — கட்டுப்பாடு) count, KV cache size, and activation memory for any open model from its config alone
- Pick the right open model for a deployment target given latency, memory, and capability constraints

## The Problem

In Lesson 04 you wrote 350 lines of numpy and had a GPT-2-shaped model. Llama 3 405B has a 200-page technical report. Your instinct is that these are different beasts. They are not. The 200 pages describe the same object with five or six well-motivated modifications, plus a thousand how to build details about scaling. The skeleton -- embedding (embedding — பொருளை எண்களாக மாற்றுதல்), transformer blocks, attention, MLP, norm, head -- is unchanged.
...
- Token embedding (embedding — பொருளை எண்களாக மாற்றுதல்) matrix (மேட்ரிக்ஸ் — எண் மேஜை) (vocab_size x hidden_dim).
...
Everything else (learning rate schedule, data mix, batch (பத்தி — தொகுப்பு) size, context length) lives in the training config, not the architecture. Six knobs.
...
Learned position embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s were a 1024-slot lookup table in GPT-2. Context 1025 is off the end of the table. Models cannot extrapolate beyond their training length.

Rotary Position embedding (embedding — பொருளை எண்களாக மாற்றுதல்) (RoPE, Su et al. 2021) injects position by rotating each Q and K vector (வெக்டர் — எண் பட்டியல்) in pairs before the attention dot product. The angle of rotation is a deterministic function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) of position, so there is nothing learned and nothing to run out of. With scaling tricks (NTK-aware interpolation, YaRN), a model trained on 8k context can stretch to 128k at inference with modest accuracy loss.
...
For any model above ~13B parameter (பாரமீட்டர் — கட்டுப்பாடு)s, GQA or MLA is effectively mandatory. Full MHA at scale is a KV cache disaster.

### Knob 5: Mixture of Experts

A dense MLP activates all its parameter (பாரமீட்டர் — கட்டுப்பாடு)s for every token. An MoE MLP has K experts per block and a router that picks the top-k experts per token (usually top-2). Only those experts' weights see a forward pass for that token.
...
The appeal: you can have 64 experts of size 7B each (so total param count is huge) while only running 2 of them per token (so per-token compute matches a dense 7B model). Mixtral 8x7B has 47B total parameter (பாரமீட்டர் — கட்டுப்பாடு)s but activates only 13B per token. DeepSeek-V3 has 671B total parameters but activates only 37B per token.
...
Pros: same compute, more parameter (பாரமீட்டர் — கட்டுப்பாடு)s, better capacity. Cons: the expert memory still has to live somewhere (so serving needs more VRAM than a dense equivalent), load-balancing the router is hard, and fine-tuning the router during alignment is its own research area.
...
```
{
  ...
  "max_position_embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s": 131072,
  "rope_theta": 500000.0,
  "rms_norm_eps": 1e-5,
  "vocab_size": 128256
}
...
- `hidden_size`: embedding (embedding — பொருளை எண்களாக மாற்றுதல்) dimension (பரிமாணம் — கோணம்).
...
From these alone you compute total parameter (பாரமீட்டர் — கட்டுப்பாடு)s, KV cache, and peak activation memory. See `code/main.py` for the exact formulas.

### Activation memory budget

Activations dominate training memory above a few billion parameter (பாரமீட்டர் — கட்டுப்பாடு)s. The rule of thumb for pre-training (with gradient (சாய்வு — மாற்றத்தின் दिशा) checkpointing):

```
activation_mem ~ batch (பத்தி — தொகுப்பு)_size * seq_len * hidden_size * num_layers * bytes_per_element
```

For Llama 3 8B at batch (பத்தி — தொகுப்பு) 1, seq 8192, BF16, 32 layers, hidden 4096: roughly 8 GB just for activations with checkpointing, 40 GB without. This is why flash-attention and ring-attention matter -- they rewrite the attention calculation so activations fit.
...
The lesson's code is a calculator. Given any config.json, it prints parameter (பாரமீட்டர் — கட்டுப்பாடு) count by component, KV cache at max context, SwiGLU MLP ratio, and a short verdict on the architecture (dense / GQA / MLA / MoE).
...
The script walks the architecture field by field, computes param counts for embedding (embedding — பொருளை எண்களாக மாற்றுதல்), attention (with GQA reduction), MLP (with SwiGLU expansion), layernorms, and the head. It then computes the KV cache at the stated context length and prints a summary.

See `code/main.py` for the how to build.

## Use It

Run the calculator on Llama 3 8B, Mistral 7B, Mixtral 8x7B, and DeepSeek V3 configs bundled in the script. Compare the parameter (பாரமீட்டர் — கட்டுப்பாடு) breakdowns. Notice that the MoE models have a total param count that dwarfs the dense models but an active param count that is often smaller. Notice that DeepSeek V3's KV cache is smaller than Llama 3 405B's despite having more total parameters -- that is MLA in action.
...
1. Read the Qwen 2.5 72B config from HuggingFace. Compute total parameter (பாரமீட்டர் — கட்டுப்பாடு)s from scratch. Compare to the HF-reported value and identify where any delta comes from (head dim rounding, KV sharing factor, etc.).
...
| RoPE | "Rotary positions" | Rotate each Q and K vector (வெக்டர் — எண் பட்டியல்) in 2D pairs by an angle that depends on position — extrapolates beyond training length with scaling tricks |
...
| Active params | "What runs per token" | For MoE models, the parameter (பாரமீட்டர் — கட்டுப்பாடு) count that sees a forward pass per token (much smaller than total params) — governs per-token FLOPs |
...
- [Su et al., 2021 -- "RoFormer: Enhanced Transformer with Rotary Position embedding (embedding — பொருளை எண்களாக மாற்றுதல்)"](https://arxiv.org/abs/2104.09864) -- the RoPE paper
