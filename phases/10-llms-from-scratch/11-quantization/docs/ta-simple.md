...
- Implement symmetric and asymmetric quantization from FP16 to INT8 and INT4, including per-tensor (டென்சர் — பல வடிவ எண் வரிசை) and per-channel scaling
...
- Apply GPTQ or AWQ to quantize a real model and measure the accuracy-memory choice on a benchmark

## The Problem

Llama 3 70B has 70 billion parameter (பாரமீட்டர் — கட்டுப்பாடு)s. Each parameter is a 16-bit floating point number. That is 140 billion bytes. 140GB. A single A100 has 80GB of VRAM. You cannot even load the weights, let alone run inference, on a single GPU. You need two A100s at $2/hour each just to serve one model.

But 16 bits per parameter (பாரமீட்டர் — கட்டுப்பாடு) is wasteful. Most weights in a neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு) cluster near zero. The full dynamic range of FP16 (from 0.000000059 to 65,504) is almost entirely unused. If you measure the actual distribution of weights in Llama 3 70B, 95% of them fall between -0.1 and +0.1. You are burning 16 bits to represent values that could fit in 4.
...
Every floating-point number has three parts: sign, exponent, and mantissa (also called significand). The sign is one bit. The exponent determines the range (how large or small the number is). The mantissa determines the precision (how many decimal places you get).
...
**FP32** is full precision. 23 mantissa bits give you about 7 decimal digits of precision. Range: roughly 1.2 x 10^-38 to 3.4 x 10^38. Training used to happen exclusively in FP32. It still does for accumulation (running sums during matrix (மேட்ரிக்ஸ் — எண் மேஜை) multiplication).

**FP16** halves the bits. 10 mantissa bits give about 3.3 decimal digits. The exponent shrinks to 5 bits, reducing the range dramatically (max value ~65,504). This is fine for weights (which cluster near zero) but dangerous for activations and gradient (சாய்வு — மாற்றத்தின் दिशा)s that can spike during training. FP16 training requires loss scaling to prevent underflow.

**BF16** (Brain Float 16) keeps the 8-bit exponent from FP32 but shrinks the mantissa to 7 bits. Same range as FP32, less precision than FP16. Google designed it specifically for deep learning (ஆழ் கற்றல் — பல்லாய கொட்ட பயிற்சி). The simple idea: range matters more than precision for neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)s. A gradient (சாய்வு — மாற்றத்தின் दिशा) of 10^-20 that underflows to zero in FP16 survives in BF16. A weight of 0.07342 that rounds to 0.0734 in BF16 is close enough. Every modern training run uses BF16 or a BF16/FP32 mix.

**FP8** comes in two flavors. E4M3 (4 exponent, 3 mantissa) helps for weights and activations during inference. E5M2 (5 exponent, 2 mantissa) helps for gradient (சாய்வு — மாற்றத்தின் दिशा)s during training where range matters more than precision. FP8 inference on H100 GPUs achieves 30-50% speedup over FP16 with negligible quality loss.

**INT8** is an integer format. No exponent, no mantissa. Just 256 evenly spaced values from -128 to 127. You need a scale factor to map floating-point weights into this range. The advantage: integer arithmetic is faster and more power-efficient than floating-point. INT8 matrix (மேட்ரிக்ஸ் — எண் மேஜை) multiplication on an A100 runs at 624 TOPS versus 312 TFLOPS for FP16.
...
The core operation is simple. Take a tensor (டென்சர் — பல வடிவ எண் வரிசை) of floating-point values, find a scale factor, multiply, round to the nearest integer, and store the integers plus the scale factor.

**Quantize:**
```
scale = max(abs(tensor (டென்சர் — பல வடிவ எண் வரிசை))) / max_int_value
...
```
scale = max(abs(tensor (டென்சர் — பல வடிவ எண் வரிசை))) / 127
quantized = clamp(round(tensor / scale), -128, 127)
```

The error is the rounding error. Each value is off by at most `scale / 2`. The total error across a layer depends on how many weights you have and how sensitive the model is to perturbations in those weights.

**Per-tensor (டென்சர் — பல வடிவ எண் வரிசை) vs per-channel quantization.** Per-tensor uses one scale factor for the entire weight matrix (மேட்ரிக்ஸ் — எண் மேஜை). Simple but lossy: if one column has large values and another has small values, the small values lose most of their precision. Per-channel uses one scale factor per output channel (per row or column of the weight matrix). More extra work (you store N scale factors instead of 1) but dramatically better quality. Every production quantization method uses per-channel or finer granularity.

**Asymmetric quantization** adds a zero-point offset: `quantized = round(tensor (டென்சர் — பல வடிவ எண் வரிசை) / scale) + zero_point`. This handles distributions that are not centered at zero. ReLU activations, for example, are always non-negative. Symmetric quantization wastes half the integer range on negative values that never appear. Asymmetric quantization maps the actual range [min, max] to the full integer range.
...
**Attention logits (most sensitive).** The softmax in attention is highly sensitive to small changes in its inputs. A quantization error of 0.01 in a pre-softmax logit can shift the attention distribution meaningfully. Most quantization schemes keep attention calculation in higher precision (FP16 or BF16) even when everything else is quantized.
...
| When to use | Deployment, iteration (மறுயுதல் — மீண்டும் செய்தல்) | Maximum quality at low bit-width |
...
The pattern: FP8 is nearly free. INT4 costs 1-2 MMLU points but doubles throughput and quarters memory. The choice is worth it for almost every deployment.
...
FP16 to INT8 (LLM.int8()): 2x memory reduction, < 0.5% quality loss. The mixed-precision approach keeps outlier feature (பண்பு — தகவல் நெடுவரிசை)s in FP16 while quantizing everything else to INT8.
...
### Step 2: Symmetric Quantization (Per-tensor (டென்சர் — பல வடிவ எண் வரிசை) and Per-Channel)

The basic quantization operations. Per-tensor (டென்சர் — பல வடிவ எண் வரிசை) uses one scale for the whole matrix (மேட்ரிக்ஸ் — எண் மேஜை). Per-channel uses one scale per row or column.

```python
def quantize_symmetric(tensor (டென்சர் — பல வடிவ எண் வரிசை), num_bits=8):
    qmin = -(2 ** (num_bits - 1))
    qmax = 2 ** (num_bits - 1) - 1
...
def quantize_per_channel(tensor (டென்சர் — பல வடிவ எண் வரிசை), num_bits=8, axis=0):
    qmin = -(2 ** (num_bits - 1))
    qmax = 2 ** (num_bits - 1) - 1

    if axis == 0:
        abs_max = np.max(np.abs(tensor (டென்சர் — பல வடிவ எண் வரிசை)), axis=1, keepdims=True)
    else:
        abs_max = np.max(np.abs(tensor), axis=0, keepdims=True)

    abs_max = np.where(abs_max == 0, 1.0, abs_max)
    scales = abs_max / qmax
    quantized = np.clip(np.round(tensor (டென்சர் — பல வடிவ எண் வரிசை) / scales), qmin, qmax).astype(np.int32)
...
def quantize_asymmetric(tensor (டென்சர் — பல வடிவ எண் வரிசை), num_bits=8):
    qmin = 0
    qmax = 2 ** num_bits - 1
    t_min = np.min(tensor)
    t_max = np.max(tensor)
...
Measure how much information quantization destroys. Mean squared error, signal-to-noise ratio, and cosine similarity between original and reconstructed tensor (டென்சர் — பல வடிவ எண் வரிசை)s.
...
def compare_quantization_methods(tensor (டென்சர் — பல வடிவ எண் வரிசை), num_bits=8):
    q_pt, s_pt = quantize_symmetric(tensor, num_bits)
    recon_pt = dequantize_symmetric(q_pt, s_pt)
    err_pt = quantization_error(tensor, recon_pt)

    q_pc, s_pc = quantize_per_channel(tensor (டென்சர் — பல வடிவ எண் வரிசை), num_bits, axis=0)
    recon_pc = dequantize_per_channel(q_pc, s_pc, axis=0)
    err_pc = quantization_error(tensor, recon_pc)

    q_asym, s_asym, zp = quantize_asymmetric(tensor (டென்சர் — பல வடிவ எண் வரிசை), num_bits)
    recon_asym = dequantize_asymmetric(q_asym, s_asym, zp)
    err_asym = quantization_error(tensor, recon_asym)

    print(f"\n  Quantization Comparison ({num_bits}-bit, tensor (டென்சர் — பல வடிவ எண் வரிசை) shape {tensor.shape}):")
    ...
    return {"per_tensor (டென்சர் — பல வடிவ எண் வரிசை)": err_pt, "per_channel": err_pc, "asymmetric": err_asym}
```

### Step 4: Bit-Width Sweep

Quantize the same tensor (டென்சர் — பல வடிவ எண் வரிசை) at different bit widths (2, 3, 4, 8, 16) and measure quality at each level. This shows exactly where the quality cliff is.

```python
def bit_width_sweep(tensor (டென்சர் — பல வடிவ எண் வரிசை)):
    ...
    for bits in [2, 3, 4, 8, 16]:
        q, s = quantize_per_channel(tensor (டென்சர் — பல வடிவ எண் வரிசை), bits, axis=0)
...
Simulate quantizing different parts of a transformer and measure which components are most sensitive. This shows the sensitivity hierarchy: weights < activations < KV cache < attention.
...
def sensitivity_experiment(batch (பத்தி — தொகுப்பு)_size=2, seq_len=16, d_model=64, num_bits=8):
    np.random.seed(42)
    ...
    quant_attn_out = dequantize_per_channel(q_act, s_act, axis=0).reshape(batch (பத்தி — தொகுப்பு)_size, seq_len, d_model)
    act_quant_output = quant_attn_out @ weights["out"]
    experiments["Activations only"] = quantization_error(baseline_output, act_quant_output)

    q_k, s_k = quantize_per_channel(fresh_internals["k"].reshape(-1, d_model), num_bits, axis=0)
    q_v, s_v = quantize_per_channel(fresh_internals["v"].reshape(-1, d_model), num_bits, axis=0)
    quant_k = dequantize_per_channel(q_k, s_k, axis=0).reshape(batch (பத்தி — தொகுப்பு)_size, seq_len, d_model)
...
```python
def simulated_gptq(weight_matrix (மேட்ரிக்ஸ் — எண் மேஜை), calibration_inputs, num_bits=4):
    ...
    quantized = np.zeros_like(weight_matrix (மேட்ரிக்ஸ் — எண் மேஜை), dtype=np.int32)
    scales = np.zeros(n_out)
    errors = np.zeros(n_out)

    W = weight_matrix (மேட்ரிக்ஸ் — எண் மேஜை).copy()
...
```python
def simulated_awq(weight_matrix (மேட்ரிக்ஸ் — எண் மேஜை), calibration_inputs, num_bits=4, salient_fraction=0.01):
    n_in, n_out = weight_matrix.shape
    qmin = -(2 ** (num_bits - 1))
    qmax = 2 ** (num_bits - 1) - 1
    ...
    for idx in salient_indices:
        col_max = np.max(np.abs(weight_matrix (மேட்ரிக்ஸ் — எண் மேஜை)[idx, :]))
        if col_max > 0:
            scale_factors[idx] = min(4.0, 1.0 / (col_max + 1e-8) * np.mean(np.abs(weight_matrix)))

    scaled_weights = weight_matrix (மேட்ரிக்ஸ் — எண் மேஜை) * scale_factors.reshape(-1, 1)
    ...
    err = quantization_error(weight_matrix (மேட்ரிக்ஸ் — எண் மேஜை), result)
...
Wire everything together. Compare naive quantization, per-channel, GPTQ, and AWQ on the same weight matrix (மேட்ரிக்ஸ் — எண் மேஜை).
...
def full_quantization_comparison(d_in=256, d_out=512, num_bits=4, n_calibration=32):
    ...
    print(f"\n  Full Quantization Comparison ({num_bits}-bit, {d_in}x{d_out} matrix (மேட்ரிக்ஸ் — எண் மேஜை))")
    print(f"  Matrix has {len(outlier_rows)} outlier rows (10x scale)")
    print()
    print(f"  {'Method':<20} {'MSE':>14} {'SNR (dB)':>10} {'Cosine Sim':>12}")
    print(f"  {'-'*58}")
    print(f"  {'Naive per-tensor (டென்சர் — பல வடிவ எண் வரிசை)':<20} {err_naive['mse']:>14.8f} {err_naive['snr_db']:>10.2f} {err_naive['cosine_similarity']:>12.8f}")
...
if __name__ == "__main__":
    ...
    print("-" * 50)
    weight_matrix (மேட்ரிக்ஸ் — எண் மேஜை) = np.random.randn(128, 256) * 0.02
    weight_matrix[0] *= 15
    weight_matrix[42] *= 8
    ...
    print("\n\nSTEP 4: Bit-Width Sweep")
    print("-" * 50)
    sweep_tensor (டென்சர் — பல வடிவ எண் வரிசை) = np.random.randn(64, 128) * 0.05
...
#
# calibration = [tokenizer(t, return_tensor (டென்சர் — பல வடிவ எண் வரிசை)s="pt") for t in calibration_texts[:128]]
...
vLLM natively supports AWQ and GPTQ models. It handles the dequantization during matrix (மேட்ரிக்ஸ் — எண் மேஜை) multiplication and uses paged attention for the KV cache. For FP8 on H100, add `--dtype float8_e4m3fn`.

## Ship It

This lesson produces `outputs/skill-quantization.md`, a decision framework for choosing the right quantization strategy. Given your model size, target hardware, and quality requirements, it tells you which format, method, and validation steps to use. It includes memory budget calculations, per-component precision recommendations, and deployment recipes for vLLM, llama.cpp, and tensor (டென்சர் — பல வடிவ எண் வரிசை)RT-LLM.

## Exercises

1. Implement group quantization. Instead of one scale per channel, use one scale per group of 128 weights within a channel. This is what GPTQ and AWQ actually use. Compare group sizes of 32, 64, 128, and 256 on the same weight matrix (மேட்ரிக்ஸ் — எண் மேஜை). Smaller groups give better quality but more storage extra work for scale factors.
...
3. Implement the straight-through estimator (STE) for quantization-aware training. Insert fake quantize/dequantize operations in the forward pass of a simple two-layer network trained on a regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) task. Compare final loss between a model trained normally (then PTQ to INT4) versus a model trained with QAT from the start.
...
| Per-channel quantization | "One scale per row" | Uses a separate scale factor for each output channel instead of one for the whole tensor (டென்சர் — பல வடிவ எண் வரிசை), dramatically reduces error |
...
- [Dettmers et al., 2022 -- "LLM.int8(): 8-bit matrix (மேட்ரிக்ஸ் — எண் மேஜை) Multiplication for Transformers at Scale"](https://arxiv.org/abs/2208.07339) -- mixed-precision INT8 that keeps outlier feature (பண்பு — தகவல் நெடுவரிசை)s in FP16, enabling INT8 inference without quality loss
...
- [Micikevicius et al., 2022 -- "FP8 Formats for deep learning (ஆழ் கற்றல் — பல்லாய கொட்ட பயிற்சி)"](https://arxiv.org/abs/2209.05433) -- the NVIDIA/ARM/Intel paper defining E4M3 and E5M2 formats now native on H100
