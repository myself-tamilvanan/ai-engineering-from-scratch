...
- Derive the GRPO objective (DeepSeek-R1's group-relative policy optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)) and contrast it with PPO's value-function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) baseline
...
where `A` is the advantage, usually estimated with GAE using a learned value network `V(s)`. The value network is a second model the same size as the policy. It doubles memory and introduces its own training loop.

GRPO throws out the value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). For each prompt, it samples a group of G responses (usually G=16 or 64). The reward for each response is computed, then normalized within the group:

```
A_i = (r_i - mean(r_1, ..., r_G)) / std(r_1, ..., r_G)
```

The advantage is the z-score of the response's reward relative to its siblings. No value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). The group acts as its own baseline.
...
The revise function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) is a stand-in. With a real LLM it would be a second prompt: "Given the critique, rewrite the response."
...
If every sample in the group has the same reward, the advantage is zero and no gradient (சாய்வு — மாற்றத்தின் दिशा) signal flows. This is a feature (பண்பு — தகவல் நெடுவரிசை). It tells you the prompt is either trivially solved or impossibly hard for the current policy, and the step should skip it.

### Step 5: GRPO Update

One step, symbolic gradient (சாய்வு — மாற்றத்தின் दिशा). In production this would be a torch autograd pass. Here we show the update rule directly.
...
This is PPO's clipped surrogate with one change: the advantages came from group-relative z-scores, not from a value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). No V(s) to train. No GAE. The group is the baseline.
...
4. Add entropy regularization to the GRPO objective. The term `-alpha * entropy(policy)` with alpha=0.01 encourages many different sampling. Measure whether it delays mode collapse across 5 rounds of self-improvement.
...
| GRPO | "PPO without a value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)" | Group-Relative Policy optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) -- sample G responses per prompt, use z-scored group rewards as advantages |
| ORM | "Reward the answer" | Outcome Reward Model -- a single scalar (scalar — ஒரு எண்) reward on the final answer only |
...
| Mode collapse | "The model stopped being many different" | Post-training policy concentrates on a narrow region of the response space. measured as falling reward std across a group |
