...
# Inference optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)

> Two phases define LLM inference. Prefill processes your prompt in parallel -- compute-bound. Decode generates tokens one at a time -- memory-bound. Every optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) targets one or both.
...
- Implement KV-cache to eliminate redundant calculation during autoregressive token generation
...
- Implement continuous batch (பத்தி — தொகுப்பு)ing and PagedAttention concepts to maximize GPU utilization under concurrent requests
- Compare inference optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) techniques (KV-cache, speculative decoding, flash attention) and their throughput/latency tradeoffs
...
The model itself does not change between 1 user and 100 users. Same weights, same architecture, same math. What changes is how you schedule the work. Naive inference wastes 90%+ of available GPU compute. A user waiting for token 47 holds an entire batch (பத்தி — தொகுப்பு) slot open while the GPU memory bus sits idle between matmuls. Meanwhile, a new user's 2,000-token prompt could fill that dead time with useful compute.

This is not a scaling problem. It is a scheduling problem. The techniques in this lesson -- KV caching, continuous batch (பத்தி — தொகுப்பு)ing, PagedAttention, speculative decoding, prefix caching -- are what separate a $25k/month inference bill from a $5k/month one serving the same traffic.

vLLM serving Llama 3 70B on 4xA100-80GB achieves ~50 tokens/second/user at low concurrency, and sustains 15-25 TPS/user at 100 concurrent requests through continuous batch (பத்தி — தொகுப்பு)ing and PagedAttention. Without these optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)s, the same hardware serves 5 TPS/user at that concurrency. Same GPUs, same model, 4x the throughput.
...
**Prefill** processes the entire input prompt. All tokens are known, so attention is computed in parallel across the full sequence. This is a large matrix (மேட்ரிக்ஸ் — எண் மேஜை) multiplication -- GPU cores stay busy. The bottleneck is compute: how many FLOPS your hardware can deliver per second. An A100 does 312 TFLOPS (BF16). Prefill for a 4,096-token prompt on a 70B model takes ~400ms on a single A100.

**Decode** generates output tokens one at a time. Each new token attends to all previous tokens, but only one token is produced per forward pass. The weight matrices are the same size as during prefill, but you are multiplying them by a single vector (வெக்டர் — எண் பட்டியல்) instead of a matrix (மேட்ரிக்ஸ் — எண் மேஜை). The GPU cores finish in microseconds, then wait for the next batch (பத்தி — தொகுப்பு) of weights to arrive from memory. The bottleneck is memory bandwidth: how fast you can stream model weights from HBM to the compute units. An A100 has 2 TB/s bandwidth. A 70B model in FP16 is 140 GB. Reading the full model once takes 70ms -- that is your floor for a single decode step.
...
The **ops:byte ratio** (also called arithmetic intensity) captures this choice. It measures how many operations you perform per byte loaded from memory.

```
ops:byte ratio = FLOPs per token / bytes read from memory
```

During prefill with a batch (பத்தி — தொகுப்பு) of 4,096 tokens, you perform ~4,096 multiply-accumulate operations per weight loaded. The ratio is high -- you are compute-bound. During decode with batch size 1, you perform ~1 operation per weight loaded. The ratio is low -- you are memory-bound.

The basic insight: *decode is memory-bound because you read the entire model to produce a single token*. Every optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) below either reduces what you read, increases the batch (பத்தி — தொகுப்பு) of tokens processed per read, or avoids reads entirely.

### KV Cache

During attention, each token's query attends to every previous token's key and value vector (வெக்டர் — எண் பட்டியல்)s. Without caching, generating token N requires recomputing the key and value projections for all N-1 preceding tokens. Token 1 gets projected when generating token 2, then again for token 3, then again for token 4. By token 1,000, you have projected token 1 a total of 999 times.
...
A single 128K-context conversation for Llama 3 70B consumes 40 GB of KV cache -- half an A100's memory. With 100 concurrent users at 4K tokens each, KV cache alone requires 128 GB. This is why KV cache management is the central challenge of inference optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்).

### Continuous batch (பத்தி — தொகுப்பு)ing

Static batch (பத்தி — தொகுப்பு)ing waits until a batch of N requests arrives, processes them together, and waits until *all* finish before accepting new requests. If one request needs 500 tokens and another needs 10, the short request sits idle for 490 decode steps after it finishes.

Continuous batch (பத்தி — தொகுப்பு)ing (also called iteration (மறுயுதல் — மீண்டும் செய்தல்)-level batching) inserts new requests into the batch as soon as any request completes. The batch is reevaluated at every decode step. A request that finishes after 10 tokens is immediately replaced by a waiting request.
...
sequenceDiagram
    ...
    Note over GPU: Static batch (பத்தி — தொகுப்பு)ing
    ...
    Note over GPU: Continuous batch (பத்தி — தொகுப்பு)ing
    GPU->>R1: Process batch [R1, R2, R3]
    Note over R2: R2 done at step 10
...
The throughput improvement depends on how much output lengths vary. With uniform lengths, continuous batch (பத்தி — தொகுப்பு)ing matches static batching. With variable lengths (the common case), continuous batching can deliver 2-5x higher throughput because GPU slots never sit empty.
...
PagedAttention (from vLLM) applies OS-style virtual memory to KV cache. Instead of allocating one contiguous block per request, it allocates fixed-size "pages" (usually 16 tokens each). Pages is anywhere in physical GPU memory. A page table maps each request's logical sequence positions to physical page locations.
...
| Method | Draft source | Acceptance rate | extra work |
|--------|-------------|-----------------|----------|
| Draft-target (Leviathan et al.) | Separate small model | 70-85% | Draft model memory |
| EAGLE (Li et al.) | Lightweight head on target | 75-90% | ~1% extra parameter (பாரமீட்டர் — கட்டுப்பாடு)s |
| N-gram lookup | Token n-gram table | 40-60% | Negligible |

**EAGLE** trains a small autoregressive head on top of the target model's hidden states. It predicts the next token's embedding (embedding — பொருளை எண்களாக மாற்றுதல்) using the target model's second-to-last layer feature (பண்பு — தகவல் நெடுவரிசை)s. Because it operates on the target model's own representations (not a separate model's), it achieves higher acceptance rates with minimal extra memory. EAGLE-2 adds a dynamic draft tree that adjusts candidate count based on context.

**N-gram speculative decoding** maintains a table of n-gram continuations from the current context or a prebuilt corpus. If the draft matches what appeared before in the same conversation (repetitive patterns, code, structured output), it fires with zero neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு) extra work. Acceptance rates are lower on average but the cost per speculation is essentially free.
...
|--------|---------------|----------|
| vLLM | PagedAttention, continuous batch (பத்தி — தொகுப்பு)ing | General-purpose serving, highest compatibility |
| SGLang | RadixAttention (prefix caching), structured generation | Multi-turn chatbots, constrained decoding |
| tensor (டென்சர் — பல வடிவ எண் வரிசை)RT-LLM | NVIDIA kernel fusion, FP8 quantization | Maximum single-GPU throughput on NVIDIA hardware |

**vLLM** is the default starting point. It supports the widest range of models, runs on any GPU vendor (NVIDIA, AMD, Intel), and achieves strong throughput through PagedAttention + continuous batch (பத்தி — தொகுப்பு)ing. The OpenAI-compatible API means you can drop it in as a replacement for any OpenAI API call.
...
**tensor (டென்சர் — பல வடிவ எண் வரிசை)RT-LLM** compiles models into optimized NVIDIA GPU kernels. It fuses operations (attention + linear + activation in one kernel), uses FP8 on H100 GPUs, and integrates with NVIDIA Triton Inference Server for production deployment. It achieves the highest single-GPU throughput on NVIDIA hardware but requires more setup and only works on NVIDIA GPUs.

Real-world numbers for Llama 3 70B (4xA100-80GB, BF16):

| Metric | vLLM | SGLang | tensor (டென்சர் — பல வடிவ எண் வரிசை)RT-LLM |
|--------|------|--------|---------------|
...
You cannot optimize what you do not measure. The ops:byte ratio tells you whether you are compute-bound or memory-bound, which determines which optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)s matter.
...
When ops:byte is low (decode, small batch (பத்தி — தொகுப்பு)es), you hit the memory bandwidth roof. Adding more compute (higher clock, more cores) does not help. You need to reduce memory reads (quantization, KV cache compression) or increase the batch size to amortize reads across more useful work.

When ops:byte is high (prefill, large batch (பத்தி — தொகுப்பு)es), you hit the compute roof. Memory bandwidth optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) does not help. You need faster GPUs, kernel fusion, or reduced precision to squeeze more FLOPS.

| Scenario | ops:byte | Bound | Optimize with |
|----------|----------|-------|---------------|
| Prefill, batch (பத்தி — தொகுப்பு)=1 | ~4,096 | Compute | Kernel fusion, FP8 |
| Decode, batch=1 | ~1 | Memory | Quantization, KV compression |
| Decode, batch=32 | ~32 | Memory | Larger batch, continuous batching |
| Decode, batch=256 | ~256 | Transitioning | Both matter |
| Decode, batch=1024 | ~1,024 | Compute | Kernel fusion, tensor (டென்சர் — பல வடிவ எண் வரிசை) parallelism |

The crossover point on A100 is around ops:byte = 156 (312 TFLOPS / 2 TB/s). Below 156, you are memory-bound. Above 156, you are compute-bound. Continuous batch (பத்தி — தொகுப்பு)ing pushes decode toward this crossover by packing more tokens per iteration (மறுயுதல் — மீண்டும் செய்தல்).
...
We build a multi-head KV cache that stores key and value projections per layer, per head, and shows the memory growth pattern.
...
class MultiHeadAttention:
    ...
    def forward(self, x, kv_cache=None, layer_idx=0):
        batch (பத்தி — தொகுப்பு), seq_len, d_model = x.shape
        ...
        attn_out = attn_out.transpose(0, 2, 1, 3).reshape(batch (பத்தி — தொகுப்பு), -1, d_model)
        return np.matmul(attn_out, self.W_o)
```

### Step 3: Continuous batch (பத்தி — தொகுப்பு)ing Simulator

This simulates the scheduling difference between static and continuous batch (பத்தி — தொகுப்பு)ing.
...
def simulate_static_batch (பத்தி — தொகுப்பு)ing(requests, batch_size):
    step = 0
    completed = []
    queue = list(requests)
...
def simulate_continuous_batch (பத்தி — தொகுப்பு)ing(requests, batch_size):
    step = 0
    completed = []
    ...
    while queue_idx < len(queue) or active or waiting:
        ...
        while waiting and len(active) < batch (பத்தி — தொகுப்பு)_size:
            r = waiting.pop(0)
            r.start_step = step
            active.append(r)
...
def batch (பத்தி — தொகுப்பு)ing_stats(completed):
...
llm = LLM(
    ...
    tensor (டென்சர் — பல வடிவ எண் வரிசை)_parallel_size=4,
...
outputs = llm.generate(["Explain inference optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) in one paragraph."], params)
```
...
@sgl.function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)
def classify(s, text):
...
results = classify.run_batch (பத்தி — தொகுப்பு)([
...
With tensor (டென்சர் — பல வடிவ எண் வரிசை)RT-LLM:

```python
import tensor (டென்சர் — பல வடிவ எண் வரிசை)rt_llm
from tensorrt_llm.runtime import ModelRunner

runner = ModelRunner.from_dir("./llama-70b-trt-engine/", rank=0)

outputs = runner.generate(
    batch (பத்தி — தொகுப்பு)_input_ids=[tokenizer.encode("Explain KV caching.")],
    max_new_tokens=256,
    temperature=0.7,
)
```
...
- `outputs/skill-inference-optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்).md` -- a skill for diagnosing and optimizing LLM inference serving
...
|------|----------------|----------------------|
| Prefill | "Processing the prompt" | Computing attention over all input tokens in parallel -- compute-bound because the full matrix (மேட்ரிக்ஸ் — எண் மேஜை) multiplication keeps GPU cores busy |
...
| Continuous batch (பத்தி — தொகுப்பு)ing | "Dynamic batching" | Inserting new requests into the running batch as soon as any request finishes, evaluated at every decode iteration (மறுயுதல் — மீண்டும் செய்தல்) quite than waiting for the whole batch |
...
- Li et al., "EAGLE: Speculative Sampling Requires Rethinking feature (பண்பு — தகவல் நெடுவரிசை) Uncertainty" (2024) -- achieves higher acceptance rates by training a head on the target model's own features instead of using a separate draft model
