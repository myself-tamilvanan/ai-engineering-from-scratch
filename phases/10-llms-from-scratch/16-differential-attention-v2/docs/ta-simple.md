...
`lambda` is a learnable scalar (scalar — ஒரு எண்) per head, parameter (பாரமீட்டர் — கட்டுப்பாடு)ized as `lambda = exp(lambda_q1 dot lambda_k1) - exp(lambda_q2 dot lambda_k2) + lambda_init`. It is negative. `lambda_init` defaults to a small positive number like 0.8.
...
V1 kept the parameter (பாரமீட்டர் — கட்டுப்பாடு) count equal to the baseline Transformer. To get two queries per head it halved the head dimension (பரிமாணம் — கோணம்). That cost head expressiveness and — more painfully — halved the value cache per head. Decode had to load the value cache twice per step (once per softmax branch). Result: decode slower than baseline despite matching parameter count.

V2 doubles the number of query heads and keeps the KV heads the same (borrowing parameter (பாரமீட்டர் — கட்டுப்பாடு)s from the up-projection). The head dimension (பரிமாணம் — கோணம்) stays the same as baseline. After the subtraction, the extra dimension is projected back down to match baseline Transformer's O_W projection. Three things happen at once:
...
| Needle-in-haystack benchmarks | large accuracy lift past 32k |
...
| feature (பண்பு — தகவல் நெடுவரிசை) | Compatible with DIFF V2? |
|---------|------------------------|
...
Stdlib matrix (மேட்ரிக்ஸ் — எண் மேஜை) ops: lists of lists, manual matmul, softmax with numerical-stability subtraction of the max.
...
V1 style: halve the head dimension (பரிமாணம் — கோணம்). V2 style: keep the head dimension and double the number of heads. The toy how to build uses V1 for pedagogical clarity — the math is identical, only the bookkeeping differs.
...
Note: the output weights is negative. That is fine — the value cache still handles signed contributions. The subsequent V projection absorbs the sign.
...
### Step 5: V1 vs V2 parameter (பாரமீட்டர் — கட்டுப்பாடு) accounting

Given a config (hidden=4096, heads=32, d_head=128), print:

- Baseline Transformer: Q, K, V each size `hidden * hidden`, MLP at 4 * hidden.
- DIFF V1: Q, K each size `hidden * hidden`, V size `hidden * hidden` (unchanged), head dim halved internally. Adds per-head `lambda` parameter (பாரமீட்டர் — கட்டுப்பாடு)s (O(heads * d_head)).
...
The toy measures the extra parameter (பாரமீட்டர் — கட்டுப்பாடு) cost for V2 (roughly `hidden * hidden` extra per attention block) and prints it.
...
2. Compute the parameter (பாரமீட்டர் — கட்டுப்பாடு)-count delta from baseline to DIFF V1 and from baseline to DIFF V2 for a 7B-class model (hidden=4096, heads=32, d_head=128, 32 layers). Show which components gained parameters and which stayed the same.
...
| lambda | "The subtraction scale" | Per-head learnable scalar (scalar — ஒரு எண்) parameter (பாரமீட்டர் — கட்டுப்பாடு)ized as `exp(lq1.lk1) - exp(lq2.lk2) + lambda_init`. is negative |
...
- [Shared DIFF Transformer (arXiv:2501.17900)](https://arxiv.org/html/2501.17900) — parameter (பாரமீட்டர் — கட்டுப்பாடு)-sharing variant
