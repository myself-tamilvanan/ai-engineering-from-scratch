...
- Explain why SFT is necessary: base models continue text quite than answer questions
...
Simple and widely used. The `input` field is optional -- many instructions don't need extra context. Stanford released 52,000 examples in this format, generated by GPT-3.5 for $600. This kicked off the open-source instruction tuning movement.
...
### Training Hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s

SFT uses dramatically different hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s than pre-training. You're not training from scratch. You're adjusting a model that already works.

| parameter (பாரமீட்டர் — கட்டுப்பாடு) | Pre-Training (Llama 2 7B) | SFT (Llama 2 Chat) |
|-----------|---------------------------|---------------------|
| Learning rate | 3e-4 (peak) | 2e-5 |
| epoch (யுக் — ஒரு முழு சுழற்சி)s | 1 (single pass over data) | 2 |
| batch (பத்தி — தொகுப்பு) size | 4M tokens | 64 examples |
...
Two epoch (யுக் — ஒரு முழு சுழற்சி)s means the model sees each training example twice. More than 3 epochs on a small dataset leads to memorization -- the model starts reproducing training examples verbatim instead of generalizing.
...
1. **Low learning rate.** 1e-5 to 5e-5. Smaller updates mean less destruction of pre-trained feature (பண்பு — தகவல் நெடுவரிசை)s.

2. **Short training.** 1-3 epoch (யுக் — ஒரு முழு சுழற்சி)s. Stop before the model overfits.
...
- 2 epoch (யுக் — ஒரு முழு சுழற்சி)s = 10.24M tokens total
...
graph TD
    subgraph SFT["Supervised Fine-Tuning Pipeline"]
        ...
        L --> BW["Backward pass"]
        BW --> U["Update weights\n(lr=2e-5, 1-3 epoch (யுக் — ஒரு முழு சுழற்சி)s)"]
    end
...
INSTRUCTION_DATA = [
    ...
    },
    {
        ...
        "response": "Machine learning is a field where algorithm (அல்கோரிதம் — வழிமுறை)s learn patterns from data to make predictions."
    },
]
```
...
Standard cross-entropy, but multiplied by the loss mask. Only response tokens contribute to the gradient (சாய்வு — மாற்றத்தின் दिशा).
...
The denominator is `num_response_tokens`, not `seq_len`. If you divide by the total sequence length, longer instructions dilute the gradient (சாய்வு — மாற்றத்தின் दिशा) signal. Dividing by response token count ensures equal weight per response token regardless of instruction length.
...
from main import MiniGPT, LayerNorm, FeedForward, MultiHeadAttention, TransformerBlock, embedding (embedding — பொருளை எண்களாக மாற்றுதல்)


def sft_train(model, dataset, num_epoch (யுக் — ஒரு முழு சுழற்சி)s=2, lr=2e-5, seq_len=64):
    formatted_data = []
    ...
    print(f"SFT Training: {len(formatted_data)} examples, {num_epoch (யுக் — ஒரு முழு சுழற்சி)s} epochs, lr={lr}")
    ...
    for epoch (யுக் — ஒரு முழு சுழற்சி) in range(num_epochs):
        ...
        for idx in indices:
            ...
            batch (பத்தி — தொகுப்பு)_size, s_len, v_size = logits.shape
            ...
            num_batch (பத்தி — தொகுப்பு)es += 1
            losses.append(loss)

        avg_loss = epoch (யுக் — ஒரு முழு சுழற்சி)_loss / max(num_batch (பத்தி — தொகுப்பு)es, 1)
        print(f"Epoch {epoch + 1}/{num_epochs} | Avg Loss: {avg_loss:.4f}")

    return model, losses
```

The learning rate is 2e-5, matching Llama 2 Chat. Compare this to the 3e-4 used in pre-training -- 15x smaller. The gradient (சாய்வு — மாற்றத்தின் दिशा) is masked: instruction tokens produce zero gradient. Only response tokens push the weights.
...
def measure_forgetting(model, test_text, seq_len=64):
    ...
    for start in range(0, len(tokens) - seq_len - 1, seq_len):
        ...
        batch (பத்தி — தொகுப்பு), s_len, vocab_size = logits.shape
        logits_flat = logits.reshape(-1, vocab_size)
...
In real fine-tuning, you would track this metric throughout training. If the raw text loss increases by more than 10-15%, your SFT is too aggressive. Lower the learning rate or reduce the number of epoch (யுக் — ஒரு முழு சுழற்சி)s.
...
The model learns to predict the next token given all previous tokens."""
    ...
    print(f"Model: {model.count_parameter (பாரமீட்டர் — கட்டுப்பாடு)s():,} parameters")
    ...
    model, losses = sft_train(
        model, INSTRUCTION_DATA, num_epoch (யுக் — ஒரு முழு சுழற்சி)s=3, lr=2e-5, seq_len=128
    )
...
2. Implement data mixing. Create a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) that takes an SFT dataset and a raw text corpus, then produces training batch (பத்தி — தொகுப்பு)es where 5% of examples are raw text (no masking) and 95% are instruction pairs (masked). Run 3 epoch (யுக் — ஒரு முழு சுழற்சி)s and compare forgetting metrics against pure SFT training.
...
| Loss masking | "Ignoring the prompt" | Setting loss to zero for instruction tokens so gradient (சாய்வு — மாற்றத்தின் दिशा)s only flow from response token predictions |
...
| Weight tying | "Shared embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s" | Using the same matrix (மேட்ரிக்ஸ் — எண் மேஜை) for input token embeddings and output prediction head, saving parameter (பாரமீட்டர் — கட்டுப்பாடு)s and improving coherence |
