...
Not because BPE is wrong -- because the how to build is incomplete. A production tokenizer handles raw bytes in any encoding, normalizes Unicode before splitting, manages special tokens that never get merged, chains pre-tokenization with subword splitting, and does all of this fast enough to not bottleneck a training pipeline processing 15 trillion tokens.
...
A production tokenizer is not one algorithm (அல்கோரிதம் — வழிமுறை). It is a pipeline of five stages, each solving a different problem.
...
Byte-level BPE solves this by treating every possible byte value (0-255) as a valid token. Your base vocabulary is exactly 256 entries. Any file -- text, binary, corrupted -- is tokenized without producing an unknown token.

GPT-2 added a trick: map each byte to a printable Unicode character so the vocabulary stays human-readable. Byte 0x20 (space) becomes the character "G" in their mapping. This is purely cosmetic. The algorithm (அல்கோரிதம் — வழிமுறை) does not care.

The real power: byte-level BPE handles every language on earth. Chinese characters are 3 UTF-8 bytes each. Japanese is 3-4 bytes. Arabic, Devanagari, emoji -- all just byte sequences. The BPE algorithm (அல்கோரிதம் — வழிமுறை) finds patterns in these byte sequences exactly the same way it finds patterns in English ASCII bytes.

### Pre-Tokenization

Before BPE touches your text, you need to split it into chunks. This prevents the merge algorithm (அல்கோரிதம் — வழிமுறை) from creating tokens that span word boundaries.
...
Llama uses SentencePiece, which skips regex entirely. It treats the raw byte stream as one long sequence and lets the BPE algorithm (அல்கோரிதம் — வழிமுறை) figure out the boundaries. This is simpler but gives BPE more freedom to create cross-word tokens.
...
| `[PAD]` | Padding for batch (பத்தி — தொகுப்பு) alignment | BERT, T5 |
...
Special tokens are never split by BPE. They are matched exactly before the merge algorithm (அல்கோரிதம் — வழிமுறை) runs, replaced with their fixed ID, and the surrounding text is tokenized normally.
...
You are building in Python to understand the algorithm (அல்கோரிதம் — வழிமுறை). In production, you would use a compiled how to build and only touch the Python wrapper.
...
The core algorithm (அல்கோரிதம் — வழிமுறை) from Lesson 01, but now operating on pre-tokenized chunks independently.
...
corpus = (
    ...
    "deep learning (ஆழ் கற்றல் — பல்லாய கொட்ட பயிற்சி) transforms how we build software. "
...
You will see different token counts for the same text. Llama 3 with 128K vocabulary is more aggressive at merging common patterns. GPT-4 with 100K sits in the middle. Mistral with 32K produces more tokens but has a smaller embedding (embedding — பொருளை எண்களாக மாற்றுதல்) layer.

The choice is always the same: larger vocabulary means shorter sequences but more parameter (பாரமீட்டர் — கட்டுப்பாடு)s.
...
3. **Hard:** Add a chat template method that takes a list of `{"role": ..., "content": ...}` messages and produces the correct token sequence for the Llama 3 chat format. Test it against the HuggingFace how to build.
...
| tiktoken | "OpenAI tokenizer" | Rust BPE how to build with Python bindings -- 10-100x faster than pure Python |
...
- [OpenAI tiktoken source](https://github.com/openai/tiktoken) -- Rust BPE how to build used by GPT-3.5/4
