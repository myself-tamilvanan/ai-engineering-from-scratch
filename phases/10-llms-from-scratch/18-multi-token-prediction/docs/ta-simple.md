...
- Compute the parameter (பாரமீட்டர் — கட்டுப்பாடு) and memory extra work of adding MTP modules to a pre-training run.
- Implement one MTP module from scratch: the shared embedding (embedding — பொருளை எண்களாக மாற்றுதல்), the per-depth transformer block, the projection, and the shared output head.
...
MTP asks: what if every hidden state were supervised to predict multiple future tokens at once? Gloeckle et al. (Meta, 2024) showed this helps. Their how to build put several independent output heads on top of the backbone, each predicting a different offset. Parallel, simple, but the heads saw the same hidden state without any hierarchical refinement — and the predictions did not chain causally, so they could not be used for speculative decoding.

DeepSeek-V3 (December 2024) re-designed MTP as sequential modules that keep the causal chain at each prediction depth. The model predicts `t+1` from `h_i^(0)`, then predicts `t+2` from a new hidden state `h_i^(1)` that combined `h_i^(0)` with the `E(t+1)` embedding (embedding — பொருளை எண்களாக மாற்றுதல்), and so on. Each depth is its own small transformer block. The shared embedding and shared output head keep parameter (பாரமீட்டர் — கட்டுப்பாடு) extra work modest. At DeepSeek-V3's scale, 14B extra parameters across MTP modules on top of 671B main-model weights. That 2% extra work bought denser training signals AND a ready-made speculative-decoding draft at inference.

This lesson builds a single MTP module and the D-depth loss from scratch. The math is tidy. The how to build is 150 lines.
...
- A projection matrix (மேட்ரிக்ஸ் — எண் மேஜை) `M_k` that combines the previous-depth hidden state with the embedding (embedding — பொருளை எண்களாக மாற்றுதல்) of the next-depth ground-truth token.
...
DeepSeek-V3's sequential design builds `h_i^(k)` from `h_i^(k-1)` plus the actual next-token embedding (embedding — பொருளை எண்களாக மாற்றுதல்) `E(t_{i+k})`. That preserves the causal chain: to predict `t_{i+k+1}`, the module at depth `k+1` sees what was at `t_{i+k}`. This is structurally identical to how an autoregressive decoder consumes its own output — making the MTP modules directly usable as speculative-decoding drafters.
...
### parameter (பாரமீட்டர் — கட்டுப்பாடு) accounting

For a model with hidden `h` and vocabulary `V`:

- Main model: billions of parameter (பாரமீட்டர் — கட்டுப்பாடு)s, plus one output head of size `V * h`.
- Shared output head: reuse the main model's head. No extra params.
- Shared embedding (embedding — பொருளை எண்களாக மாற்றுதல்): reuse the main model's embedding. No extra params.
- Per-MTP module:
  - Projection `M_k`: `(2h) * h = 2h^2`.
  - Transformer block `T_k`: attention (`4h^2` for MHA) plus MLP (usually `8h^2` for SwiGLU with ratio 8/3). About `12h^2` per block.

Total extra per module: `~14h^2`. For DeepSeek-V3's `h = 7168`, D = 1 module: `~14 * 7168^2 = ~720M` parameter (பாரமீட்டர் — கட்டுப்பாடு)s on paper. DeepSeek-V3 reports 14B — the difference is mostly expert layers being MoE in the MTP module too.
...
| dimension (பரிமாணம் — கோணம்) | EAGLE-3 | MTP (DeepSeek-V3) |
|-----------|---------|------------------|
...
`code/main.py` builds a single MTP module end to end: shared embedding (embedding — பொருளை எண்களாக மாற்றுதல்), projection, transformer block, shared output head. It then computes the per-depth cross-entropy loss on a short synthetic sequence and prints the parameter (பாரமீட்டர் — கட்டுப்பாடு) count by component. A toy vocabulary of 32 tokens keeps the numbers readable.

### Step 1: shared embedding (embedding — பொருளை எண்களாக மாற்றுதல்) table

A single `vocab_size x hidden` table helps by the main model AND by every MTP module at every depth. Not a second copy — literally the same tensor (டென்சர் — பல வடிவ எண் வரிசை).

### Step 2: the per-depth combination

```python
def combine(prev_hidden, next_token_embed, M_k):
    # concat along feature (பண்பு — தகவல் நெடுவரிசை) dim, then project down to hidden
    concat = rms_norm(prev_hidden) + rms_norm(next_token_embed)  # vector (வெக்டர் — எண் பட்டியல்) addition stand-in
    projected = matvec(M_k, concat)
    return projected
```

Real DeepSeek-V3 concatenates the two RMSNormed vector (வெக்டர் — எண் பட்டியல்)s to `[2h]` and projects with an `h x 2h` matrix (மேட்ரிக்ஸ் — எண் மேஜை). The toy uses vector addition for stdlib brevity.
...
### Step 6: parameter (பாரமீட்டர் — கட்டுப்பாடு) accounting

Print the total parameter (பாரமீட்டர் — கட்டுப்பாடு) count, the shared (embedding (embedding — பொருளை எண்களாக மாற்றுதல்), head) count, and the per-module extra count. Show the ratio of MTP extra to main-model size.
...
- Your hidden size is at least 4096. At 1B-scale the extra work hurts more than the gain helps.
...
This lesson produces `outputs/skill-mtp-planner.md`. Given a pre-training run specification (model size, data, compute), it returns a plan for integrating MTP: number of depths D, `lambda` schedule, memory extra work, and the inference-time speculative-decoding wiring.
...
2. Compute the parameter (பாரமீட்டர் — கட்டுப்பாடு) extra work for a dense 70B model (hidden 8192, 80 layers) with D=1 MTP module. Compare to the DeepSeek-V3 reported 14B extra work. Explain why DeepSeek's number is higher: the MTP transformer block inherits the same MoE structure, inflating the per-module parameter count.

3. Implement D=2 in the toy: add a second MTP module that takes h^(1) and predicts `t_{i+2}`. Verify the joint loss and the parameter (பாரமீட்டர் — கட்டுப்பாடு) accounting match the DeepSeek paper's equations 19-21.
...
| Sequential MTP | "DeepSeek-V3 style" | Each module conditions on the previous depth's hidden state plus the next token's embedding (embedding — பொருளை எண்களாக மாற்றுதல்). preserves causal chain |
| Shared output head | "Reuse the main head" | The MTP modules call the main model's LM head, not a separate output projection |
| Shared embedding | "Reuse the main table" | Same vocabulary embedding table helps everywhere. no duplicate parameter (பாரமீட்டர் — கட்டுப்பாடு)s |
| Projection matrix (மேட்ரிக்ஸ் — எண் மேஜை) M_k | "Combine hidden + next-token" | An `h x 2h` linear layer that folds the previous hidden state and the target-token embedding into the next depth's input |
