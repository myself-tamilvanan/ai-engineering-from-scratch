...
- Evaluate the effect of RLHF by comparing response quality before and after preference optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)

## The Problem

Ask a model "Explain quantum computing" and it might produce:

**Response A:** "Quantum computing uses qubits that can exist in superposition, meaning they is 0, 1, or both simultaneously. This allows quantum computers to process certain calculations exponentially faster than classical computers. Key algorithm (அல்கோரிதம் — வழிமுறை)s include Shor's algorithm for factoring large numbers and Grover's algorithm for searching unsorted databases."
...
RLHF solves this. It trains a reward model to predict which response a human would prefer, then uses that reward signal to push the language model toward higher-quality outputs. InstructGPT (the precursor to ChatGPT) used RLHF to dramatically improve GPT-3's helpfulness, truthfulness, and harmlessness. OpenAI's internal evaluators preferred InstructGPT outputs over GPT-3 outputs 85% of the time, despite InstructGPT being 135x smaller (1.3B vs 175B parameter (பாரமீட்டர் — கட்டுப்பாடு)s).
...
**Stage 2: Reward Model.** Collect human preference data: show annotators two responses to the same prompt and ask "which is better?" Train a model to predict these preferences. The reward model takes (prompt, response) as input and outputs a scalar (scalar — ஒரு எண்) score.
...
The reward model is a language model repurposed as a scorer. Take the SFT model, replace the language modeling head (which outputs a distribution over vocabulary) with a scalar (scalar — ஒரு எண்) head (which outputs a single number). The architecture is identical up to the final layer.

Input: a prompt concatenated with a response. Output: a single scalar (scalar — ஒரு எண்) reward score.
...
The loss function (சேதத் சூத்திரம் — எவ்வளவு தவறு என்பதை அளக்கும் சூத்திரம்) (சார்பு — உள்ளீட்டுக்கு வெளியீடு) uses the Bradley-Terry model of pairwise preferences:
...
### PPO: Proximal Policy optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)

PPO is a reinforcement learning algorithm (அல்கோரிதம் — வழிமுறை). In RLHF, the "environment" is the reward model, the "agent" is the language model, and the "action" is generating a token.
...
**InstructGPT numbers:** PPO training used lr=1.5e-5, KL coefficient beta=0.02, 256K episodes (prompt-response pairs), and 4 PPO epoch (யுக் — ஒரு முழு சுழற்சி)s per batch (பத்தி — தொகுப்பு). The entire RLHF pipeline took several days on a cluster of GPUs.
...
graph LR
    subgraph PPO["PPO Training Loop"]
        ...
        SCORE --> OBJ["Objective:\nreward - beta * KL"]
        KL --> OBJ
        OBJ --> UPDATE["PPO gradient (சாய்வு — மாற்றத்தின் दिशा) update\n(clipped surrogate loss)"]
...
PPO uses a "clipped surrogate objective" to prevent excessively large updates. The ratio between the new policy and old policy probabilities is clipped to the range [1 - epsilon, 1 + epsilon], where epsilon is usually 0.2.
...
The advantage function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) estimates how much better the current response is compared to the expected quality. In RLHF:
...
The reward model reuses the transformer architecture from the mini GPT, but replaces the vocabulary-sized output head with a single scalar (scalar — ஒரு எண்) projection.
...
The reward model takes the hidden state at the *last* token position and projects it to a scalar (scalar — ஒரு எண்). Why the last token? Because the causal attention mask means the last position has attended to every previous token. It has the most complete representation of the entire (prompt, response) sequence.
...
def train_reward_model(rm, preference_data, num_epoch (யுக் — ஒரு முழு சுழற்சி)s=10, lr=1e-4, max_seq_len=128):
    ...
    for epoch (யுக் — ஒரு முழு சுழற்சி) in range(num_epochs):
        ...
        for idx in indices:
            ...
            if r_preferred > r_rejected:
                epoch (யுக் — ஒரு முழு சுழற்சி)_correct += 1

            diff = r_preferred - r_rejected
            grad = sigmoid(diff) - 1.0

            rm.reward_head -= lr * grad * rm.ln_f.forward(
                rm.embedding (embedding — பொருளை எண்களாக மாற்றுதல்).forward(preferred_ids)
            )[:, -1, :].flatten()

            epoch (யுக் — ஒரு முழு சுழற்சி)_loss += loss
            num_pairs += 1

        avg_loss = epoch (யுக் — ஒரு முழு சுழற்சி)_loss / max(num_pairs, 1)
        accuracy = epoch_correct / max(num_pairs, 1)
        losses.append(avg_loss)
        accuracies.append(accuracy)

        if epoch (யுக் — ஒரு முழு சுழற்சி) % 2 == 0:
...
Full PPO is complex. This how to build captures the core mechanism: generate responses, score them, compute the advantage, and update the policy with a KL penalty.
...
if __name__ == "__main__":
    ...
    rm, rm_losses, rm_accuracies = train_reward_model(rm, PREFERENCE_DATA, num_epoch (யுக் — ஒரு முழு சுழற்சி)s=10, lr=1e-4)
    print()
...
| Reward model | "A model that scores responses" | A transformer with a scalar (scalar — ஒரு எண்) output head, trained on pairwise human preferences using the Bradley-Terry loss |
| Bradley-Terry | "The comparison model" | A probabilistic model where P(A > B) = sigmoid(score(A) - score(B)), converting pairwise preferences into a consistent scoring function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) |
| PPO | "The RL algorithm (அல்கோரிதம் — வழிமுறை)" | Proximal Policy optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்): updates the policy to maximize reward while clipping the update magnitude to prevent instability |
...
| Preference pair | "Which is better, A or B?" | A training example consisting of (prompt, preferred_response, rejected_response) -- the basic unit of RLHF training data |
| Reference model | "The frozen SFT checkpoint" | A copy of the SFT model whose weights never change -- used as the anchor for KL divergence calculation |
...
- [Schulman et al., 2017 -- "Proximal Policy optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) algorithm (அல்கோரிதம் — வழிமுறை)s"](https://arxiv.org/abs/1707.06347) -- the original PPO paper from OpenAI
...
- [Christiano et al., 2017 -- "Deep reinforcement learning from human preferences"](https://arxiv.org/abs/1706.03741) -- the foundational work on learning reward function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s from human comparisons
