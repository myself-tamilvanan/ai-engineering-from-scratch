...
- Build a streaming data pipeline that tokenizes, chunks, shuffles, and batch (பத்தி — தொகுப்பு)es terabytes of text without loading it all into memory
...
Not a dataset. Not a CSV file. Terabytes of text -- cleaned, deduplicated, filtered for quality, tokenized into fixed-length sequences, and served in randomized batch (பத்தி — தொகுப்பு)es fast enough that your 8-GPU cluster never waits for the next batch.
...
2. **MinHash:** For each document's shingle set, compute k hash values. Each hash value is the minimum hash across all shingles under a different hash function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). This creates a fixed-size "signature" that approximates the Jaccard similarity between any two documents.
...
4. **Verify:** For each candidate pair, compute exact Jaccard similarity. Remove one copy if similarity exceeds a threshold (usually 0.8).
...
In practice, this means you should scale model size and dataset size roughly equally. A model with 10x more parameter (பாரமீட்டர் — கட்டுப்பாடு)s needs roughly 10x more training tokens to reach the same loss.

| Model | parameter (பாரமீட்டர் — கட்டுப்பாடு)s | Training Tokens | Chinchilla-Optimal? |
|-------|-----------|----------------|-------------------|
...
The `num_hashes=128` and `bands=16` parameter (பாரமீட்டர் — கட்டுப்பாடு)s control the precision-recall choice. More hashes give more accurate similarity estimates. More bands increase recall (catch more duplicates) at the cost of more false positives. These values work well for typical web text.
...
Yield randomized batch (பத்தி — தொகுப்பு)es of packed sequences. This is what the training loop consumes.

```python
import random

class PreTrainingDataLoader:
    def __init__(self, sequences, attention_masks, batch (பத்தி — தொகுப்பு)_size, shuffle=True):
    ...
    def __len__(self):
        return (len(self.sequences) + self.batch (பத்தி — தொகுப்பு)_size - 1) // self.batch_size
...
Compression ratio tells you how efficient the tokenizer is on this corpus. English text usually compresses to about 3-4 characters per token. If you see 1.5 characters per token, your tokenizer is splitting too aggressively. If you see 8+, it has learned very domain-specific merges.
...
The HuggingFace pipeline uses Rust tokenizers under the hood and parallel processing across 4 cores. Your pure Python pipeline will be 10-50x slower. That gap is why production teams use compiled tokenizers. The algorithm (அல்கோரிதம் — வழிமுறை) is the same. The how to build language is the difference.
...
| Deduplication | "Removing copies" | Eliminating exact and near-duplicate documents -- usually removes 30-40% of raw web data |
