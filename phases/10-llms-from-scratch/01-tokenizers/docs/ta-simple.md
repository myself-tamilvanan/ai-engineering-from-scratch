...
- Implement BPE, WordPiece, and Unigram tokenization algorithm (அல்கோரிதம் — வழிமுறை)s from scratch and compare their merge strategies
- Explain how vocabulary size affects model speed: too small creates long sequences, too large wastes embedding (embedding — பொருளை எண்களாக மாற்றுதல்) parameter (பாரமீட்டர் — கட்டுப்பாடு)s
...
BPE is a greedy compression algorithm (அல்கோரிதம் — வழிமுறை) repurposed for tokenization. The idea is simple enough to fit on an index card.
...
"embedding (embedding — பொருளை எண்களாக மாற்றுதல்)"   -> ["em", "##bed", "##ding"]
```
...
SentencePiece supports two algorithm (அல்கோரிதம் — வழிமுறை)s:
...
graph LR
    subgraph Small["Small Vocab (32K)\ne.g., BERT, T5"]
        ...
        S2["Longer sequences"]
        S3["Smaller embedding (embedding — பொருளை எண்களாக மாற்றுதல்) matrix (மேட்ரிக்ஸ் — எண் மேஜை)"]
...
Concrete numbers. For a 128K vocabulary with 4,096-dimension (பரிமாணம் — கோணம்)al embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s, the embedding matrix (மேட்ரிக்ஸ் — எண் மேஜை) alone is 128,000 x 4,096 = 524 million parameter (பாரமீட்டர் — கட்டுப்பாடு)s. For a 32K vocabulary, it is 131 million parameters. That is a 400M parameter difference from the tokenizer choice alone.
...
Tokenizers trained primarily on English are brutal to other languages. Korean text in GPT-2's tokenizer averages 2-3 tokens per word. Chinese is worse. This means a Korean user effectively has a context window that is half the size of an English user's -- paying the same price for less information density.
...
The real how to build. We train on raw bytes (like GPT-2), count pairs, merge the most frequent, and record every merge in order. The merge table is the tokenizer.
...
tiktoken uses the exact same algorithm (அல்கோரிதம் — வழிமுறை) but trained on hundreds of gigabytes of text with 100,000 merges. The algorithm is identical. The difference is the training data and the number of merges. Your tokenizer trained on a paragraph with 40 merges cannot compete with tiktoken's 100K merges on a massive corpus. But the mechanism is the same.
...
tiktoken is written in Rust with Python bindings. It encodes millions of tokens per second. Same BPE algorithm (அல்கோரிதம் — வழிமுறை), industrial-strength how to build.
...
This lesson produces `outputs/prompt-tokenizer-analyzer.md` -- a reusable prompt that analyzes tokenization speed for any text and model combination. Feed it a text sample and it tells you which model's tokenizer handles it best.
...
4. Build a multilingual tokenizer speed benchmark. Take 10 sentences in English, Spanish, Chinese, Korean, and Arabic. Tokenize each with tiktoken (cl100k_base) and measure the average tokens per character. Quantify the "multilingual tax" for each language.
...
| SentencePiece | "A tokenizer library" | A language-agnostic tokenizer that operates on raw Unicode without pre-tokenization, supporting BPE and Unigram algorithm (அல்கோரிதம் — வழிமுறை)s |
...
| Fertility | "Not a tokenizer term" | Average number of tokens per word -- measures tokenizer speed across languages (1.0 is perfect, 3.0 means the model works three times harder) |
...
- [Sennrich et al., 2016 -- "Neural Machine Translation of Rare Words with Subword Units"](https://arxiv.org/abs/1508.07909) -- the paper that introduced BPE for NLP, turning a 1994 compression algorithm (அல்கோரிதம் — வழிமுறை) into the foundation of modern tokenization
...
- [OpenAI tiktoken repository](https://github.com/openai/tiktoken) -- production BPE how to build in Rust with Python bindings, used by GPT-3.5/4/4o
