...
**Type:** Build
**Languages:** Python
**Prerequisites:** Phase 7 Lesson 02 (Self-Attention), Phase 4 Lesson 04 (Image classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்))
**Time:** ~45 minutes

## Learning Objectives

- Implement patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்), learned positional embedding, class token, and transformer encoder blocks from scratch to build a minimal ViT
...
```mermaid
flowchart LR
    IMG["Image<br/>(3, 224, 224)"] --> PATCH["Patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்)<br/>conv 16x16 s=16<br/>-> (768, 14, 14)"]
...
### Patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்)

The first conv is the secret. Kernel size 16, stride 16, so a 224x224 image becomes a 14x14 grid of 16x16 patches, each projected to a 768-dim embedding (embedding — பொருளை எண்களாக மாற்றுதல்). That single conv both patchifies and linearly projects.
...
196 patches = 196 tokens. Each token's feature (பண்பு — தகவல் நெடுவரிசை) dimension (பரிமாணம் — கோணம்) is 768 (ViT-B), 1024 (ViT-L), or 1280 (ViT-H).

### Class token

A single learned vector (வெக்டர் — எண் பட்டியல்) prepended to the sequence:

```
tokens = [CLS. patch_1. patch_2. .... patch_196]   shape (197, 768)
```

After N transformer blocks, the `[CLS]` output is the global image representation. classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) head reads only this one vector (வெக்டர் — எண் பட்டியல்).

### Positional embedding (embedding — பொருளை எண்களாக மாற்றுதல்)

Transformers have no built-in notion of spatial position. Add a learned vector (வெக்டர் — எண் பட்டியல்) to every token:

```
tokens = tokens + learned_pos_embedding (embedding — பொருளை எண்களாக மாற்றுதல்)   (also shape (197, 768))
```

The embedding (embedding — பொருளை எண்களாக மாற்றுதல்) is a parameter (பாரமீட்டர் — கட்டுப்பாடு) of the model. gradient (சாய்வு — மாற்றத்தின் दिशा)-based training adapts it to 2D image structure. Sinusoidal 2D alternatives exist but are rarely used in practice.
...
ViT-B/16 stacks 12 of these blocks, each with 12 attention heads, totalling 86M parameter (பாரமீட்டர் — கட்டுப்பாடு)s.
...
3. Repeated augmentation (same image sampled 3 times per batch (பத்தி — தொகுப்பு)).
...
### Step 1: Patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்)

```python
import torch
import torch.nn as nn

class Patchembedding (embedding — பொருளை எண்களாக மாற்றுதல்)(nn.Module):
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4, dropout=0.0):
        ...
        self.attn = nn.MultiheadAttention(dim, num_heads, dropout=dropout, batch (பத்தி — தொகுப்பு)_first=True)
...
`nn.MultiheadAttention` handles the splitting into heads, the scaled dot-product, and the output projection. `batch (பத்தி — தொகுப்பு)_first=True` so shapes are `(N, seq, dim)`.
...
class ViT(nn.Module):
    def __init__(self, image_size=64, patch_size=16, in_channels=3,
        ...
        super().__init__()
        self.patch = Patchembedding (embedding — பொருளை எண்களாக மாற்றுதல்)(in_channels, patch_size, dim, image_size)
        num_patches = self.patch.num_patches
        self.cls_token = nn.parameter (பாரமீட்டர் — கட்டுப்பாடு)(torch.zeros(1, 1, dim))
...
print(f"params: {sum(p.numel() for p in vit.parameter (பாரமீட்டர் — கட்டுப்பாடு)s()):,}")
```

About 2.8M parameter (பாரமீட்டர் — கட்டுப்பாடு)s — a tiny ViT tractable on CPU. Real ViT-B is 86M. same class definition with `dim=768, depth=12, num_heads=12`.
...
- `outputs/skill-vit-patch-and-pos-embed-inspector.md` — a skill that verifies a ViT's patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்) and positional embedding shapes match the model's expected sequence length, catching the most common porting bugs.

## Exercises

1. **(Easy)** Print the shapes of every intermediate tensor (டென்சர் — பல வடிவ எண் வரிசை) for a forward pass through the tiny ViT above. Confirm: input `(N, 3, 64, 64)` -> patches `(N, 16, 192)` -> with CLS `(N, 17, 192)` -> classifier input `(N, 192)` -> output `(N, num_classes)`.
...
|------|----------------|----------------------|
| Patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்) | "The first conv" | A conv with kernel size = stride = patch size. turns the image into a grid of token embeddings |
| Class token | "[CLS]" | A learned vector (வெக்டர் — எண் பட்டியல்) prepended to the token sequence. its final output is the global image representation |
