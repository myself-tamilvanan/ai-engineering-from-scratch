...
`beta_t` is a small variance schedule, usually linear from 0.0001 to 0.02 over T=1000 steps. Each step slightly shrinks the signal and injects fresh noise.
...
The forward process is fixed. The reverse process `p(x_{t-1} | x_t)` is what the neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு) learns. Diffusion models do not predict `x_{t-1}` directly. they predict the noise `epsilon` added at step t, and the math derives `x_{t-1}` from it.
...
That is it. The neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு) learns to predict the noise at any timestep. The loss is MSE. There is no adversarial game, no collapse, no oscillation.
...
The network `epsilon_theta(x_t, t)` needs to know which timestep it is denoising. Modern diffusion models inject `t` via sinusoidal time embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s (same idea as positional encoding in transformers) that get added to feature (பண்பு — தகவல் நெடுவரிசை) maps at every U-Net level.

```
t_embedding (embedding — பொருளை எண்களாக மாற்றுதல்) = sinusoidal(t)
feature (பண்பு — தகவல் நெடுவரிசை)_map += MLP(t_embedding)
```
...
One-line closed form. `t` is a batch (பத்தி — தொகுப்பு) of timesteps, one per image in the batch.
...
def timestep_embedding (embedding — பொருளை எண்களாக மாற்றுதல்)(t, dim=64):
    half = dim // 2
...
class TinyUNet(nn.Module):
    ...
    def forward(self, x, t):
        t_emb = timestep_embedding (embedding — பொருளை எண்களாக மாற்றுதல்)(t, self.t_dim)
...
def sample(model, schedule, shape, T=1000, device="cpu"):
    ...
    for t in reversed(range(T)):
        t_batch (பத்தி — தொகுப்பு) = torch.full((shape[0],), t, dtype=torch.long, device=device)
        eps = model(x, t_batch)
...
1000 forward passes to produce one batch (பத்தி — தொகுப்பு) of samples. In real code you would swap this for a DDIM 50-step sampler.
...
def sample_ddim(model, schedule, shape, steps=50, T=1000, device="cpu", eta=0.0):
    ...
    for i in range(steps):
        ...
        a_prev = alphas_cumprod[t_prev] if t_prev >= 0 else torch.tensor (டென்சர் — பல வடிவ எண் வரிசை)(1.0, device=device)
...
2. **(Medium)** Train the TinyUNet on the synthetic-circles dataset for 20 epoch (யுக் — ஒரு முழு சுழற்சி)s and sample 16 circles. Compare DDPM (1000 steps) and DDIM (50 steps) sampling — do they produce similar images from the same noise seed?
...
| Time conditioning | "Tell the model which t" | Sinusoidal embedding (embedding — பொருளை எண்களாக மாற்றுதல்) of t injected into the U-Net so it knows the noise level |
...
- [Improved DDPM (Nichol & Dhariwal, 2021)](https://arxiv.org/abs/2102.09672) — cosine schedule and v-parameter (பாரமீட்டர் — கட்டுப்பாடு)isation
