...
> A vision encoder converts an image into tokens. An MLP projector maps those tokens into the LLM's embedding (embedding — பொருளை எண்களாக மாற்றுதல்) space. A language model does the rest. That pattern — ViT-MLP-LLM — is every production VLM in 2026.
...
- Compare Qwen3-VL, InternVL3.5, LLaVA-Next, and GLM-4.6V on parameter (பாரமீட்டர் — கட்டுப்பாடு) count, context length, and benchmark performance
- Explain DeepStack: why multi-level ViT feature (பண்பு — தகவல் நெடுவரிசை)s tighten vision-language alignment better than a single last-layer feature
- Measure VLM hallucination in production with Cross-Modal Error Rate (CMER) and act on the signal

## The Problem

CLIP (Phase 4 Lesson 18) gives you a shared embedding (embedding — பொருளை எண்களாக மாற்றுதல்) space for images and text, which is enough for zero-shot classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) and retrieval. It cannot answer "how many red cars are in this image?" because CLIP does not generate text — it only scores similarities.
...
2. **Projector** — a small module (2-4 layer MLP, or a Q-former) that maps vision tokens into the LLM's embedding (embedding — பொருளை எண்களாக மாற்றுதல்) dimension (பரிமாணம் — கோணம்). This is where most of the fine-tuning happens.
...
All three pieces are trainable in principle. In practice, the vision encoder and LLM stay mostly frozen while the projector trains — a few billion parameter (பாரமீட்டர் — கட்டுப்பாடு)s of signal for cheap.

### DeepStack

Vanilla projection uses only the last ViT layer. DeepStack (Qwen3-VL) samples feature (பண்பு — தகவல் நெடுவரிசை)s from multiple ViT depths and stacks them. Deeper layers carry high-level semantics. shallower layers carry fine-grained spatial and textural information. Feeding both into the LLM closes the gap between "what does the image contain" (semantics) and "where exactly" (spatial grounding).
...
VLMs need to know **when** a frame is in a video. Qwen3-VL evolved from T-RoPE (temporal rotary position embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s) to **text-based time alignment** — explicit timestamp text tokens interleaved with video frames. The model sees "`<timestamp 00:32>` frame, prompt" and can reason about temporal relationships.
...
Input is a `(N_patches, d_vit)` token tensor (டென்சர் — பல வடிவ எண் வரிசை). Output is `(N_patches, d_llm)`. The LLM treats every output row as just another token.
...
class MinimalVLM(nn.Module):
    ...
    def forward(self, image, input_ids, attention_mask):
        # 1. vision feature (பண்பு — தகவல் நெடுவரிசை)s
        vision_tokens = self.vit(image)                     # (B, N_patches, d_vit)
        vision_embeds = self.projector(vision_tokens)       # (B, N_patches, d_llm)

        # 2. text embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s
    ...
    def _merge(self, text_embeds, vision_embeds, input_ids):
        ...
        for b in range(input_ids.size(0)):
            ...
            if len(positions) != expected:
                raise ValueError(
                    f"batch (பத்தி — தொகுப்பு) item {b} has {len(positions)} image tokens but vision_embeds has {expected} patches."
                    " Every sample in the batch must be pre-padded to the same number of image placeholder tokens.")
            out[b, positions] = vision_embeds[b]
        return out
```

The `<image>` placeholder token in the text gets replaced with real image embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s — same pattern LLaVA, Qwen-VL, and InternVL use.

### Step 3: CMER calculation
...
def cross_modal_error_rate(image_emb, text_emb, text_confidence, sim_threshold=0.25, conf_threshold=0.8):
    """
    image_emb, text_emb: embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s of image and generated text (normalised internally)
...
Demonstrate the projector trains. Fake "ViT feature (பண்பு — தகவல் நெடுவரிசை)s" go in. a tiny LLM-style token predicts a class.
...
One can fit this on synthetic (feature (பண்பு — தகவல் நெடுவரிசை), class) pairs in under 200 steps — enough to show the projector pattern works.
...
inputs = processor.apply_chat_template(messages, add_generation_prompt=True, tokenize=True, return_dict=True, return_tensor (டென்சர் — பல வடிவ எண் வரிசை)s="pt").to("cuda")
...
| Projector | "The bridge" | 2-4 layer MLP (or Q-former) that maps vision tokens into LLM embedding (embedding — பொருளை எண்களாக மாற்றுதல்) space |
| DeepStack | "Qwen3-VL feature (பண்பு — தகவல் நெடுவரிசை) trick" | Multi-level ViT features stacked quite than last-layer only |
