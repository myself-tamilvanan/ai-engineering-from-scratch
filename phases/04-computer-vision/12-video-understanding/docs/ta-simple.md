...
> A video is a sequence of images plus the physics that connects them. Every video model either treats time as an extra axis (3D conv), a sequence to attend over (transformer), or a feature (பண்பு — தகவல் நெடுவரிசை) to extract once and pool (2D+pool).
...
A 30-second video at 30 fps is 900 images. Naively, video classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) is image classification run 900 times followed by some kind of aggregation. That works when the action is visible in almost every frame (sports, cooking, exercise videos) and fails badly when the action is defined by motion itself: "pushing something from left to right" looks like two still objects in every single frame.
...
Take a 2D CNN (ResNet, EfficientNet, ViT). Run it independently on every sampled frame. Average (or max-pool, or attention-pool) the per-frame embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s. Feed the pooled vector (வெக்டர் — எண் பட்டியல்) to a classifier.
...
- **AVA** — action *localisation* in space and time. harder than classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்).

## Build It

### Step 1: Frame sampler

Uniform and dense samplers that work on a list of frames (or a video tensor (டென்சர் — பல வடிவ எண் வரிசை)).
...
Both return `T` indices that you use to slice the video tensor (டென்சர் — பல வடிவ எண் வரிசை).

### Step 2: A 2D+pool baseline

Run a 2D ResNet-18 over every frame, average-pool feature (பண்பு — தகவல் நெடுவரிசை)s, classify.
...
class FramePool(nn.Module):
    def __init__(self, num_classes=400, pretrained=True):
        ...
        self.feature (பண்பு — தகவல் நெடுவரிசை)s = nn.Sequential(*(list(backbone.children())[:-1]))  # global avg pool kept
    ...
    def forward(self, x):
        ...
        feats = self.feature (பண்பு — தகவல் நெடுவரிசை)s(x).view(N, T, -1)
...
Eleven million parameter (பாரமீட்டர் — கட்டுப்பாடு)s, ImageNet pretrained, runs per-frame, averages, classifies. This baseline is often within 5-10 points of proper 3D models on appearance-heavy tasks — sometimes better, because it reuses a stronger ImageNet backbone.
...
The division by `time_kernel` keeps the activation magnitudes roughly constant — important for not breaking batch (பத்தி — தொகுப்பு)-norm statistics on the first pass.

### Step 4: Factorised (2+1)D conv

Split a 3D conv into a 2D (spatial) and a 1D (temporal) conv. Same receptive field, fewer parameter (பாரமீட்டர் — கட்டுப்பாடு)s, better accuracy on some benchmarks.
...
class Conv2Plus1D(nn.Module):
    def __init__(self, in_c, out_c, kernel_size=3):
        ...
        self.bn = nn.batch (பத்தி — தொகுப்பு)Norm3d(mid_c)
...
|------|----------------|----------------------|
| 2D + pool | "Per-frame classifier" | Run a 2D CNN on every sampled frame, average-pool feature (பண்பு — தகவல் நெடுவரிசை)s across time, classify |
...
| (2+1)D | "Factorised conv" | Split 3D into 2D spatial + 1D temporal. fewer parameter (பாரமீட்டர் — கட்டுப்பாடு)s, extra non-linearity between |
