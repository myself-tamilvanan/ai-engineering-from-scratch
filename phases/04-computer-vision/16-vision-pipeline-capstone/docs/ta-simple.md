...
- Ship a minimal FastAPI service that accepts an image upload, runs the pipeline, and returns detections with classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)s
...
This capstone sets up the minimum viable pipeline: detection + classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) + structured output + a serving layer. Everything else in Phase 4 slots into this skeleton: swap Mask R-CNN for YOLOv8, add a OCR head, add a segmentation branch, add a tracker. The architecture is stable. the pieces are pluggable.
...
PipelineResult(
    ...
    classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)s: list[Classification],
    inference_ms: float,
)
```
...
- **Tiny crops** — skip classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) for boxes smaller than the classifier's minimum input.
...
### batch (பத்தி — தொகுப்பு)ing

A production service serves multiple clients. batch (பத்தி — தொகுப்பு)ing detections and classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)s across requests multiplies throughput. The trade-off: extra latency from waiting for a batch to fill. Typical setup: collect requests for up to 20ms, batch together, process, distribute responses. `torchserve` and `triton` do this natively. small services with predictable load roll their own micro-batcher.
...
class classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)(BaseModel):
    detection_index: int
    class_id: int
    class_name: str
    score: float = Field(ge=0, le=1)
...
class VisionPipeline:
    ...
    def preprocess(self, image):
        ...
        returns: CHW float tensor (டென்சர் — பல வடிவ எண் வரிசை) on device
        """
    ...
    @torch.no_grad()
    def detect(self, image_tensor (டென்சர் — பல வடிவ எண் வரிசை)):
    ...
    def classify(self, crops):
        if len(crops) == 0:
            return []
        batch (பத்தி — தொகுப்பு) = torch.stack(crops).to(self.device)
    ...
    def run(self, image, image_id="anonymous"):
        ...
        tensor (டென்சர் — பல வடிவ எண் வரிசை) = self.preprocess(image)
        ...
        for i, (box, score, cls) in enumerate(zip(det["boxes"], det["scores"], det["labels"])):
            ...
            x2 = min(x2, tensor (டென்சர் — பல வடிவ எண் வரிசை).shape[-1])
            ...
            crop = torch.nn.function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)al.interpolate(
                crop.unsqueeze(0),
                size=(224, 224),
                mode="bilinear",
                align_corners=False,
        ...
        classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)s = []
        ...
        return PipelineResult(
            ...
            classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)s=classifications,
...
def benchmark(pipe, num_runs=20, image_size=(400, 600)):
    ...
    for _ in range(num_runs):
        t0 = time.perf_counter()
        tensor (டென்சர் — பல வடிவ எண் வரிசை) = pipe.preprocess(img)
...
- **Fallback path** — if the classifier times out, return detections without classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)s quite than failing the whole request.
- **Safety filters** — NSFW / PII filters run after classification, before the response leaves the service.
- **batch (பத்தி — தொகுப்பு) endpoint** — a `/detect_batch` accepting a list of image URLs for bulk processing.

For production serving, `torchserve`, `Triton Inference Server`, and `BentoML` handle batch (பத்தி — தொகுப்பு)ing, versioning, metrics, and health checks out of the box. Running `FastAPI` directly is fine for prototypes and small-scale products.
...
3. **(Hard)** Add a micro-batch (பத்தி — தொகுப்பு)er in front of the classifier: collect crops for up to 10 ms, classify them all in one GPU call, return results per request. Measure the throughput gain at 5 concurrent requests per second and the latency added.
...
| Microbatch (பத்தி — தொகுப்பு)er | "Collect then forward" | Aggregator that waits a fixed window for multiple requests, runs a single batched forward pass |
| Trace ID | "Request id" | Per-request identifier logged at every stage so slow requests is traced end-to-end |
...
- [Full Stack deep learning (ஆழ் கற்றல் — பல்லாய கொட்ட பயிற்சி) — Deploying Models](https://fullstackdeeplearning.com/course/2022/lecture-5-deployment/) — the canonical overview of production ML deployment
- [BentoML docs](https://docs.bentoml.com) — serving framework with batch (பத்தி — தொகுப்பு)ing, versioning, and metrics
