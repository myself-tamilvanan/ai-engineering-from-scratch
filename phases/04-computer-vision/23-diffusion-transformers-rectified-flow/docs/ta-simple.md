...
- Distinguish model variants (SD3, FLUX.1-dev, FLUX.1-schnell, Z-Image, Qwen-Image) by architecture, parameter (பாரமீட்டர் — கட்டுப்பாடு) count, and licensing
...
- **FLUX** (Black Forest Labs, 2024) — first N blocks double-stream like SD3, later blocks concatenate and share weights (single-stream) for speed at higher depth.
- **Z-Image** (2025) — an efficient single-stream DiT at 6B parameter (பாரமீட்டர் — கட்டுப்பாடு)s that challenges "scale at all costs".
...
DiTs condition on timestep and class/text via **adaptive layer norm**: predict `scale` and `shift` from the conditioning vector (வெக்டர் — எண் பட்டியல்) and apply them after LayerNorm. Much cleaner than FiLM-style modulation in U-Nets and the default in every modern DiT.
...
- **SD3** uses three text encoders: two CLIP models + T5-XXL. embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s concatenated and fed to the image stream as text conditioning.
...
FLUX.1-schnell is the 2026 open-source default. Z-Image is the speed leader. FLUX.2 and SD4 are the current quality tips.
...
class DiTBlock(nn.Module):
    def __init__(self, dim=192, heads=3, mlp_ratio=4, cond_dim=192):
        ...
        self.attn = nn.MultiheadAttention(dim, heads, batch (பத்தி — தொகுப்பு)_first=True)
...
```python
def timestep_embedding (embedding — பொருளை எண்களாக மாற்றுதல்)(t, dim):
    import math
    half = dim // 2
...
class TinyDiT(nn.Module):
    def __init__(self, image_size=16, patch_size=2, in_channels=3, dim=96, depth=4, heads=3):
        ...
        self.pos = nn.parameter (பாரமீட்டர் — கட்டுப்பாடு)(torch.zeros(1, self.num_patches, dim))
    ...
    def forward(self, x, t):
        ...
        t_emb = self.time_mlp(timestep_embedding (embedding — பொருளை எண்களாக மாற்றுதல்)(t, self.pos.size(-1)))
...
2. **(Medium)** Add text conditioning by concatenating a learned class embedding (embedding — பொருளை எண்களாக மாற்றுதல்) to the time embedding (10 blob "classes" by colour). Sample with class 0, 5, and 9 and verify colours match.
...
| Single-stream / double-stream | "FLUX trick" | First N blocks double-stream (separate weights per modality), later blocks single-stream (concat + shared weights) for speed |
...
| Velocity target | "epsilon - x_0" | The regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) target in rectified flow. points from clean data to noise |
