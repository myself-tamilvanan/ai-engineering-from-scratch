...
Yann LeCun's digit recogniser. 60,000 parameter (பாரமீட்டர் — கட்டுப்பாடு)s. Two conv-pool blocks, two fully connected layers, tanh activations. It defined the template every CNN inherits:
...
1. **ReLU** instead of tanh. gradient (சாய்வு — மாற்றத்தின் दिशा)s stop vanishing. Training speeds up by a factor of six.
2. **Dropout** in the fully connected head. Regularisation becomes a layer, not a trick.
3. **Depth and width**. Five conv layers, three dense layers, 60M parameter (பாரமீட்டர் — கட்டுப்பாடு)s, trained on two GPUs with the model split across them.
...
Two 3x3 convs see the same 5x5 input area as one 5x5 conv but with fewer parameter (பாரமீட்டர் — கட்டுப்பாடு)s (2*9*C^2 = 18C^2 vs 25*C^2) and an extra ReLU in between. VGG turned this observation into an entire architecture. The simplicity — one block type, repeated — made it the reference point for everything that came after.

Cost: 138M parameter (பாரமீட்டர் — கட்டுப்பாடு)s, slow to train, expensive at inference.
...
```mermaid
flowchart LR
    IN["Input feature (பண்பு — தகவல் நெடுவரிசை) map"] --> A["1x1 conv"]
    IN --> B["3x3 conv"]
    IN --> C["5x5 conv"]
    IN --> D["3x3 max pool"]
...
Each branch specialises — 1x1 for channel mixing, 3x3 for local texture, 5x5 for larger patterns, pooling for shift-invariant feature (பண்பு — தகவல் நெடுவரிசை)s — and the concat lets the next layer pick whichever branch is useful. Inception v1 used 1x1 convolutions inside each branch as a bottleneck to keep parameter (பாரமீட்டர் — கட்டுப்பாடு) counts sane.

### The degradation problem

By 2015, VGG-19 worked and VGG-32 did not. Depth was supposed to help, but past ~20 layers both training and test loss got worse. That is not overfitting. That is the optimiser failing to find useful weights because gradient (சாய்வு — மாற்றத்தின் दिशा)s shrink multiplicatively through every layer.

```
Plain deep network:
  y = f_L( f_{L-1}( ... f_1(x) ... ) )

gradient (சாய்வு — மாற்றத்தின் दिशा) wrt early layer:
  dL/dW_1 = dL/dy * df_L/df_{L-1} * ... * df_2/df_1 * df_1/dW_1

Each multiplicative term has magnitude roughly (weight magnitude) * (activation gain).
Stack 100 of them with gains < 1 and the gradient (சாய்வு — மாற்றத்தின் दिशा) is effectively zero.
```

VGG worked at 19 layers because batch (பத்தி — தொகுப்பு) norm (published simultaneously) kept activations well-scaled. But even batch norm could not rescue depth beyond 30-ish layers.
...
The idea was not really about image classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்). It was about turning deep networks from "cross-your-fingers and hope gradient (சாய்வு — மாற்றத்தின் दिशा)s survive" into a reliable, scalable engineering tool. Every transformer you will read about next phase has the exact same skip connection in every block. Without ResNet, there is no GPT.
...
print(f"params: {sum(p.numel() for p in net.parameter (பாரமீட்டர் — கட்டுப்பாடு)s()):,}")
```
...
One reusable block: two 3x3 convs, ReLU, batch (பத்தி — தொகுப்பு) norm, max pool.
...
class VGGBlock(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        self.bn1 = nn.batch (பத்தி — தொகுப்பு)Norm2d(out_c)
...
Three VGG blocks on CIFAR-sized input, an adaptive pool, one linear layer. ~290k parameter (பாரமீட்டர் — கட்டுப்பாடு)s. Plenty for CIFAR-10.
...
`bias=False` on conv layers is a batch (பத்தி — தொகுப்பு)-norm convention — BN's beta parameter (பாரமீட்டர் — கட்டுப்பாடு) already handles the bias, so carrying conv bias as well is a waste. The `shortcut` only needs a real conv when stride or channel count changes. otherwise it is a no-op identity.
...
class TinyResNet(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.stem = nn.Sequential(
            ...
            nn.batch (பத்தி — தொகுப்பு)Norm2d(32),
...
Four groups of two blocks each. Stride 2 at the start of groups 2, 3, 4. Channel count doubles at every downsample. Roughly 2.8M parameter (பாரமீட்டர் — கட்டுப்பாடு)s. That is the standard recipe that scales cleanly up to ResNet-152.

### Step 5: Compare parameter (பாரமீட்டர் — கட்டுப்பாடு)-to-feature (பண்பு — தகவல் நெடுவரிசை) speed

Run the same input through all three networks and compare parameter (பாரமீட்டர் — கட்டுப்பாடு) counts.

```python
def summary(name, net, x):
    y = net(x)
    params = sum(p.numel() for p in net.parameter (பாரமீட்டர் — கட்டுப்பாடு)s())
...
Three models, three eras, three orders of magnitude in parameter (பாரமீட்டர் — கட்டுப்பாடு) count. For CIFAR-10 accuracy, you need roughly: LeNet 60%, MiniVGG 89%, TinyResNet 93% after a few epoch (யுக் — ஒரு முழு சுழற்சி)s of training.
...
print(f"ResNet-18 params: {sum(p.numel() for p in r18.parameter (பாரமீட்டர் — கட்டுப்பாடு)s()):,}")
print(r18.layer1[0])
print()

v16 = vgg16(weights=VGG16_Weights.IMAGENET1K_V1)
v16.eval()
print(f"VGG-16   params: {sum(p.numel() for p in v16.parameter (பாரமீட்டர் — கட்டுப்பாடு)s()):,}")
```

ResNet-18 has 11.7M parameter (பாரமீட்டர் — கட்டுப்பாடு)s. VGG-16 has 138M. Similar ImageNet top-1 accuracy (69.8% vs 71.6%). Residual connections buy you a 12x parameter speed win. That is why ResNet variants dominated from 2016 until ViT arrived in 2021 — and still dominate real-world deployments where compute is the constraint.

For transfer learning, the recipe is always the same: load pretrained, freeze the backbone, replace the classifier head.

```python
for p in r18.parameter (பாரமீட்டர் — கட்டுப்பாடு)s():
    p.requires_grad = False
r18.fc = nn.Linear(r18.fc.in_feature (பண்பு — தகவல் நெடுவரிசை)s, 10)
```
...
1. **(Easy)** Count parameter (பாரமீட்டர் — கட்டுப்பாடு)s by hand for `TinyResNet` layer by layer. Compare against `sum(p.numel() for p in net.parameters())`. Where does the majority of the parameter budget go — convs, BN, or the classifier head?
...
3. **(Hard)** Remove the skip connection from `BasicBlock`, train a 34-block "plain" network and a 34-block ResNet on CIFAR-10 for 10 epoch (யுக் — ஒரு முழு சுழற்சி)s each. Plot training loss vs epoch for both. Reproduce the He et al. Figure 1 result where the plain deep network converges to higher loss than its shallower twin.

## Key Terms

| Term | What people say | What it actually means |
|------|----------------|----------------------|
| Backbone | "The model" | The stack of convolutional blocks that produces the feature (பண்பு — தகவல் நெடுவரிசை) map fed to the task head |
...
- [ImageNet classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) with Deep CNNs (Krizhevsky et al., 2012)](https://papers.nips.cc/paper_files/paper/2012/hash/c399862d3b9d6b76c8436e924a68c45b-Abstract.html) — AlexNet. the paper that ended the hand-crafted-feature (பண்பு — தகவல் நெடுவரிசை) era
