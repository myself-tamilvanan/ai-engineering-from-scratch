...
- Distinguish top-down and bottom-up pose estimation and state when each helps
...
### Heatmap regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)
...
Why heatmaps work better than direct regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்): the network's spatial structure (conv feature (பண்பு — தகவல் நெடுவரிசை) map) aligns naturally with spatial output. Gaussian targets also regularise — a small localisation error produces a small loss, not zero.
...
OpenPose's trick for bottom-up association. For each pair of connected keypoints (e.g. left shoulder to left elbow), predict a 2-channel field that encodes the unit vector (வெக்டர் — எண் பட்டியல்) pointing from one to the other. To associate a shoulder with its elbow, integrate the PAF along the line connecting candidate pairs. the pair with the highest integral is matched.

```
For each connection (limb):
  PAF channels: 2 (unit vector (வெக்டர் — எண் பட்டியல்) x, y)
...
- **3D pose** — world / camera coordinates. still active research. Common approaches:
  ...
  - Direct 3D regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) from image (PyMAF, MHFormer).
...
Per-keypoint heatmaps stacked along a channel axis give the full target tensor (டென்சர் — பல வடிவ எண் வரிசை).
...
opt = torch.optim.Adam(model.parameter (பாரமீட்டர் — கட்டுப்பாடு)s(), lr=3e-3)

for step in range(200):
    batch (பத்தி — தொகுப்பு) = [make_synthetic_sample() for _ in range(16)]
...
|------|----------------|----------------------|
| Keypoint | "A landmark" | A specific ordered point on an object (joint, corner, feature (பண்பு — தகவல் நெடுவரிசை)) |
...
| Heatmap | "Gaussian target" | H x W tensor (டென்சர் — பல வடிவ எண் வரிசை) per keypoint with peak at the true location. the preferred regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) target |
| PAF | "Part Affinity Field" | 2-channel unit vector (வெக்டர் — எண் பட்டியல்) field encoding limb directions. used to group keypoints into instances |
