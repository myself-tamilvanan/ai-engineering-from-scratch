...
Positional encoding is 3D: a rotary or learned embedding (embedding — பொருளை எண்களாக மாற்றுதல்) per (t, h, w) coordinate. Attention is:
...
| Model | Use | parameter (பாரமீட்டர் — கட்டுப்பாடு)s | Output | License |
|-------|-----|------------|--------|---------|
...
Rotary Position embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s (RoPE) separately applied along `t`, `h`, `w` axes:
...
def rope_3d(tokens, t_dim, h_dim, w_dim, grid):
    ...
    freqs_t = torch.exp(-torch.log(torch.tensor (டென்சர் — பல வடிவ எண் வரிசை)(10000.0)) * torch.arange(t_dim // 2, device=tokens.device) / (t_dim // 2))
...
class DividedAttentionBlock(nn.Module):
    def __init__(self, dim=64, heads=2):
        super().__init__()
        self.time_attn = nn.MultiheadAttention(dim, heads, batch (பத்தி — தொகுப்பு)_first=True)
...
2. **(Medium)** Swap the divided attention block above for a full joint attention block and measure the shape and parameter (பாரமீட்டர் — கட்டுப்பாடு) count. Explain why divided attention is necessary for real video models.
3. **(Hard)** Build a minimal latent-action video model: take a dataset of (frame_t, action_t, frame_{t+1}) triples (any simple 2D game), train a tiny video DiT conditioned on action embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s, and show that different actions produce different next frames.
