...
- Distinguish tracking-by-detection from query-based tracking and name the algorithm (அல்கோரிதம் — வழிமுறை) families (SORT, DeepSORT, ByteTrack, BoT-SORT, SAM 2 memory tracker, SAM 3.1 Object Multiplex)
...
Tracking is essential to every video-facing product: sports analytics, surveillance, autonomous driving, medical video analysis, wildlife monitoring, wordmark counting. The core building blocks are shared: a per-frame detector, a motion model (Kalman filter or something richer), an association step (Hungarian algorithm (அல்கோரிதம் — வழிமுறை) on IoU / cosine / learned feature (பண்பு — தகவல் நெடுவரிசை)s), and a track lifecycle (birth, update, death).

2026 brought two new patterns: **SAM 2 memory-based tracking** (feature (பண்பு — தகவல் நெடுவரிசை)-memory instead of motion-model association) and **SAM 3.1 Object Multiplex** (shared memory for many instances of the same concept). This lesson walks the classical stack first, then the memory-based approach.
...
- **DeepSORT** (2017): SORT + a CNN-based appearance feature (பண்பு — தகவல் நெடுவரிசை) per track (ReID embedding (embedding — பொருளை எண்களாக மாற்றுதல்)). Handles crossings better.
...
### The Hungarian algorithm (அல்கோரிதம் — வழிமுறை)

Given a `M x N` cost matrix (மேட்ரிக்ஸ் — எண் மேஜை) (tracks x detections), find the one-to-one assignment that minimises total cost. Cost is usually `1 - IoU(track_bbox, detection_bbox)` or negative cosine similarity of appearance feature (பண்பு — தகவல் நெடுவரிசை)s. Runtime is O((M+N)^3). for M, N up to ~1000 it is fast enough in Python via `scipy.optimize.linear_sum_assignment`.
...
### Step 1: IoU-based cost matrix (மேட்ரிக்ஸ் — எண் மேஜை)
...
def bbox_iou(a, b):
    ...
    Returns (N_a, N_b) IoU matrix (மேட்ரிக்ஸ் — எண் மேஜை).
    """
...
60 lines. Takes per-frame detections, returns per-frame track IDs. Real systems add the Kalman predict, ByteTrack's second-stage re-match, and appearance feature (பண்பு — தகவல் நெடுவரிசை)s.
...
- Heavy occlusions with identifiable appearance: **DeepSORT / StrongSORT** (ReID feature (பண்பு — தகவல் நெடுவரிசை)s).
...
| Hungarian algorithm (அல்கோரிதம் — வழிமுறை) | "Optimal assignment" | Solves the minimum-cost bipartite matching problem. `scipy.optimize.linear_sum_assignment` |
| ByteTrack | "Low-confidence second pass" | Re-match unmatched tracks to low-confidence detections to recover short occlusions |
| DeepSORT | "SORT + appearance" | Adds a ReID feature (பண்பு — தகவல் நெடுவரிசை) for cross-frame matching. better for ID preservation |
...
- [DeepSORT (Wojke et al., 2017)](https://arxiv.org/abs/1703.07402) — adds appearance feature (பண்பு — தகவல் நெடுவரிசை)
