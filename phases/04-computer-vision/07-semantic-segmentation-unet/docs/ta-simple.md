...
> Segmentation is classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) at every pixel. U-Net makes it work by pairing a downsampling encoder with an upsampling decoder and wiring skip connections between them.
...
classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) outputs one label per image. Detection outputs a handful of boxes per image. Segmentation outputs one label per pixel. For an input of size `H x W`, the output is a tensor (டென்சர் — பல வடிவ எண் வரிசை) of shape `H x W` (semantic) or `H x W x N_instances` (instance). That is millions of predictions per image, not one.

The structure of segmentation is why it powers almost every dense-prediction vision product: medical imaging (tumour masks), autonomous driving (road, lane, obstacle), satellite (building footprints, crop boundaries), document parsing (layout zones), robotics (graspable regions). None of those tasks is solved by putting a box around the object. they need the exact silhouette.
...
The encoder halves spatial resolution four times and doubles channels. The decoder reverses: doubles spatial resolution four times and halves channels. The skip connections concatenate matching encoder feature (பண்பு — தகவல் நெடுவரிசை)s with decoder features at every resolution. The final 1x1 conv maps `64 -> num_classes` at full resolution.

Why skip connections are necessary: the decoder has seen only small feature (பண்பு — தகவல் நெடுவரிசை) maps by the time it tries to output pixel-level predictions. Without the skips it cannot localise edges accurately because that information was compressed away in the encoder. Skip connections hand it the high-resolution feature maps the encoder computed on the way down.

### Transposed vs bilinear upsample

The decoder has to expand spatial dimension (பரிமாணம் — கோணம்)s. Two options:
...
- **Bilinear upsample + 3x3 conv** — smooth upsample followed by a conv. Fewer artifacts, fewer parameter (பாரமீட்டர் — கட்டுப்பாடு)s, now the modern default.

Both appear in the wild. For a first U-Net, bilinear is safer.

### Cross-entropy on a pixel grid

For semantic segmentation with C classes, the model output is `(N, C, H, W)`. The target is `(N, H, W)` with integer class IDs. Cross-entropy is identical to the classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) case, just applied at every spatial position:
...
Cross-entropy gives stable gradient (சாய்வு — மாற்றத்தின் दिशा)s early in training. Dice focuses the tail of training on actually matching the mask shape. This combination is the medical-imaging default and hard to beat on any class-imbalanced dataset.

### Evaluation metrics

- **Pixel accuracy** — percent of pixels predicted correctly. Cheap. Broken on imbalanced data for the same reason as accuracy in classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்).
...
Two 3x3 convs with batch (பத்தி — தொகுப்பு) norm and ReLU. The first conv changes channel count. the second keeps it.
...
class DoubleConv(nn.Module):
    def __init__(self, in_c, out_c):
        ...
        self.net = nn.Sequential(
            ...
            nn.batch (பத்தி — தொகுப்பு)Norm2d(out_c),
...
The spatial-only shape check (`shape[-2:]`) handles inputs whose dimension (பரிமாணம் — கோணம்)s are not divisible by 16. a safe `F.interpolate` aligns the tensor (டென்சர் — பல வடிவ எண் வரிசை) before the concat. Comparing the full shape would also trigger on channel-count differences, which should be a loud error, not a silent interpolate.
...
Output shape `(1, 2, 256, 256)` — same spatial size as the input, `num_classes` channels. About 7.7M parameter (பாரமீட்டர் — கட்டுப்பாடு)s at `base=32`.
...
Dice is computed per class then averaged (macro Dice). The `eps` prevents division by zero on classes absent from the batch (பத்தி — தொகுப்பு).
...
def iou_per_class(logits, targets, num_classes):
    ...
    for c in range(num_classes):
        ...
        ious[c] = (inter / union) if union > 0 else torch.tensor (டென்சர் — பல வடிவ எண் வரிசை)(float("nan"))
    return ious
```

Returns a vector (வெக்டர் — எண் பட்டியல்) of length C. `nan` marks classes absent from the batch (பத்தி — தொகுப்பு) — do not average over those when computing mIoU.
...
```python
def train_one_epoch (யுக் — ஒரு முழு சுழற்சி)(model, loader, optimizer, device, num_classes):
    model.train()
...
Run this for 10-30 epoch (யுக் — ஒரு முழு சுழற்சி)s on the synthetic dataset and watch mIoU climb past 0.9 for the shape classes. Note the `nan_to_num(0)` treats classes absent from a batch (பத்தி — தொகுப்பு) as zero. for accurate per-class IoU, mask by presence and use `torch.nanmean` across batches at evaluation time quite than averaging here.
...
|------|----------------|----------------------|
| Semantic segmentation | "Label every pixel" | Per-pixel classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) into C classes. instances of the same class merge |
...
| Skip connection | "U-Net bridge" | Concatenation of encoder feature (பண்பு — தகவல் நெடுவரிசை)s into matching-resolution decoder features. preserves high-frequency detail |
