...
**Type:** Learn + Build
**Languages:** Python
**Prerequisites:** Phase 4 Lesson 03 (CNNs), Phase 1 Lesson 12 (tensor (டென்சர் — பல வடிவ எண் வரிசை) Operations)
**Time:** ~45 minutes

## Learning Objectives

- Distinguish explicit (point cloud, mesh, voxel) and implicit (signed distance field, NeRF) 3D representations and when each helps
- Understand PointNet's symmetric-function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) trick that makes a neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு) permutation-invariant over an unordered set of points
...
A camera produces a 2D image. A LIDAR produces a set of 3D points with no ordering. A structure-from-motion pipeline produces a sparse cloud of 3D keypoints. A NeRF reconstructs an entire 3D scene from a handful of posed images. All of these are "vision" but none of them look like the dense tensor (டென்சர் — பல வடிவ எண் வரிசை) a CNN wants.
...
The two representations dominate for different reasons. Point clouds are what sensors give you for free. NeRFs and their successors (3D Gaussian splatting, neural SDFs) are what you get when you ask a neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு) to learn a scene.

## The Concept

### Point clouds

A point cloud is an unordered set of N points in R^3, optionally each with feature (பண்பு — தகவல் நெடுவரிசை)s (colour, intensity, normal).
...
No grid, no connectivity. Two properties make this hard for neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)s:
...
PointNet (Qi et al., 2017) solved both with one idea: apply a shared MLP to every point, then aggregate with a symmetric function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) (max pool). The result is a fixed-size vector (வெக்டர் — எண் பட்டியல்) that does not depend on order.

```
f(P) = max_{p in P} MLP(p)
```

This is the entire core of PointNet. Deeper variants (PointNet++, Point Transformer) add hierarchical sampling and local aggregation but the symmetric-function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) trick is unchanged.
...
flowchart LR
    ...
    MAX --> FEAT["global feature (பண்பு — தகவல் நெடுவரிசை)<br/>(1024,)"]
...
"Shared MLP" means the same MLP runs on every point independently. Implemented as a 1x1 conv over the point dimension (பரிமாணம் — கோணம்) for speed.

### Neural Radiance Fields (NeRFs)

NeRFs (Mildenhall et al., 2020) took the question "can we reconstruct a 3D scene from N photos?" and answered with a neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு) that is the scene. The network maps `(x, y, z, viewing_direction)` to `(density, colour)`. Rendering a new view is a ray-casting loop over this network.
...
A vanilla MLP on `(x, y, z)` cannot represent high-frequency details because MLPs are spectrally biased toward low frequencies. NeRF fixes this by encoding each coordinate into a Fourier feature (பண்பு — தகவல் நெடுவரிசை) vector (வெக்டர் — எண் பட்டியல்) before the MLP:
...
- **ShapeNet** — classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) and segmentation of 3D CAD models as point clouds.
...
class PointNet(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.mlp1 = nn.Sequential(
            nn.Conv1d(3, 64, 1),    nn.batch (பத்தி — தொகுப்பு)Norm1d(64),   nn.ReLU(inplace=True),
...
About 1.6M parameter (பாரமீட்டர் — கட்டுப்பாடு)s. Runs on 1,024 points per cloud.
...
2. **(Medium)** Implement a minimal ray-generation function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) that, given camera intrinsics and pose, produces ray origins and directions for every pixel of an H x W image.
3. **(Hard)** Train a TinyNeRF on a synthetic dataset of rendered views of a coloured cube (generated via differentiable rendering or a simple ray tracer). Report rendering loss at epoch (யுக் — ஒரு முழு சுழற்சி) 1, 10, and 100. At what epoch does the model produce recognisable views?

## Key Terms

| Term | What people say | What it actually means |
|------|----------------|----------------------|
| Point cloud | "3D points from LIDAR" | Unordered set of (x, y, z) + optional feature (பண்பு — தகவல் நெடுவரிசை)s per point |
...
| SDF | "Signed distance field" | function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) returning signed distance to the nearest surface. another implicit representation |
