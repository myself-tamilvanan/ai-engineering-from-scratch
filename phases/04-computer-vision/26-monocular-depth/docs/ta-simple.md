...
- Explain why monocular depth works at all from a single image (perspective cues, texture gradient (சாய்வு — மாற்றத்தின் दिशा)s, learned priors) and what it cannot recover (absolute scale, occluded geometry)
...
flowchart LR
    ...
    ENC --> FEATS["Dense feature (பண்பு — தகவல் நெடுவரிசை)s<br/>(H/14, W/14, d)"]
...
Depth Anything V3 freezes the encoder and trains only the DPT-style decoder. The encoder provides rich feature (பண்பு — தகவல் நெடுவரிசை)s. the decoder interpolates them back to image resolution and regresses depth.
...
- **Texture gradient (சாய்வு — மாற்றத்தின் दिशा)** — surfaces far away have smaller, denser texture.
...
- DPT decoder.
- Trained on posed image pairs from many different sources (no explicit depth supervision needed beyond photometric consistency).
...
One function (சார்பு — உள்ளீட்டுக்கு வெளியீடு), every 3D-lifted application. Export the point cloud to `.ply` and open in MeshLab or CloudCompare.
...
def synthetic_depth(size=96):
    ...
    # Floor: linear gradient (சாய்வு — மாற்றத்தின் दिशा) from near (top) to far (bottom)
...
| DINOv2 backbone | "The reason it works" | Self-supervised feature (பண்பு — தகவல் நெடுவரிசை)s that generalise across domains without depth labels |
