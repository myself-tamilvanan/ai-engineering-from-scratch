...
flowchart LR
    ...
    TE --> CT["Text<br/>embedding (embedding — பொருளை எண்களாக மாற்றுதல்)"]
...
- **Text encoder** — CLIP text encoder (SD 1.x/2.x), CLIP-L + CLIP-G (SDXL), or T5-XXL (SD3/FLUX). Produces a sequence of token embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s.
- **U-Net** — the denoiser. Has cross-attention layers that attend from latents to the text embedding at every resolution level.
- **Scheduler** — the sampling algorithm (அல்கோரிதம் — வழிமுறை) (DDIM, Euler, DPM-Solver++). Picks sigmas, blends predicted noise back into the latent.
- **Safety checker** — optional NSFW / illegal-content filter on the output image.

### Classifier-free guidance (CFG)

Plain text conditioning learns `epsilon_theta(x_t, t, c)` for every prompt `c`. CFG trains the same network with `c` dropped 10% of the time (replaced by an empty embedding (embedding — பொருளை எண்களாக மாற்றுதல்)), giving a single model that predicts both the conditional and the unconditional noise. At inference:
...
- **Transformer blocks** at every spatial resolution, containing self-attention + cross-attention to the text embedding (embedding — பொருளை எண்களாக மாற்றுதல்).
- **Time embedding** via MLP on sinusoidal encoding.
- **Skip connections** between encoder and decoder at matching resolutions.

Total parameter (பாரமீட்டர் — கட்டுப்பாடு)s in SD 1.5: ~860M. SDXL: ~2.6B. FLUX: ~12B. The jump in params is mostly in attention layers.

### LoRA fine-tuning

Full fine-tuning of Stable Diffusion needs 20+ GB of VRAM and updates 860M parameter (பாரமீட்டர் — கட்டுப்பாடு)s. LoRA (Low-Rank Adaptation) keeps the base model frozen and injects small rank-decomposition matrices into the attention layers. A LoRA adapter for SD is usually 10-50 MB, trains in 10-60 minutes on a single consumer GPU, and loads at inference time as a drop-in modification.
...
This lesson uses `diffusers` end-to-end quite than rebuilding Stable Diffusion from scratch. The pieces you would need to rebuild (VAE, text encoder, U-Net, scheduler) are topics of their own lessons. here the goal is fluency with the production API.
...
```python
# Pseudocode
for step, batch (பத்தி — தொகுப்பு) in enumerate(dataloader):
    images, prompts = batch
    latents = vae.encode(images).latent_dist.sample() * 0.18215

    t = torch.randint(0, num_train_timesteps, (batch (பத்தி — தொகுப்பு)_size,))
...
Only the LoRA matrices receive gradient (சாய்வு — மாற்றத்தின் दिशा). the base U-Net, VAE, and text encoder are frozen. With a batch (பத்தி — தொகுப்பு) size of 1 and gradient checkpointing this fits in 8 GB of VRAM.
...
For batch (பத்தி — தொகுப்பு) generation, `AUTO1111` / `ComfyUI` are the community tools. for production APIs, `diffusers` + `accelerate` or `optimum-nvidia` with tensor (டென்சர் — பல வடிவ எண் வரிசை)RT compilation.
...
- `outputs/skill-lora-training-setup.md` — a skill that writes a full LoRA training config for a custom dataset including captions, rank, batch (பத்தி — தொகுப்பு) size, and learning rate.
...
| Scheduler | "Sampler" | The algorithm (அல்கோரிதம் — வழிமுறை) that turns noise + model predictions into a denoised latent trajectory |
