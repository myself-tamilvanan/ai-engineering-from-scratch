...
2. Open-vocabulary — the concept is anything describable in natural language.
3. Returns multiple instances at once quite than one mask per prompt.
...
- **Memory bank** — stores per-instance feature (பண்பு — தகவல் நெடுவரிசை)s across frames for video tracking (same mechanism SAM 2 used).
...
- **Decoupled mask injection** — decoder receives pre-computed mask feature (பண்பு — தகவல் நெடுவரிசை)s instead of re-running.
...
SAM 3 accepts one concept per forward pass. for multi-concept queries, loop or batch (பத்தி — தொகுப்பு) them.
...
- **Real-time annotation** — SAM 3 + CVAT's label-as-text-prompt feature (பண்பு — தகவல் நெடுவரிசை). Annotators select a label name. SAM 3 pre-labels every matching instance. Review and correct.
