...
> Somebody else spent a million GPU hours teaching a network what edges, textures, and object parts look like. You should borrow those feature (பண்பு — தகவல் நெடுவரிசை)s before training your own.

**Type:** Build
**Languages:** Python
**Prerequisites:** Phase 4 Lesson 03 (CNNs), Phase 4 Lesson 04 (Image classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்))
**Time:** ~75 minutes

## Learning Objectives

- Distinguish feature (பண்பு — தகவல் நெடுவரிசை) extraction from fine-tuning and pick the right one based on dataset size, domain distance, and compute budget
...
Getting transfer right has three bugs waiting for you: destroying pretrained feature (பண்பு — தகவல் நெடுவரிசை)s with a too-high learning rate, starving the model of information by freezing too much, and letting batch (பத்தி — தொகுப்பு)Norm's running statistics drift toward a tiny dataset that the rest of the network never learnt from. This lesson walks each of them on purpose.

## The Concept

### feature (பண்பு — தகவல் நெடுவரிசை) extraction vs fine-tuning

Two regimes, picked by how much you trust the pretrained feature (பண்பு — தகவல் நெடுவரிசை)s and how much data you have.

```mermaid
flowchart TB
    subgraph FE["feature (பண்பு — தகவல் நெடுவரிசை) extraction — backbone frozen"]
        FE1["Pretrained backbone<br/>(no gradient (சாய்வு — மாற்றத்தின் दिशा))"] --> FE2["New head<br/>(trained)"]
    end
...
"Close to ImageNet" roughly means natural RGB photos with object-like content. Medical CT scans, extra work satellite imagery, and microscopy are far domains — the feature (பண்பு — தகவல் நெடுவரிசை)s still help, but you will need to let more layers adapt.

### Why freezing works at all

The ImageNet feature (பண்பு — தகவல் நெடுவரிசை)s a CNN learns are not specialised to the 1,000 categories. They are specialised to the statistics of natural images: edges at specific orientations, textures, contrast patterns, shape primitives. Those statistics are stable across almost every visual domain a human can name. That is why a model trained on ImageNet and evaluated zero-shot on CIFAR-10 with just a new linear head (no fine-tuning of the backbone) reaches 80%+ accuracy. The head is learning which of the already-learnt features to weight for this task.

### Discriminative learning rates

When you do unfreeze, early layers should train slower than late layers. Early layers encode generic feature (பண்பு — தகவல் நெடுவரிசை)s that you want to preserve. late layers encode task-specific structure that you need to move a lot.
...
In PyTorch this is just a list of parameter (பாரமீட்டர் — கட்டுப்பாடு) groups passed to the optimizer. One model, five learning rates, zero extra code.

### The batch (பத்தி — தொகுப்பு)Norm problem
...
3. **Replace BN with GroupNorm.** Removes the moving-average problem entirely. Used in detection and segmentation backbones where batch (பத்தி — தொகுப்பு) size per GPU is tiny.
...
```
backbone.fc = nn.Linear(backbone.fc.in_feature (பண்பு — தகவல் நெடுவரிசை)s, num_classes)          # ResNet
...
print("feature (பண்பு — தகவல் நெடுவரிசை) dim:", backbone.fc.in_features)
```

`ResNet18` has four stages (`layer1..layer4`) plus a stem and a `fc` head. Every torchvision classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) backbone has an analogous structure.

### Step 2: feature (பண்பு — தகவல் நெடுவரிசை) extraction — freeze everything, replace the head

```python
def make_feature (பண்பு — தகவல் நெடுவரிசை)_extractor(num_classes=10):
...
model = make_feature (பண்பு — தகவல் நெடுவரிசை)_extractor(num_classes=10)
trainable = sum(p.numel() for p in model.parameter (பாரமீட்டர் — கட்டுப்பாடு)s() if p.requires_grad)
frozen = sum(p.numel() for p in model.parameters() if not p.requires_grad)
print(f"trainable: {trainable:>10,}")
print(f"frozen:    {frozen:>10,}")
```

Only `model.fc` is trainable. The backbone is a frozen feature (பண்பு — தகவல் நெடுவரிசை) extractor.

### Step 3: Discriminative fine-tuning

A utility that builds parameter (பாரமீட்டர் — கட்டுப்பாடு) groups with stage-specific learning rates.
...
def discriminative_param_groups(model, base_lr=1e-3, decay=0.3):
    ...
    for i, names in enumerate(stages):
        ...
        params = [p for n, p in model.named_parameter (பாரமீட்டர் — கட்டுப்பாடு)s()
...
model = resnet18(weights=ResNet18_Weights.IMAGENET1K_V1)
model.fc = nn.Linear(model.fc.in_feature (பண்பு — தகவல் நெடுவரிசை)s, 10)
for p in model.parameter (பாரமீட்டர் — கட்டுப்பாடு)s():
...
### Step 4: batch (பத்தி — தொகுப்பு)Norm handling
...
def freeze_bn_stats(model):
    for m in model.modules():
        if isinstance(m, (nn.batch (பத்தி — தொகுப்பு)Norm1d, nn.BatchNorm2d, nn.BatchNorm3d)):
            m.eval()
            for p in m.parameter (பாரமீட்டர் — கட்டுப்பாடு)s():
                p.requires_grad = False
    return model
```

Call it after you set `model.train()` at the start of every epoch (யுக் — ஒரு முழு சுழற்சி). `model.train()` flips everything to training mode. this reverses it only for BN layers.
...
def fine_tune(model, train_loader, val_loader, device, epoch (யுக் — ஒரு முழு சுழற்சி)s=5, base_lr=1e-3, freeze_bn=False):
    ...
    for epoch (யுக் — ஒரு முழு சுழற்சி) in range(epochs):
        ...
        print(f"epoch (யுக் — ஒரு முழு சுழற்சி) {epoch}  train {tr_loss/tr_total:.3f}/{tr_correct/tr_total:.3f}  "
              f"val {va_correct/va_total:.3f}")
    return model
```

Five epoch (யுக் — ஒரு முழு சுழற்சி)s with the above recipe on CIFAR-10 takes `ResNet18-IMAGENET1K_V1` from ~70% zero-shot linear-probe accuracy to ~93% fine-tuned accuracy. The head alone would plateau around 86% without ever touching the backbone.

### Step 6: Progressive unfreezing

A schedule that unfreezes one stage per epoch (யுக் — ஒரு முழு சுழற்சி) from the end toward the beginning. Mitigates feature (பண்பு — தகவல் நெடுவரிசை) drift at the cost of some extra epochs.
...
def progressive_unfreeze_schedule(model):
    ...
    def unfreeze(epoch (யுக் — ஒரு முழு சுழற்சி)):
        if epoch < len(stages):
            name = stages[epoch]
            yielded.add(name)
            for n, p in model.named_parameter (பாரமீட்டர் — கட்டுப்பாடு)s():
...
Call `start()` once before the first epoch (யுக் — ஒரு முழு சுழற்சி). Call `unfreeze(epoch)` at the start of each epoch. Rebuild the optimizer whenever the set of trainable parameter (பாரமீட்டர் — கட்டுப்பாடு)s changes, otherwise the frozen params still hold cached moments that confuse it.
...
model.fc = nn.Linear(model.fc.in_feature (பண்பு — தகவல் நெடுவரிசை)s, num_classes)
optimizer = torch.optim.AdamW(model.parameter (பாரமீட்டர் — கட்டுப்பாடு)s(), lr=1e-4, weight_decay=1e-4)
```
...
- For transformers, `transformers.AutoModelForImageclassification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்).from_pretrained(name, num_labels=N)` gives you ViT / BEiT / DeiT with the same loading semantics as text models.

## Ship It

This lesson produces:

- `outputs/prompt-fine-tune-planner.md` — a prompt that picks feature (பண்பு — தகவல் நெடுவரிசை)-extraction vs progressive vs end-to-end fine-tuning based on dataset size, domain distance, and compute budget.
- `outputs/skill-freeze-inspector.md` — a skill that, given a PyTorch model, reports which parameter (பாரமீட்டர் — கட்டுப்பாடு)s are trainable, which batch (பத்தி — தொகுப்பு)Norm layers are in eval mode, and whether the optimizer is actually being fed the trainable parameters.

## Exercises

1. **(Easy)** Train a `ResNet18` as a linear probe (backbone frozen) and as a full fine-tune on the same synthetic-CIFAR dataset. Report both accuracies side by side. Explain which gap tells you the feature (பண்பு — தகவல் நெடுவரிசை)s transfer well and which tells you they do not.
...
|------|----------------|----------------------|
| feature (பண்பு — தகவல் நெடுவரிசை) extraction | "Freeze and train head" | Backbone parameter (பாரமீட்டர் — கட்டுப்பாடு)s frozen, only the new classifier head receives gradient (சாய்வு — மாற்றத்தின் दिशा) |
...
| BN statistics drift | "Running mean is wrong" | batch (பத்தி — தொகுப்பு)Norm running_mean/var computed on a different distribution than the current task, silently hurting accuracy |
...
- [How transferable are feature (பண்பு — தகவல் நெடுவரிசை)s in deep neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)s? (Yosinski et al., 2014)](https://arxiv.org/abs/1411.1792) — the paper that quantified feature transferability across layers
