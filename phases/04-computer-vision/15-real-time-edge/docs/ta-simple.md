...
**Type:** Learn + Build
**Languages:** Python
**Prerequisites:** Phase 4 Lesson 04 (Image classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)), Phase 10 Lesson 11 (Quantization)
**Time:** ~75 minutes
...
- Export to ONNX and compile with ONNX Runtime or tensor (டென்சர் — பல வடிவ எண் வரிசை)RT. name the three most common export failures and their fixes
- Explain when to pick MobileNetV3, EfficientNet-Lite, ConvNeXt-Tiny, or MobileViT for an edge constraint

## The Problem

A training-time vision model is a floating-point monster. 100M parameter (பாரமீட்டர் — கட்டுப்பாடு)s, 10 GFLOPs per forward pass, 2 GB of VRAM. None of that fits on a phone, a car's infotainment unit, an industrial camera, or a drone. Shipping a vision system means fitting the same predictions into a budget that is 100x smaller.

Three knobs do most of the work: model choice (a smaller architecture with the same recipe), quantisation (INT8 instead of FP32), and the inference runtime (ONNX Runtime, tensor (டென்சர் — பல வடிவ எண் வரிசை)RT, Core ML, TFLite). Getting them right is the difference between a demo that runs on a workstation and a product that ships on a $30 camera module.
...
FLOPs (floating-point operations per inference) is a cheap, device-independent proxy for latency. Useful for architecture comparison, misleading as absolute wall-clock. A model with 10% more FLOPs is 2x faster in practice because it uses hardware-friendly ops (depthwise convs compile well, large 7x7 convs do not).

Rule: use FLOPs for architecture search, use on-device latency for deployment decisions.

### Quantisation in one paragraph

Replace FP32 weights and activations with INT8. Model size drops 4x, memory bandwidth drops 4x, compute drops 2-4x on hardware that has INT8 kernels (every modern mobile SoC, every NVIDIA GPU with tensor (டென்சர் — பல வடிவ எண் வரிசை) Cores). Accuracy loss on vision tasks is usually 0.1-1 percentage points with post-training static quantisation.
...
- **Pruning** — remove unimportant weights (magnitude-based) or channels (structured). Works well on overparameter (பாரமீட்டர் — கட்டுப்பாடு)ised models. less useful on already-compact architectures.
...
- **ONNX Runtime** — the neutral runtime. CPU, CUDA, CoreML, tensor (டென்சர் — பல வடிவ எண் வரிசை)RT, OpenVINO all have ONNX providers. Start here.
...
### Step 2: parameter (பாரமீட்டர் — கட்டுப்பாடு) and FLOP counts

```python
def parameter (பாரமீட்டர் — கட்டுப்பாடு)_count(model):
...
def flops_estimate(model, input_shape):
    ...
    def linear_hook(m, inp, out):
        nonlocal total
        total += 2 * m.in_feature (பண்பு — தகவல் நெடுவரிசை)s * m.out_features
    hooks = []
...
def export_onnx(model, sample_input, path="model.onnx"):
    ...
    torch.onnx.export(
        ...
        input_names=["input"],
        output_names=["output"],
        dynamic_axes={"input": {0: "batch (பத்தி — தொகுப்பு)"}, "output": {0: "batch"}},
        opset_version=17,
    )
    return path
```

`opset_version=17` is the safe default in 2026. `dynamic_axes` lets you run the ONNX model with arbitrary batch (பத்தி — தொகுப்பு) size.
...
def compare_regimes():
    ...
    params = parameter (பாரமீட்டர் — கட்டுப்பாடு)_count(model)
...
Run the same function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) for `resnet50`, `efficientnet_v2_s`, and `convnext_tiny` and you have the comparison table you need for a deployment decision.
...
- **NVIDIA edge (Jetson, GPU server)**: PyTorch -> ONNX -> tensor (டென்சர் — பல வடிவ எண் வரிசை)RT. Best latency, biggest engineering effort.
- **Mobile**: PyTorch -> ONNX -> Core ML (iOS) or TFLite (Android). Quantise before export.

For measurement, `torch-tb-profiler`, `nvprof` / `nsys`, and Instruments on macOS give layer-by-layer breakdowns. `benchmark_app` (OpenVINO) and `trtexec` (tensor (டென்சர் — பல வடிவ எண் வரிசை)RT) give standalone CLI numbers.
...
| tensor (டென்சர் — பல வடிவ எண் வரிசை)RT | "NVIDIA compiler" | Compiles ONNX into an optimised engine for NVIDIA GPUs |
...
- [A Practical Guide to tensor (டென்சர் — பல வடிவ எண் வரிசை)RT optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) (NVIDIA)](https://developer.nvidia.com/blog/accelerating-model-inference-with-tensorrt-tips-and-best-practices-for-pytorch-users/) — how to actually get the throughput numbers in the paper
