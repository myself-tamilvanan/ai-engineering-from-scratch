...
**Type:** Build
**Languages:** Python
**Prerequisites:** Phase 4 Lesson 13 (3D Vision & NeRF), Phase 1 Lesson 12 (tensor (டென்சர் — பல வடிவ எண் வரிசை) Operations), Phase 4 Lesson 10 (Diffusion basics optional)
**Time:** ~90 minutes

## Learning Objectives

- Explain why 3D Gaussian Splatting replaced NeRF as the production default for photorealistic 3D reconstruction in 2026
- State the six per-Gaussian parameter (பாரமீட்டர் — கட்டுப்பாடு)s (position, rotation quaternion, scale, opacity, spherical harmonics colour, optional feature (பண்பு — தகவல் நெடுவரிசை)) and how many floats each contributes
...
A scene usually has 1-5 million Gaussians. Each stores roughly 60 floats (3 + 4 + 3 + 1 + 48 + misc). That is 240 MB for a five-million-Gaussian scene — far smaller than the equivalent point cloud with per-point texture, and an order of magnitude smaller than a NeRF's MLP weights re-rendered at high resolution.
...
The 2D Gaussian's footprint is an ellipse whose axes are the eigenvector (வெக்டர் — எண் பட்டியல்)s of `Sigma'`. Every pixel inside that ellipse receives the Gaussian's contribution, weighted by `exp(-0.5 * (p - mu')^T Sigma'^-1 (p - mu'))`.
...
Every step — projection, tile assignment, alpha compositing, SH evaluation — is differentiable with respect to the Gaussian parameter (பாரமீட்டர் — கட்டுப்பாடு)s. Given a ground-truth image, compute rendered pixel loss, backprop through the rasteriser, update all `(mu, q, s, alpha, c_lm)` by gradient (சாய்வு — மாற்றத்தின் दिशा) descent. Over ~30,000 iteration (மறுயுதல் — மீண்டும் செய்தல்)s the Gaussians find their right positions, scales, and colours.

### Densification and pruning

A fixed set of Gaussians cannot cover a complex scene. Training includes two adaptive mechanisms:

- **Clone** a Gaussian at its current position when its gradient (சாய்வு — மாற்றத்தின் दिशा) magnitude is high but its scale is small — the reconstruction needs more detail here.
...
Densification runs every N iteration (மறுயுதல் — மீண்டும் செய்தல்)s. A scene usually grows from ~100k initial Gaussians (seeded from SfM points) to 1-5M at the end of training.

### Spherical harmonics in one paragraph

View-dependent colour is a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) `c(direction)` on the unit sphere. Spherical harmonics are the sphere's Fourier basis. Truncate at degree `L` and you get `(L+1)^2` basis functions per channel. Evaluating the colour for a new view is a dot product between the learned SH coefficients and the basis evaluated at the viewing direction. Degree 0 = one coefficient = constant colour. Degree 3 = 16 coefficients = enough to capture Lambertian shading, specular, and mild reflection. SD Gaussian Splatting papers use degree 3 by default.
...
- **4D Gaussian Splatting** — Gaussians are function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s of time. used for volumetric video (Superman 2026, A$AP Rocky's "Helicopter").
...
Alpha-compositing front-to-back. Depth in 2D is meaningless, so we use a learned per-Gaussian scalar (scalar — ஒரு எண்) for order.
...
def rasterise_2d(means, covs, colours, opacities, depths, image_size):
    ...
    colours:   (G, 3)
    opacities: (G,)     in [0, 1]
    depths:    (G,)     per-Gaussian scalar (scalar — ஒரு எண்) used for ordering
    image_size: (H, W)
...
Not fast — a real how to build uses tile-based CUDA kernels — but exactly the right math and fully differentiable.
...
class Splats2D(nn.Module):
    def __init__(self, num_splats=128, image_size=64, seed=0):
        ...
        self.means = nn.parameter (பாரமீட்டர் — கட்டுப்பாடு)(torch.rand(num_splats, 2, generator=g) * torch.tensor (டென்சர் — பல வடிவ எண் வரிசை)([W, H]))
...
`log_scale`, `opacity_logit`, and `colour_logits` are all unconstrained parameter (பாரமீட்டர் — கட்டுப்பாடு)s mapped through the right activation at render time. This is the standard pattern for every 3DGS how to build.
...
opt = torch.optim.Adam(model.parameter (பாரமீட்டர் — கட்டுப்பாடு)s(), lr=0.05)
...
Over 200 steps the 64 Gaussians settle into the two shapes. That is the entire idea — gradient (சாய்வு — மாற்றத்தின் दिशा)-descent on explicit geometric primitives.
...
5. Depth-sort is from actual camera-space z instead of a learned scalar (scalar — ஒரு எண்).

Every production how to build (`gsplat`, `inria/gaussian-splatting`, `nerfstudio`) does exactly this on the GPU with tile-based CUDA kernels.
...
def eval_sh_degree_3(sh_coeffs, dirs):
    ...
    dirs:      (..., 3)       unit vector (வெக்டர் — எண் பட்டியல்)s
    returns:   (..., 3)
    """
...
Learned `sh_coeffs` store the "colour in every direction" for that Gaussian. At render time you evaluate against the current view direction and get a 3-vector (வெக்டர் — எண் பட்டியல்) RGB.
...
2. **(Medium)** Extend the 2D rasteriser to support per-Gaussian RGB colours that depend on a scalar (scalar — ஒரு எண்) "view angle" through a degree-2 harmonic. Train on a pair of target images and verify the model reconstructs both.
...
| Spherical harmonics | "View-dependent colour" | Fourier basis on the sphere. stores colour as a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) of viewing direction |
