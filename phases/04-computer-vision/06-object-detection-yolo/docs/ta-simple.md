...
> Detection is classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) plus regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்), run at every position in a feature (பண்பு — தகவல் நெடுவரிசை) map, then cleaned up with non-maximum suppression.

**Type:** Build
**Languages:** Python
**Prerequisites:** Phase 4 Lesson 03 (CNNs), Phase 4 Lesson 04 (Image classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)), Phase 4 Lesson 05 (Transfer Learning)
**Time:** ~75 minutes

## Learning Objectives

- Explain the grid-and-anchor design that turns detection into a dense prediction problem and state what every number in the output tensor (டென்சர் — பல வடிவ எண் வரிசை) means
- Compute Intersection-over-Union between boxes and implement non-maximum suppression from scratch
- Build a minimal YOLO-style head on top of a pretrained backbone, including the classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்), objectness, and box-regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) losses
- Read a detection metric row (precision@0.5, recall, mAP@0.5, mAP@0.5:0.95) and pick which knob to turn next

## The Problem

classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) says "this image is a dog." Detection says "there is a dog at pixels (112, 40, 280, 210), there is a cat at (400, 180, 560, 310), and nothing else in the frame." That one structural change — predicting a variable number of labelled boxes instead of one label per image — is what every autonomous system, every surveillance product, every document layout parser, and every factory vision line depends on.

Detection is also where every engineering trade-off in vision shows up at once. You want boxes that are accurate (regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) head), you want the right class for each box (classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) head), you want the model to know when there is nothing to detect (objectness score), and you want exactly one prediction per real object (non-maximum suppression). Miss any of these and the pipeline either misses objects, reports hallucinated boxes, or predicts the same object fifteen times in slightly different positions.
...
flowchart LR
    ...
    BB --> FM["feature (பண்பு — தகவல் நெடுவரிசை) map<br/>(C_feat, 13, 13)"]
    FM --> HEAD["Detection head<br/>(1x1 convs)"]
    HEAD --> OUT["Output tensor (டென்சர் — பல வடிவ எண் வரிசை)<br/>(13, 13, B * (5 + C))"]
...
Plain regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) would predict `(x, y, w, h)` for every object as an absolute coordinate. That is hard for a conv network because translating the image should not translate all predictions by the same amount — each object is spatially anchored. The grid answers this by assigning each ground-truth box to the grid cell its centre falls in. only that cell is responsible for that object.

Anchors address a second problem. A 3x3 conv cannot easily regress a 500-pixel-wide box out of a 16-pixel receptive field feature (பண்பு — தகவல் நெடுவரிசை) cell. Instead, we pre-define `B` prior box shapes (anchors) per cell and predict small deltas from each anchor. The model learns to pick the right anchor and nudge it quite than regress from nothing.
...
The raw `tx, ty, tw, th` are not box coordinates. they are regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) targets to be transformed before plotting:
...
IoU = 1 means identical. IoU = 0 means no overlap. IoU between the prediction and the ground-truth box is what decides whether a prediction counts as a true positive (usually IoU >= 0.5). IoU between two predictions is what NMS uses to deduplicate.
...
Report all four. A detector that is strong on mAP@0.5 but weak on mAP@0.5:0.95 is localising roughly but not tightly. fix with better box-regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) loss. A detector with high precision and low recall is too conservative. lower the confidence threshold or increase the objectness weight.
...
Returns an `(N_a, N_b)` matrix (மேட்ரிக்ஸ் — எண் மேஜை) of pairwise IoUs. Use it against a single ground-truth box by making one of the arrays shape `(1, 4)`.
...
One 1x1 conv on a feature (பண்பு — தகவல் நெடுவரிசை) map, reshaping to `(B, S, S, num_anchors, 5 + C)`.
...
Output shape: `(N, H, W, num_anchors, 5 + C)`. The last dimension (பரிமாணம் — கோணம்) holds `[tx, ty, tw, th, obj, cls_0, ..., cls_{C-1}]`.
...
def yolo_loss(pred, target, has_obj, lambda_coord=5.0, lambda_obj=1.0, lambda_noobj=0.5, lambda_cls=1.0):
    ...
    # box-regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) loss: only on cells with objects
    box_pred = pred[..., :4][has_obj_t]
    box_true = target_t[..., :4][has_obj_t]
    loss_box = torch.nn.function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)al.mse_loss(box_pred, box_true, reduction="sum")

    # objectness loss
    obj_pred = pred[..., 4]
    obj_true = target_t[..., 4]
    loss_obj_pos = torch.nn.function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)al.binary_cross_entropy_with_logits(
        obj_pred[has_obj_t], obj_true[has_obj_t], reduction="sum")
    loss_obj_neg = torch.nn.functional.binary_cross_entropy_with_logits(
        obj_pred[~has_obj_t], obj_true[~has_obj_t], reduction="sum")

    # classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) loss on cells with objects
    cls_pred = pred[..., 5:][has_obj_t]
    cls_true = target_t[..., 5:][has_obj_t]
    loss_cls = torch.nn.function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)al.binary_cross_entropy_with_logits(
...
Five hyper-parameter (பாரமீட்டர் — கட்டுப்பாடு)s that every YOLO tutorial either hardcodes or sweeps. The ratios matter: `lambda_coord=5, lambda_noobj=0.5` mirrors the original YOLOv1 paper and still works as a reasonable default.
...
```python
def postprocess(pred_tensor (டென்சர் — பல வடிவ எண் வரிசை), anchors, stride, img_size, conf_threshold=0.25, iou_threshold=0.45):
...
2. **(Medium)** Port `yolo_loss` to a version that uses `CIoU` box loss instead of MSE. Show on a 100-image synthetic dataset that CIoU converges to a better final mAP@0.5:0.95 than MSE in the same number of epoch (யுக் — ஒரு முழு சுழற்சி)s.
...
| NMS | "Deduplicate" | Greedy algorithm (அல்கோரிதம் — வழிமுறை) that keeps highest-score predictions and removes overlapping ones above a threshold |
| Objectness | "Is there something here" | Per-anchor, per-cell scalar (scalar — ஒரு எண்) predicting whether an object is centred in that cell |
