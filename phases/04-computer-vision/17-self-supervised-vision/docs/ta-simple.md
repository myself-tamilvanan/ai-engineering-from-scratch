...
> Labels are the bottleneck of supervised vision. Self-supervised pretraining removes them: learn visual feature (பண்பு — தகவல் நெடுவரிசை)s from 100M unlabelled images, fine-tune on 10k labelled ones.

**Type:** Learn + Build
**Languages:** Python
**Prerequisites:** Phase 4 Lesson 04 (Image classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)), Phase 4 Lesson 14 (ViT)
**Time:** ~75 minutes
...
- Implement an InfoNCE loss from scratch and explain why a batch (பத்தி — தொகுப்பு) of 512 works but a batch of 32 fails
...
Take one image, apply two random augmentations, get two views. Feed both through the same encoder plus a projection head. Minimise a loss that says "these two embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s should be close" and "this embedding should be far from every other image's embeddings in the batch (பத்தி — தொகுப்பு)."

```
Loss for positive pair (z_i, z_j) among 2N views per batch (பத்தி — தொகுப்பு):

   L_ij = -log( exp(sim(z_i, z_j) / tau) / sum_k in batch (பத்தி — தொகுப்பு) \ {i} exp(sim(z_i, z_k) / tau) )

sim = cosine similarity
tau = temperature (0.1 standard)
```

This is the InfoNCE loss. It requires many negatives per positive, so batch (பத்தி — தொகுப்பு) size matters — SimCLR needs 512-8192. MoCo introduced a momentum queue of past batches to decouple negative count from batch size.
...
Why it does not collapse to "predict a constant": the teacher's output is centred (subtract per-dimension (பரிமாணம் — கோணம்) mean) and sharpened (divide by small temperature). Centering prevents one dimension from dominating. sharpening prevents output collapse to uniform.

DINO is what DINOv2 scales up, on 142M curated images. The resulting feature (பண்பு — தகவல் நெடுவரிசை)s are the current SOTA for zero-shot visual retrieval and dense prediction.
...
```
Encoder:  visible 25% of patches -> feature (பண்பு — தகவல் நெடுவரிசை)s
...
- **75% mask ratio** — high. Forces the encoder to learn semantic feature (பண்பு — தகவல் நெடுவரிசை)s. reconstructing 25% would be near-trivial (neighbouring pixels are so correlated that a CNN could nail it).
...
After pretraining, discard the decoder. The encoder is the feature (பண்பு — தகவல் நெடுவரிசை) extractor.
...
Linear probe is a pure measure of feature (பண்பு — தகவல் நெடுவரிசை) quality. fine-tuning usually adds 2-5 points but also mixes in the effect of head retraining.
...
two_view_train = lambda: T.Compose([
    ...
    T.Totensor (டென்சர் — பல வடிவ எண் வரிசை)(),
])
...
def info_nce(z1, z2, tau=0.1):
    """
    z1, z2: (N, D) L2-normalised embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s of paired views
    """
    N, D = z1.shape
...
L2-normalise embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s before calling. `tau=0.1` is the SimCLR default. lower makes the loss sharper and requires more negatives.
...
Identical pairs should give a low loss (close to 0 for a large batch (பத்தி — தொகுப்பு) and cold temperature). Random pairs should give log(2N-1) = ~log(31) = ~3.4 with a 16-pair batch.
...
Simple, fast, and deterministic for a given seed. Real MAE implementations batch (பத்தி — தொகுப்பு) this and keep per-sample masks.
...
# Per-image embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s for zero-shot retrieval
with torch.no_grad():
    inputs = processor(images=[pil_image], return_tensor (டென்சர் — பல வடிவ எண் வரிசை)s="pt")
    outputs = model(**inputs)
    embedding = outputs.last_hidden_state[:, 0]  # CLS token
```

The resulting 768-dim embedding (embedding — பொருளை எண்களாக மாற்றுதல்) is the backbone of modern image retrieval, dense correspondence, and zero-shot transfer pipelines. Fine-tuning on a downstream task rarely needs more than a linear head.

For image-text embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s, SigLIP or OpenCLIP is the equivalent. for MAE-style fine-tuning, the `timm` repo ships every MAE checkpoint.
...
1. **(Easy)** Verify that InfoNCE loss drops when you decrease temperature for well-aligned embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s and rises when you decrease temperature for random embeddings. Produce a plot `tau in [0.05, 0.1, 0.2, 0.5]` vs loss.
2. **(Medium)** Implement a DINO-style centre buffer. Show that without the centring, the student collapses to a constant vector (வெக்டர் — எண் பட்டியல்) within a few epoch (யுக் — ஒரு முழு சுழற்சி)s.
...
| Linear probe | "Frozen encoder + linear head" | Standard SSL evaluation: train only a linear classifier on top of frozen feature (பண்பு — தகவல் நெடுவரிசை)s |
...
| Representation collapse | "Constant output" | SSL failure where the encoder outputs a constant vector (வெக்டர் — எண் பட்டியல்) for all inputs. prevented by centring, sharpening, or negatives |
...
- [DINOv2 (Oquab et al., 2023)](https://arxiv.org/abs/2304.07193) — scaling self-supervised ViT to production feature (பண்பு — தகவல் நெடுவரிசை)s
