...
> A GAN is two neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)s in a fixed game. One draws, one critiques. They get better together until the drawings fool the critic.
...
- Read training curves that distinguish healthy convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) from mode collapse, oscillation, and discriminator-wins-completely

## The Problem

classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) teaches a network to map images to labels. Generation inverts the problem: sample new images that look like they came from the same distribution. There is no "correct" output you can diff against. there is only a distribution you want to mimic.

The standard loss function (சேதத் சூத்திரம் — எவ்வளவு தவறு என்பதை அளக்கும் சூத்திரம்) (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s (MSE, cross-entropy) cannot measure "did this sample come from the real distribution." Minimising per-pixel error produces blurry averages, not realistic samples. The breakthrough was to learn the loss: train a second network whose job is to tell real from fake, and use its judgement to push the generator.

GANs (Goodfellow et al., 2014) defined that framework. By 2018 StyleGAN was producing 1024x1024 faces indistinguishable from photographs. Diffusion models have since taken the throne on quality and controllability, but every trick that makes diffusion practical — normalisation choices, latent spaces, feature (பண்பு — தகவல் நெடுவரிசை) losses — was first understood on GANs.
...
The **generator** G takes a vector (வெக்டர் — எண் பட்டியல்) of noise `z` and outputs an image. The **discriminator** D takes an image and outputs a single scalar (scalar — ஒரு எண்): the probability that the image is real.
...
The form above is numerically unstable. Early in training, `D(G(z))` is near zero for every fake, so `log(1 - D(G(z)))` has vanishing gradient (சாய்வு — மாற்றத்தின் दिशा)s with respect to G. The fix: flip G's loss.
...
Now when `D(G(z))` is near zero, G's loss is large and its gradient (சாய்வு — மாற்றத்தின் दिशा) is informative. Every modern GAN trains with this variant.
...
2. Use batch (பத்தி — தொகுப்பு) norm in both generator and discriminator, except output of G and input of D.
...
flowchart LR
    ...
    M2["Vanishing gradient (சாய்வு — மாற்றத்தின் दिशा)s<br/>D wins completely"] --> S2["D accuracy ~100%,<br/>G loss huge and static"]
...
- **Mode collapse**: G finds one image that fools D and produces only that. Fix: add minibatch (பத்தி — தொகுப்பு) discrimination, spectral norm, or label-conditioning.
- **Discriminator wins**: D becomes too strong too fast, G's gradient (சாய்வு — மாற்றத்தின் दिशा)s vanish. Fix: smaller D, lower D learning rate, or apply label smoothing on the real labels.
...
- **Sample inspection** — just look at 64 samples at the end of every epoch (யுக் — ஒரு முழு சுழற்சி). Non-negotiable.
- **FID (Fréchet Inception Distance)** — distance between Inception-v3 feature (பண்பு — தகவல் நெடுவரிசை) distributions of real and generated sets. Lower is better. Community standard.
...
class Generator(nn.Module):
    def __init__(self, z_dim=64, img_channels=3, feat=64):
        ...
        self.net = nn.Sequential(
            ...
            nn.batch (பத்தி — தொகுப்பு)Norm2d(feat * 4),
...
Mirror of the generator. LeakyReLU, strided convs, ends with a scalar (scalar — ஒரு எண்) logit.
...
class Discriminator(nn.Module):
    def __init__(self, img_channels=3, feat=64):
        ...
        self.net = nn.Sequential(
            ...
            nn.batch (பத்தி — தொகுப்பு)Norm2d(feat * 2),
            nn.LeakyReLU(0.2, inplace=True),
...
The last conv reduces a `4x4` feature (பண்பு — தகவல் நெடுவரிசை) map to `1x1`. Output is a single scalar (scalar — ஒரு எண்) per image. apply sigmoid only during loss calculation.

### Step 3: Training step

Alternate: update D once, then G once, every batch (பத்தி — தொகுப்பு).
...
`G(z).detach()` in the D step is critical: we do not want gradient (சாய்வு — மாற்றத்தின் दिशा)s flowing into G during its update. Forgetting that is the classic beginner bug.
...
loader = DataLoader(tensor (டென்சர் — பல வடிவ எண் வரிசை)Dataset(data), batch (பத்தி — தொகுப்பு)_size=64, shuffle=True)

G = Generator(z_dim=64, img_channels=3, feat=32).to(device)
D = Discriminator(img_channels=3, feat=32).to(device)
opt_g = torch.optim.Adam(G.parameter (பாரமீட்டர் — கட்டுப்பாடு)s(), lr=2e-4, betas=(0.5, 0.999))
opt_d = torch.optim.Adam(D.parameters(), lr=2e-4, betas=(0.5, 0.999))

for epoch (யுக் — ஒரு முழு சுழற்சி) in range(10):
    for (batch (பத்தி — தொகுப்பு),) in loader:
...
Always switch to eval mode before sampling. For DCGAN this matters because batch (பத்தி — தொகுப்பு) norm running stats are used instead of the batch's stats.
...
Swap `Discriminator` for `build_sn_discriminator()` and you often do not need the TTUR trick. Spectral norm is the easiest single strength upgrade you can apply.
...
1. **(Easy)** Train the DCGAN above on the synthetic circle dataset and save a grid of 16 samples at the end of each epoch (யுக் — ஒரு முழு சுழற்சி). By which epoch do the generated circles become clearly circular?
2. **(Medium)** Replace the discriminator's batch (பத்தி — தொகுப்பு) norm with spectral norm. Train both versions side by side. Which one converges faster? Which one has lower variance across three seeds?
3. **(Hard)** Implement a conditional DCGAN: feed the class label into both G and D (concat one-hot to the noise in G, concat a class embedding (embedding — பொருளை எண்களாக மாற்றுதல்) channel in D). Train on the synthetic "circles vs squares" dataset from lesson 7 and show that class conditioning works by sampling with specific labels.
...
| Non-saturating loss | "The numerically sane version" | G's loss is -log(D(G(z))) instead of log(1 - D(G(z))) to avoid vanishing gradient (சாய்வு — மாற்றத்தின் दिशा)s early in training |
| Mode collapse | "Generator makes one thing" | G produces only a small subset of the data distribution. fix with SN, minibatch (பத்தி — தொகுப்பு) discrimination, or larger batch |
| TTUR | "Two learning rates" | D learns faster than G, usually by a factor of 2-4. stabilises training |
...
| FID | "Fréchet Inception Distance" | Distance between Inception-v3 feature (பண்பு — தகவல் நெடுவரிசை) distributions of real and generated sets. the standard evaluation metric |
