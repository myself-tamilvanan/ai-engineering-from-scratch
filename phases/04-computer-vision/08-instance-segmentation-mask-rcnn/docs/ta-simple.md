...
Mask R-CNN (He et al., 2017) solved this by reframing instance segmentation as detection-plus-a-mask. The design was so clean that for the next five years almost every instance segmentation paper was a Mask R-CNN variant, and the torchvision how to build is still the production default for small to medium datasets.

The hard engineering problem is sampling: how do you crop a fixed-size feature (பண்பு — தகவல் நெடுவரிசை) region out of a proposal box whose corners do not align with pixel boundaries? Getting that wrong costs tenths of a mAP point everywhere. RoIAlign is the answer.
...
flowchart LR
    ...
    BB --> FPN["feature (பண்பு — தகவல் நெடுவரிசை)<br/>Pyramid Network"]
...
1. **Backbone** — ResNet-50 or ResNet-101 trained on ImageNet. Produces a hierarchy of feature (பண்பு — தகவல் நெடுவரிசை) maps at strides 4, 8, 16, 32.
...
- `L_rpn_cls`, `L_rpn_box` — objectness + box regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) for the RPN proposals.
...
Each loss has its own default weight. the torchvision how to build exposes them as constructor arguments.
...
def roi_align_single(feature (பண்பு — தகவல் நெடுவரிசை), box, output_size=7, spatial_scale=1 / 16.0):
    """
    ...
    sampled = F.grid_sample(feature (பண்பு — தகவல் நெடுவரிசை).unsqueeze(0), grid, mode="bilinear",
                            align_corners=False)
    return sampled.squeeze(0)
```

Every number is at a bilinearly-sampled position. No rounding, no quantisation, no dropped gradient (சாய்வு — மாற்றத்தின் दिशा)s.
...
feature (பண்பு — தகவல் நெடுவரிசை) = torch.randn(1, 16, 50, 50)
boxes = torch.tensor (டென்சர் — பல வடிவ எண் வரிசை)([[0, 10, 20, 100, 90]], dtype=torch.float32)  # (batch (பத்தி — தொகுப்பு)_idx, x1, y1, x2, y2)

ours = roi_align_single(feature (பண்பு — தகவல் நெடுவரிசை)[0], boxes[0, 1:].tolist(), output_size=7, spatial_scale=1/4)
...
model.eval()
print(f"params: {sum(p.numel() for p in model.parameter (பாரமீட்டர் — கட்டுப்பாடு)s()):,}")
print(f"classes (including background): {len(model.roi_heads.box_predictor.cls_score.out_feature (பண்பு — தகவல் நெடுவரிசை)s * [0])}")
```

46M parameter (பாரமீட்டர் — கட்டுப்பாடு)s, 91 classes (COCO). The first class (id 0) is background. everything the model actually detects starts at id 1.
...
The mask tensor (டென்சர் — பல வடிவ எண் வரிசை) is shape `(N, 1, H, W)`. Threshold at 0.5 to get a binary mask per object:
...
def build_custom_maskrcnn(num_classes):
    model = maskrcnn_resnet50_fpn_v2(weights=MaskRCNN_ResNet50_FPN_V2_Weights.DEFAULT)
    in_feature (பண்பு — தகவல் நெடுவரிசை)s = model.roi_heads.box_predictor.cls_score.in_features
...
print(f"custom cls_score.out_feature (பண்பு — தகவல் நெடுவரிசை)s: {custom.roi_heads.box_predictor.cls_score.out_features}")
```
...
On small datasets, freeze the backbone and the FPN. Only the RPN objectness + regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) and the two heads learn.

```python
def freeze_backbone_and_fpn(model):
    # torchvision Mask R-CNN packs the FPN inside `model.backbone` (as
    # `model.backbone.fpn`), so iterating `model.backbone.parameter (பாரமீட்டர் — கட்டுப்பாடு)s()` covers
    # both the ResNet feature (பண்பு — தகவல் நெடுவரிசை) layers and the FPN lateral/output convs.
    for p in model.backbone.parameters():
        p.requires_grad = False
    return model

custom = freeze_backbone_and_fpn(custom)
trainable = sum(p.numel() for p in custom.parameter (பாரமீட்டர் — கட்டுப்பாடு)s() if p.requires_grad)
print(f"trainable after freeze: {trainable:,}")
```

On 500-image datasets this is the difference between convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) and overfitting.
...
The `targets` list must have per-image dicts with `boxes`, `labels`, and `masks` (as `(num_instances, H, W)` binary tensor (டென்சர் — பல வடிவ எண் வரிசை)s). The model returns a dict of four losses during training and a list of predictions during eval, keyed on `model.training`.
...
1. **(Easy)** Verify your RoIAlign against `torchvision.ops.roi_align` on 100 random boxes. Report the max absolute difference. Also run RoIPool (pre-2017 behaviour) and show it diverges by ~1-2 feature (பண்பு — தகவல் நெடுவரிசை)-map pixels on boxes near the border.
2. **(Medium)** Fine-tune `maskrcnn_resnet50_fpn_v2` on a 50-image custom dataset (any two classes: balloons, fish, pothole, logos). Freeze the backbone, train for 20 epoch (யுக் — ஒரு முழு சுழற்சி)s, report mask AP@0.5.
...
| FPN | "feature (பண்பு — தகவல் நெடுவரிசை) pyramid" | Top-down + lateral connections that give every stride level C channels of semantic-rich features |
...
- [FPN: feature (பண்பு — தகவல் நெடுவரிசை) Pyramid Networks (Lin et al., 2017)](https://arxiv.org/abs/1612.03144) — the FPN paper. every modern detector uses it
