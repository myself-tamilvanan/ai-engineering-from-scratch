...
> An image is a tensor (டென்சர் — பல வடிவ எண் வரிசை) of light samples. Every vision model you will ever use starts from this one fact.

**Type:** Build
**Languages:** Python
**Prerequisites:** Phase 1 Lesson 12 (tensor (டென்சர் — பல வடிவ எண் வரிசை) Operations), Phase 3 Lesson 11 (Intro to PyTorch)
**Time:** ~45 minutes
...
flowchart LR
    ...
    H --> I["batch (பத்தி — தொகுப்பு)<br/>CHW → NCHW"]
    I --> J["Model"]
...
- **Intensity quantization** decides how finely the voltage is bucketed. 8 bits gives 256 levels and is standard for display. 10, 12, 16 bits give smoother gradient (சாய்வு — மாற்றத்தின் दिशा)s and matter for medical imaging, HDR, and raw sensor pipelines.
...
Same tensor (டென்சர் — பல வடிவ எண் வரிசை), two orderings. Every library picks one.
...
v |R G B|R G B|R G B|                   v |G G G G G G|
  +-----+-----+-----+                     +-----+-----+
   ...
   PIL, OpenCV, matplotlib,              PyTorch, most deep learning (ஆழ் கற்றல் — பல்லாய கொட்ட பயிற்சி)
   almost every image file on disk       frameworks, cuDNN kernels
```

CHW exists because convolution kernels slide across H and W. Keeping the channel axis first means each kernel sees a contiguous 2D plane per channel, which vector (வெக்டர் — எண் பட்டியல்)izes cleanly. Disk formats keep HWC because that matches how scanlines come out of a sensor.

The one-line conversion you will type a thousand times:

```
img_chw = img_hwc.transpose(2, 0, 1)      # NumPy
img_chw = img_hwc.permute(2, 0, 1)        # PyTorch tensor (டென்சர் — பல வடிவ எண் வரிசை)
```
...
- **Grayscale** — OCR, document models, any case where color is nuisance variable quite than signal.
...
- **Resize directly to target** — stretches the image. Cheap, distorts geometry, fine for many classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) tasks.
...
Go from raw bytes to the exact tensor (டென்சர் — பல வடிவ எண் வரிசை) a pretrained ImageNet model expects, then back.
...
pipeline = transforms.Compose([
    ...
    transforms.Totensor (டென்சர் — பல வடிவ எண் வரிசை)(),
    transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]),
])

x = pipeline(img)
print(f"tensor (டென்சர் — பல வடிவ எண் வரிசை) type:  {type(x).__name__}")
print(f"tensor dtype: {x.dtype}")
print(f"tensor shape: {tuple(x.shape)}      # (C, H, W)")
print(f"per-channel mean: {x.mean(dim=(1, 2)).tolist()}")
print(f"per-channel std:  {x.std(dim=(1, 2)).tolist()}")

batch (பத்தி — தொகுப்பு) = x.unsqueeze(0)
print(f"\nbatched shape: {tuple(batch.shape)}   # (N, C, H, W) — ready for a model")
```

Four steps, in this exact order: `Resize(256)` scales the shorter side to 256. `CenterCrop(224)` takes a 224x224 patch from the middle. `Totensor (டென்சர் — பல வடிவ எண் வரிசை)()` divides by 255 and swaps HWC to CHW. `Normalize` subtracts the ImageNet mean and divides by std. Reversing that order silently changes what reaches the model.
...
- `outputs/skill-image-tensor (டென்சர் — பல வடிவ எண் வரிசை)-inspector.md` — a skill that, given any image-shaped tensor or array, reports dtype, layout, range, and whether it looks raw, normalized, or standardized.
...
2. **(Medium)** Write `standardize(img, mean, std)` and its inverse that together pass a `roundtrip_max_diff <= 1` test on any uint8 image. Your function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s must work on a single image in HWC and on a batch (பத்தி — தொகுப்பு) in NCHW with the same call.
3. **(Hard)** Take a 3-channel ImageNet-standardized tensor (டென்சர் — பல வடிவ எண் வரிசை) and run it through a 1x1 conv that learns a weighted mixture of RGB into a single grayscale channel. Initialize the weights to `[0.299, 0.587, 0.114]`, freeze them, and verify the output matches your manual `rgb_to_grayscale` to within floating-point error. What other classical color-space transforms is written as 1x1 convolutions?
...
| Channel | "The colour" | One of the parallel spatial grids stacked into an image tensor (டென்சர் — பல வடிவ எண் வரிசை). last axis in HWC, first in CHW |
...
- [How JPEG Works (Colt McAnlis)](https://www.youtube.com/watch?v=F1kYBnY6mwg) — a sharp visual tour of chroma subsampling, DCT, and why JPEG encodes YCbCr quite than RGB
