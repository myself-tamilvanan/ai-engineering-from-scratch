...
> A retrieval system ranks candidates by a distance in embedding (embedding — பொருளை எண்களாக மாற்றுதல்) space. Metric learning is the discipline of shaping that space so the distances mean what you want.
...
- Use DINOv2, CLIP, and SigLIP as off-the-shelf embedding (embedding — பொருளை எண்களாக மாற்றுதல்) backbones and know when each wins
...
flowchart LR
    ...
    ENC --> EMB["Query embedding (embedding — பொருளை எண்களாக மாற்றுதல்)"]
    EMB --> IDX["FAISS index"]
...
| **NT-Xent / InfoNCE** | Pairs + batch (பத்தி — தொகுப்பு)-mined negatives | Scales to large batches | Needs big batch or momentum queue |
| **Proxy-based (ProxyNCA)** | Class labels only | Fast, stable, no mining | Can overfit to proxies on small datasets |

For most production use cases, start with a pretrained backbone and only add a metric-learning fine-tune if the off-the-shelf embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s underperform on your test set.
...
- **Cosine**: angle between vector (வெக்டர் — எண் பட்டியல்)s. Requires L2-normalised embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s.
- **L2**: Euclidean distance. Works on raw or normalised embeddings, but is usually paired with L2-normalised + squared L2.

For most modern nets the two are equivalent: `||a - b||^2 = 2 - 2 cos(a, b)` when `||a|| = ||b|| = 1`. Pick the convention that matches your embedding (embedding — பொருளை எண்களாக மாற்றுதல்) training. mixing them silently changes what "nearest" means.
...
Report recall@1, @5, @10 side by side. A recall@10 above 0.95 with recall@1 below 0.5 means the embedding (embedding — பொருளை எண்களாக மாற்றுதல்) space has the right structure but the ranking is noisy — try longer fine-tunes or a re-ranking step.
...
- `IndexFlatIP` / `IndexFlatL2` — brute force, exact, no training. Use up to ~1M vector (வெக்டர் — எண் பட்டியல்)s.
...
For 100k vector (வெக்டர் — எண் பட்டியல்)s you probably want `IndexFlatIP` on cosine similarity. For 10M you want `IndexIVFFlat`. For 100M+ combined with product quantisation (`IndexIVFPQ`).

### Instance-level vs category-level retrieval

Two very different problems with the same name:

- **Category-level** — "find cats in my catalogue." Class-conditional similarity. off-the-shelf CLIP / DINOv2 embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s work well.
...
One line. Works on L2-normalised or raw embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s.

### Step 2: Semi-hard mining

Given a batch (பத்தி — தொகுப்பு) of embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s and labels, find the hardest semi-hard negative for each anchor.
...
Top-k by inner product on L2-normalised embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s equals top-k by cosine. Report the mean proportion of queries with at least one correct neighbour.
...
def sample_batch (பத்தி — தொகுப்பு)(bs=32):
    labels = torch.randint(0, num_classes, (bs,))
    x = protos[labels] + 0.15 * torch.randn(bs, 128)
    return x, labels

enc = Encoder()
opt = Adam(enc.parameter (பாரமீட்டர் — கட்டுப்பாடு)s(), lr=3e-3)

for step in range(200):
    x, y = sample_batch (பத்தி — தொகுப்பு)(32)
    emb = enc(x)
...
After a few hundred steps the embedding (embedding — பொருளை எண்களாக மாற்றுதல்) clusters form one cluster per class.
...
- **Milvus / Weaviate / Qdrant** — managed vector (வெக்டர் — எண் பட்டியல்) DB wrappers around FAISS or HNSW.

For SOTA instance retrieval, the recipe is: DINOv2 backbone, add an embedding (embedding — பொருளை எண்களாக மாற்றுதல்) head, fine-tune with a triplet or InfoNCE loss on instance-labelled pairs, index in FAISS.
...
1. **(Easy)** Run the toy example above. Plot the embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s with PCA before and after training to see the six clusters form.
2. **(Medium)** Add a ProxyNCA loss how to build: one learned "proxy" per class, standard cross-entropy on cosine similarity. Compare convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) speed vs triplet loss on the toy data.
...
| Instance retrieval | "Find this exact thing" | Fine-grained matching. off-the-shelf feature (பண்பு — தகவல் நெடுவரிசை)s usually underperform |
| FAISS | "The NN library" | Facebook's nearest-neighbour library. supports exact and approximate indexes |
| HNSW | "Graph index" | Hierarchical navigable small world. fast approximate NN with small memory extra work |

## Further Reading

- [FaceNet: A Unified embedding (embedding — பொருளை எண்களாக மாற்றுதல்) for Face Recognition (Schroff et al., 2015)](https://arxiv.org/abs/1503.03832) — the triplet loss / semi-hard mining paper
