...
> The encoder, projection, and decoder are wired. Now train them together. Two objectives drive learning: a contrastive image-text loss (InfoNCE) that pulls matching pairs together in the joint embedding (embedding — பொருளை எண்களாக மாற்றுதல்) space, and a language modeling loss that asks the decoder to caption each image. Combined, they teach the network both to find the right image for a caption and to write a caption for the image.
...
- Implement InfoNCE contrastive loss across a batch (பத்தி — தொகுப்பு) of image-caption pairs.
...
Stack the N image embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s as rows and the N text embeddings as rows. L2-normalize both. Compute the `N x N` matrix (மேட்ரிக்ஸ் — எண் மேஜை) `S = I T^T / tau` where `tau` is a learned temperature. The diagonal entries are the matching pairs. off-diagonal entries are negatives. Apply cross-entropy with the target `argmax` running down the diagonal: row `i` should have its highest entry in column `i`. Do the same symmetrically along columns. The total is the average of the two. This is the CLIP loss in eight lines.

### Temperature matters

The temperature `tau` controls how peaked the softmax is. Too small (e.g. `tau = 0.01`) and the gradient (சாய்வு — மாற்றத்தின் दिशा) comes only from the very hardest negative, training is noisy. Too large and the softmax flattens and gradient vanishes. CLIP learns `tau` as a parameter (பாரமீட்டர் — கட்டுப்பாடு). the demo here does the same.
...
`total = contrastive + lm_weight * lm` where `lm_weight` is a scalar (scalar — ஒரு எண்) (often 1.0). The two losses share gradient (சாய்வு — மாற்றத்தின் दिशा)s into the encoder and projection. only the decoder receives LM-loss gradient. This is the multi-task recipe that CoCa, BLIP, and SigLIP-style models all use, with various weightings.
...
- A training loop running 50 steps with batch (பத்தி — தொகுப்பு) size 16, Adam optimizer, and a learned log-temperature parameter (பாரமீட்டர் — கட்டுப்பாடு). Both losses are printed every 5 steps.
...
Output: contrastive loss drops from about `ln(16) = 2.77` toward 2.4. LM loss drops from a random-uniform baseline of `ln(512) ≈ 6.24` toward about 4.7. Both decreases prove the gradient (சாய்வு — மாற்றத்தின் दिशा) is wired correctly. Real models train for millions of steps. the dynamics are the same.
...
- **SigLIP (2023).** Switches InfoNCE for a sigmoid pair loss. same contrastive role, different function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)al form.
...
- InfoNCE loss returns 0 when the similarity matrix (மேட்ரிக்ஸ் — எண் மேஜை) is a perfect diagonal of large positive numbers
...
1. Replace InfoNCE with SigLIP-style sigmoid pair loss and compare convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) on the mock corpus.

2. Add a hard-negative mining step: every other batch (பத்தி — தொகுப்பு), select the hardest off-diagonal pair from the previous batch and append it. Train and inspect whether contrastive loss drops faster.

3. Add an image-text matching binary head on top of the joint embedding (embedding — பொருளை எண்களாக மாற்றுதல்) (true/false: do these match?) for a third loss, replicating BLIP's three-head setup.

4. Replace the mock corpus with caption-id sequences drawn from a Markov chain whose transition matrix (மேட்ரிக்ஸ் — எண் மேஜை) is conditioned on image hash. The captioning loss should drop further because there is actual learnable signal.
...
| Term | What it means |
|------|---------------|
| InfoNCE | Noise contrastive estimation: cross-entropy on a similarity matrix (மேட்ரிக்ஸ் — எண் மேஜை) |
| Temperature | scalar (scalar — ஒரு எண்) that controls how peaked the contrastive softmax is |
...
| Joint embedding (embedding — பொருளை எண்களாக மாற்றுதல்) space | The shared space where image and text vector (வெக்டர் — எண் பட்டியல்)s live after projection |
