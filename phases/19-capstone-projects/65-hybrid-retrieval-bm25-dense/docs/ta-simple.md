...
# Hybrid Retrieval with BM25 and Dense embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s
...
**Type:** Build
**Languages:** Python
**Prerequisites:** Phase 11 lessons 04 (embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s), 06 (RAG). Phase 19 Track B foundations (lessons 20-29). Phase 19 lesson 64 (chunking strategies)
**Time:** ~90 minutes
...
- Build a dense retriever on top of a deterministic mock embedding (embedding — பொருளை எண்களாக மாற்றுதல்) so the loop runs offline.
...
Field weighting lets you tell BM25 that a match on the symbol name counts more than a match in the body. how to build is a multiplier on the term counts during indexing, not at scoring time. That keeps the math identical and avoids a separate score per field.

### Dense retrieval in one paragraph

Embed each chunk into a fixed-dimension (பரிமாணம் — கோணம்) vector (வெக்டர் — எண் பட்டியல்) with an embedding (embedding — பொருளை எண்களாக மாற்றுதல்) model. At query time, embed the query, cosine-rank every chunk by similarity, and return the top-k. The model is the variable that decides quality. The retrieval algorithm (அல்கோரிதம் — வழிமுறை) itself is two lines: dot product and sort.

This lesson uses a deterministic hash-based embedding (embedding — பொருளை எண்களாக மாற்றுதல்) so you can read the fusion math without a network call. The hash sums token-keyed offsets into a 96-dimension (பரிமாணம் — கோணம்)al vector (வெக்டர் — எண் பட்டியல்) and normalizes. The cosine ranks are deterministic across runs, which is what the test suite requires.

### Reciprocal rank fusion, the published formula

Two ranked lists. For each candidate that appears in either list, sum its reciprocal-rank contributions. The 2009 paper used `1 / (k + rank)` with k equal to 60 as the default. Sort by total score. That is the whole algorithm (அல்கோரிதம் — வழிமுறை).
...
Two tunable knobs in our how to build. The `k` constant. A pair of per-modality weights so you can boost BM25 or dense when you have prior evidence one is better on your corpus. Multiplying the rank contribution by the weight is the simplest principled how to build. it preserves the rank-decay shape and stays scale-free.
...
- `mock_embed`, `DenseIndex` - the same deterministic embedding (embedding — பொருளை எண்களாக மாற்றுதல்) as lesson 64 so chunks are comparable.
...
Tune by re-running lesson 68's eval harness on your held-out query set, not by simple idea.

## Failure modes the demo will hide

**Out-of-vocabulary tokens.** BM25's IDF is computed from the corpus, so terms only in the query contribute zero. Dense embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s hallucinate a vector (வெக்டர் — எண் பட்டியல்) for the same term. On out-of-corpus identifiers the dense modality returns plausible-looking but wrong neighbors. The fusion absorbs this because BM25 returns nothing and the rank contribution drops out, but only if you de-duplicate by document, not by chunk.
...
- Index BM25 in process. the bottleneck is the term-frequency dictionary, not the vector (வெக்டர் — எண் பட்டியல்)s.
...
4. Implement BM25F properly (per-field length normalization quite than the multiplier trick) and compare on a corpus where symbol matches matter most.
...
- [Vespa: Hybrid Retrieval with BM25 and embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s](https://docs.vespa.ai/en/tutorials/hybrid-search.html)
