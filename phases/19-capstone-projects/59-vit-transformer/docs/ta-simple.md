...
> Patches alone do not see. A 12-layer pre-LN transformer with 12 attention heads turns the sequence of patch tokens into a sequence of contextual tokens, with the CLS token pooling whole-image feature (பண்பு — தகவல் நெடுவரிசை)s in its final hidden state. This lesson is the engine room of every modern vision-language model.
...
Original Transformer placed LayerNorm after the residual. Pre-LN (LayerNorm before each sub-layer) is the version every modern vision-language model uses, because it trains stably without learning-rate warm-up tricks. The difference is one line in the forward pass, and the gradient (சாய்வு — மாற்றத்தின் दिशा) flow at depth 12+ is night and day.

### Multi-head self-attention

Each head projects the token vector (வெக்டர் — எண் பட்டியல்) to its own `(query, key, value)` triple with dimension (பரிமாணம் — கோணம்) `head_dim = hidden / num_heads`. With `hidden = 768` and `heads = 12`, each head has `dim = 64`. The 12 heads attend in parallel, then their outputs concat back to dimension 768 and pass through an output projection. The point of multi-head is that one head can learn "attend to the cat eye" while another learns "attend to the background gradient (சாய்வு — மாற்றத்தின் दिशा)" without interference.
...
| Component | parameter (பாரமீட்டர் — கட்டுப்பாடு)s at ViT-Base scale |
|-----------|------------------------------|
...
ViT-Base is a 86M-parameter (பாரமீட்டர் — கட்டுப்பாடு) encoder. That is small by 2026 standards (SigLIP-So400M is 400M, the Qwen-VL ViT is 675M), but the architecture is identical up to width and depth.
...
The CLS token starts as a learned parameter (பாரமீட்டர் — கட்டுப்பாடு), has no patch content of its own, and accumulates information through attention across every block. By the final layer, the CLS row is a vector (வெக்டர் — எண் பட்டியல்) summary of the whole image. downstream heads project this single vector into class logits, contrastive embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s, or cross-attention keys for a text decoder.
...
- `VisionEncoder`, which wires `VisionFrontEnd` from lesson 58 to the `ViT` stack and exposes a `forward()` returning the contextual sequence and the pooled CLS vector (வெக்டர் — எண் பட்டியல்).
- A demo that runs a synthesized 224x224 fixture image through the full encoder and prints input shape, output shape, parameter (பாரமீட்டர் — கட்டுப்பாடு) count, and the CLS norm at every other layer.
...
Output: the fixture is encoded to a `(1, 197, 768)` tensor (டென்சர் — பல வடிவ எண் வரிசை). The CLS norm drifts upward as the layers compose, then stabilizes at the final LayerNorm. Total parameter (பாரமீட்டர் — கட்டுப்பாடு)s report at about 86M.
...
- a single block preserves shape and is invariant to input batch (பத்தி — தொகுப்பு) size
...
- gradient (சாய்வு — மாற்றத்தின் दिशा)s flow to the patch projection from the CLS output
...
1. Add register tokens (4 learned vector (வெக்டர் — எண் பட்டியல்)s prepended after CLS) and rerun. Compare attention map smoothness via the entropy of the softmax distribution on the last layer.

2. Swap pre-LN for post-LN and train for one epoch (யுக் — ஒரு முழு சுழற்சி) on a synthetic shape classifier. Observe which one trains stably without LR warm-up.

3. Implement causal masking as an `attn_mask` argument so the same block is reused as a decoder block. The mask shape is `(seq, seq)`, lower-triangular.

4. Profile a forward pass at batch (பத்தி — தொகுப்பு) sizes 1, 8, 64 with `torch.profiler`. The MLP layer dominates wall time, not attention.

5. Replace one attention head's q-k-v projections with a low-rank LoRA adapter, freeze the rest, and verify the gradient (சாய்வு — மாற்றத்தின் दिशा) only flows where you expect.
