...
**Type:** Build
**Languages:** Python
**Prerequisites:** Phase 19 lessons 30 to 33 (tokenizer, embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s, attention math, batch (பத்தி — தொகுப்பு)ed data loader)
**Time:** ~90 minutes
...
- Track gradient (சாய்வு — மாற்றத்தின் दिशा) flow through both variants on a 12 layer stack and read the result without hand waving.
- Reuse the block as a drop-in unit when the next lesson assembles a 124 million parameter (பாரமீட்டர் — கட்டுப்பாடு) GPT.

## The Problem

A transformer is one block repeated. Get the block wrong once, repeat it twelve times, and you ship a model that diverges in the first epoch (யுக் — ஒரு முழு சுழற்சி) or that needs warmup hacks the rest of the way. The two failure modes you will see in this lesson are not exotic. They show up the first time a learner stacks blocks naively. One is the attention layer attending to the future. The other is the LayerNorm placed where it cannot tame the residual signal at depth.
...
Every decoder only transformer block is a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) that takes a tensor (டென்சர் — பல வடிவ எண் வரிசை) of shape `(batch (பத்தி — தொகுப்பு), sequence, embedding (embedding — பொருளை எண்களாக மாற்றுதல்))` and returns a tensor of the same shape. Inside, two sublayers do the work.

```mermaid
flowchart TB
  X[Input embedding (embedding — பொருளை எண்களாக மாற்றுதல்)<br/>shape B, T, D] --> N1[LayerNorm 1]
...
Shape is identical. Training behavior is not. With post-LN, the gradient (சாய்வு — மாற்றத்தின் दिशा) that flows back through the residual path must pass through the LayerNorm. At depth twelve and learning rate `3e-4`, that gradient shrinks fast enough to need a warmup schedule. Pre-LN leaves the residual path unnormalized, so gradients propagate cleanly to the embedding (embedding — பொருளை எண்களாக மாற்றுதல்) layer. Pre-LN is the configuration GPT-2 onward ships with for that reason.

### Causal multi head attention

The attention sublayer projects the input three ways into query, key, value tensor (டென்சர் — பல வடிவ எண் வரிசை)s. Each is reshaped from `(B, T, D)` to `(B, H, T, D/H)` where `H` is the head count. Scaled dot product attention computes `softmax(Q K^T / sqrt(d_k))` per head, masks the upper triangle to negative infinity, applies the mask via softmax, then multiplies by `V`. Heads are concatenated back into a single `(B, T, D)` tensor and projected once more. The mask is the only piece that makes the model causal. Forget the mask and you train a model that cheats.

### The MLP

The position wise MLP applies the same two layer network to every token independently. The hidden width is four times the embedding (embedding — பொருளை எண்களாக மாற்றுதல்) width, the activation is GELU, and a dropout follows the second linear. No tokens talk to each other inside the MLP. All token mixing happens in attention.

### Residual connections do two things

They make the gradient (சாய்வு — மாற்றத்தின் दिशा) path additive across depth, which keeps the gradient norm in scale through twelve layers. They also let each block learn an additive update to the running representation quite than a full replacement. Both effects are why the block scales.

## Build It

`code/main.py` implements:

- `class LayerNorm` with learnable scale and shift, biased eps, applied per token vector (வெக்டர் — எண் பட்டியல்).
...
- A demo that builds a 6 layer pre-LN stack and a 6 layer post-LN stack with identical inputs and prints (a) output shape, (b) gradient (சாய்வு — மாற்றத்தின் दिशा) norm at the embedding (embedding — பொருளை எண்களாக மாற்றுதல்) after one backward pass.
...
Output: shape check on both stacks, gradient (சாய்வு — மாற்றத்தின் दिशा) norms side by side. The pre-LN stack's embedding (embedding — பொருளை எண்களாக மாற்றுதல்) gradient is order of magnitude larger than the post-LN stack at the same learning rate, which is the empirical signal pre-LN trains without warmup.

## Stack

- `torch` for the tensor (டென்சர் — பல வடிவ எண் வரிசை) math, autograd, and `nn.Module` plumbing.
...
1. Add a `bias=False` flag to every linear in the block. Modern open weights LLMs ship without biases on the linear layers. Measure how many parameter (பாரமீட்டர் — கட்டுப்பாடு)s you save in a 12 layer 768 dim model.
2. Replace `nn.LayerNorm` with a hand rolled RMSNorm and verify the output shape is unchanged.
3. Add a flag that returns the attention weights for the first head as a `(B, T, T)` tensor (டென்சர் — பல வடிவ எண் வரிசை). Plot the upper triangle to confirm it is zero after softmax.
...
| Residual stream | "Skip connection" | The unnormalized tensor (டென்சர் — பல வடிவ எண் வரிசை) that flows top to bottom through every block. what each block adds to |
