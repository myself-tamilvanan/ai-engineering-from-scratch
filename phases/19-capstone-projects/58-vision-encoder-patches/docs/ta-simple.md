...
> A vision model that reads pixels needs a tokenizer for pixels. Patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்) is that tokenizer. Cut the image into a grid of squares, flatten each square, project it through one linear layer, then add a 2D position signal so the transformer knows where each square sat in the original image.
...
- Tokenize an image into a fixed-length sequence of patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s.
...
A transformer eats a sequence of vector (வெக்டர் — எண் பட்டியல்)s. An image is a 3-channel grid. Reading every pixel as a token explodes the sequence length: a 224x224 RGB image is 150,528 tokens, which a 12-layer transformer cannot afford in attention. Reading the image as one giant flat vector throws away locality, which the attention layer cannot recover from. The job of the encoder front end is to compress the pixel grid into a few hundred tokens that each summarize a square region.

Patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்) solves this with one linear projection. A 224x224 image cut into 16x16 patches produces a 14x14 grid of 196 patches. Each patch is flattened from `(3, 16, 16) = 768` pixel values into one vector (வெக்டர் — எண் பட்டியல்), then a linear layer maps it to the model's hidden dimension (பரிமாணம் — கோணம்). The transformer sees 196 tokens of dimension `hidden` (commonly 768) plus a CLS token. That is a sequence the rest of the network can chew on.
...
Each patch is treated as an independent vector (வெக்டர் — எண் பட்டியல்). The projection learns a basis: edge detectors, color filters, simple textures. A single linear layer is small (`768 * 768 = 589,824` parameter (பாரமீட்டர் — கட்டுப்பாடு)s for ViT-Base) and trains fast. Deeper convolutional stems exist (the "hybrid" ViT), but a flat linear projection is the standard, and most modern open-weight encoders ship with this exact shape.
...
### Position embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s

Tokens carry no order out of the projection. The 2D sinusoidal embedding (embedding — பொருளை எண்களாக மாற்றுதல்) gives each token a fixed signal that encodes its `(row, col)` position. Half the embedding dimension (பரிமாணம் — கோணம்) encodes row position with sin/cos at multiple frequencies. the other half encodes column position. The encoding is deterministic so you can swap resolutions without retraining, and it interpolates cleanly to grids the model never saw at training time.

| Component | Shape | parameter (பாரமீட்டர் — கட்டுப்பாடு)s |
|-----------|-------|------------|
| Patch projection (`Conv2d`) | `(hidden, 3, patch, patch)` | `3 * P * P * hidden + hidden` |
| Position embedding (embedding — பொருளை எண்களாக மாற்றுதல்) (fixed) | `(num_patches, hidden)` | 0 (computed, not learned) |
| CLS token (learned) | `(1, hidden)` | `hidden` |

For ViT-Base/16 at 224 resolution: 590,592 parameter (பாரமீட்டர் — கட்டுப்பாடு)s in the projection, 768 in the CLS token, and zero for sinusoidal position. The next lesson (59) stacks a 12-layer transformer on top of this front end.
...
- `sinusoidal_2d(grid_h, grid_w, dim)`, a stateless function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) that builds the 2D position table.
- `VisionFrontEnd`, which composes patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்), CLS prepend, and position addition into one forward pass.
...
Output: the 224x224 fixture is tokenized to a sequence of shape `(1, 197, 768)`. The first token is the CLS. the next 196 are patch tokens. The position embedding (embedding — பொருளை எண்களாக மாற்றுதல்) norms are uniform within a row, which is the sinusoidal signature.
...
- output shape matches `(batch (பத்தி — தொகுப்பு), num_patches + 1, hidden)`
...
1. Replace the sinusoidal position with a learned `nn.parameter (பாரமீட்டர் — கட்டுப்பாடு)` and compare the first-epoch (யுக் — ஒரு முழு சுழற்சி) loss on a tiny synthetic classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) task. Learned positions win at fixed resolution. sinusoidal wins when you change resolution after training.
...
4. Profile the patch step at batch (பத்தி — தொகுப்பு) sizes 1, 8, 64. The patch projection is rarely the bottleneck. the attention layers downstream dominate.

5. Train the front end as a frozen feature (பண்பு — தகவல் நெடுவரிசை) extractor on a 4-class synthetic shape dataset (circles, squares, triangles, stars). The CLS token output should linearly separate.

## Key Terms

| Term | What it means |
|------|---------------|
| Patch | A square sub-region of the image, usually 14x14 or 16x16 |
| Patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்) | Linear projection of one flattened patch to the hidden dim |
...
| CLS token | Learned vector (வெக்டர் — எண் பட்டியல்) prepended to the sequence as the pooling head |
