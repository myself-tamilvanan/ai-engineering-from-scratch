...
- Pin behaviour with tests on the loss math, the gradient (சாய்வு — மாற்றத்தின் दिशा) sign, and the reference invariance.
...
where `r` is some latent reward function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). RLHF first fits `r` from preferences, then trains a policy `pi` to maximise `r` with a KL anchor:
...
This is the loss. It is a sigmoid over a single scalar (scalar — ஒரு எண்) per example, computed from four log-probabilities. No separate reward model. No PPO. No KL term in the loss. the KL constraint is baked into the closed-form derivation.
...
## The Sign of the gradient (சாய்வு — மாற்றத்தின் दिशा)

A useful sanity check before any training run. Take the gradient (சாய்வு — மாற்றத்தின் दिशा) with respect to `log pi_theta(y_w | x)`:

```text
d L_DPO / d log pi_theta(y_w | x) = - beta * (1 - sigmoid(z))
```

where `z` is the argument to the sigmoid. This is negative for all `z`, which means: increasing the policy's log-probability of the chosen completion decreases the loss. Symmetrically, the gradient (சாய்வு — மாற்றத்தின் दिशा) with respect to `log pi_theta(y_l | x)` is positive: increasing the rejected log-probability increases the loss. Training pushes the chosen up and the rejected down. The reference is frozen. it does not move.
...
A DPO how to build has to handle the reference model carefully. The reference is the SFT model frozen in place. Three properties have to hold:

- The reference parameter (பாரமீட்டர் — கட்டுப்பாடு)s never receive gradient (சாய்வு — மாற்றத்தின் दिशा)s.
- The reference log-probabilities never change between epoch (யுக் — ஒரு முழு சுழற்சி)s.
...
The how to build enforces these by:

- Wrapping the reference in `torch.no_grad()` during forward passes.
- Setting `requires_grad=False` on every reference parameter (பாரமீட்டர் — கட்டுப்பாடு).
...
5. `dpo_loss`: takes the four log-probabilities and `beta`, returns the per-example loss tensor (டென்சர் — பல வடிவ எண் வரிசை) and the implicit reward delta for logging.
6. `train_dpo`: per-epoch (யுக் — ஒரு முழு சுழற்சி) loop that computes chosen and rejected log-probs under policy and reference, applies the loss, and steps Adam.
...
DPO is mathematically equivalent to RLHF under the Bradley-Terry preference model, up to the parameter (பாரமீட்டர் — கட்டுப்பாடு)isation of the reward. The implicit reward `r(x, y) = beta * (log pi(y|x) - log pi_ref(y|x))` is identifiable from preferences up to a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) of `x`, which cancels in the difference. The closed-form policy lets you skip the explicit reward model. The KL constraint is enforced structurally: any deviation of `pi` from `pi_ref` makes the log-ratio larger, and the sigmoid saturates, which damps the gradient (சாய்வு — மாற்றத்தின் दिशा) when the policy moves too far. The reference is your safety net.
...
- Add the IPO variant of the loss: replace the sigmoid + log with `(z - 1)^2`. Compare convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) on the fixture.
- Add a label-smoothing parameter (பாரமீட்டர் — கட்டுப்பாடு) that interpolates between the hard chosen-rejected label and a uniform 0.5.
- Replace the reference with a smaller cheaper model (knowledge distillation flavour).

The how to build gives you the loss, the reference invariance, and the training loop. The math is the lesson. The code makes the math concrete.
