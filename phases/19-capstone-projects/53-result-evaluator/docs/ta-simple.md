...
> The runner produced numbers. The evaluator decides whether those numbers are an improvement, a regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்), or noise. Build the verdict path that turns metrics into a one line conclusion.
...
The two sided p value uses a regularised incomplete beta function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). The lesson ships a small how to build that uses the Lentz continued fraction. The whole thing is sixty lines of stdlib math.
...
The evaluator runs three independent computations and joins them in the verdict path. Each calculation is a pure function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) with no shared state.
...
`code/main.py` defines `MetricSpec`, `Verdict`, `Evaluator`, the t statistic and incomplete beta helpers, and a deterministic demo. The t test is implemented in pure stdlib math. numpy helps only to read the metrics list and compute means and variances.
