...
2. **Webhook receiver.** Lambda function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) accepts issue label / PR comment webhooks. Filters by label `@agent fix this`. Enqueues to SQS.
...
# on github.com
  ...
  - body:
    ...
    > Added regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) test widget_test.go::TestDedupeNullComparator.
    > Coverage delta: +0.12%
