...
- Capture the full training state into a single payload that is reloaded into a fresh process.
...
The right artifact is a single file that holds everything needed to continue: model parameter (பாரமீட்டர் — கட்டுப்பாடு)s, optimizer state, scheduler state, the loss history for plots, the current step and epoch (யுக் — ஒரு முழு சுழற்சி) and batch (பத்தி — தொகுப்பு)-in-epoch counters, and the RNG state for every source of randomness. Without the RNG state the resumed loss curve is a different curve. Same model, same data, different shuffle, different dropout mask, different number on the dashboard.
...
flowchart TD
  ...
  ckpt --> tr[train state: step, epoch (யுக் — ஒரு முழு சுழற்சி), batch (பத்தி — தொகுப்பு)_in_epoch, losses]
...
| Optimizer | Momentum and adaptive moments. without these the next step is a different optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) problem. |
| Scheduler | Where the learning rate is on its curve. cosine schedules in particular care. |
| Train counters | Step, epoch (யுக் — ஒரு முழு சுழற்சி), batch (பத்தி — தொகுப்பு)-in-epoch, plus the loss history that draws the dashboard. |
...
When the model gets large the single-file payload becomes too big to load fast, too big to inspect, and too painful when a network share hiccups mid-read. The fix is to split the parameter (பாரமீட்டர் — கட்டுப்பாடு) state into shards and write a small index that ties them together.
...
### Resume continues mid epoch (யுக் — ஒரு முழு சுழற்சி)

A resume that snaps to the start of the next epoch (யுக் — ஒரு முழு சுழற்சி) wastes anywhere from minutes to a day. The fix is `(epoch, batch (பத்தி — தொகுப்பு)_in_epoch)` plus the RNG state. After load, the training loop fast-forwards the random number generator past the batches already consumed in the current epoch and continues from `batch_in_epoch`. The lesson code does this exactly. the assertion is that the loss trajectory after resume matches the uninterrupted baseline within 1e-4.
...
`capture_rng_state` returns a dict with Python's `random.getstate`, NumPy's `np.random.get_state`, and PyTorch CPU and CUDA RNG bytes. `restore_rng_state` reverses it. The CPU tensor (டென்சர் — பல வடிவ எண் வரிசை) is a uint8 byte buffer that PyTorch's RNG knows how to consume.
...
`save_sharded_checkpoint` round-robins the parameter (பாரமீட்டர் — கட்டுப்பாடு) keys across N shards, writes each shard with its own atomic save, writes a meta file with optimizer and scheduler and train state, and writes the JSON index with shard sha256s. `load_sharded_checkpoint` verifies every shard before merging.

### Step 5: resume demo

`run_resume_demo` trains a small model for `total_steps`, saves a checkpoint at `interrupt_at`, then continues. A second process restores the checkpoint and runs the remaining steps. The function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) returns the max absolute difference between the two loss trajectories after the interruption point. With RNG restored, the difference is zero or floating-point noise.
...
1. Replace round-robin sharding with sharding by parameter (பாரமீட்டர் — கட்டுப்பாடு) group (layers ending in `.weight` vs `.bias`). When is each layout preferable?
...
5. Implement a `migrate_v1_to_v2` function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) that adds a new field to the payload and bumps the schema string. Make load tolerate both versions.
...
| State dict | "The weights" | Model parameter (பாரமீட்டர் — கட்டுப்பாடு)s and buffers, keyed by parameter name |
...
| Mid-epoch (யுக் — ஒரு முழு சுழற்சி) resume | "Restart" | Fast-forward the RNG and continue from the next batch (பத்தி — தொகுப்பு) in the same epoch |
...
- Phase 19 lesson 46 covers the gradient (சாய்வு — மாற்றத்தின் दिशा) accumulation that this lesson's checkpoint payload survives across.
