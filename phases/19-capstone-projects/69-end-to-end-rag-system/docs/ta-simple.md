...
The pipeline is a small graph. Each stage is a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) with a clear signature.
...
This is the shape a CI smoke test takes. The pipeline runs offline, fast, deterministic. The thresholds are deliberately tight on the fixture so a regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) in any of the six lessons fails the demo.
...
- Persist the metric trace per CI run so you can attribute regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)s to a stage swap.
...
| Stage interface | "function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) signature" | The stable input and output type of each pipeline stage |
