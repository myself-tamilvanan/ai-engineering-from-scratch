...
> A detector is a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) from prompt to confidence and category. Anything else is a vibe.
...
flowchart LR
  ...
  V --> M[metrics runner]
  M --> CM[confusion matrix (மேட்ரிக்ஸ் — எண் மேஜை) per category]
```
...
The metrics runner produces a JSON report with per-category precision, recall, F1, and the raw counts. The detector is wrong on purpose for some fixtures (especially the benign-looking role-play prompts). the report exposes that quite than hiding it.
...
|---|---|---|
| detector | a model that blocks attacks | a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) returning category and confidence, evaluated by precision and recall |
| normalize | a preprocessing step | a transform that exposes hidden tokens to subsequent rules |
| confusion matrix (மேட்ரிக்ஸ் — எண் மேஜை) | a 2x2 table | the per-category breakdown of TP, FP, TN, FN used to compute precision and recall |
