...
# iteration (மறுயுதல் — மீண்டும் செய்தல்) Scheduler
...
- Surface a per-iteration (மறுயுதல் — மீண்டும் செய்தல்) trace with branch scores, slot occupancy, and pruning decisions.
...
When a high-yield result lands, the scheduler can call the user-supplied `expander` to produce one or more follow-up hypotheses on the same branch. The expander is a pure function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) from `Result` to `list[Hypothesis]`. The lesson ships a deterministic expander that produces two follow-ups for any result whose reward exceeds the paper threshold.
...
`code/main.py` defines `Hypothesis`, `Result`, `BranchStats`, `iteration (மறுயுதல் — மீண்டும் செய்தல்)Scheduler`, and a `make_deterministic_runner` factory that returns an asyncio experiment runner with predictable rewards. The runner sleeps for a fixed `delay_ms` (default `5ms`) so concurrency is observable.
...
Three extensions a real how to build will want. First, persistent UCB stats across sessions: the current statistics live in memory. a real scheduler would checkpoint them so a restart preserves the exploration budget already spent. Second, multi-objective scoring: instead of a scalar (scalar — ஒரு எண்) reward, each result emits a vector (வெக்டர் — எண் பட்டியல்) and UCB becomes a Pareto-style picker. Third, contextual bandits: the picker conditions on hypothesis feature (பண்பு — தகவல் நெடுவரிசை)s (length, complexity) so similar hypotheses share exploration.
