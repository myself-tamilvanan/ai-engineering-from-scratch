...
When `n - c < k` the numerator is undefined and the value is `1`. The how to build handles the edge case directly. We expose `pass_at_k(n, c, k)` for use by the leaderboard layer in lesson 74.
