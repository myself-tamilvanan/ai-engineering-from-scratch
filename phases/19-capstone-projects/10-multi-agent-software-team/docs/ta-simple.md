...
Single-agent coding harnesses hit a ceiling on large tasks. Not because any individual agent is weak, but because a 200k-token context cannot hold an architecture plan plus four parallel codebase slices plus reviewer commentary plus test output. Multi-agent factories split the problem: an architect owns the plan, coders own how to build in parallel worktrees, a reviewer gates, a tester verifies. SWE-AF's "factory" architecture, MetaGPT's roles, AutoGen's typed actor graph — all three framings describe the same shape.
...
2. **Architect.** Reads the GitHub issue, runs Opus 4.7 with a plan template requiring explicit subtask interfaces (files touched, public function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s, test impact). Emits one `plan_request` with a DAG of subtasks.
...
| 20 | Token speed | Total tokens per solved issue vs single-agent |
...
| Token amplification | "Multi-agent extra work" | Total tokens across roles / single-agent tokens for the same task |
