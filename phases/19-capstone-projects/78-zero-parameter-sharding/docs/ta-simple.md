...
- Use reduce_scatter to deliver each rank only its shard's gradient (சாய்வு — மாற்றத்தின் दिशा) sum, then allgather to broadcast the updated parameter (பாரமீட்டர் — கட்டுப்பாடு) shards back.
...
Vanilla DDP replicates everything: parameter (பாரமீட்டர் — கட்டுப்பாடு)s, gradient (சாய்வு — மாற்றத்தின் दिशा)s, and optimiser state are present in full on every rank. For a 7B-parameter model in fp16 that means 14 GB of parameters, 14 GB of gradients, and 28 GB of optimiser state per rank. The optimiser state is the largest term and the easiest to shard because it is only touched during the step, not during forward or backward.

ZeRO stage 1 shards the optimiser state. Each rank holds 1/N of the Adam moments. After backward, instead of allreducing the full gradient (சாய்வு — மாற்றத்தின் दिशा) and stepping locally, ZeRO reduce_scatters so each rank receives only its shard's summed gradient. The rank applies the optimiser step to its shard of the master parameter (பாரமீட்டர் — கட்டுப்பாடு)s. The updated parameter shards then allgather back so every rank has the full model for the next forward. The optimiser memory drops by N. The wire traffic per step is the same as DDP: one reduce_scatter plus one allgather equals one allreduce by bandwidth. Memory wins, throughput holds.
...
Stage 1 is the cheapest win because optimiser state dominates the budget. Stage 2 needs gradient (சாய்வு — மாற்றத்தின் दिशा)-shard accumulation logic but the bandwidth is the same. Stage 3 (FSDP) pays per-layer comm for every forward and backward, gaining the parameter (பாரமீட்டர் — கட்டுப்பாடு)-shard memory drop. The lesson implements stage 1 in full.

### The memory math, real numbers

For a model with P parameter (பாரமீட்டர் — கட்டுப்பாடு)s trained with Adam in mixed precision:
...
Allreduce gives every rank the full summed gradient (சாய்வு — மாற்றத்தின் दिशा). If you only need shard r, the (N-1)/N of the gradient that was reduced is wasted on rank r. Reduce_scatter delivers exactly the shard each rank owns. the per-rank bytes are the same as allreduce (since allreduce is reduce_scatter + allgather) but the second half is replaced by the parameter (பாரமீட்டர் — கட்டுப்பாடு)-shard allgather later. Net wire is identical to DDP, memory is divided.

## Build It

`code/main.py` implements:

- `flatten_params(module)` and `unflatten_into(module, flat)` that pack a model's parameter (பாரமீட்டர் — கட்டுப்பாடு)s into one contiguous tensor (டென்சர் — பல வடிவ எண் வரிசை) and unpack back. The flat layout is what makes sharding by rank a simple slice.
- `ZeroOptimizer(model, world_size, rank, lr)` that owns the rank's shard of the master copy and Adam moments.
- `step()` that runs reduce_scatter on the flat gradient (சாய்வு — மாற்றத்தின் दिशा), applies Adam to the rank's shard, and allgathers the updated parameters back.
...
**Stage 1 is a near-free win.** The comm is identical to DDP by bandwidth. The memory savings are linear in N. The only cost is the bookkeeping for the optimiser shard. Production stacks default to stage 1 unless the parameter (பாரமீட்டர் — கட்டுப்பாடு) shard memory is also a problem. then stage 2 or 3 trades comm for memory.

## Use It

Production patterns:

- **DeepSpeed ZeRO.** The reference how to build. `deepspeed_config.json` selects stage 1/2/3 and partition sizes.
...
1. Extend to ZeRO-2 by sharding gradient (சாய்வு — மாற்றத்தின் दिशा)s: each rank only stores the gradient for its shard, achieved by zeroing out the non-shard portion after backward.
...
| ZeRO-2 | "Shard grads too" | Each rank also drops the non-shard gradient (சாய்வு — மாற்றத்தின் दिशा)s after reduce_scatter |
| ZeRO-3 | "Shard params" | Each rank holds 1/N of fp16 params. allgather per layer in forward |
| Master copy | "fp32 weights" | The high-precision parameter (பாரமீட்டர் — கட்டுப்பாடு) copy the optimiser updates |
| Reduce_scatter | "Split the sum" | Deliver each rank only its shard's summed gradient |

## Further Reading

- [Rajbhandari et al, ZeRO: Memory optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)s Toward Training Trillion parameter (பாரமீட்டர் — கட்டுப்பாடு) Models](https://arxiv.org/abs/1910.02054)
