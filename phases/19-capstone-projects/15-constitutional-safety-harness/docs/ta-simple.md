...
The red-team range runs on a scheduler. PAIR and TAP autonomously discover jailbreaks. GCG runs gradient (சாய்வு — மாற்றத்தின் दिशा)-based suffix attacks. ASCII / base64 / rot13 encoding attacks. Multi-turn attacks (persona adoption, memory exploitation). Code-switch attacks (mix English with Swahili or Thai). Each run produces a structured findings file with CVSS scoring and disclosure timeline.
...
5. **Attack suite.** Six attack families: (1) PAIR automated jailbreak, (2) TAP tree-of-attacks, (3) GCG gradient (சாய்வு — மாற்றத்தின் दिशा) suffix, (4) ASCII / base64 / rot13 encoding, (5) multi-turn persona, (6) multilingual code-switch. Report success rate per family.
...
8. **CVSS scoring.** For each successful jailbreak, score on CVSS 4.0 (attack vector (வெக்டர் — எண் பட்டியல்), complexity, impact). Produce a disclosure timeline and mitigation plan.

9. **Range automation.** Everything above runs on a cron. findings write to a queue. over-refusal regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) alerts fire to Slack.
...
| GCG | "Greedy coordinate gradient (சாய்வு — மாற்றத்தின் दिशा)" | Gradient-based adversarial suffix attack |
| Constitutional self-critique | "Anthropic-style training" | Target drafts -> critic scores -> rewrite -> retrain |
| XSTest | "Benign probe set" | Benchmark for over-refusal regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) |
