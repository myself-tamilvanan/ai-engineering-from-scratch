...
Enterprises sit on PDFs that OCR pipelines mangle: scanned 10-Ks with rotated tables, scientific papers dense with equations, charts that only make sense as images, handwritten annotations. Treating these as text-first means losing half the signal. The 2026 answer is late-interaction multi-vector (வெக்டர் — எண் பட்டியல்) retrieval on raw page images. ColPali (Illuin Tech) introduced it. ColQwen2.5-v0.2 and ColQwen3-omni pushed accuracy. On ViDoRe v3, vision-first retrieval scores above OCR-then-text by meaningful margins — and the gap widens on charts, tables, and handwriting.

The trade-off is storage and latency. A ColQwen embedding (embedding — பொருளை எண்களாக மாற்றுதல்) is ~2048 patch vector (வெக்டர் — எண் பட்டியல்)s per page, not a single 1024-dim vector. Raw storage balloons. DocPruner (2026) brings 50% pruning without measurable accuracy loss. You will index 10k pages, measure ViDoRe v3 nDCG@5, serve answers under 2s, and compare directly against an OCR-then-text baseline.

## Concept

Late interaction means every query token scores against every patch token, and the maximum score per query token is summed. You get fine-grained matching without needing a single pooled vector (வெக்டர் — எண் பட்டியல்). A multi-vector index (Vespa, Qdrant multi-vector, or AstraDB) stores the per-patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s and runs MaxSim at retrieval time.
...
Evaluation is a two-dimension (பரிமாணம் — கோணம்)al matrix (மேட்ரிக்ஸ் — எண் மேஜை). One axis: content type (plain text paragraphs, dense tables, bar/line charts, handwritten notes, equations). Other axis: retrieval approach (vision-first late interaction vs OCR-then-text vs hybrid). Each cell gets nDCG@5 and answer accuracy. The report is the deliverable.
...
PDFs -> page renderer (PyMuPDF, 180 DPI)
           |
           v
  ColQwen2.5-v0.2 embed (multi-vector (வெக்டர் — எண் பட்டியல்) per page, ~2048 patches)
           |
...
- Index: Vespa with multi-vector (வெக்டர் — எண் பட்டியல்) field, or Qdrant multi-vector, or AstraDB with MaxSim
...
2. **Embed.** Run ColQwen2.5-v0.2 on each page image. Output shape ~2048 patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s of dim 128. Apply DocPruner to keep the highest-signal half. Write to Vespa multi-vector (வெக்டர் — எண் பட்டியல்) field or Qdrant multi-vector.

3. **Query.** For each incoming query, embed with the query tower (token-level embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s). Run MaxSim against the index: for every query token, take the max dot-product over page patch embeddings, sum. Return top-k pages.
...
7. **Eval.** Run ViDoRe v3 (retrieval nDCG@5) and M3DocVQA (multi-page QA accuracy). Also run OCR-then-text pipeline on the same corpus with the same synthesizer. Produce a content-type × approach matrix (மேட்ரிக்ஸ் — எண் மேஜை).
...
2. Prune embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s aggressively (75%, 90%). Find the compression cliff: the point where ViDoRe nDCG@5 drops below the OCR baseline.
...
| Multi-vector (வெக்டர் — எண் பட்டியல்) | "Per-patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்)" | Each document has many vectors, not one pooled vector |
...
- [Vespa multi-vector (வெக்டர் — எண் பட்டியல்) tutorial](https://docs.vespa.ai/en/colpali.html) — reference serving stack
