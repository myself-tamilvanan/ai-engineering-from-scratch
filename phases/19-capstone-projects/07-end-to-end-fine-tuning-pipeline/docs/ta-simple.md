...
The pipeline has five stages. **Data**: dedup (MinHash / Datatrove), quality filter (Nemotron-CC style classifier), PII scrub, split-hygiene check against public benchmark contamination. **SFT**: Axolotl YAML, ZeRO-3 on 8xH100, cosine schedule, packed sequences, 2-3 epoch (யுக் — ஒரு முழு சுழற்சி)s. **DPO or GRPO**: TRL config, 1 epoch, preference pairs either human-labeled or model-judged, beta tuning. **Quantize**: GPTQ + AWQ + GGUF for deployment flexibility. **Serve**: vLLM 0.7 with EAGLE-3 speculative heads (or SGLang with SpecForge), K8s deployment, HPA on queue-wait.

Ablations are the deliverable: SFT-only vs SFT+DPO vs SFT+GRPO on three task-specific benchmarks. Serving metrics: tokens/s at batch (பத்தி — தொகுப்பு) 1 / 8 / 32, EAGLE-3 acceptance rate, $/1M tokens. Safety eval: Llama Guard 4 pass rate. Model card: bias evaluations, reproducibility seeds, data licensing.
...
Axolotl SFT config (YAML)  ---> 8xH100, ZeRO-3
    |
    v
TRL DPO / GRPO config       ---> 4xH100, 1 epoch (யுக் — ஒரு முழு சுழற்சி)
    |
    v
...
- Preference tuning: TRL 0.15 for DPO or GRPO. Unsloth for single-GPU iteration (மறுயுதல் — மீண்டும் செய்தல்)
...
3. **Axolotl SFT.** YAML with ZeRO-3, FA3, sequence packing. 2-3 epoch (யுக் — ஒரு முழு சுழற்சி)s on 8xH100. Log to W&B.

4. **TRL DPO / GRPO.** Take the SFT checkpoint, run one epoch (யுக் — ஒரு முழு சுழற்சி) of DPO on preference pairs (or GRPO with a verifiable reward on math/code). Sweep beta.

5. **Quantize.** Produce three quants: GPTQ-INT4-Marlin, AWQ-INT4, GGUF-Q4_K_M for llama.cpp. Record size and nominal throughput.

6. **Serve with speculative decoding.** vLLM 0.7 config with EAGLE-3 draft heads trained via Red Hat Speculators. Measure acceptance rate and tail latency at batch (பத்தி — தொகுப்பு) 1 / 8 / 32. Report $/1M tokens vs Anthropic / OpenAI on the same eval.

7. **Eval matrix (மேட்ரிக்ஸ் — எண் மேஜை).** Run lm-eval-harness, RewardBench-2, MT-Bench-v2, MMLU-Pro on base, SFT-only, SFT+DPO, SFT+GRPO. Produce a table.
...
[SFT]     3 epoch (யுக் — ஒரு முழு சுழற்சி)s, 8xH100, 6h12m, val loss 1.42 -> 1.03
...
| 20 | Serving speed | tokens/s at bs=1/8/32, EAGLE-3 acceptance rate, $/1M tokens |
...
| GRPO | "Group-relative policy optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)" | DeepSeek R1's RL recipe with verifiable rewards |
...
- [Unsloth](https://github.com/unslothai/unsloth) — single-GPU iteration (மறுயுதல் — மீண்டும் செய்தல்) reference
