...
An AST-aware ingestion pipeline parses each file with tree-sitter, extracts function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) and class nodes, and chunks at node boundaries quite than fixed token windows. Each chunk gets three representations: a dense embedding (embedding — பொருளை எண்களாக மாற்றுதல்) (Voyage-code-3 or nomic-embed-code), sparse BM25 terms, and a short natural-language summary. The summary adds a third retrievable modality — users ask "how is X authorized" and the summary mentions "authz", even if the code only has `check_permission`.
...
git push --> webhook --> ingest worker (LlamaIndex Workflow)
        ...
        (Voyage / bge)  (Tantivy)        (Haiku 4.5)
            |              |                |
            +------> Qdrant / pgvector (வெக்டர் — எண் பட்டியல்) <----+
                            |
                            v
...
- Dense embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s: Voyage-code-3 (hosted) or nomic-embed-code-v1.5 (self-host), bge-code-v1 fallback
- Sparse index: Tantivy (Rust) with BM25F, field-weighted on symbol name vs body
- vector (வெக்டர் — எண் பட்டியல்) DB: Qdrant 1.12 with hybrid search, or pgvector + pgvectorscale for teams under 50M vectors
...
1. **Ingestion walker.** Iterate git history on every push hook. Collect changed files. For each file, parse with tree-sitter, extract function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) and class nodes with their full source span. Emit chunk records `{repo, path, start_line, end_line, symbol, body}`.

2. **Chunk summarizer.** batch (பத்தி — தொகுப்பு) chunks into Haiku 4.5 calls with prompt caching on the system preamble. Prompt: "Summarize this function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) in one sentence, naming its public contract and side effects." Store summary alongside the chunk.

3. **embedding (embedding — பொருளை எண்களாக மாற்றுதல்) pool.** Two parallel queues: dense (Voyage-code-3 batch (பத்தி — தொகுப்பு) 128) and summary (same model, but on the summary string). Write vector (வெக்டர் — எண் பட்டியல்)s to Qdrant with payload `{repo, path, start_line, end_line, symbol, kind}`.

4. **BM25 index.** Field-weighted Tantivy index: symbol name weight 4, symbol body weight 1, summary weight 2. Enables "find the function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) named X" queries alongside "find the function that does X".

5. **Symbol graph.** For each chunk, record edges: imports (this file uses symbol Y from repo Z), calls (this function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) calls method M on class C), inheritance. Store in kuzu. Used at query time to expand retrieval across repo boundaries.
...
3. Benchmark Qdrant hybrid search vs pgvector (வெக்டர் — எண் பட்டியல்) + pgvectorscale at your corpus size. Report p99 at batch (பத்தி — தொகுப்பு) size 1.

4. Add a sampling-based drift check: weekly, rerun the 100-question eval. Alert on MRR@10 drop > 5%.

5. Extend to cross-language symbol resolution: a Python function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) that calls a Go service over gRPC. Use the symbol graph to link them.

## Key Terms

| Term | What people say | What it actually means |
|------|-----------------|------------------------|
| AST-aware chunking | "function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)-level splits" | Cutting code at tree-sitter node boundaries instead of fixed token windows |
| Hybrid search | "Dense + sparse" | Run BM25 and vector (வெக்டர் — எண் பட்டியல்) search in parallel, merge top-k, rerank |
| Cross-encoder rerank | "Second-stage rank" | Model that scores each (query, candidate) pair together, more accurate than cosine |
| Prompt caching | "Cached system prompt" | 2026 Claude / OpenAI feature (பண்பு — தகவல் நெடுவரிசை) that discounts repeat prefix tokens up to 90% |
...
- [Qdrant hybrid search docs](https://qdrant.tech/documentation/concepts/hybrid-queries/) — reference how to build
- [Voyage AI code embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s](https://docs.voyageai.com/docs/embeddings) — Voyage-code-3 details
