...
> A vision encoder produces image tokens. A text decoder consumes text tokens. The two live in different vector (வெக்டர் — எண் பட்டியல்) spaces. A small two-layer MLP projects image tokens into the text embedding (embedding — பொருளை எண்களாக மாற்றுதல்) space, and a cosine alignment loss against a paired caption pulls the two spaces into agreement. That projection is the smallest piece of a vision-language model and the one that matters most for transfer.
...
- Build a two-layer MLP projection that maps image feature (பண்பு — தகவல் நெடுவரிசை)s into the text embedding (embedding — பொருளை எண்களாக மாற்றுதல்) space.
...
You have a vision encoder (lessons 58-59) producing tokens of dimension (பரிமாணம் — கோணம்) `vision_hidden = 768`. You have a text decoder you want to bolt on top with embedding (embedding — பொருளை எண்களாக மாற்றுதல்) dimension `text_hidden = 512` (any other number is just as plausible). The decoder expects text-shaped tokens. The image tokens are not text-shaped: they live in a basis the encoder learned during vision-only pretraining, with no relationship to the decoder's word vector (வெக்டர் — எண் பட்டியல்)s.

Two-layer MLP projection (linear, GELU, linear) bridges the gap. It is small enough (about `768 * 1024 + 1024 * 512 = 1.3M` parameter (பாரமீட்டர் — கட்டுப்பாடு)s) to train in minutes on a single GPU, and it is the only piece that has to learn during the alignment phase. The vision encoder stays frozen. The text embedding (embedding — பொருளை எண்களாக மாற்றுதல்) table stays frozen. Only the projection moves. This is the recipe LLaVA shipped in 2023, that BLIP-2 reframed as a Q-Former, and that every open-weight VLM since has adopted in some form.
...
flowchart LR
  ...
  Proj --> Img[image embedding (embedding — பொருளை எண்களாக மாற்றுதல்) B x 512]
...
- `MockTextembedding (embedding — பொருளை எண்களாக மாற்றுதல்)(vocab_size, dim)`, a frozen embedding table with deterministic init from a seed.
...
- **LLaVA 1.5.** Two-layer GELU MLP projection from CLIP-ViT-L hidden to LLaMA embedding (embedding — பொருளை எண்களாக மாற்றுதல்) dim. Frozen vision encoder, frozen LLM, train only the projection (then unfreeze the LLM in stage two).
...
The shape varies but the role is identical: pool image tokens, project to text embedding (embedding — பொருளை எண்களாக மாற்றுதல்) dim, train alone.
...
- frozen text embedding (embedding — பொருளை எண்களாக மாற்றுதல்) table has zero `requires_grad` parameter (பாரமீட்டர் — கட்டுப்பாடு)s
- cosine loss is zero on identical vector (வெக்டர் — எண் பட்டியல்)s and is 2 on anti-parallel vectors
- projector gradient (சாய்வு — மாற்றத்தின் दिशा) flows after one backward pass
...
2. Add a learned scalar (scalar — ஒரு எண்) temperature to the cosine loss (`cos / tau`) and observe what happens when `tau` is too small (gradient (சாய்வு — மாற்றத்தின் दिशा) noise) or too large (loss plateaus high).

3. Swap the two-layer MLP for a single linear layer and quantify the loss gap. The non-linearity matters more on natural image feature (பண்பு — தகவல் நெடுவரிசை)s and less on synthetic ones.
...
| Term | What it means |
|------|---------------|
| Modality alignment | The act of making image and text embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s comparable in one shared space |
...
| Frozen encoder | The vision (or text) model has all parameter (பாரமீட்டர் — கட்டுப்பாடு)s with `requires_grad=False` |
