...
- Read tokens back through HDF5's page-cache-backed chunked layout so the dataloader copies into batch (பத்தி — தொகுப்பு) buffers only at batch time.
...
flowchart TD
  ...
  Window --> Train[Train batch (பத்தி — தொகுப்பு)]
```
...
A demo at the bottom of the file builds a tiny in-memory corpus, tokenizes into two shards, opens them via memory map, runs the dataloader for 10 batch (பத்தி — தொகுப்பு)es, and prints the per-batch shape and a checksum.
...
The script exits zero and prints batch (பத்தி — தொகுப்பு) checksums.
...
**Sharded sha256 with parallel verification.** Each shard has its own sha256 over the token bytes. The trainer can verify all shards in parallel before training starts. A wrong sha256 fails the run early, not on epoch (யுக் — ஒரு முழு சுழற்சி) three after sixteen hours.
...
2. Add a deterministic seed to the sliding-window dataloader and verify two runs with the same seed produce identical batch (பத்தி — தொகுப்பு)es.
