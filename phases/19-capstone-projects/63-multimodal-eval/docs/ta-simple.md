...
> Training is half the loop. The other half is measurement. This lesson builds three evaluation surfaces from primitives: image-caption retrieval reported as R@1, R@5, R@10. visual question answering reported as exact match accuracy. and image captioning reported as BLEU-4. Each metric is a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) over the model's outputs and a synthetic eval suite that runs in seconds.
...
- Compute Recall@K from a similarity matrix (மேட்ரிக்ஸ் — எண் மேஜை) between image and caption embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s.
...
The temptation is to declare a multimodal model finished when the training loss plateaus. Training loss measures fit on the training distribution. it does not measure whether the model can rank pairs in a held-out batch (பத்தி — தொகுப்பு), answer a question, or write a caption a human would accept. Three eval surfaces are standard:

- **Retrieval (R@1, R@5, R@10).** Build the joint embedding (embedding — பொருளை எண்களாக மாற்றுதல்) for a query caption. rank every image in the eval pool by cosine. report whether the matching image lands in the top 1, top 5, top 10. Symmetric (image-to-text) form runs the same way.
...
Each metric is a thin function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). The lesson builds them all in code so the math is concrete and the surface stays under your control. Real benchmark suites (MS-COCO, VQA v2, GQA, OK-VQA) plug into the same function shapes.

## The Concept

```mermaid
flowchart TB
  Model[trained multimodal model] --> Embed[joint embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s on eval set]
  Embed --> Sim[similarity matrix (மேட்ரிக்ஸ் — எண் மேஜை)]
  Sim --> R1[R at 1]
  Sim --> R5[R at 5]
  Sim --> R10[R at 10]
...
Smoothing is needed for small samples where some `p_n` is zero. The how to build uses Chen and Cherry "method 1" (add 1 to numerator and denominator for any zero count), which is the safest default for low-count regimes.
...
- `recall_at_k(sim_matrix (மேட்ரிக்ஸ் — எண் மேஜை), k)`, returning a float in `[0, 1]` for both directions.
...
- **Retrieval.** MS-COCO 5K val, Flickr30K, ImageNet zero-shot are all R@K problems on the same similarity matrix (மேட்ரிக்ஸ் — எண் மேஜை). Replace the synthetic eval with the real files and the function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) signature is unchanged.
...
For real benchmarks, swap `build_eval_suite` for a real loader and keep the function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) bodies. The math is benchmark-agnostic.

## Tests

`code/test_main.py` covers:

- recall@k returns 1.0 on a perfect identity similarity matrix (மேட்ரிக்ஸ் — எண் மேஜை) and 0.0 on a flipped one for k < N
...
- MS-COCO captioning eval scripts for the canonical reference how to build.
