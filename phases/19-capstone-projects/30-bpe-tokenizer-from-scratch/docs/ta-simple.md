...
The first 256 ids are reserved for the raw bytes 0x00 through 0xFF. That guarantees every input string is expressed in the vocabulary before any merge happens. After the byte block we reserve a small range for special tokens. The training loop never proposes those ids as merge targets because we keep them out of the pretokenized stream entirely.
...
- `<|pad|>` fills out short sequences so a batch (பத்தி — தொகுப்பு) is a rectangular tensor (டென்சர் — பல வடிவ எண் வரிசை). The loss mask hides it during training.
