...
- Replace a language-model head with a classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) head without re-initialising the body.
...
- Compute precision, recall, F1, and a confusion matrix (மேட்ரிக்ஸ் — எண் மேஜை) from raw logits.
- Reason about the trade-off between parameter (பாரமீட்டர் — கட்டுப்பாடு) count, training time, and head-room.
...
```mermaid
flowchart LR
  T[Tokens] --> E[Token + position<br/>embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s]
...
The model is a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) `f_theta(tokens) -> hidden_states`. The head is a function `g_phi(hidden) -> logits`. Swapping heads means keeping `theta` and replacing `g_phi`. The body's parameter (பாரமீட்டர் — கட்டுப்பாடு)s are the expensive part. The head is a single linear layer.

Two trainable parameter (பாரமீட்டர் — கட்டுப்பாடு) sets matter:

- `theta` (the body): tens of thousands of weights per attention block.
- `phi` (the head): `hidden_dim * num_classes` weights plus a bias.

In head-only training you compute gradient (சாய்வு — மாற்றத்தின் दिशा)s against `phi` and zero them against `theta`. PyTorch lets you do this by setting `requires_grad=False` on body parameter (பாரமீட்டர் — கட்டுப்பாடு)s. The optimiser then sees only the head and the body stays frozen.

In full fine-tuning you let gradient (சாய்வு — மாற்றத்தின் दिशा)s flow back through the whole stack. The body's weights drift to fit the classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) objective. The risk is catastrophic forgetting on small data: the body's pretraining gets washed out by overfitting noise.

## The Pooling Question

A classifier needs one vector (வெக்டர் — எண் பட்டியல்) per sequence, not one vector per token. Three common choices:
...
Binary classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) with class 1 as the positive class (spam). Counts are:
...
A confusion matrix (மேட்ரிக்ஸ் — எண் மேஜை) prints the four counts as a 2x2 grid. The demo writes this to stdout for both training regimes.
...
flowchart TD
  ...
  Tok --> DS[classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)Dataset<br/>pad + mask]
  DS --> DL[DataLoader<br/>batch (பத்தி — தொகுப்பு)ed]
...
The body is a deliberately tiny transformer: vocab 260, hidden 64, 4 heads, 2 blocks, max sequence 32. It is small enough to train both regimes to convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) inside ninety seconds on CPU. It is not pretrained in the lesson. instead, the `pretrain_quick` helper does five epoch (யுக் — ஒரு முழு சுழற்சி)s of LM training on the same fixture's text to give the body a non-trivial starting point. This keeps the lesson self-contained.

## What you will build

The how to build is one `main.py` plus one test module (`code/tests/test_main.py`).
...
3. `LMBody`: token + position embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s plus a stack of blocks. Returns hidden states.
...
6. `freeze_body` and `unfreeze_body`: toggle `requires_grad` on body parameter (பாரமீட்டர் — கட்டுப்பாடு)s.
...
The head-only regime usually trains faster and underfits more gracefully. On this fixture you usually see precision near 0.9 and recall near 0.85 after twenty epoch (யுக் — ஒரு முழு சுழற்சி)s of head-only training. Full fine-tuning takes about three times longer and lands within a couple of points either way, depending on the random seed.

The lesson does not pick a winner. It teaches you to read the numbers and the cost. On 800 examples and a tiny body, head-only is the right call. On 80,000 examples and a bigger body, full fine-tuning starts to pay off. The contract you take from this lesson is the API: the same `train_classifier` function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) handles both, and the toggle is one call.
...
The how to build gives you the hooks. The tests pin the contract. The numbers are yours to push.
