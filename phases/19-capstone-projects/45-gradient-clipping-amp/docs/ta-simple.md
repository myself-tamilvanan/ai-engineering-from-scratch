...
- Compute the global L2 norm over all parameter (பாரமீட்டர் — கட்டுப்பாடு) gradient (சாய்வு — மாற்றத்தின் दिशा)s and clip in place when it exceeds a configured threshold.
...
The global L2 norm is the Euclidean norm of the concatenated gradient (சாய்வு — மாற்றத்தின் दिशा) vector (வெக்டர் — எண் பட்டியல்), not the per-parameter (பாரமீட்டர் — கட்டுப்பாடு) norm. PyTorch implements this as `torch.nn.utils.clip_grad_norm_(parameters, max_norm)`. The function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) returns the pre-clip norm so the lesson can log both the natural and the clipped value, which is necessary for the "we are clipping at every step" diagnosis.

### autocast and GradScaler

`torch.amp.autocast(device_type)` is the context manager that selectively runs eligible operations (most matmul-class operations) in FP16. `torch.amp.GradScaler(device_type)` is the helper that scales the loss before backward and inverse-scales the gradient (சாய்வு — மாற்றத்தின் दिशा)s before the optimizer step. The two are designed together. using one without the other is a configuration error the test should catch.
...
- `has_non_finite_grad` - a helper that scans gradient (சாய்வு — மாற்றத்தின் दिशा)s for NaN and Inf.
...
**Skip counter as an alert, not a log line.** A handful of skipped steps per training run is healthy. Hundreds of skips per epoch (யுக் — ஒரு முழு சுழற்சி) are a hard alert: the model is in a regime FP16 cannot hold and the loop is silently failing. The lesson tracks a 1,000-step rolling skip rate and would, in production, page on a rate above 5 percent.

**Clip threshold lives in the config.** `max_norm = 1.0` is the modern default for language-model training. Sweep it on a small model first. larger thresholds let the model recover from genuinely difficult batch (பத்தி — தொகுப்பு)es. smaller thresholds bound the worst case at the cost of a noisier loss curve. The threshold belongs in the same YAML or JSON config as the schedule from lesson 44.

**Norm log goes to a CSV with the schedule.** The CSV columns are `step, lr, grad_l2_pre_clip, grad_l2_post_clip, loss, skipped, skip_reason, scaler_scale`. A reviewer who opens the file sees the schedule, the gradient (சாய்வு — மாற்றத்தின் दिशा) story, the scaling factor, and the skip outcome (with its reason) in one row. Splitting the columns across files is a recipe for misaligned analyses.
...
- **Loss check before backward.** `torch.isfinite(loss).all()` is one tensor (டென்சர் — பல வடிவ எண் வரிசை) reduction. the cost is negligible and the savings on a NaN loss are an entire training step. Always run it.
- **`set_to_none=True` in `zero_grad`.** Sets gradient (சாய்வு — மாற்றத்தின் दिशा)s to `None` instead of zero, which lets the optimizer skip calculation for unaffected parameter (பாரமீட்டர் — கட்டுப்பாடு) groups. The setting is a free throughput improvement and a slight bug-surface reduction.
...
1. Replace the synthetic Inf injection with a real loss spike (multiply one batch (பத்தி — தொகுப்பு)'s target by 1e8) and verify the skip path triggers.
...
3. Add a unit test that the gradient (சாய்வு — மாற்றத்தின் दिशा)-clip wrapper returns the pre-clip and post-clip norm correctly when no clipping occurs.
4. Add a rolling-window skip-rate calculation and a CLI flag that fails the run if the rate exceeds a configured threshold for 100 consecutive steps.
...
|------|-----------------|------------------------|
| Global L2 norm | "Clip target" | Euclidean norm of the concatenated gradient (சாய்வு — மாற்றத்தின் दिशा) vector (வெக்டர் — எண் பட்டியல்) across all trainable parameter (பாரமீட்டர் — கட்டுப்பாடு)s |
...
- [Pascanu, Mikolov, Bengio, On the difficulty of training recurrent neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)s (arXiv 1211.5063)](https://arxiv.org/abs/1211.5063) - the gradient (சாய்வு — மாற்றத்தின் दिशा)-clipping reference paper
