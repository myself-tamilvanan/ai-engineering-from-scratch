...
> Twelve Labs productized Marengo + Pegasus. VideoDB shipped the CRUD-for-video API. AI2's Molmo 2 published open VLM checkpoints. Gemini long-context handles hours of video natively. TimeLens-100K defined temporal grounding at scale. The 2026 pipeline is settled: scene segmentation, per-scene caption + embedding (embedding — பொருளை எண்களாக மாற்றுதல்), transcript alignment, multi-vector (வெக்டர் — எண் பட்டியல்) index, and a query that answers with (start, end) timestamps plus frame previews. The capstone is ingesting 100 hours, hitting public benchmarks, and measuring hallucination on counting and action questions.
...
Three pipelines run in parallel at ingest. **Scene segmentation** cuts the video into scenes. **VLM captioning** generates a caption per scene and a frame embedding (embedding — பொருளை எண்களாக மாற்றுதல்) from a keyframe. **ASR alignment** produces word-level timestamps. The three streams are joined by (scene_id, time range). Each scene gets three vector (வெக்டர் — எண் பட்டியல்) types in a multi-vector index (Qdrant): caption embedding, keyframe embedding, transcript embedding.

At query time, the natural-language question fires against all three vector (வெக்டர் — எண் பட்டியல்)s. results merge with RRF. a temporal-grounding adapter (TimeLens-style) refines the (start, end) window within the top scene. The VLM synthesizer (Gemini 2.5 Pro or Qwen3-VL-Max) takes query + top scenes + cropped frames and answers with cited timestamps and a frame preview.
...
PySceneDetect / TransNetV2  (scene segmentation)
      |
      +--- per-scene keyframe --- VLM caption + frame embedding (embedding — பொருளை எண்களாக மாற்றுதல்)
      |                            (Gemini 2.5 Pro / Qwen3-VL-Max / Molmo 2)
      |
      +--- audio channel --- Whisper-v3-turbo ASR + word timestamps
      |
      v
multi-vector (வெக்டர் — எண் பட்டியல்) Qdrant: {caption_emb, keyframe_emb, transcript_emb}
      |
query:
...
- Index: Qdrant with multi-vector (வெக்டர் — எண் பட்டியல்) support (caption / frame / transcript)
...
4. **VLM captioning.** Per scene, call Gemini 2.5 Pro (or Qwen3-VL-Max) with the keyframe and a short caption template. Produce caption + frame embedding (embedding — பொருளை எண்களாக மாற்றுதல்).

5. **Multi-vector (வெக்டர் — எண் பட்டியல்) index.** Qdrant collection with three named vectors. Payload: `{video_id, scene_id, start_ms, end_ms, keyframe_url}`.
...
[index]    69 vector (வெக்டர் — எண் பட்டியல்)s written (23 scenes x 3)
...
2. Reduce per-scene frame embedding (embedding — பொருளை எண்களாக மாற்றுதல்) to one pooled vector (வெக்டர் — எண் பட்டியல்) instead of multi-vector. Measure the retrieval regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்).
...
| Multi-vector (வெக்டர் — எண் பட்டியல்) index | "Caption + frame + transcript" | Qdrant collection with named vectors per representation |
| Temporal grounding | "When exactly did it happen" | Refining the (start, end) window for a query answer |
| Frame embedding (embedding — பொருளை எண்களாக மாற்றுதல்) | "Visual representation" | A vector embedding of a keyframe. used for scene-visual similarity |
