...
**Type:** Learn
**Languages:** Python
**Prerequisites:** Phase 11 · 09 (function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) Calling), Phase 11 · 16 (LangGraph)
**Time:** ~45 minutes
...
![Agent framework matrix (மேட்ரிக்ஸ் — எண் மேஜை): core abstraction vs problem shape](../assets/framework-matrix.svg)
...
- **LangGraph** — you decide, via conditional edges. Routing is a Python function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) with named branches. Branches are first-class in the compiled graph. the checkpointer records which branch was taken.
...
- **AutoGen** → `function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)Tool` wraps any Python callable. MCP adapter available. Tight coupling to AG2 ecosystem for agent-to-agent patterns.
- **Agno** → `@tool` decorator or BaseTool subclass. MCP adapter. tools is shared across agents and teams.
...
5. **Budget the framework extra work.** Every framework is another dependency. If the task is two LLM calls and a tool, write 30 lines of plain Python. no framework is cheaper than no framework.
...
## The Decision matrix (மேட்ரிக்ஸ் — எண் மேஜை)
...
| Explicit routing | "Developer decides" | A Python function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) or static edge picks the next step. cheap and auditable. |
