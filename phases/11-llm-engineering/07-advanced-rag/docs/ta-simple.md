...
**Time:** ~90 minutes
**Related:** Phase 5 · 23 (Chunking Strategies for RAG) covers all six chunking algorithm (அல்கோரிதம் — வழிமுறை)s — recursive, semantic, sentence, parent-document, late chunking, contextual retrieval — with Vectara/Anthropic benchmarks. This lesson builds on top: hybrid search, reranking, query transformation.

## Learning Objectives

- Implement advanced chunking strategies (semantic, recursive, parent-child) that preserve document structure and context
- Build a hybrid search pipeline combining BM25 keyword matching with semantic vector (வெக்டர் — எண் பட்டியல்) search and a cross-encoder reranker
...
**Ambiguous query**: "What was revenue last quarter?" Semantic search returns chunks about revenue strategy, revenue projections, and the CFO's thoughts on revenue growth. All semantically similar to the word "revenue." None containing the actual number. The correct chunk says "$47.2M in Q3 2025" but uses the word "earnings" instead of "revenue." The embedding (embedding — பொருளை எண்களாக மாற்றுதல்) model thinks "revenue strategy" is closer to the query than "Q3 earnings were $47.2M."
...
**Large corpus problem**: You have 2 million chunks. The correct answer is in chunk #1,847,293. Your top-5 retrieval pulls chunks #14, #89,201, #1,200,000, #44, and #901,333. Close in embedding (embedding — பொருளை எண்களாக மாற்றுதல்) space, but none containing the answer. At this scale, approximate nearest neighbor search introduces enough error that relevant results get pushed out of the top-k.

Basic RAG fails because vector (வெக்டர் — எண் பட்டியல்) similarity is not the same as relevance. A chunk is semantically similar to a query without being useful for answering it. Advanced RAG addresses this with four techniques: hybrid search (add keyword matching), reranking (score candidates more carefully), query transformation (fix the query before searching), and better chunking (retrieve at the right granularity).

## The Concept

### Hybrid Search: Semantic + Keyword

Semantic search (vector (வெக்டர் — எண் பட்டியல்) similarity) is good at understanding meaning. "How do I cancel my subscription?" matches "Steps to terminate your plan" even though they share no words. But it misses exact matches. "Error code E-4021" might not match a chunk containing "E-4021" if the embedding (embedding — பொருளை எண்களாக மாற்றுதல்) model treats it as noise.
...
**BM25** (Best Matching 25) is the standard keyword search algorithm (அல்கோரிதம் — வழிமுறை). It has been the backbone of search engines since the 1990s. The formula:
...
You have two ranked lists: one from vector (வெக்டர் — எண் பட்டியல்) search, one from BM25. How do you combine them? Reciprocal Rank Fusion is the standard approach.
...
Where k is a constant (usually 60) that prevents the top-ranked result from dominating.

A document ranked #1 in vector (வெக்டர் — எண் பட்டியல்) search and #5 in BM25 gets: 1/(60+1) + 1/(60+5) = 0.0164 + 0.0154 = 0.0318

A document ranked #3 in vector (வெக்டர் — எண் பட்டியல்) search and #2 in BM25 gets: 1/(60+3) + 1/(60+2) = 0.0159 + 0.0161 = 0.0320
...
Retrieval (whether vector (வெக்டர் — எண் பட்டியல்), keyword, or hybrid) is fast but imprecise. It uses bi-encoders: the query and each document are embedded independently, then compared. The embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s are computed once and cached. This scales to millions of documents.
...
- ColBERTv2 / Jina-ColBERT-v2: late-interaction multi-vector (வெக்டர் — எண் பட்டியல்) rerankers — O(tokens) not O(docs) at scoring time

### Query Transformation

Sometimes the problem is not retrieval but the query itself. "What was that thing about the new policy change?" is a terrible search query. It contains no specific terms. The embedding (embedding — பொருளை எண்களாக மாற்றுதல்) is vague. No retrieval system can find the right documents from this.
...
**HyDE (Hypothetical Document embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s)**: instead of searching with the query, generate a hypothetical answer, embed that, and search for similar real documents.
...
Embed the hypothetical answer and search for real documents similar to it. The simple idea: the hypothetical answer lives closer in embedding (embedding — பொருளை எண்களாக மாற்றுதல்) space to the real answer than the original question does. Questions and answers have different linguistic structures. By generating a hypothetical answer, you bridge the gap between "question space" and "answer space" in the embedding.
...
Before running vector (வெக்டர் — எண் பட்டியல்) search, filter the corpus by metadata: date, source, category, author, language. This reduces the search space and prevents irrelevant results.
...
Production RAG systems store metadata alongside each chunk: source document, creation date, category, author, version. vector (வெக்டர் — எண் பட்டியல்) databases support pre-filtering by metadata before similarity search, which is critical for performance at scale.
...
```python
def hybrid_search(query, chunks, vector (வெக்டர் — எண் பட்டியல்)_embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s, vocab, idf, bm25_index, top_k=5, fusion_k=60):
...
### Step 5: HyDE (Hypothetical Document embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s)
...
def hyde_search(query, chunks, vector (வெக்டர் — எண் பட்டியல்)_embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s, vocab, idf, top_k=5):
...
The alpha parameter (பாரமீட்டர் — கட்டுப்பாடு) controls the balance: 0.0 = pure keyword (BM25), 1.0 = pure vector (வெக்டர் — எண் பட்டியல்), 0.5 = equal weight. Most production systems use alpha between 0.3 and 0.7.
...
1. Compare BM25 vs vector (வெக்டர் — எண் பட்டியல்) search vs hybrid search on the sample documents. For each of the 5 test queries, record which approach returns the most relevant chunk in position #1. Hybrid search should win on at least 3 out of 5.

2. Implement a metadata filter. Add a "category" field to each document (security, billing, api, product). Before running vector (வெக்டர் — எண் பட்டியல்) search, filter chunks to only the relevant category. Test with "What encryption helps?" and verify it only searches security-category chunks.

3. Build a full HyDE pipeline using the simple generate function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) from Lesson 06. Compare retrieval quality (top-3 relevance) between direct query search and HyDE search on all 5 test queries. HyDE should improve results for vague queries.
...
5. Create an evaluation dataset: 10 questions with known answer chunks. Measure Recall@3, Recall@5, and Recall@10 for (a) vector (வெக்டர் — எண் பட்டியல்) search only, (b) BM25 only, (c) hybrid search, (d) hybrid + reranking. Plot the results and identify where reranking helps most.

## Key Terms

| Term | What people say | What it actually means |
|------|----------------|----------------------|
| BM25 | "Keyword search" | A probabilistic ranking algorithm (அல்கோரிதம் — வழிமுறை) that scores documents by term frequency, inverse document frequency, and document length normalization |
| Hybrid search | "Best of both worlds" | Running semantic (vector (வெக்டர் — எண் பட்டியல்)) and keyword (BM25) search in parallel, then merging results with rank fusion |
...
| Bi-encoder | "Independent embedding (embedding — பொருளை எண்களாக மாற்றுதல்) model" | A model that embeds queries and documents independently. fast because embeddings are precomputed, but less accurate than cross-encoders |
...
- Gao et al., "Precise Zero-Shot Dense Retrieval without Relevance Labels" (2022) -- the HyDE paper demonstrating that hypothetical document embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s improve retrieval without any training data
