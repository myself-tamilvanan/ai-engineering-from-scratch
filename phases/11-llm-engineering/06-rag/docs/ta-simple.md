...
**Time:** ~90 minutes
**Related:** Phase 5 · 23 (Chunking Strategies for RAG) for the six chunking algorithm (அல்கோரிதம் — வழிமுறை)s and when each wins. Phase 5 · 22 (embedding (embedding — பொருளை எண்களாக மாற்றுதல்) Models Deep Dive) for picking the embedder. Phase 11 · 07 (Advanced RAG) for hybrid search, reranking, and query transformation.

## Learning Objectives

- Build a complete RAG pipeline: document loading, chunking, embedding (embedding — பொருளை எண்களாக மாற்றுதல்), vector (வெக்டர் — எண் பட்டியல்) storage, retrieval, and generation
...
graph LR
    ...
    subgraph "Retrieve"
        R --> Embed["Embed query"]
        Embed --> Search["Search vector (வெக்டர் — எண் பட்டியல்) store"]
...
|---------|------------|-----|
| Cost | $1,000-$100,000+ per training run | $0.01-$0.10 per query (embedding (embedding — பொருளை எண்களாக மாற்றுதல்) + LLM) |
...
| Data privacy | Training data baked into weights | Documents stay in your vector (வெக்டர் — எண் பட்டியல்) store |
...
### embedding (embedding — பொருளை எண்களாக மாற்றுதல்) Models

An embedding (embedding — பொருளை எண்களாக மாற்றுதல்) model converts text into a dense vector (வெக்டர் — எண் பட்டியல்). Similar texts produce vectors that are close together in this high-dimension (பரிமாணம் — கோணம்)al space. "How do I reset my password?" and "I need to change my password" produce nearly identical vectors despite sharing few words. "The cat sat on the mat" produces a very different vector.

Common embedding (embedding — பொருளை எண்களாக மாற்றுதல்) models (2026 lineup — see Phase 5 · 22 for full analysis):

| Model | dimension (பரிமாணம் — கோணம்)s | Provider | Notes |
|-------|-----------|----------|-------|
| text-embedding (embedding — பொருளை எண்களாக மாற்றுதல்)-3-small | 1536 (Matryoshka) | OpenAI | Best price/performance for most use cases |
...
For this lesson, we build our own simple embedding (embedding — பொருளை எண்களாக மாற்றுதல்) using TF-IDF. Not because TF-IDF is what production systems use, but because it makes the concept concrete: text goes in, a vector (வெக்டர் — எண் பட்டியல்) comes out, similar texts produce similar vectors.

### vector (வெக்டர் — எண் பட்டியல்) Similarity

Given two vector (வெக்டர் — எண் பட்டியல்)s, how do you measure similarity? Three options:

**Cosine similarity**: the cosine of the angle between two vector (வெக்டர் — எண் பட்டியல்)s. Ranges from -1 (opposite) to 1 (identical). Ignores magnitude, only cares about direction. This is the default for RAG.

```
cosine_sim(a, b) = dot(a, b) / (||a|| * ||b||)
```

**Dot product**: the raw inner product. Larger vector (வெக்டர் — எண் பட்டியல்)s get higher scores. Useful when magnitude carries information (longer documents might be more relevant).

```
dot(a, b) = sum(a_i * b_i)
```

**L2 (Euclidean) distance**: straight-line distance in the vector (வெக்டர் — எண் பட்டியல்) space. Smaller distance = more similar. Sensitive to magnitude differences.

```
L2(a, b) = sqrt(sum((a_i - b_i)^2))
```

Cosine similarity is the standard. It handles documents of different lengths gracefully because it normalizes by magnitude. When someone says "vector (வெக்டர் — எண் பட்டியல்) search," they almost always mean cosine similarity.

### Chunking Strategies

Documents are too long to embed as single vector (வெக்டர் — எண் பட்டியல்)s. A 50-page PDF might produce a terrible embedding (embedding — பொருளை எண்களாக மாற்றுதல்) because it contains dozens of topics. Instead, you split documents into chunks and embed each chunk separately.
...
### vector (வெக்டர் — எண் பட்டியல்) Databases

Once you have embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s, you need somewhere to store and search them. Options:

| Database | Type | Best for |
|----------|------|----------|
| FAISS | Library (in-process) | Prototyping, small to medium datasets |
| Chroma | Lightweight DB | Local development, small deployments |
| Pinecone | Managed service | Production without ops extra work |
| Weaviate | Open source DB | Self-hosted production |
| pgvector (வெக்டர் — எண் பட்டியல்) | Postgres extension | Already using Postgres |
| Qdrant | Open source DB | High-performance self-hosted |

For this lesson, we build a simple in-memory vector (வெக்டர் — எண் பட்டியல்) store. It stores vectors in a list and does brute-force cosine similarity search. This is equivalent to FAISS with a flat index. It scales to maybe 100,000 vectors before getting slow. Production systems use approximate nearest neighbor (ANN) algorithm (அல்கோரிதம் — வழிமுறை)s like HNSW to search millions of vectors in milliseconds.
...
graph TD
    subgraph "Indexing (offline)"
        ...
        E --> S["Store vector (வெக்டர் — எண் பட்டியல்)s + text"]
    end

    subgraph "Querying (online)"
        Q["User query"] --> QE["Embed query"]
        QE --> VS["vector (வெக்டர் — எண் பட்டியல்) search (top-k)"]
        VS --> P["Build prompt with chunks"]
        P --> LLM["LLM generates answer"]
    end

    S -.->|"same vector (வெக்டர் — எண் பட்டியல்) space"| VS
```
...
Most production RAG systems use these parameter (பாரமீட்டர் — கட்டுப்பாடு)s:
...
- **embedding (embedding — பொருளை எண்களாக மாற்றுதல்) dimension (பரிமாணம் — கோணம்)**: 384-3072 depending on model
...
### Step 2: TF-IDF embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s

We build a simple embedding (embedding — பொருளை எண்களாக மாற்றுதல்) function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). TF-IDF (Term Frequency-Inverse Document Frequency) is not a neural embedding, but it converts text to vector (வெக்டர் — எண் பட்டியல்)s in a way that captures word importance. Frequent words in a document get higher TF. Rare words across the corpus get higher IDF. The product gives a vector where important, distinctive words have high values.
...
def search(query_embedding (embedding — பொருளை எண்களாக மாற்றுதல்), stored_embeddings, top_k=5):
    scores = []
...
class RAGPipeline:
    def __init__(self):
        self.chunks = []
        self.embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s = []
        self.vocab = []
        self.idf = []
    ...
    def query(self, question, top_k=5):
        ...
        results = search(query_emb, self.embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s, top_k)
...
With a real embedding (embedding — பொருளை எண்களாக மாற்றுதல்) model and LLM, the code barely changes:
...
def embed(text):
    response = client.embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s.create(
...
The pipeline is the same. Swap the embedding (embedding — பொருளை எண்களாக மாற்றுதல்) function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). Swap the generation function. The retrieval logic, chunking, prompt construction -- all identical regardless of which models you use.

For vector (வெக்டர் — எண் பட்டியல்) storage at scale, replace the brute-force search with a proper vector database:
...
Chroma handles the embedding (embedding — பொருளை எண்களாக மாற்றுதல்) internally (it uses all-MiniLM-L6-v2 by default) and stores the vector (வெக்டர் — எண் பட்டியல்)s in a local database. Same pattern, different plumbing.
...
1. Replace the TF-IDF embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s with a simple bag-of-words approach (binary: 1 if word present, 0 if not). Compare retrieval quality on the sample documents. TF-IDF should outperform because it weights rare words higher.
...
| embedding (embedding — பொருளை எண்களாக மாற்றுதல்) | "Convert text to numbers" | A dense vector (வெக்டர் — எண் பட்டியல்) representation of text where similar meanings produce similar vectors |
...
| Chunking | "Split docs into pieces" | Breaking documents into smaller segments (usually 256-512 tokens) so each is embedded and retrieved independently |
...
| Augmented generation | "Answer using given context" | Generating a response using retrieved documents as context quite than relying solely on trained knowledge |
...
| Indexing | "Preparing docs for search" | The offline process of chunking, embedding, and storing documents so they is searched at query time |
...
- Sentence-BERT: Reimers & Gurevych (2019) -- the paper behind the all-MiniLM embedding (embedding — பொருளை எண்களாக மாற்றுதல்) models, showing how to train bi-encoders for semantic similarity
