...
**Tool definitions**: each tool adds 50-200 tokens (name, description, parameter (பாரமீட்டர் — கட்டுப்பாடு) schema). 50 tools at 150 tokens each is 7,500 tokens before any conversation happens. Dynamic tool selection -- only including tools relevant to the current query -- can reduce this by 60-80%.

**Retrieved context**: documents from a vector (வெக்டர் — எண் பட்டியல்) database, search results, file contents. The quality of retrieval directly determines the quality of the response. Bad retrieval is worse than no retrieval -- it fills the window with noise and actively misleads the model.
...
**Long-term memory**: facts and preferences that persist across conversations. "The user prefers TypeScript." "The project uses PostgreSQL." Stored in a database, retrieved on session start. Claude Code stores this in CLAUDE.md files. ChatGPT stores it in its memory feature (பண்பு — தகவல் நெடுவரிசை).

**Episodic memory**: specific past interactions that might be relevant. "Last Tuesday, we debugged a similar issue in the auth module." Stored as embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s, retrieved when the current conversation matches a past episode.
...
graph TD
    subgraph Memory["Memory Architecture"]
        ...
        EM["Episodic Memory\n(past interactions)\nembedding (embedding — பொருளை எண்களாக மாற்றுதல்)s -> retrieved on similarity"]
    end
...
def classify_intent(query):
    ...
    intent_keywords = {
        "code": ["code", "function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)", "bug", "error", "file", "implement", "refactor", "debug", "test"],
...
class ContextEngine:
    def __init__(self, max_tokens=128000, generation_reserve=4000):
        ...
        self.knowledge_base = [
            "Python 3.12 introduced type parameter (பாரமீட்டர் — கட்டுப்பாடு) syntax for generic classes using bracket notation.",
            "The project uses PostgreSQL 16 with pgvector (வெக்டர் — எண் பட்டியல்) for embedding (embedding — பொருளை எண்களாக மாற்றுதல்) storage.",
...
def run_demo():
    ...
    budget = engine.chat("What is the best approach for implementing vector (வெக்டர் — எண் பட்டியல்) search in PostgreSQL?")
    print(budget.report())

    print("\n--- Query 3: After conversation history builds up ---")
    for i in range(8):
        engine.conversation.add_turn("user", f"Follow-up question number {i+1} about the how to build details of the system")
...
Cursor indexes your entire codebase into embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s. When you type a query, it retrieves the most relevant files and code blocks using vector (வெக்டர் — எண் பட்டியல்) similarity. Only those pieces go into the context window. A 500K-line codebase is compressed into the 5-10 most relevant code blocks.
...
Retrieval-Augmented Generation is context engineering formalized. Instead of stuffing knowledge into the model's weights (training) or the system prompt (static context), you retrieve relevant documents at query time and inject them into the context window. The entire RAG pipeline -- chunking, embedding (embedding — பொருளை எண்களாக மாற்றுதல்), retrieval, reranking -- exists to solve one problem: putting the right information in the context window.

## Ship It

This lesson produces `outputs/prompt-context-optimizer.md` -- a reusable prompt that audits a context assembly strategy and recommends optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)s. Feed it your system prompt, tool count, average history length, and retrieval strategy, and it identifies token waste and suggests improvements.
...
2. Implement semantic deduplication for retrieved context. If two retrieved documents are more than 80% similar (by word overlap or cosine similarity of their embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s), keep only the higher-scored one. Measure how much token budget this recovers.
...
5. Build a multi-strategy context compressor. Implement three compression strategies (truncation, summarization, extraction of key sentences) and benchmark them on a set of 20 documents. Measure the choice between compression ratio and information retention (does the compressed version still contain the answer to the query?).
...
| Dynamic context | "Loading stuff on the fly" | Assembling the context window differently for each query based on intent classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்), relevant tool selection, and retrieval results |
...
| Episodic memory | "Remembering specific past events" | Past interactions stored as embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s and retrieved when the current query is similar to a past conversation |
...
- [LangChain documentation on RAG](https://python.langchain.com/docs/tutorials/rag/) -- practical how to build of retrieval-augmented generation as a context engineering pattern
