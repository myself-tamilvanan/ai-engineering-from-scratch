...
For long-running batch (பத்தி — தொகுப்பு) jobs, the 5-minute default expires between jobs. Set `ttl`:

```python
{"type": "text", "text": RUBRIC, "cache_control": {"type": "ephemeral", "ttl": "1h"}}
```

1-hour TTL costs 2x the write premium (50% over baseline instead of 25%) but pays back fast on any batch (பத்தி — தொகுப்பு) reusing the prefix more than 5 times.
...
| batch (பத்தி — தொகுப்பு) job reusing a prefix for 30+ minutes | Anthropic with `ttl: "1h"` |
...
| Extended TTL | "1-hour Anthropic cache" | `{"type": "ephemeral", "ttl": "1h"}`. 2x write premium but worth it for batch (பத்தி — தொகுப்பு) reuse. |
