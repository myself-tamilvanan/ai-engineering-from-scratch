...
- Calculate the parameter (பாரமீட்டர் — கட்டுப்பாடு) savings of LoRA vs full fine-tuning: rank r with d_model dimension (பரிமாணம் — கோணம்)s trains 2*r*d parameters instead of d^2
...
Full fine-tuning updates every parameter (பாரமீட்டர் — கட்டுப்பாடு) in the model. Llama 3 8B has 8 billion parameters. In fp16, each parameter takes 2 bytes. That's 16GB just to load the weights. During training, you also need gradient (சாய்வு — மாற்றத்தின் दिशा)s (16GB), optimizer states for Adam (32GB for momentum + variance), and activations. Total: roughly 56GB of VRAM for a single 8B model.

An A100 80GB can barely fit this. Two A100s cost $3-4/hour on cloud providers. Training for 3 epoch (யுக் — ஒரு முழு சுழற்சி)s on 50,000 examples takes 6-10 hours. That's $30-40 per experiment. Run 10 experiments to get the hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s right and you've spent $400 before deploying anything.
...
You need a method that trains fewer parameter (பாரமீட்டர் — கட்டுப்பாடு)s, uses less memory, and doesn't destroy the model's existing knowledge.

## The Concept

### LoRA: Low-Rank Adaptation

Edward Hu and colleagues at Microsoft published LoRA in June 2021. The paper's insight: the weight updates during fine-tuning have low intrinsic rank. You don't need to update all 16.7 million parameter (பாரமீட்டர் — கட்டுப்பாடு)s in a 4096x4096 weight matrix (மேட்ரிக்ஸ் — எண் மேஜை). The useful information in the update is captured by a matrix of rank 16 or 32.
...
Where W is a d_out x d_in matrix (மேட்ரிக்ஸ் — எண் மேஜை). For a 4096x4096 attention projection, that's 16,777,216 parameter (பாரமீட்டர் — கட்டுப்பாடு)s.
...
Where B is (d_out x r) and A is (r x d_in). The rank r is much smaller than d -- usually 8, 16, or 32.

For r=16 on a 4096x4096 layer:
- Original parameter (பாரமீட்டர் — கட்டுப்பாடு)s: 4096 x 4096 = 16,777,216
- LoRA parameters: (4096 x 16) + (16 x 4096) = 65,536 + 65,536 = 131,072
- Reduction: 131,072 / 16,777,216 = 0.78%

You're training 0.78% of the parameter (பாரமீட்டர் — கட்டுப்பாடு)s and getting 95-100% of the quality.
...
When alpha = r, the scaling is 1x. When alpha = 2r (the common default), the scaling is 2x. This hyperparameter (பாரமீட்டர் — கட்டுப்பாடு) controls the learning rate of the LoRA path independently of the base learning rate.
...
- Higher alpha means larger updates per step, which can speed convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) or cause instability
...
|------|---------------------------|----------|
| 4 | 32,768 | Simple classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்), sentiment |
...
**NF4 (Normal Float 4-bit)**: A new data type designed specifically for neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு) weights. Neural network weights follow a roughly normal distribution. NF4 places its 16 quantization levels at the quantiles of a standard normal distribution. This is information-theoretically optimal for normally distributed data. It loses less information than uniform 4-bit quantization (INT4) or standard Float4.

**Double quantization**: The quantization constants themselves take memory. Each block of 64 weights needs a fp32 scale factor (4 bytes). For a 7B model, that's an extra 0.4GB. Double quantization quantizes these constants to fp8, reducing the extra work to 0.1GB. Small but it adds up.
...
Does reducing parameter (பாரமீட்டர் — கட்டுப்பாடு)s or quantizing the base hurt quality? The results from multiple papers:
...
Fine-tuning Llama 3 8B on 50,000 examples (3 epoch (யுக் — ஒரு முழு சுழற்சி)s):
...
After training, you have two things: the frozen base model and a small LoRA adapter (usually 10-100MB). You can either:
...
2. **Merge them permanently**: Compute W' = W + (alpha/r) * BA and save the result as a new full model. The merged model is the same size as the original. No inference extra work. No adapter to manage.
...
- **TIES-Merging** (Yadav et al. 2023): Trims small-magnitude parameter (பாரமீட்டர் — கட்டுப்பாடு)s, resolves sign conflicts, then merges. Reduces interference between adapters.
...
class LoRALayer(nn.Module):
    def __init__(self, in_feature (பண்பு — தகவல் நெடுவரிசை)s, out_features, rank=8, alpha=16):
        super().__init__()
        self.rank = rank
        self.alpha = alpha
        self.scaling = alpha / rank

        self.A = nn.parameter (பாரமீட்டர் — கட்டுப்பாடு)(torch.randn(in_feature (பண்பு — தகவல் நெடுவரிசை)s, rank) * (1 / math.sqrt(rank)))
...
class LinearWithLoRA(nn.Module):
    def __init__(self, linear, rank=8, alpha=16):
        ...
        self.linear = linear
        self.lora = LoRALayer(
            linear.in_feature (பண்பு — தகவல் நெடுவரிசை)s, linear.out_features, rank, alpha
        )

        for param in self.linear.parameter (பாரமீட்டர் — கட்டுப்பாடு)s():
...
The original linear layer is frozen. Only the LoRA parameter (பாரமீட்டர் — கட்டுப்பாடு)s (A and B) are trainable.

### Step 3: Inject LoRA into a Model

```python
def inject_lora(model, target_modules, rank=8, alpha=16):
    for param in model.parameter (பாரமீட்டர் — கட்டுப்பாடு)s():
        param.requires_grad = False
...
First, freeze every parameter (பாரமீட்டர் — கட்டுப்பாடு) in the model. Then walk the model tree, find linear layers matching your target names, and replace them with LoRA-wrapped versions. The LoRA A and B matrices are the only trainable parameters in the entire model.

### Step 4: Count parameter (பாரமீட்டர் — கட்டுப்பாடு)s

```python
def count_parameter (பாரமீட்டர் — கட்டுப்பாடு)s(model):
...
After merging, the LoRA layers are gone. The model is the same size as the original with the adaptation baked into the weights. No inference extra work.

### Step 6: Simulated QLoRA Quantization

```python
def quantize_to_nf4(tensor (டென்சர் — பல வடிவ எண் வரிசை), block_size=64):
...
```python
def train_lora(model, data, epoch (யுக் — ஒரு முழு சுழற்சி)s=5, lr=1e-3, batch (பத்தி — தொகுப்பு)_size=4):
    optimizer = torch.optim.AdamW(
        [p for p in model.parameter (பாரமீட்டர் — கட்டுப்பாடு)s() if p.requires_grad], lr=lr
    )
    criterion = nn.MSELoss()

    losses = []
    for epoch (யுக் — ஒரு முழு சுழற்சி) in range(epochs):
        epoch_loss = 0.0
        n_batch (பத்தி — தொகுப்பு)es = 0
        indices = torch.randperm(len(data["inputs"]))

        for i in range(0, len(indices), batch (பத்தி — தொகுப்பு)_size):
            ...
            epoch (யுக் — ஒரு முழு சுழற்சி)_loss += loss.item()
            n_batch (பத்தி — தொகுப்பு)es += 1

        avg_loss = epoch (யுக் — ஒரு முழு சுழற்சி)_loss / n_batch (பத்தி — தொகுப்பு)es
...
def demo():
    ...
    params_before = count_parameter (பாரமீட்டர் — கட்டுப்பாடு)s(model)

    lora_layers = inject_lora(
        model, target_modules=["0", "2"], rank=8, alpha=16
    )

    params_after = count_parameter (பாரமீட்டர் — கட்டுப்பாடு)s(model)

    losses = train_lora(model, data, epoch (யுக் — ஒரு முழு சுழற்சி)s=20, lr=1e-3)

    merge_lora_weights(model)
    params_merged = count_parameter (பாரமீட்டர் — கட்டுப்பாடு)s(model)
...
The demo creates a small model, injects LoRA into two layers, trains it, and merges the weights back. The parameter (பாரமீட்டர் — கட்டுப்பாடு) count drops from full trainable to ~1% trainable during LoRA training, then returns to the original architecture after merging.
...
model.print_trainable_parameter (பாரமீட்டர் — கட்டுப்பாடு)s()
```
...
training_args = TrainingArguments(
    ...
    num_train_epoch (யுக் — ஒரு முழு சுழற்சி)s=3,
    per_device_train_batch (பத்தி — தொகுப்பு)_size=4,
    gradient (சாய்வு — மாற்றத்தின் दिशा)_accumulation_steps=4,
    learning_rate=2e-4,
    fp16=True,
    logging_steps=10,
    save_strategy="epoch",
...
- `outputs/prompt-lora-advisor.md` -- a prompt that helps you decide LoRA rank, target modules, and hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s for your specific task
- `outputs/skill-fine-tuning-guide.md` -- a skill that teaches agents the decision tree for when and how to fine-tune

## Exercises

1. **Rank ablation study.** Run the demo with ranks 2, 4, 8, 16, 32, and 64. Plot final loss vs. rank. Find the point of diminishing returns where doubling the rank no longer halves the loss. For a simple classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) task on 256-dim feature (பண்பு — தகவல் நெடுவரிசை)s, this should be around r=8-16.

2. **Target module comparison.** Modify inject_lora to target only layer "0", only layer "2", only layer "4", and all three. Train each variant for 20 epoch (யுக் — ஒரு முழு சுழற்சி)s. Compare convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) speed and final loss. This mirrors the real decision of targeting q_proj vs v_proj vs all linear layers.
...
| Rank (r) | "How much the model can learn" | The inner dimension (பரிமாணம் — கோணம்) of the A and B matrices. controls expressiveness vs. parameter (பாரமீட்டர் — கட்டுப்பாடு) count |
| Alpha | "LoRA learning rate" | Scaling factor applied to the LoRA output. alpha/r scales the adaptation's contribution to the final output |
| NF4 | "4-bit quantization" | Normal Float 4: a 4-bit data type with quantization levels at normal distribution quantiles, optimal for neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு) weights |
...
| Merging | "Bake it in" | Computing W + (alpha/r) * BA and replacing the original weight, eliminating the adapter extra work at inference |
...
- PEFT library documentation (huggingface.co/docs/peft) -- the standard library for LoRA, QLoRA, and other parameter (பாரமீட்டர் — கட்டுப்பாடு)-efficient methods in the Hugging Face ecosystem
...
- [Rafailov et al., "Direct Preference optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்): Your Language Model is Secretly a Reward Model" (NeurIPS 2023)](https://arxiv.org/abs/2305.18290) -- DPO derivation. the preference-tuning stage that comes after SFT, no reward model needed.
