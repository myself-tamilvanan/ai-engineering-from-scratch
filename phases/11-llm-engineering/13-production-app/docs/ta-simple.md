...
- Wire all Phase 11 components (prompts, RAG, function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) calling, caching, guardrails) into a single production-ready service
...
Building an LLM feature (பண்பு — தகவல் நெடுவரிசை) takes an afternoon. Shipping an LLM product takes months.
...
- Your monthly bill hits $12,000 and you have no idea which feature (பண்பு — தகவல் நெடுவரிசை) caused it.
...
This is the capstone. You will build a complete production LLM service that integrates prompt management (L01-02), embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s and vector (வெக்டர் — எண் பட்டியல்) search (L04-07), function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) calling (L09), evaluation (L10), caching (L11), guardrails (L12), streaming, error handling, observability, and cost tracking. One service. Every component wired together.
...
graph LR
    ...
    Cache["Semantic Cache<br/>embedding (embedding — பொருளை எண்களாக மாற்றுதல்) Lookup"]
...
| embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s | L04 | text-embedding-3-small | Semantic similarity for cache and RAG |
| vector (வெக்டர் — எண் பட்டியல்) Store | L06-07 | In-memory (prod: Pinecone/Qdrant) | Nearest neighbor search for context retrieval |
| function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) Calling | L09 | Tool registry + JSON Schema | External data access, structured actions |
...
**Layer 2: Model failures.** The model returns malformed JSON, hallucinates a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) name, or produces an output that fails validation. Solution: retry with a corrected prompt. Include the error in the retry message so the model can self-correct.

**Layer 3: Application failures.** A downstream service is unreachable, the vector (வெக்டர் — எண் பட்டியல்) store is slow, a guardrail throws an exception. Solution: graceful degradation. If RAG context is unavailable, proceed without it. If the cache is down, bypass it. Never let a secondary system crash the primary flow.
...
| vector (வெக்டர் — எண் பட்டியல்) store down | No retry on vector store | Skip RAG context | Lower quality, still function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)al |
...
**Tracing.** A single user request touches 5-8 components. OpenTelemetry traces let you see the full journey: how long did embedding (embedding — பொருளை எண்களாக மாற்றுதல்) take? Was it a cache hit? How long was the LLM call? Did the guardrail add latency? Without tracing, debugging production issues is guesswork.
...
**Cursor.** The open file, surrounding files, recent edits, and terminal output form the context. A prompt router decides: small model for autocomplete (Cursor-small, ~20ms), large model for chat (Claude Sonnet 4.6 / GPT-5, ~3s). Context is aggressively compressed -- only relevant code sections, not entire files. Codebase embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s provide long-range context. Speculative edits stream diffs, not full files. MCP integration lets third-party tools plug in without per-tool code changes.

**ChatGPT.** Plugins, function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) calling, and MCP servers let the model access the web, run code, generate images, and query databases. A routing layer decides which capabilities to invoke. Memory persists user preferences across sessions. The system prompt is 1,500+ tokens of behavioral rules, cached via prompt caching. Multiple models serve different feature (பண்பு — தகவல் நெடுவரிசை)s: GPT-5 for chat, GPT-Image for images, Whisper for voice, o4-mini for deep reasoning.
...
- Semantic caching using cosine similarity on embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s
...
embedding (embedding — பொருளை எண்களாக மாற்றுதல்)-based cache that matches semantically similar queries. Two questions phrased differently but meaning the same thing will hit the cache.

```python
def simple_embedding (embedding — பொருளை எண்களாக மாற்றுதல்)(text, dim=64):
...
class SemanticCache:
    ...
    def get(self, query):
        query_emb = simple_embedding (embedding — பொருளை எண்களாக மாற்றுதல்)(query)
        now = time.time()
        ...
        for entry in self.entries:
            if now - entry["timestamp"] > self.ttl:
                continue
            score = cosine_similarity(query_emb, entry["embedding (embedding — பொருளை எண்களாக மாற்றுதல்)"])
            if score > best_score:
                best_score = score
                best_entry = entry
    ...
    def put(self, query, response):
        ...
        self.entries.append({
            "query": query,
            "embedding (embedding — பொருளை எண்களாக மாற்றுதல்)": simple_embedding(query),
            "response": response,
            "timestamp": time.time(),
        })
...
SIMULATED_RESPONSES = {
    "general": "Based on the information available, here is a clear and concise answer to your question. "
               "The key points are: first, the basic concept involves understanding the relationship "
               "between the components. Second, practical how to build requires attention to error handling "
               "and edge cases. Third, performance optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) comes from measuring before optimizing. "
               "Let me know if you need more detail on any specific aspect.",
    "rag": "According to the provided context, the answer is as follows. The documentation states that "
           "the system processes requests through a pipeline of validation, transformation, and execution stages. "
           "Each stage is configured independently. The context specifically mentions that caching reduces "
           "latency by 40-60% for repeated queries.",
    "code_review": "Code Review Findings:\n\n"
                   "1. Line 12: SQL query uses string concatenation instead of parameter (பாரமீட்டர் — கட்டுப்பாடு)ized queries. "
                   "This is a SQL injection vulnerability. Use prepared statements.\n\n"
                   "2. Line 28: The try/except block catches all exceptions silently. "
                   "Log the exception and re-raise or handle specific exception types.\n\n"
                   "3. Line 45: No input validation on user_id parameter. "
                   "Validate that it matches the expected UUID format before database lookup.\n\n"
                   "4. Performance: The loop on line 33-40 makes a database query per iteration (மறுயுதல் — மீண்டும் செய்தல்). "
                   "batch (பத்தி — தொகுப்பு) the queries into a single SELECT with an IN clause.",
}
...
1. **Add RAG integration.** Build a simple in-memory vector (வெக்டர் — எண் பட்டியல்) store with 20 documents. When the template is `rag_answer`, embed the query, find the 3 most similar documents, and inject them as context. Measure how response quality changes with and without RAG context. Track retrieval latency separately from LLM latency.

2. **Implement real function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) calling.** Add a tool registry (from Lesson 09) to the service. When a user asks a question that requires external data (weather, calculation, search), the pipeline should detect this, execute the tool, and include the result in the prompt. Add a `tools_used` field to the response.
...
| Semantic Cache | "Smart cache" | A cache keyed by embedding (embedding — பொருளை எண்களாக மாற்றுதல்) similarity quite than exact string match -- two differently-phrased identical questions return the same cached response |
...
| Graceful Degradation | "Partial failure handling" | When a secondary component fails (cache, RAG, guardrails), the system continues with reduced function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)ality quite than crashing |
...
- [Anthropic API Reference](https://docs.anthropic.com/en/api/messages-streaming) -- streaming how to build details for Claude, including server-sent events and tool use during streaming
...
- [Hugging Face TGI](https://huggingface.co/docs/text-generation-inference/index) -- Text Generation Inference: Rust server with continuous batch (பத்தி — தொகுப்பு)ing, Flash Attention, and Medusa speculative decoding. the HF-native alternative to vLLM.
- [NVIDIA tensor (டென்சர் — பல வடிவ எண் வரிசை)RT-LLM documentation](https://nvidia.github.io/TensorRT-LLM/) -- the highest-throughput path on NVIDIA hardware. quantization, in-flight batching, and FP8 kernels for enterprise deployments.
