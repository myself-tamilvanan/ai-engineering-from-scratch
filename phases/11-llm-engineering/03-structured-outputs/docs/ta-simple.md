...
- Implement JSON-mode and schema-constrained outputs using OpenAI and Anthropic API parameter (பாரமீட்டர் — கட்டுப்பாடு)s
...
graph TD
    subgraph ToolUse["Tool Use Flow"]
        ...
        TC --> V["Validate against\nfunction (சார்பு — உள்ளீட்டுக்கு வெளியீடு) schema"]
...
Tool use is preferred when the model needs to choose which function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) to call, not just fill in parameter (பாரமீட்டர் — கட்டுப்பாடு)s. If you have 10 different extraction schemas and the model must pick the right one based on the input, tool use gives you both the schema selection and the structured output.
...
1. Extend the schema validator to support `oneOf` (the data must match exactly one of several schemas). This handles polymorphic outputs -- for example, a field that is either a `Product` or a `Service` object with different shapes.
...
| function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) calling | "Tool use" | LLM outputs a structured function invocation (name + typed arguments) instead of free text -- OpenAI and Anthropic both support this |
