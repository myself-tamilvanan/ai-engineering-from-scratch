...
2. **Nodes.** Python function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s `state -> partial_state`. Each is a discrete step: "call the model," "run tools," "summarize."
...
A compiled graph is a node in another graph. The outer graph sees a single node. the inner graph has its own state and its own checkpoints. This is how teams build supervisor-worker agents: the supervisor graph routes user intent to a per-domain worker subgraph.
...
3. **Draw the edges.** Static unless the next step depends on model output. Every conditional edge needs a router function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) with named branches.
...
| Reducer | "How the field merges" | A function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) `(old, new) -> merged` applied when a node returns an update for that field. default is overwrite, `add_messages` appends. |
...
- Phase 11 · 09 (function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) Calling) — the tool-call primitive every LangGraph agent node reuses.
