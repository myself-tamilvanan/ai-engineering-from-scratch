...
# Caching, Rate Limiting & Cost optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)

> Most AI startups do not die from bad models. They die from bad unit economics. A single GPT-4o call costs fractions of a cent. Ten thousand users making ten calls per day costs $250 in input tokens alone -- before you charge a single dollar. The companies that survive are the ones that treat every API call as a financial transaction, not a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) call.

**Type:** Build
**Languages:** Python
**Prerequisites:** Phase 11 Lesson 09 (function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) Calling)
**Time:** ~45 minutes
...
- Build a cost optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) layer with prompt compression, model routing (expensive vs cheap), and response caching
...
That is just the LLM. Add embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s, vector (வெக்டர் — எண் பட்டியல்) database hosting, infrastructure. You are looking at $30,000/month for a chatbot.
...
You are paying full price for redundant calculation.
...
**OpenAI's approach** is automatic. Any prompt prefix that matches a previous request gets a 50% discount. No markers needed. The choice: less discount, less control, but zero how to build effort.
...
"What is the return policy?" and "How do I return an item?" are different strings but identical intent. A semantic cache embeds both queries, computes cosine similarity, and returns the cached response if similarity exceeds a threshold (usually 0.92-0.95).
...
flowchart TD
    ...
    E --> F[Cache Response<br/>with embedding (embedding — பொருளை எண்களாக மாற்றுதல்)]
    F --> G[Return Response]
    D --> G
```

The embedding (embedding — பொருளை எண்களாக மாற்றுதல்) costs are negligible. OpenAI's text-embedding-3-small costs $0.02 per million tokens. Checking the cache costs almost nothing compared to a full LLM call.
...
- function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) calling with identical tool definitions
- batch (பத்தி — தொகுப்பு) processing where the same document gets processed multiple times

### Rate Limiting: Protecting Your Budget

Rate limiting is not just about fairness. It is about survival.

**Token bucket algorithm (அல்கோரிதம் — வழிமுறை):** each user gets a bucket of N tokens that refills at rate R per second. A request consumes tokens from the bucket. If the bucket is empty, the request is rejected. This allows bursts (use the full bucket at once) while enforcing an average rate.
...
This data reveals which feature (பண்பு — தகவல் நெடுவரிசை)s are expensive, which users are heavy consumers, and where caching has the most impact.

### batch (பத்தி — தொகுப்பு)ing: Bulk Discounts

OpenAI's batch (பத்தி — தொகுப்பு) API processes requests asynchronously at a 50% discount. You submit a batch of up to 50,000 requests, and results come back within 24 hours.

Use batch (பத்தி — தொகுப்பு)ing for:
- Nightly document processing
- Bulk classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)
- Evaluation runs
...
### The optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) Stack

Apply these techniques in order. Each layer compounds on the previous ones.

| Layer | Technique | Typical Savings | how to build Effort |
|-------|-----------|----------------|----------------------|
| 1 | Provider prompt caching | 30-50% | Low (add cache markers) |
| 2 | Exact caching | 10-20% | Low (hash + dict) |
| 3 | Semantic caching | 15-30% | Medium (embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s + similarity) |
| 4 | Model routing | 40-70% | Medium (classifier) |
| 5 | Rate limiting | Budget protection | Low (token bucket) |
| 6 | Prompt compression | 10-30% | Medium (rewrite prompts) |
| 7 | batch (பத்தி — தொகுப்பு)ing | 50% on eligible | Low (batch API) |

A RAG app applying layers 1-5 usually reduces costs from $22,500/month to $4,000-6,000/month. That is the difference between burning runway and building a business.

### Real Savings: Before and After

Here is a real breakdown for a RAG chatbot serving 10,000 DAU.

| Metric | Before optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) | After Optimization | Savings |
|--------|--------------------|--------------------|---------|
...
| Monthly embedding (embedding — பொருளை எண்களாக மாற்றுதல்) cost | $0 | $180 | (new cost) |
| Total monthly cost | $22,500 | $5,380 | 76% |

The embedding (embedding — பொருளை எண்களாக மாற்றுதல்) cost for semantic caching ($180/month) pays for itself within the first hour of cache hits.
...
class SemanticCache:
    ...
    def get(self, query):
        query_embedding (embedding — பொருளை எண்களாக மாற்றுதல்) = simple_embed(query)
        now = time.time()
        best_match = None
        best_sim = 0.0
    ...
    def put(self, query, response):
        ...
        self.entries.append({
            "query": query,
            "embedding (embedding — பொருளை எண்களாக மாற்றுதல்)": simple_embed(query),
            "response": response,
            "timestamp": time.time(),
            "access_count": 1,
        })
...
def run_demo():
    print("=" * 60)
    print("  Caching, Rate Limiting & Cost optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) Demo")
    print("=" * 60)
    ...
    print("\n--- Semantic Cache ---")
    test_queries = [
        ("What is the return policy?", "Items is returned within 30 days with receipt."),
    ...
    print("\n--- Full Pipeline: Before vs After optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) ---")
    queries = [
        ...
        "Write a Python function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) to sort a list",
...
### OpenAI batch (பத்தி — தொகுப்பு) API
...
#     })
#
# with open("batch (பத்தி — தொகுப்பு)_input.jsonl", "w") as f:
#     for r in requests:
...
batch (பத்தி — தொகுப்பு) API gives a flat 50% discount on all tokens. Results arrive within 24 hours. Perfect for non-real-time workloads: evaluations, data labeling, bulk summarization.
...
# client = OpenAI()
#
# def get_embedding (embedding — பொருளை எண்களாக மாற்றுதல்)(text):
...
In production, replace the linear scan with a vector (வெக்டர் — எண் பட்டியல்) index (Redis Vector Search, Pinecone, or pgvector). Linear scan works for <1,000 entries. Beyond that, use ANN (approximate nearest neighbor) for O(log n) lookup.

## Ship It

This lesson produces `outputs/prompt-cost-optimizer.md` -- a reusable prompt that analyzes your LLM application and recommends specific cost optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)s with projected savings.
...
4. **Build a model routing classifier.** Replace the keyword-based classifier with an embedding (embedding — பொருளை எண்களாக மாற்றுதல்)-based one. Embed 50 labeled queries (simple/medium/complex), then classify new queries by finding the nearest labeled example. Measure classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) accuracy against a test set of 20 queries.
...
| Semantic caching | "Smart caching" | embedding (embedding — பொருளை எண்களாக மாற்றுதல்) the query, computing similarity to past queries, and returning the cached response if similarity exceeds a threshold -- catches paraphrases that exact matching misses |
...
| Token bucket | "Rate limiter" | An algorithm (அல்கோரிதம் — வழிமுறை) where each user has a bucket of N tokens that refills at rate R per second -- allows bursts up to N while enforcing an average rate of R |
...
| Cost tracking | "Metering" | Logging every API call with model, tokens, latency, cost, and user ID so you know exactly where money goes and which feature (பண்பு — தகவல் நெடுவரிசை)s are expensive |
...
| batch (பத்தி — தொகுப்பு) API | "Bulk discount" | OpenAI's asynchronous processing at 50% discount -- submit up to 50,000 requests, get results within 24 hours |
...
| Cache hit rate | "Cache speed" | The percentage of requests served from cache instead of calling the LLM -- 40-60% is typical for production chatbots, saves proportionally on cost |
...
- [OpenAI batch (பத்தி — தொகுப்பு) API](https://platform.openai.com/docs/guides/batch) -- 50% discount for asynchronous processing, JSONL format, 24-hour completion window, and 50K request limits
- [GPTCache](https://github.com/zilliztech/GPTCache) -- open-source semantic caching library supporting multiple embedding (embedding — பொருளை எண்களாக மாற்றுதல்) backends, vector (வெக்டர் — எண் பட்டியல்) stores, and eviction policies
