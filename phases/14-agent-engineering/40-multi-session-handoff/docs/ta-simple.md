...
So the session does not end when the feature (பண்பு — தகவல் நெடுவரிசை) works. It ends when the workbench is in a state the generator can summarize and the next session can trust. Cleanup is its own phase, run before the handoff, and it is a check, not a habit, because a habit is the thing that gets skipped on a hard day.
...
| feature (பண்பு — தகவல் நெடுவரிசை) board | `feature_list.json` status reflects reality (Phase 14 · 36) | A stale board sends the next session to work that is already done |
...
- A `generate_handoff(snapshot) -> (markdown, payload)` function (சார்பு — உள்ளீட்டுக்கு வெளியீடு).
