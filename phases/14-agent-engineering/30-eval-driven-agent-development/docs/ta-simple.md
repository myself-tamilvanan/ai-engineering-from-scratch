...
1. **Static benchmarks** — SWE-bench Verified for code (Lesson 19), WebArena/OSWorld for browsing / desktop (Lesson 20), GAIA for generalist (Lesson 19), BFCL V4 for tool use (Lesson 06). Use for cross-model comparison and regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) gating. Contamination is real: SWE-bench+ found 32.67% solution leakage. Always report Verified / +-audited scores.
...
- Gate merge on eval scores (e.g. "no regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) > 5% vs main").
...
| 19-20 Benchmarks | SWE-bench Verified score, WebArena success rate, OSWorld speed |
...
- CI gate: aggregate pass rate + regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) against baseline.
...
Output: per-case pass/fail, regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) flag, CI gate verdict.

## Use It

- Write eval cases in the same repo as your agent code.
- Run them on every PR via CI.
- Fail the build on regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்).
- Track pass rate over time.
- Tie every production failure to a new case.

## Ship It

`outputs/skill-eval-suite.md` builds a three-layer eval suite for an agent product with CI gates and regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) tracking.

## Exercises

1. Take one of your production failures. Write an eval case that reproduces it. Does your agent pass it now?
2. Build an LLM-judge rubric for your domain with three dimension (பரிமாணம் — கோணம்)s (factual, tone, scope). Score 50 sessions.
3. Wire the eval suite into CI. Fail the build on >=5% regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்).
4. Add a trajectory-speed metric: how many steps did the agent take vs a gold trajectory?
...
| CI gate | "Merge blocker" | Fail the build on eval regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) |
| Baseline | "Last-known-good" | Reference score to detect regression |
| Trajectory speed | "Steps over gold" | Agent step count divided by human expert minimum |
...
- [Berkeley function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) Calling Leaderboard](https://gorilla.cs.berkeley.edu/leaderboard.html) — tool-use benchmark
