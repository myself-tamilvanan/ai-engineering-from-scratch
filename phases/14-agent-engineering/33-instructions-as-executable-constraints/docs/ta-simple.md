...
Each rule has a slug, a category, a one-line description, and a `check` field that names a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) in `rule_checker.py`. Adding a rule means adding a check. the checker grows with the workbench.
...
feature (பண்பு — தகவல் நெடுவரிசை)_list.json          # the backlog (Phase 14 · 36)
```
...
- `rule_checker.py` style checker function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s, one per `check` reference.
...
The rule set is portable across all three because it is just markdown plus function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) names.
...
- [logi-cmd/agent-guardrails](https://github.com/logi-cmd/agent-guardrails) — merge-gate how to build: scope, mutation testing, violation budgets
