...
- **Agent.** `role + goal + backstory + tools + (optional) llm`. The backstory is load-bearing. It shapes tone, judgment, when the agent stops. Tools are function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s the agent can call (more below).
...
1. **`@tool` decorator.** Pure function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s become tools. Signature is the schema. docstring is the description the LLM sees. Best for one-off helpers.
...
- **Long-term.** Persisted across runs. Stored in a vector (வெக்டர் — எண் பட்டியல்) DB (Chroma by default, swappable). Retrieved by similarity to the current task.
...
Enable on the Crew with `memory=True` or per-type config. Backed by an embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s provider you configure (defaults to OpenAI, swappable to local). Memory is one of the places CrewAI earns its keep against thinner frameworks. pure LangGraph requires you to wire each of these yourself.
...
Lesson 17 (Agent Framework Tradeoffs) lays this out in a matrix (மேட்ரிக்ஸ் — எண் மேஜை). The short version: CrewAI sits in the "collaborative role-based" corner.

### Dependency shape

Independent of LangChain. Python 3.10 to 3.13. Uses `uv`. Star count: see [crewAIInc/crewAI](https://github.com/crewAIInc/crewAI) (snapshot as of 2026-05). AWS Bedrock integration is documented. vendor benchmarks report a large speedup vs LangGraph on QA workloads, but the methodology (dataset, hardware, evaluation metric) is not published, so treat framework-vendor numbers as directional only.
...
`outputs/skill-crew-or-flow.md` picks Crew vs Flow for a task and scaffolds the minimal how to build. Hard rejects on Crew-without-backstory, Flow-without-explicit-topics, Hierarchical with under three specialists.
...
- **Memory always-on.** Long-term writes every run. vector (வெக்டர் — எண் பட்டியல்) DB grows. Retrieval gets noisy. Scope writes to tasks where the fact is persistent.
...
| `@tool` | "function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) tool" | Decorator that turns a function into a tool the Agent can call |
...
| Long-term memory | "Cross-run memory" | vector (வெக்டர் — எண் பட்டியல்)-backed memory that survives between kickoffs |
