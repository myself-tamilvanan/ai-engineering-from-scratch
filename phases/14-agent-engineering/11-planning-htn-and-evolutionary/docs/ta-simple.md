...
> Symbolic planning handles the cases where the plan is provably correct. Evolutionary code search handles the cases where the fitness function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) is machine-checkable. ChatHTN (2025) and AlphaEvolve (2025) show what each unlocks when paired with an LLM.
...
2. **optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)s with a machine-checkable fitness function (சார்பு — உள்ளீட்டுக்கு வெளியீடு).** matrix (மேட்ரிக்ஸ் — எண் மேஜை) multiplication, scheduling heuristics, compiler passes — the goal is not "a correct plan" but "the best plan."
...
Online method learning (OpenReview `gwYEDY9j2x`, 2025 follow-up) adds a learner that generalizes LLM-produced decompositions by regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) — cutting LLM query frequency up to 75%.
...
- First improvement over Strassen for 4x4 complex matrix (மேட்ரிக்ஸ் — எண் மேஜை) multiplication in 56 years (48 scalar (scalar — ஒரு எண்) multiplications).
- 0.7% recovered Google compute via a Borg scheduling heuristic.
- 32% FlashAttention speedup on a frontier workload.

The hard constraint: the fitness function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) must be machine-checkable. Evolutionary search over prose answers does not converge.
...
| Compiler optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) | AlphaEvolve | Machine-checkable fitness |
...
- **AlphaEvolve without a real evaluator.** "Ask the LLM if the code is better" is not a fitness function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). The evaluator must be deterministic and fast.
...
3. Swap the evolutionary search evaluator to a real test suite. Evolve a sort function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) that passes 20 test cases. report generations to convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்).
4. Read AlphaEvolve's evaluator design notes. Design an evaluator for a domain you care about (SQL query optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்), test-suite minimization, deployment YAML).
...
| Fitness function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) | "Evaluator" | Deterministic, machine-checkable score over outputs |
