...
Strip the agent label off for a moment. An agent run is calculation that crosses time, processes, and machines. To make that reliable you need the same primitives any production system needs.

| Primitive | What it is | What it carries for an agent |
|-----------|------------|------------------------------|
| function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) | Typed handler. Pure where possible. Owns its inputs and outputs. | A tool call, a rule check, a verification step, a model invocation |
...
- **Instructions** — policy + function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) metadata. Rules are checks (functions). The router (`AGENTS.md`) is policy attached to the runtime's startup.
...
The agent loop itself is a worker that consumes events (user message, tool result, timer tick), calls function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s (the model, then the tools the model picks), writes records (state, feedback), and emits triggers (verify, review, handoff). No mystery. the same shape as a job processor.
...
| Guides and Sensors (Birgitta Böckeler / Thoughtworks) — feedforward rules + feedback observability | Authorization policy + verification function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s + observability traces |
...
- Harvey — legal agents more than doubled accuracy through harness optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) alone (MongoDB).
...
Read these as data points, not as authorities. Each one is a partial taxonomy. Translate every concept back to a primitive (function (சார்பு — உள்ளீட்டுக்கு வெளியீடு), worker, trigger, runtime, HTTP/RPC, queue, persistence, policy) before deciding whether to adopt it.
...
- [OpenAI, Unrolling the Codex agent loop](https://openai.com/index/unrolling-the-codex-agent-loop/) — the agent loop reduced to a `while` over function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) calls
...
- [andrewgarst/agentic_harness](https://github.com/andrewgarst/agentic_harness) — production-ready reference how to build with Redis-backed memory and eval suite
