...
> LangGraph is the 2026 reference for low-level stateful orchestration. Agent is a state machine. nodes are function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s. edges are transitions. state is immutable and checkpointed after every step. Resume from any failure exactly where it left off.
...
- Describe LangGraph's core model: state machine with immutable state, function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) nodes, conditional edges, and post-step checkpoints.
...
- **Nodes.** Pure function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s `(state) -> state_update`. Updates are merged into state after return.
...
1. Add a conditional edge from `classify` to `end` when classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) confidence is below a threshold. Resume the run after a human sets `route` manually.
2. Swap the SQLite-like fake for a real SQLite checkpointer. Measure per-step serialization extra work.
...
| Reducer | "State merger" | function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) that combines current state with a node's update |
...
- [AutoGen v0.4, Microsoft Research](https://www.microsoft.com/en-us/research/articles/autogen-v0-4-reimagining-the-foundation-of-agentic-ai-for-scale-extensibility-and-strength/) — actor-model alternative
