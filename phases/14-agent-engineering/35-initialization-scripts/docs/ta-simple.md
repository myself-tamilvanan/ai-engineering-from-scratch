...
Rules (Phase 14 · 33) describe what must be true to act. Init is the script that establishes that those rules is checked. Rules without init become "be careful." Init without rules becomes a polished failure.
...
4. Move probes from hardcoded function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s to a YAML registry. Defend the trade-off.
...
|------|----------------|------------------------|
| Probe | "A check" | A deterministic function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) returning `(name, status, detail)` |
