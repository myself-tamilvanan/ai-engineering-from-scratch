...
- Explain why ReWOO's Planner / Worker / Solver split saves tokens and improves strength over ReAct's interleaved loop.
...
ReWOO (Xu et al., arXiv:2305.18323, May 2023) noticed this and made a bet: plan the whole thing up front, fetch evidence in parallel, compose the answer at the end. One LLM call to plan, N tool calls for evidence (is parallel), one LLM call to solve. The trade is less flexibility (the plan is static) for much better token speed and clearer failure modes.
...
4. Read Section 4 of the ReWOO paper on planner distillation. Reproduce the 175B -> 7B result in simple idea: what training data do you need, and how do you score plan quality?
...
| Token speed | "Fewer round trips" | 5x fewer tokens on HotpotQA vs ReAct in the paper |
