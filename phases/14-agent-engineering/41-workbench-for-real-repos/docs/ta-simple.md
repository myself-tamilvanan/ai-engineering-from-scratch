...
**Harvey 2x accuracy via harness alone.** Legal agents more than doubled their accuracy through harness optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்), no model change.
...
3. Add a "false negative" pass: tasks where prompt-only would have been faster and the workbench extra work is real cost. Defend keeping the workbench anyway.
