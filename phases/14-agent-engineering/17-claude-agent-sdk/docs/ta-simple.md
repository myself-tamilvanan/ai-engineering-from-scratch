...
- **Subagent over-spawn.** Spawning 100 subagents for 100 tiny tasks. extra work dominates. batch (பத்தி — தொகுப்பு) instead.
...
1. Add a subagent spawner that batch (பத்தி — தொகுப்பு)es 20 tasks into groups of 5 parallel subagents. Measure orchestrator context size vs one-per-task.
