...
Most agents emit primitive commands. Voyager emits JavaScript function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s. A skill is:

```
async function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) craftIronPickaxe(bot) {
  await mineIron(bot, 3). await mineStick(bot, 2). await placeCraftingTable(bot). await craft(bot, 'iron_pickaxe'). }
```

Composed from sub-skills. Stored keyed on description and embedding (embedding — பொருளை எண்களாக மாற்றுதல்). Retrieved as a program, not a prompt.
...
For production agents this translates to a "what's missing" operator: given the current skill library and a domain, what skills are we not yet covering? Teams usually implement this manually as curriculum review.
...
- **Retrieval quality.** vector (வெக்டர் — எண் பட்டியல்) retrieval over skill descriptions degrades as the library grows past a few hundred. Supplement with tag filters and hard constraints ("only skills with `category=tooling`").
...
3. Replace token-overlap retrieval with sentence-transformers embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s (or a BM25 stdlib impl). Measure retrieval@5 on a 50-skill toy library.
...
| Action-space-as-code | "Programmatic actions" | Emit function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s, not primitive commands, for temporally extended behavior |
