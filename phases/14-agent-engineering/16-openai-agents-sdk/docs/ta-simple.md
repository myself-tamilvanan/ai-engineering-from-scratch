...
3. **Guardrail.** Validation on input (first agent only), output (last agent only), or tool invocation (per function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) tool).
...
- **Tool guardrails.** Run per-function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)-tool. Validate arguments, check permissions, audit execution.
...
- **Guardrail bypass.** Tool guardrails only fire on function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) tools. built-in tools (file reader, web fetch) need separate policy.
...
- `Agent`, `function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)Tool`, `Handoff` (as a function tool with transfer semantics).
