...
Self-evaluation is the load-bearing piece. The paper shows three variants: `sure / likely / impossible` classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்), `1..10` numeric score, and vote among candidates. All three beat CoT substantially on Game of 24 (4% -> 74% with GPT-4).
...
- **Value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)**: score a partial trajectory (ToT-style self-eval).
- **Self-reflector**: on failure, write a natural-language reflection (Reflexion-style) and use it to reseed future rollouts.

Environment feedback (observations) mixes into the value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) so the search is informed by real tool results, not just model opinions. Results at paper time: HumanEval pass@1 92.7% with GPT-4 (SOTA), WebShop average 75.9 with GPT-3.5 (approaching gradient (சாய்வு — மாற்றத்தின் दिशा)-based fine-tuning).

### MCTS, minimally

Four phases per iteration (மறுயுதல் — மீண்டும் செய்தல்):

1. **Select** — walk from root to a leaf using UCT (upper confidence bound for trees).
2. **Expand** — generate K children via the policy.
3. **Simulate** — rollout from a child using the policy, score the leaf with the value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) (or environment reward).
...
- Tasks with a cheap, reliable value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) (unit tests for code, explicit target for math).
...
- Coding agents that run tests as the value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) (HumanEval-style).
...
- A value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) that composes a symbolic score plus a self-eval score.
...
2. Swap the value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) for a noisier scorer (add random jitter). Does MCTS still find the best leaf? What is the minimum signal-to-noise it tolerates?
...
| Value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) | "How good is this state" | Prompted LLM score or environment reward. feeds backprop |
