...
| Disk | external context | vector (வெக்டர் — எண் பட்டியல்) DB, KV, graph store |
...
MemGPT introduces memory-as-interrupt: mid-conversation the agent can invoke a memory tool, the runtime executes it, and the result splices into the next assistant turn as a new observation. in simple idea identical to a Unix `read()` syscall that blocks the process, returns bytes, and the process continues.
...
- **Mem0** (Lesson 09) — vector (வெக்டர் — எண் பட்டியல்) + KV + graph fused with a scoring layer.
...
5. Port the how to build to use the MemGPT research repo's core-memory JSON schema (`cpacker/MemGPT`). What changes when you switch from flat strings to typed sections?
