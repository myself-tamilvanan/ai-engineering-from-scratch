...
- Choose between scalar (scalar — ஒரு எண்), heuristic, and self-evaluated feedback sources for a given task.
- Explain why verbal reinforcement catches errors that gradient (சாய்வு — மாற்றத்தின் दिशा)-based RL would need thousands of trials to fix.

## The Problem

An agent fails a task. In standard RL you would run thousands more trials, compute gradient (சாய்வு — மாற்றத்தின் दिशा)s, update weights. Expensive, slow, and most production agents do not have a training budget for every failure.

Reflexion (Shinn et al., arXiv:2303.11366) asks a different question: what if the agent just thought about why it failed and tried again with that thought in its prompt? No weight updates. No gradient (சாய்வு — மாற்றத்தின் दिशा). Just natural language stored between trials.

The result: on ALFWorld it beats ReAct and other non-fine-tuned baselines. On HotpotQA it improves over ReAct. On code generation (HumanEval/MBPP) it sets state of the art at the time. All without a single gradient (சாய்வு — மாற்றத்தின் दिशा) step.
...
1. **scalar (scalar — ஒரு எண்)** — an external binary signal. ALFWorld succeeds or fails. HumanEval tests pass or fail. Simplest, highest-signal.
...
The 2026 default is a mix: scalar (scalar — ஒரு எண்) when available, self-eval when not, heuristics as safety rails.

### Why this generalizes

Reflexion is not a new algorithm (அல்கோரிதம் — வழிமுறை) so much as a named pattern. Almost every production "self-healing" agent runs some variant:
...
- The task class is reproducible (the same type of question is asked again).
...
1. Switch from binary to scalar (scalar — ஒரு எண்) evaluator that returns a distance metric (how far from target). Does it converge faster?
...
5. Read Section 4 of the Reflexion paper on AlfWorld. Reproduce the 130% success-rate improvement in simple idea: what is the key delta vs vanilla ReAct?
...
| Verbal reinforcement | "Learning without gradient (சாய்வு — மாற்றத்தின் दिशा)s" | Natural-language reflection prepended to the next trial's prompt |
| Episodic memory | "Per-task reflections" | Bounded buffer of prior reflections for one task class |
| scalar (scalar — ஒரு எண்) evaluator | "Binary success signal" | Pass/fail or numeric score from ground truth |
