...
### Two altitudes of scope: the feature (பண்பு — தகவல் நெடுவரிசை) list and the task contract
...
| Field | Purpose |
|-------|---------|
| `active` | The single feature (பண்பு — தகவல் நெடுவரிசை) the current session may touch. empty means pick one and set it |
