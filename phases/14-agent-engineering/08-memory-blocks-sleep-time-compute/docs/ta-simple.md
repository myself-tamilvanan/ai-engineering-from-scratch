...
> MemGPT became Letta in 2024. The 2026 evolution adds two ideas: discrete function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)al memory blocks the model can edit directly, and a sleep-time agent that consolidates memory asynchronously while the primary agent is idle. This is how you scale memory beyond one conversation.
...
| Archival | Arbitrary facts | vector (வெக்டர் — எண் பட்டியல்) + KV + graph | Agent tool call + sleep-time ingest |
...
- **Stronger model allowed.** The sleep-time agent is a more expensive, slower model because it is not latency-constrained.
...
- **Letta** (letta.com) for the reference how to build. Self-host or managed cloud.
