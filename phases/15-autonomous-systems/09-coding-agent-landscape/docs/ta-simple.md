...
- **2026**: Claude Sonnet 4.5 and frontier competitors at 70–80%+ on SWE-bench Verified. epoch (யுக் — ஒரு முழு சுழற்சி) AI's leaderboard tracks this live.
...
- **CodeAct**: one action is a whole program. compositional. requires a hardened sandbox (OpenHands uses Docker isolation). failure modes include anything the sandbox runtime allows.
...
| Aider | Apache-2 | edit-via-diff in local repo | Minimal scaffold, strong regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) stability |
...
The OpenHands authors and epoch (யுக் — ஒரு முழு சுழற்சி) AI both flag that SWE-bench Verified has an easy tail: 161 of 500 tasks need only 1–2 lines of change. High scores are driven partly by this tail. SWE-bench Pro restricts to 10+ line changes and returns scores in the 23–59% range even for frontier systems. Your production distribution is almost certainly closer to Pro than to Verified.
...
| JSON tool call | "function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) calling" | Each action is a structured JSON payload validated before execution |
...
- [epoch (யுக் — ஒரு முழு சுழற்சி) AI — SWE-bench leaderboard](https://epoch.ai/benchmarks) — live-tracked scores.
