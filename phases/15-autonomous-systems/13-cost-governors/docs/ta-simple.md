...
# Action Budgets, iteration (மறுயுதல் — மீண்டும் செய்தல்) Caps, and Cost Governors

> A mid-sized e-commerce agent's monthly LLM cost jumped from $1,200 to $4,800 after its team enabled the "order-tracking" skill. That is not a pricing bug. That is an agent that found a new loop and kept spending inside it. Microsoft's Agent Governance Toolkit (April 2, 2026) codifies the defense against this class: per-request `max_tokens`, per-task token and dollar budgets, per-day/month caps, iteration (மறுயுதல் — மீண்டும் செய்தல்) caps, tiered model routing, prompt caching, context windowing, HITL checkpoints on expensive actions, kill switches on budget breach. Anthropic's Claude Code Agent SDK ships the same primitives under different names. Financial velocity limits — e.g. cut access on >$50 in 10 minutes — catch loops faster than monthly caps.
...
5. **iteration (மறுயுதல் — மீண்டும் செய்தல்) cap (`max_turns`).** Total agent loop iterations. prevents infinite reasoning loops.
...
- `max_turns` — iteration (மறுயுதல் — மீண்டும் செய்தல்) cap.
...
1. Run `code/main.py`. Confirm the velocity limit fires before the iteration (மறுயுதல் — மீண்டும் செய்தல்) cap on a polling-loop trajectory. Now disable the velocity limit and measure how much the agent "spends" before the iteration cap catches it.
...
| max_turns | "iteration (மறுயுதல் — மீண்டும் செய்தல்) cap" | Ceiling on agent loop iterations in a session |
