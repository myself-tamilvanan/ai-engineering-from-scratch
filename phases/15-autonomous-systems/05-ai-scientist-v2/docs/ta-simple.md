...
Research is an open-ended task. Unlike AlphaEvolve's algorithm (அல்கோரிதம் — வழிமுறை)ic search or DGM's benchmark-bounded self-modification, a research result does not have a machine-checkable correctness criterion. A paper is judged by reviewers, not unit tests. That makes the loop harder to close — and more valuable if closed, because research is where compounding progress lives.
...
|---|---|---|---|---|
| AlphaEvolve | algorithm (அல்கோரிதம் — வழிமுறை)s | code | unit + benchmark | bounded by evaluator rigor |
...
1. Run `code/main.py` with default parameter (பாரமீட்டர் — கட்டுப்பாடு)s. What fraction of loop runs produce a "clean" paper? What fraction produce a paper with an experiment-failure flaw the figure critique polished over?
...
3. Read Sakana's AI Scientist v2 repo README on sandbox requirements. Name two extra restrictions (beyond Docker) you would apply for a multi-day autonomous run.

4. Read Beel et al. Section 4 on presentation-quality gap. Design one extra evaluator that would catch polished-looking but experimentally flawed papers.
