...
The convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்): all three label long-range autonomy as a capability class worth tracking. All three acknowledge deceptive behavior (alignment faking, sandbagging) as a specific class of risk. All three have an internal review body. The divergence: OpenAI splits categories into "Tracked" (mandatory mitigation) and "Research" (no automatic trigger). DeepMind folds autonomy into two domains quite than naming it separately. The lab names Tracked vs Research, or Critical vs Moderate, or Tier-1 vs Tier-2. the operational consequence of which bucket a capability lives in is different across labs.

Reading them together is the useful exercise. The same capability is "mandatory mitigation" at Anthropic, "monitored but not triggering" at OpenAI, and "tracked in a specific domain" at DeepMind. The policy posture matters.
...
- **Tracked Capability Levels**: extra granularity added in April 2026. Concrete example: ML R&D autonomy level 1 = fully automate the AI R&D pipeline at competitive cost vs human + AI tools.
...
2. Read OpenAI Preparedness Framework v2 in full. Identify each Research Category. For each, write one sentence on why it is in Research quite than Tracked.
...
5. Compare the three policies on a specific capability (your choice). Name which policy's classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) you find most rigorous and which least. Justify with source text.
