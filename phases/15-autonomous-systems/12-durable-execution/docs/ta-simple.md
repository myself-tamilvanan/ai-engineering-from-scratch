...
- **Workflow**: deterministic orchestration code. Defines the sequence of activities, the branches, the waits. Must be deterministic so it is replayed from the event log without surprising divergence.
...
- Runs shorter than a few minutes with no human input. extra work > benefit.
...
- A workflow function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) that sequences activities.
