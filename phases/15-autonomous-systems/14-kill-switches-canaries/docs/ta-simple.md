...
All three are pre-LLM engineering. Classical deception, rate-limit breakers, and feature (பண்பு — தகவல் நெடுவரிசை)-flag kills predate autonomous agents. What's new is the attack surface: agents read untrusted content (Lesson 11), edit their own memory, and can compose many safe-looking actions into an unsafe one. The detectors named here work because they do not trust the agent's self-report.

## The Concept

### Kill switches

A kill switch is a boolean the agent reads but cannot write. how to build choices:

- **feature (பண்பு — தகவல் நெடுவரிசை) flag in a managed service.** LaunchDarkly, Statsig, Unleash. Updates propagate in seconds.
...
Statistical detectors (EWMA, CUSUM, z-score on tool-call rates) adapt to a moving baseline. That is a feature (பண்பு — தகவல் நெடுவரிசை) for legitimate drift and a bug for adversarial drift. A patient attacker can shift the baseline slowly enough that no alarm fires.
...
| EWMA | "Moving average" | Exponentially weighted. adapts to drift (feature (பண்பு — தகவல் நெடுவரிசை) + bug) |
