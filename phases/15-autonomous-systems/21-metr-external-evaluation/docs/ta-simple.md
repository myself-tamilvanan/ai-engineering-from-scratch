...
3. Fit a logistic curve: P(success) as a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) of log(expert completion time).
...
`code/main.py` implements a logistic fit of task-success vs log(expert time), given a synthetic result set. It reports the 50% horizon (METR's headline), 10% horizon (conservative), and 90% horizon (optimistic). Also shows what changes when the success rate is artificially inflated by eval-context gaming.
...
- [epoch (யுக் — ஒரு முழு சுழற்சி) AI — METR Time Horizons benchmark](https://epoch.ai/benchmarks/metr-time-horizons) — live tracking.
