...
Every frontier LLM in 2026 ships on one of three algorithm (அல்கோரிதம் — வழிமுறை)s (BPE, Unigram, WordPiece), wrapped in one of three libraries (tiktoken, SentencePiece, HF Tokenizers). You cannot ship a language model without picking one.

## The Concept

![BPE vs Unigram vs WordPiece, character-by-character](../assets/subword-tokenization.svg)

**BPE (Byte-Pair Encoding).** Start with a character-level vocabulary. Count every adjacent pair. Merge the most frequent pair into a new token. Repeat until you hit the target vocabulary size. Dominant algorithm (அல்கோரிதம் — வழிமுறை): GPT-2/3/4, Llama, Gemma, Qwen2, Mistral.

**Byte-level BPE.** Same algorithm (அல்கோரிதம் — வழிமுறை) but over raw bytes (256 base tokens) instead of Unicode characters. Guarantees zero `[UNK]` tokens — any byte sequence encodes. GPT-2 uses 50,257 tokens (256 bytes + 50,000 merges + 1 special).
...
**WordPiece.** Merge pairs that maximize likelihood of the training corpus quite than raw frequency. Used by BERT, DistilBERT, ELECTRA.
...
Three facts the algorithm (அல்கோரிதம் — வழிமுறை) encodes. `</w>` marks word end so "low" (suffix) and "lower" (prefix) stay distinct. Frequency weighting makes high-frequency pairs win early. The merge list is ordered — inference applies merges in training order.
...
description: Pick tokenizer algorithm (அல்கோரிதம் — வழிமுறை), vocab size, library for a given corpus and deployment target.
version: 1.0.0
phase: 5
lesson: 19
...
1. algorithm (அல்கோரிதம் — வழிமுறை). BPE, Unigram, or WordPiece. One-sentence reason.
