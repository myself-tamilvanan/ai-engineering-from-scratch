...
2. **embedding (embedding — பொருளை எண்களாக மாற்றுதல்)-based (ESS / REL / Blink).** Encode mention + context. Encode each candidate's description. Pick max cosine. The 2020-2024 default.
...
The Jaccard overlap is a toy. Replace with cosine similarity on embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s (see `code/main.py` step-2 for the transformer version).

### Step 3: embedding (embedding — பொருளை எண்களாக மாற்றுதல்)-based (BLINK-style)
...
def embed_mention(text, mention_span):
    ...
    return encoder.encode([marked], normalize_embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s=True)[0]

def embed_entity(entity_id, description):
    return encoder.encode([f"{entity_id}: {description}"], normalize_embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s=True)[0]
```
...
GENRE decodes the entity's Wikipedia title character-by-character. Constrained decoding (see lesson 20) ensures only valid titles is output. Tight integration with a KB-backed trie. The modern descendant is REL-GEN and LLM-prompted EL with structured output.
...
2. Candidate generator. Alias-index, embedding (embedding — பொருளை எண்களாக மாற்றுதல்), or hybrid. Target mention recall @ K.
...
2. **Medium.** Encode 50 ambiguous mentions with a sentence transformer. Embed each candidate's description. Compare embedding (embedding — பொருளை எண்களாக மாற்றுதல்)-based disambiguation to Jaccard context overlap.
...
- [Wu et al. (2020). Zero-shot Entity Linking with Dense Entity Retrieval (BLINK)](https://arxiv.org/abs/1911.03814) — the embedding (embedding — பொருளை எண்களாக மாற்றுதல்)-based workhorse.
