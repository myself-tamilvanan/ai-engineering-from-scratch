...
Transition + emission probabilities are local. They cannot capture that `saw` is a noun in "I bought a saw" but a verb in "I saw the movie." A CRF with arbitrary feature (பண்பு — தகவல் நெடுவரிசை)s (suffix, word shape, word before and after, word itself) hits ~97%. A BiLSTM-CRF or transformer hits ~98%+.
...
- **Transition-based** parsers (arc-eager, arc-standard) act like a shift-reduce parser: they read tokens, shift them onto a stack, and apply reduce actions that create arcs. Greedy decoding is fast. Classic how to build is MaltParser. Modern neural version: Chen and Manning's transition-based parser.
- **Graph-based** parsers (Eisner's algorithm (அல்கோரிதம் — வழிமுறை), Dozat-Manning biaffine) score every possible head-dependent edge and pick the maximum spanning tree. Slower but more accurate.
...
- [spaCy linguistic feature (பண்பு — தகவல் நெடுவரிசை)s guide](https://spacy.io/usage/linguistic-features) — practical reference for every attribute exposed on `Token`.
- [Chen and Manning (2014). A Fast and Accurate Dependency Parser using neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)s](https://nlp.stanford.edu/pubs/emnlp2014-depparser.pdf) — the paper that brought neural parsers into the mainstream.
