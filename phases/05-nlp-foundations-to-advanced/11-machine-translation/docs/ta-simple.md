...
inputs = tok(src, return_tensor (டென்சர் — பல வடிவ எண் வரிசை)s="pt")
...
print(tok.batch (பத்தி — தொகுப்பு)_decode(out, skip_special_tokens=True)[0])
```
...
Always use `sacrebleu`. It normalizes tokenization so scores are comparable across papers. Rolling your own BLEU calculation is how misleading benchmarks happen.
...
- **Heuristic** (BLEU, chrF). Fast, reference-based, interpretable, insensitive to paraphrase. Use for legacy comparison and regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) detection.
...
args = TrainingArguments(output_dir="out", per_device_train_batch (பத்தி — தொகுப்பு)_size=4, num_train_epoch (யுக் — ஒரு முழு சுழற்சி)s=3, learning_rate=3e-5)
...
LLMs now outperform specialized MT models on several language pairs as of 2026, particularly on idiomatic content and long context. The choice is per-token cost and latency. Pick an LLM when context length, stylistic consistency, or domain adaptation via prompting matters more than throughput.
