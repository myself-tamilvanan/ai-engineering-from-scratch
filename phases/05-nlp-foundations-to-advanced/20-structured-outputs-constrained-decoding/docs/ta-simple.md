...
- **XGrammar / llguidance.** Context-free grammar engines. Handle recursive JSON Schema. Near-zero decoding extra work. OpenAI credited llguidance in their 2025 structured output how to build.
...
See `code/main.py` for a standalone FSM how to build. The core idea in 30 lines:
...
| batch (பத்தி — தொகுப்பு) processing with retries acceptable | Instructor + cheapest model |
...
| Logit processor | The thing that constrains | function (சார்பு — உள்ளீட்டுக்கு வெளியீடு): `(logits, state) -> masked_logits`. |
...
- [vLLM — Structured Outputs](https://docs.vllm.ai/en/latest/feature (பண்பு — தகவல் நெடுவரிசை)s/structured_outputs.html) — inference server integration.
