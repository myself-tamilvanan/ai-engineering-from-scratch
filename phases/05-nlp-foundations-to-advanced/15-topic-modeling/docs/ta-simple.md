...
> LDA: documents are mixtures of topics, topics are distributions over words. BERTopic: documents cluster in embedding (embedding — பொருளை எண்களாக மாற்றுதல்) space, clusters are topics. Same goal, different decompositions.
...
Two algorithm (அல்கோரிதம் — வழிமுறை)ic families dominate. LDA (2003) treats each document as a mixture of latent topics and each topic as a distribution over words. Inference is Bayesian. It still ships in production where you need mixed-membership topic assignments and explainable word-level probability distributions.

BERTopic (2020) encodes documents with BERT, reduces dimension (பரிமாணம் — கோணம்)ality with UMAP, clusters with HDBSCAN, and extracts topic words via class-based TF-IDF. It wins on short text, social media, and anything where semantic similarity matters more than word overlap. One document gets one topic, which is a limitation for long-form content.

This lesson builds simple idea for both and names which one to pick for a given corpus.
...
- `doc_topic`: matrix (மேட்ரிக்ஸ் — எண் மேஜை) `(n_docs, n_topics)`, each row sums to 1 (document's topic mixture).
- `topic_word`: matrix `(n_topics, vocab_size)`, each row sums to 1 (topic's word distribution).

**BERTopic pipeline.**

1. Encode each document with a sentence transformer (e.g., `all-MiniLM-L6-v2`). 384-dim vector (வெக்டர் — எண் பட்டியல்)s.
2. Reduce dimension (பரிமாணம் — கோணம்)ality with UMAP to ~5 dimensions. BERT embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s are too high-dim for clustering.
...
Output is one topic per document (plus a -1 outlier label). Optionally, a soft membership via HDBSCAN's probability vector (வெக்டர் — எண் பட்டியல்).
...
def fit_lda(documents, n_topics=5, max_feature (பண்பு — தகவல் நெடுவரிசை)s=1000):
    cv = Countvector (வெக்டர் — எண் பட்டியல்)izer(
...
def print_top_words(lda, feature (பண்பு — தகவல் நெடுவரிசை)_names, n_top=10):
...
Notice: stopwords removed, min_df and max_df filter rare and ubiquitous terms, Countvector (வெக்டர் — எண் பட்டியல்)izer (not TfidfVectorizer) because LDA expects raw counts.
...
topic_model = BERTopic(
    embedding (embedding — பொருளை எண்களாக மாற்றுதல்)_model="sentence-transformers/all-MiniLM-L6-v2",
    min_topic_size=15,
    verbose=True,
)
...
- **Topic coherence (c_v).** Combines NPMI (normalized pointwise mutual information) of top-word pairs over sliding-window contexts, aggregates the scores into topic vector (வெக்டர் — எண் பட்டியல்)s, and compares those vectors via cosine similarity. Higher is better. Use `gensim.models.CoherenceModel` with `coherence="c_v"`.
...
The biggest practical consideration is document length. BERT embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s truncate. LDA counts work on whatever length. For documents longer than the embedding model's context, either chunk + aggregate or use LDA.
...
- **NMF.** Non-negative matrix (மேட்ரிக்ஸ் — எண் மேஜை) factorization. Fast alternative to LDA, comparable quality on short text.
...
1. algorithm (அல்கோரிதம் — வழிமுறை). LDA / NMF / BERTopic / Top2Vec / FASTopic. One-sentence reason.
2. Configuration. Number of topics: `recommended = max(5, round(sqrt(n_docs)))`, clamped to 200 for corpora under 40,000 docs. permit >200 only when the corpus is genuinely large (>40k) and note the increased compute cost. `min_df` / `max_df` filters and embedding (embedding — பொருளை எண்களாக மாற்றுதல்) model for neural approaches also belong here.
3. Evaluation. Topic coherence (c_v) via `gensim.models.CoherenceModel`, topic diversity, and a 20-sample human read.
4. Failure mode to probe. For LDA, "junk topics" absorbing stopwords and frequent terms. For BERTopic, the -1 outlier cluster swallowing ambiguous documents.

Refuse BERTopic on documents longer than the embedding (embedding — பொருளை எண்களாக மாற்றுதல்) model's context window without a chunking strategy. Refuse LDA on very short text (tweets, reviews under 10 tokens) as coherence collapses. Flag any n_topics choice below 5 as likely wrong. flag >200 on corpora under 40k docs as likely over-splitting.
```

## Exercises

1. **Easy.** Fit LDA with 5 topics on the 20 Newsgroups dataset. Print top 10 words per topic. Label each topic by hand. Did the algorithm (அல்கோரிதம் — வழிமுறை) find the real categories?
...
| UMAP | dimension (பரிமாணம் — கோணம்)ality reduction | Manifold learning that preserves local structure. used in BERTopic. |
