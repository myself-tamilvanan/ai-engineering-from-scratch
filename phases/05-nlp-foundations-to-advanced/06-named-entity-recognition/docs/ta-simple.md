...
**Type:** Build
**Languages:** Python
**Prerequisites:** Phase 5 · 02 (BoW + TF-IDF), Phase 5 · 03 (Word embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s)
**Time:** ~75 minutes
...
- **CRF.** Conditional Random Field. Like HMM but discriminative, so you can mix arbitrary feature (பண்பு — தகவல் நெடுவரிசை)s (word shape, capitalization, neighboring words). Still the classical production workhorse in 2026 for low-resource deployments.
...
- **Transformer-based.** Fine-tune BERT with a token-classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) head. Best accuracy. Most compute.
...
### Step 2: hand-crafted feature (பண்பு — தகவல் நெடுவரிசை)s

For classical (non-neural) NER, feature (பண்பு — தகவல் நெடுவரிசை)s are the game. Useful ones:

```python
def token_feature (பண்பு — தகவல் நெடுவரிசை)s(token, prev_token, next_token):
    return {
        "lower": token.lower(),
...
def to_feature (பண்பு — தகவல் நெடுவரிசை)s(tokens):
    out = []
...
crf = sklearn_crfsuite.CRF(algorithm (அல்கோரிதம் — வழிமுறை)="lbfgs", c1=0.1, c2=0.1, max_iteration (மறுயுதல் — மீண்டும் செய்தல்)s=100, all_possible_transitions=True)
X_train = [to_feature (பண்பு — தகவல் நெடுவரிசை)s(s) for s in sentences_tokenized]
...
feature (பண்பு — தகவல் நெடுவரிசை)s become learned. Inputs: token embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s (GloVe or fastText). LSTM reads left-to-right and right-to-left. Concatenated hidden states go through a CRF output layer. The CRF still enforces tag-sequence consistency. the LSTM replaces hand-crafted features with learned ones.
...
class BiLSTM_CRF_Head(nn.Module):
    def __init__(self, vocab_size, embed_dim, hidden_dim, n_labels):
        super().__init__()
        self.embed = nn.embedding (embedding — பொருளை எண்களாக மாற்றுதல்)(vocab_size, embed_dim)
        self.lstm = nn.LSTM(embed_dim, hidden_dim, bidirectional=True, batch (பத்தி — தொகுப்பு)_first=True)
...
Notice `iPhone` labeled `ORG` quite than `PRODUCT` — spaCy's small model has weak product-entity coverage. The large model (`en_core_web_lg`) does better. The transformer model (`en_core_web_trf`) does better still.
...
- [Devlin et al. (2018). BERT: Pre-training of Deep Bidirectional Transformers](https://arxiv.org/abs/1810.04805) — introduces the token-classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) pattern that became standard.
- [spaCy linguistic feature (பண்பு — தகவல் நெடுவரிசை)s — named entities](https://spacy.io/usage/linguistic-features#named-entities) — practical reference for every attribute on `Doc.ents` and `Span`.
