...
**Extractive.** Treat the article as a graph where nodes are sentences and edges are similarities. Run PageRank (or something like it) over the graph to score sentences by how connected they are to everything else. Highest-scoring sentences are the summary. The canonical how to build is **TextRank** (Mihalcea and Tarau, 2004).
...
def textrank(text, top_k=3, damping=0.85, iteration (மறுயுதல் — மீண்டும் செய்தல்)s=50, epsilon=1e-4):
    ...
    scores = [1.0] * n
    for _ in range(iteration (மறுயுதல் — மீண்டும் செய்தல்)s):
...
Two things worth naming. The similarity function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) uses log-normalized word overlap, which is the original TextRank variant. Cosine of TF-IDF vector (வெக்டர் — எண் பட்டியல்)s works too. The damping factor 0.85 and iteration (மறுயுதல் — மீண்டும் செய்தல்) count are the PageRank defaults.
...
- **BERTScore** (contextual embedding (embedding — பொருளை எண்களாக மாற்றுதல்) similarity) gained ground through 2023 and is now reported alongside ROUGE in most summarization papers.
...
LLMs with long context often beat specialized models in 2026 when compute is not a constraint. The choice is cost and reproducibility. specialized models give more consistent outputs.
