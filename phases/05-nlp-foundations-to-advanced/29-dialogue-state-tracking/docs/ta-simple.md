...
**Task structure.** A schema defines domains (restaurant, hotel, taxi) and their slots (cuisine, area, price, people). Each slot is empty, filled with a value from a closed set (price: {cheap, moderate, expensive}), or a free-form value (name: "The Copper Kettle").

**Two DST formulations.**

- **classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்).** For each (slot, candidate_value) pair, predict yes/no. Works for closed-vocab slots. Standard pre-2020.
...
On a detected correction, overwrite the last-updated slot quite than appending. Hard to get right without LLM help. The modern pattern: always let the LLM regenerate the whole state from history quite than incrementally updating — this naturally handles corrections.
...
| Slot | Unit of user intent | Named parameter (பாரமீட்டர் — கட்டுப்பாடு) the backend needs (cuisine, date). |
