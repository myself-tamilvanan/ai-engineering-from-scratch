...
Sample `x_0` from data, pick a random `t`, sample `ε ~ N(0, I)`, compute the noisy `x_t` in one shot via the closed form, and regress on the noise. One loss, no minimax, no KL, no reparameter (பாரமீட்டர் — கட்டுப்பாடு)ization tricks.
...
1. **Denoising is easy. generating is hard.** At `t=T`, the data is pure noise — the net has to solve a trivial problem. At `t=0`, the net only has to clean up a few pixels. At intermediate `t`, the problem is hard but the net has many gradient (சாய்வு — மாற்றத்தின் दिशा)s flowing through the same weights from every noise level.

2. **Score matching in disguise.** Vincent (2011) proved that predicting the noise is equivalent to estimating `∇_x log q(x_t | x_0)`, the *score*. The reverse SDE uses this score to walk up the density gradient (சாய்வு — மாற்றத்தின் दिशा) — a guided random walk toward high-probability regions.

3. **The ELBO reduces to simple MSE.** The full variational lower bound has a KL term per timestep. With DDPM's parameter (பாரமீட்டர் — கட்டுப்பாடு)ization those KL terms simplify to MSE on noise prediction with specific coefficients. Ho dropped the coefficients (calling it "simple" loss) and quality *improved*.
...
def train_step(x0, model, alpha_bars, rng):
    ...
    loss = (eps - eps_hat) ** 2
    return loss, gradient (சாய்வு — மாற்றத்தின் दिशा)_step(model, ...)
```
...
For a 1-D problem with 40 timesteps and a 24-unit MLP, this learns the two-mode mixture in ~200 epoch (யுக் — ஒரு முழு சுழற்சி)s.

## Time conditioning

The net needs to know which timestep it is denoising. Two standard options:

- **Sinusoidal embedding (embedding — பொருளை எண்களாக மாற்றுதல்).** Like Transformer positional encoding. `embed(t) = [sin(t/ω_0), cos(t/ω_0), sin(t/ω_1), ...]`. Pass through an MLP, broadcast into the net.
...
- **Timestep embedding (embedding — பொருளை எண்களாக மாற்றுதல்) is fragile.** Passing raw `t` as a float works for toy 1-D but fails for images. always use a proper embedding.
...
3. **Caching and compilation.** `torch.compile(unet, mode="reduce-extra work")`, tensor (டென்சர் — பல வடிவ எண் வரிசை)RT-LLM's diffusion backends, `xformers`/SDPA attention, bf16 weights. Cuts per-step latency ~2×. Stacks with (1) and (2).

For a production diffusion server the budget conversation is the same as production literature describes for LLMs: latency is `num_steps × step_cost + VAE_decode`, throughput is `batch (பத்தி — தொகுப்பு)_size × (num_steps × step_cost)^-1`. TTFT is small (one step). TPOT-equivalent is the full response time because image generation is "all-at-once" from the user's perspective.
