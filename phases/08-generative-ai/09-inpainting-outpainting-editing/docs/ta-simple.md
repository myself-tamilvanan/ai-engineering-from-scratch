...
`code/main.py` implements a toy 1-D inpainting scheme on 5-dimension (பரிமாணம் — கோணம்)al data. We train a DDPM on 5-D mixture data where each sample is 5 floats from one of two clusters. At inference, we "mask" 2 of the 5 dimensions, inject the noisy-forward version of the unmasked three at each step, and regenerate only the masked dimensions.
...
- **Seams.** The naive approach leaves visible boundaries because gradient (சாய்வு — மாற்றத்தின் दिशा) info doesn't flow across the mask. Fix: dilate the mask by 8-16 pixels, or use a proper inpainting model.
...
1. **Easy.** In `code/main.py`, vary the fraction of dimension (பரிமாணம் — கோணம்)s masked from 0.2 to 0.8. At what fraction does the inpaint quality (residual in masked dims) equal unconditional generation?
...
Users editing an image expect sub-5-second round trips. A 30-step SDXL-Inpaint at 1024² is 3-4 s on an L4, plus SAM mask generation (~200 ms) and VAE encode/decode (~500 ms combined). In production framing, this is TTFT-bound quite than throughput-bound — batch (பத்தி — தொகுப்பு) 1, low concurrency, minimize every stage:

- **SAM-H is the slow one.** SAM-H at 1024² is ~200 ms. SAM-ViT-B is ~40 ms with minor quality loss. SAM 2 (video) adds temporal extra work. do not use it for single-image edits.
