...
- **FID (Fréchet Inception Distance).** A distance between two distributions — real and generated — in an Inception network's feature (பண்பு — தகவல் நெடுவரிசை) space. Lower is better.
- **CLIP score.** Cosine similarity between a generated image's CLIP-image embedding (embedding — பொருளை எண்களாக மாற்றுதல்) and a prompt's CLIP-text embedding. Higher is better. Measures prompt adherence.
...
1. Extract Inception-v3 feature (பண்பு — தகவல் நெடுவரிசை)s (2048-D) for N real images and N generated.
2. Fit a Gaussian to each pool: compute mean `μ_r, μ_g` and covariance `Σ_r, Σ_g`.
3. FID = `||μ_r - μ_g||² + Tr(Σ_r + Σ_g - 2 · (Σ_r · Σ_g)^0.5)`.

Interpretation: Fréchet distance between two multivariate Gaussians in feature (பண்பு — தகவல் நெடுவரிசை) space. Lower = more similar distributions.

Failure modes:
- **Biased on small N.** FID is mean-squared over the feature (பண்பு — தகவல் நெடுவரிசை) distribution — small N under-estimates covariance, gives falsely low FID. Always use N ≥ 10,000.
...
Average across 30k generated images → a scalar (scalar — ஒரு எண்) comparable between models.
...
CMMD (Jayasumana et al., 2024) fixes some of these: uses CLIP feature (பண்பு — தகவல் நெடுவரிசை)s instead of Inception, maximum-mean discrepancy instead of Fréchet. Better at detecting subtle quality differences.
...
- **PartiPrompts (Google)**: 1,600 many different prompts, 12 categories.
...
`code/main.py` implements FID, CLIP-score-like, and Elo aggregation on synthetic "feature (பண்பு — தகவல் நெடுவரிசை) vector (வெக்டர் — எண் பட்டியல்)s" (we use 4-D vectors as stand-ins for Inception features). You see:

- FID calculation on a small N and on a large N — the bias.
- "CLIP score" as cosine similarity between feature (பண்பு — தகவல் நெடுவரிசை) pools.
- Elo update rule from a synthetic preference stream.

### Step 1: FID in four lines

```python
def fid(real_feature (பண்பு — தகவல் நெடுவரிசை)s, gen_features):
...
- **Comparing FID across resolutions.** Inception's 299×299 resize changes the feature (பண்பு — தகவல் நெடுவரிசை) distribution. Compare at matched resolution only.
...
2. **Medium.** Implement CMMD from synthetic CLIP-style feature (பண்பு — தகவல் நெடுவரிசை)s (see Jayasumana et al., 2024 for the formula). Compare sensitivity to quality differences vs FID.
...
|------|-----------------|-----------------------|
| FID | "Fréchet Inception Distance" | Fréchet distance of Gaussian fits to real vs gen Inception feature (பண்பு — தகவல் நெடுவரிசை)s. |
| CLIP score | "Text-image similarity" | Cosine similarity between CLIP image and text embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s. |
...
- **batch (பத்தி — தொகுப்பு) hard, forget latency.** Offline eval = static batching at the largest size that fits in memory. `pipe(...).images` with `num_images_per_prompt=8` on an 80GB H100 runs 4-6× faster wall-clock than single-request.
- **Cache the real feature (பண்பு — தகவல் நெடுவரிசை)s.** The Inception (FID) or CLIP (CLIP-score, CMMD) feature extraction over the real reference set is run *once*, stored as a `.npz`. Do not recompute per eval.

For CI / regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) gates: run FID + CLIP score on a 500-sample subset per PR (~30 min). run full 10k FID + HPSv2 + Elo nightly.
