...
**Type:** Learn
**Languages:** Python
**Prerequisites:** Phase 2 (ML Fundamentals), Phase 3 (deep learning (ஆழ் கற்றல் — பல்லாய கொட்ட பயிற்சி) Core), Phase 7 · 14 (Transformers)
**Time:** ~45 minutes
...
| Simulation / physics | Flow matching, score SDE | Straight-line paths, smooth vector (வெக்டர் — எண் பட்டியல்) fields. |
...
| Score | "gradient (சாய்வு — மாற்றத்தின் दिशा) of log-density" | `∇_x log p(x)`. diffusion and SDE models learn this field. |
| Manifold hypothesis | "Data lives on a surface" | High-dim data concentrates on a low-dim manifold. why dimension (பரிமாணம் — கோணம்)ality reduction works. |
...
- **Autoregressive (bucket 1 and 5).** Sequential decode dominates latency. KV-cache, continuous batch (பத்தி — தொகுப்பு)ing, and speculative decoding all apply directly.
