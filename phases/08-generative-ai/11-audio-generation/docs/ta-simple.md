...
**Type:** Build
**Languages:** Python
**Prerequisites:** Phase 6 · 02 (Audio feature (பண்பு — தகவல் நெடுவரிசை)s), Phase 6 · 04 (ASR), Phase 8 · 06 (DDPM)
**Time:** ~45 minutes
...
Encodec (Meta, 2022), SoundStream (Google, 2021), Descript Audio Codec (DAC, 2023). A convolutional encoder compresses waveform to a per-timestep vector (வெக்டர் — எண் பட்டியல்). residual vector quantization (RVQ) converts each vector to a cascade of K codebook indices. Decoder reverses it. 24 kHz audio at 2 kbps using 8 RVQ codebooks at 75 Hz = 600 tokens/sec.
...
| Voice prompt | "3-second sample" | Speaker embedding (embedding — பொருளை எண்களாக மாற்றுதல்) or token prefix that steers the cloned voice. |
...
- **Flow-matching audio models cannot stream trivially.** Stable Audio 2.5 and AudioCraft 2 render a fixed clip length in one pass. To stream, you chunk the clip and overlap boundaries — think sliding-window diffusion — adding 100-300ms of latency extra work vs a codec AR model.
