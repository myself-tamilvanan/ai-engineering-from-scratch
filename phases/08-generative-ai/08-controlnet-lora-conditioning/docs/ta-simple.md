...
Training a new conditional model from scratch for every signal (pose, depth, canny, segmentation) is prohibitive. You want to keep the 2.6B-param SDXL backbone frozen, attach a small side-network that reads the conditioning, and have it nudge the backbone's intermediate feature (பண்பு — தகவல் நெடுவரிசை)s. That is ControlNet.
...
```
SD U-Net decoder:   ... ← orig_enc_feature (பண்பு — தகவல் நெடுவரிசை)s + zero_conv(controlnet_enc(condition))
```
...
```
feature (பண்பு — தகவல் நெடுவரிசை)s += weight_a * control_a(depth) + weight_b * control_b(pose)
```
...
with `r << d`. Rank 4-16 is standard for attention, rank 64-128 for heavy fine-tunes. Number of new parameter (பாரமீட்டர் — கட்டுப்பாடு)s: `2 · d · r` instead of `d²`. For SDXL attention with `d=640`, `r=16`: 20k params per adapter instead of 410k — a 20x reduction. Across the whole model: a LoRA is usually 20-200MB vs the base 5GB.

At inference you can scale the LoRA: `W' = W + α · B @ A`. `α = 0.5-1.5` is normal. Multiple LoRAs stack additively (with the usual warning that they interact in non-linear ways).
...
## Composability matrix (மேட்ரிக்ஸ் — எண் மேஜை)
...
2. **ControlNet-lite.** A "frozen base" predictor and a "side network" that reads an extra signal. The side network's output is gated by a learnable scalar (scalar — ஒரு எண்) initialized to zero (our version of zero-conv). Train and watch the gate ramp up.
...
- **LoRA on the wrong base.** SDXL LoRAs silently no-op on SD 1.5 because the attention dimension (பரிமாணம் — கோணம்)s do not match. Diffusers will warn in 0.30+.
...
| Textual Inversion | "New token" | Learn a new word embedding (embedding — பொருளை எண்களாக மாற்றுதல்) only. legacy, mostly replaced. |

## Production note: LoRA swaps, ControlNet lanes, multi-tenant serving

A real text-to-image SaaS serves hundreds of LoRAs and a dozen ControlNets over the same base checkpoint. The serving problem looks a lot like LLM multi-tenancy (the production literature covers the LLM case under continuous batch (பத்தி — தொகுப்பு)ing and LoRAX / S-LoRA):
...
- **ControlNet as a second attention lane.** The cloned encoder runs in parallel with the base. Two ControlNets at weight 1.0 each = two extra forward passes per step, not one merged pass. batch (பத்தி — தொகுப்பு)-size headroom drops quadratically. Budget for ~1.5× step cost per active ControlNet.
...
Flux-specific: Niels' Flux-on-8GB notebook quantizes the base to 4-bit. stacking a style LoRA (`pipe.load_lora_weights("user/style-lora")`) on that quantized base at `weight_name="pytorch_lora_weights.safetensor (டென்சர் — பல வடிவ எண் வரிசை)s"` still works. This is the recipe most SaaS agencies ship in 2026.
