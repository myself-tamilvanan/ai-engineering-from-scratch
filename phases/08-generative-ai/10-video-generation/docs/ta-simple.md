...
> An image is a 2-D tensor (டென்சர் — பல வடிவ எண் வரிசை). A video is a 3-D one. The theory is the same. the compute is 10-100x harder. OpenAI's Sora (Feb 2024) proved it was possible. By 2026 Veo 2, Kling 1.5, Runway Gen-3, Pika 2.0, and WAN 2.2 ship production video from text at 1080p — and the open-weights stack (CogVideoX, HunyuanVideo, Mochi-1, WAN 2.2) is 12 months behind.
...
A transformer processes the flat sequence of patches. Each patch has a 3D positional embedding (embedding — பொருளை எண்களாக மாற்றுதல்) (time + y + x). Attention is usually factorized:
...
### Step 2: position embedding (embedding — பொருளை எண்களாக மாற்றுதல்) per frame
...
Instead of denoising each frame independently, our tiny net concatenates all frame values + their position embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s and predicts the noise for all frames jointly.
...
| Motion brush | "Directional hint" | UI input where the user paints motion vector (வெக்டர் — எண் பட்டியல்)s onto the image. |
...
- **Frame batch (பத்தி — தொகுப்பு)ing = continuous batching.** At generation time, video is in simple idea a batch of frames linked by attention. Continuous batching (in-flight scheduling) applies: start rendering frame `t+1` while frame `t-1` is being returned, if the model architecture allows sliding-window generation.
