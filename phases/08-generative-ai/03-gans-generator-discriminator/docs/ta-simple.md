...
**Generator `G(z)`.** Maps a noise vector (வெக்டர் — எண் பட்டியல்) `z ~ N(0, I)` to a sample `x̂`. A decoder-shaped network (dense or transposed conv).

**Discriminator `D(x)`.** Maps a sample to a scalar (scalar — ஒரு எண்) probability (or score). Real → 1, fake → 0.

**Loss.** Two alternating updates:

- **Train `D`:** `loss_D = -[ log D(x) + log(1 - D(G(z))) ]`. Binary cross-entropy on real=1, fake=0.
- **Train `G`:** `loss_G = -log D(G(z))`. This is the *non-saturating* form Goodfellow used (original `log(1 - D(G(z)))` saturates and kills gradient (சாய்வு — மாற்றத்தின் दिशा)s when `D` is confident).

**Training loop.** One step of `D`, one step of `G`. Repeat.

**Why it works.** If `G` perfectly matches `p_data`, then `D` cannot do better than chance and outputs 0.5 everywhere. `G` gets no more gradient (சாய்வு — மாற்றத்தின் दिशा). Equilibrium.

**Why it breaks.** Mode collapse (`G` finds one mode `D` can't classify and mints it forever), vanishing gradient (சாய்வு — மாற்றத்தின் दिशा) (`D` learns too fast and `log D` saturates), training instability (learning rates, batch (பத்தி — தொகுப்பு) sizes, anything).

## Variants that made GANs work

| Year | Innovation | Fix |
|------|------------|-----|
| 2015 | DCGAN | Conv/deconv, batch (பத்தி — தொகுப்பு) norm, LeakyReLU — the first stable architecture. |
| 2017 | WGAN, WGAN-GP | Replace BCE with Wasserstein distance + gradient (சாய்வு — மாற்றத்தின் दिशा) penalty. Fixes vanishing gradient. |
...
`code/main.py` trains a tiny GAN on 1-D data: a mixture of two Gaussians. Generator and discriminator are single-hidden-layer MLPs. We implement forward, backward, and the minimax loop by hand. The goal is to see the two key failure modes (mode collapse + vanishing gradient (சாய்வு — மாற்றத்தின் दिशा)) as they happen.

### Step 1: non-saturating loss

The vanilla Goodfellow loss `log(1 - D(G(z)))` goes to 0 when D classifies G's fake as fake with high confidence. At that point the gradient (சாய்வு — மாற்றத்தின் दिशा) for G is basically zero — G cannot improve. The non-saturating form `-log D(G(z))` has the opposite asymptote: it blows up when D is confident, giving G a strong signal.
...
for step in range(steps):
    # train D
    real_batch (பத்தி — தொகுப்பு) = sample_real(batch_size)
    fake_batch = [G(z) for z in sample_noise(batch_size)]
    update_D(real_batch, fake_batch)

    # train G
    fake_batch (பத்தி — தொகுப்பு) = [G(z) for z in sample_noise(batch_size)]  # fresh fakes
    update_G(fake_batch)
```

Fresh fakes for G, otherwise gradient (சாய்வு — மாற்றத்தின் दिशा)s are stale.
...
- **Generator memorizes a mode.** Add noise to D inputs, use a minibatch (பத்தி — தொகுப்பு)-discriminator layer, or switch to WGAN-GP.
...
2. **Medium.** Replace the Goodfellow BCE loss with the WGAN loss: `loss_D = E[D(fake)] - E[D(real)]`, `loss_G = -E[D(fake)]`, and clip D's weights to `[-0.01, 0.01]`. Is training more stable? Compare wall-clock convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்).
3. **Hard.** Extend the 1-D example to 2-D data (mixture of 8 Gaussians on a ring). Track how many of the 8 modes the generator captures at steps 1k, 5k, 10k. Implement minibatch (பத்தி — தொகுப்பு) discrimination and re-measure.
...
| Mode collapse | "G memorized one thing" | Generator produces few distinct outputs despite many different data. |
| WGAN | "Wasserstein" | Replace BCE with Earth-Mover distance + gradient (சாய்வு — மாற்றத்தின் दिशा) penalty. smoother gradient. |
...
- **No KV-cache pressure.** The only state is the weights. batch (பத்தி — தொகுப்பு) size is bounded by activation memory, not cache.
