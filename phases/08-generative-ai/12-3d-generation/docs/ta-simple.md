...
Represent a scene as a cloud of ~1M 3D Gaussians. Each has 59 parameter (பாரமீட்டர் — கட்டுப்பாடு)s: position (3), covariance (6, or quaternion 4 + scale 3), opacity (1), spherical-harmonics color (48 at degree 3, 3 at degree 0).

Rendering = projection + alpha-compositing. Fast (~100 fps at 1080p on a 4090). Differentiable. Fit by gradient (சாய்வு — மாற்றத்தின் दिशा) descent against ground-truth photos. A scene fits in 5-30 minutes on a consumer GPU.
...
### Step 3: fit by gradient (சாய்வு — மாற்றத்தின் दिशा) descent

```python
for step in range(steps):
    pred = render(size, gaussians)
    loss = mse(pred, target)
    gradient (சாய்வு — மாற்றத்தின் दिशा)s = compute_grads(pred, target, gaussians)
...
Save `outputs/skill-3d-pipeline.md`. Skill takes a 3D brief (input: text / one image / few images. output: mesh / splat / NeRF. usage: render / game / VR) and outputs: pipeline (multi-view diffusion + fit, or direct mesh model), base model, iteration (மறுயுதல் — மீண்டும் செய்தல்) budget, topology post-processing, material channels needed.
...
| Triplane | "Three 2-D planes" | Factor 3D into three 2-D axis-aligned feature (பண்பு — தகவல் நெடுவரிசை) grids. cheaper than volumetric. |
| SDS | "Score distillation sampling" | Train 3D model by using 2D-diffusion score as pseudo-gradient (சாய்வு — மாற்றத்தின் दिशा). |
| Multi-view diffusion | "Many views at once" | Diffusion model that outputs a batch (பத்தி — தொகுப்பு) of consistent camera views. |
...
- **NeRF / triplane.** Inference is ray-marching + an MLP forward per sample. A 512² render requires millions of MLP forwards. batch (பத்தி — தொகுப்பு) the ray samples aggressively. SDPA/xformers applies.
...
- **SDS / DreamFusion.** Per-asset optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்), not inference. Build jobs, not request handlers.
