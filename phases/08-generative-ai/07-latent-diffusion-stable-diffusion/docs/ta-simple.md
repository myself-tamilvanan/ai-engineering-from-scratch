...
Pixel-space diffusion at 512² means the U-Net runs on tensor (டென்சர் — பல வடிவ எண் வரிசை)s of shape `[B, 3, 512, 512]`. Each sampling step is ~100 GFLOPS for a 500M-param U-Net. Fifty steps is 5 TFLOPS per image. Train on a billion images and the compute bill is absurd.
...
**Text conditioning.** Two extra components. A frozen text encoder (CLIP-L for SD 1.x, CLIP-L+OpenCLIP-G for SD 2/XL, T5-XXL for SD3 and Flux). A cross-attention injection: every U-Net block takes `[Q = image feature (பண்பு — தகவல் நெடுவரிசை)s, K = V = text tokens]` and mixes them in. The tokens are the only way text influences the image.

**The loss function (சேதத் சூத்திரம் — எவ்வளவு தவறு என்பதை அளக்கும் சூத்திரம்) (சார்பு — உள்ளீட்டுக்கு வெளியீடு) is identical to Lesson 06.** Same DDPM / flow matching MSE on noise. You just swap the data domain.
...
Replace the class label with a frozen text encoder output. Feed the text embedding (embedding — பொருளை எண்களாக மாற்றுதல்) to the U-Net via cross-attention:
...
- [Hugging Face Diffusers docs](https://huggingface.co/docs/diffusers/index) — reference how to build for every checkpoint above.
