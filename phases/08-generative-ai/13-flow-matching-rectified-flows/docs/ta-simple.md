...
If you could train the model such that the path from noise to data was a *straight line*, a single Euler step from `t=1` to `t=0` would work. Flow matching builds this directly: define a straight-line interpolation from `x_1 ∼ N(0, I)` to `x_0 ∼ data`, train a vector (வெக்டர் — எண் பட்டியல்) field `v_θ(x, t)` to match its time derivative, integrate at inference.

Rectified flow (Liu 2022) goes further: iteratively straighten the paths with a reflow procedure that produces a progressively closer-to-linear ODE. After two reflow iteration (மறுயுதல் — மீண்டும் செய்தல்)s, a 2-step sampler matches 50-step DDPM quality.
...
Define a neural vector (வெக்டர் — எண் பட்டியல்) field `v_θ(x_t, t)` and train it to match this derivative:
...
At inference, integrate the learned vector (வெக்டர் — எண் பட்டியல்) field *backwards* in time:
...
In practice 2 reflow iteration (மறுயுதல் — மீண்டும் செய்தல்)s get you to near-linear, enabling 2-4 step inference. SDXL-Turbo, SD3-Turbo, LCM are all distilled-from-flow-matching models.
...
`code/main.py` implements 1-D flow matching on a two-mode Gaussian mixture. The vector (வெக்டர் — எண் பட்டியல்) field `v_θ(x, t)` is a tiny MLP trained with the straight-line target. At inference, integrate 1, 2, 4, and 20 Euler steps and compare sample quality.
...
- **Time parameter (பாரமீட்டர் — கட்டுப்பாடு)ization.** Flow matching uses `t ∈ [0, 1]` with `t=0` at data, `t=1` at noise. DDPM uses `t ∈ [0, T]` with `t=0` at data, `t=T` at noise. Same direction, different scale. Papers get this wrong constantly.
...
| Science / physics (particle trajectories, molecules) | Flow matching + equivariant vector (வெக்டர் — எண் பட்டியல்) field |
...
3. **Hard.** Implement one reflow iteration (மறுயுதல் — மீண்டும் செய்தல்): generate paired (x_0, x_1) by integrating the first model, train a second model on the pairs, and compare 1-step sample quality.
...
| Logit-normal t | "SD3 sampling" | Concentrate `t` sampling toward mid-values where gradient (சாய்வு — மாற்றத்தின் दिशा)s are strongest. |
