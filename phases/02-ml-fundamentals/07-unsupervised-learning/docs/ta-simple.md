...
> No labels, no teacher. The algorithm (அல்கோரிதம் — வழிமுறை) finds structure on its own.
...
- Explain when DBSCAN outperforms K-Means and identify which algorithm (அல்கோரிதம் — வழிமுறை) handles non-spherical clusters and outliers
...
The catch: without labels, you cannot directly measure "right" or "wrong." You need different tools to evaluate whether the structure your algorithm (அல்கோரிதம் — வழிமுறை) found is meaningful.
...
Lloyd's algorithm (அல்கோரிதம் — வழிமுறை):
...
The objective function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) (inertia) measures the total squared distance from each point to its assigned centroid. K-Means minimizes this, but only finds a local minimum. Different initializations can give different results.
...
Two parameter (பாரமீட்டர் — கட்டுப்பாடு)s:
...
The "closeness" between clusters is measured as:
...
GMM assumes the data is generated from a mixture of K Gaussian distributions, each with its own mean and covariance. The Expectation-Maximization (EM) algorithm (அல்கோரிதம் — வழிமுறை) alternates between:
...
| DBSCAN | Unknown K, arbitrary shapes, outlier detection | Varying densities, very high dimension (பரிமாணம் — கோணம்)s |
...
def kmeans(data, k, max_iteration (மறுயுதல் — மீண்டும் செய்தல்)s=100, seed=42):
    random.seed(seed)
    n_feature (பண்பு — தகவல் நெடுவரிசை)s = len(data[0])
    ...
    for iteration (மறுயுதல் — மீண்டும் செய்தல்) in range(max_iterations):
        ...
        if all(
            ...
            print(f"  Converged at iteration (மறுயுதல் — மீண்டும் செய்தல்) {iteration + 1}")
            break
...
### Step 4: Gaussian Mixture Model (EM algorithm (அல்கோரிதம் — வழிமுறை))

```python
def gmm(data, k, max_iteration (மறுயுதல் — மீண்டும் செய்தல்)s=100, seed=42):
    random.seed(seed)
    n = len(data)
    d = len(data[0])
    ...
    for iteration (மறுயுதல் — மீண்டும் செய்தல்) in range(max_iterations):
        ...
        if shift < 1e-6:
            print(f"  GMM converged at iteration (மறுயுதல் — மீண்டும் செய்தல்) {iteration + 1}")
            break
...
With scikit-learn, the same algorithm (அல்கோரிதம் — வழிமுறை)s are one-liners:
...
This lesson produces working implementations of K-Means, DBSCAN, and GMM from scratch. The clustering code is reused as a foundation for more advanced unsupervised methods.

## Exercises

1. Implement K-Means++ initialization: instead of picking random centroids, pick the first randomly and each subsequent centroid with probability proportional to its squared distance from the nearest existing centroid. Compare convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) speed to random initialization.
...
| EM algorithm (அல்கோரிதம் — வழிமுறை) | "Soft K-Means" | Expectation-Maximization: iteratively compute membership probabilities (E-step) and update distribution parameter (பாரமீட்டர் — கட்டுப்பாடு)s (M-step) |
...
- [scikit-learn Clustering Guide](https://scikit-learn.org/stable/modules/clustering.html) - practical comparison of all clustering algorithm (அல்கோரிதம் — வழிமுறை)s with visual examples
