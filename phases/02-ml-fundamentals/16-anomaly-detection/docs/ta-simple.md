...
- Explain why anomaly detection is framed as modeling normal data quite than classifying anomalies
- Compare unsupervised anomaly detection with supervised classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) and evaluate the choice between novel anomaly coverage and precision

## The Problem

A credit card helps in New York at 2pm, then in Tokyo at 2:05pm. A factory sensor reads 150 degrees when the normal range is 80-120. A server sends 50,000 requests per second when the daily average is 200.
...
Most methods detect point anomalies. Contextual anomalies need time or location feature (பண்பு — தகவல் நெடுவரிசை)s. Collective anomalies need sequence-aware methods.
...
In standard classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்), you have labels for both classes. In anomaly detection, you usually have one of three situations:
...
The key insight: anomaly detection is fundamentally different from classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்). You are modeling the distribution of normal data, not the decision boundary between two classes.

### Supervised vs Unsupervised: The choice

If you do have labeled anomalies, should you use them for training (supervised classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)) or for evaluation only (unsupervised detection)?

**Supervised (treat as classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)):**
...
The simplest approach. Compute the mean and standard deviation of each feature (பண்பு — தகவல் நெடுவரிசை). Flag any point more than k standard deviations from the mean.
...
**When it works well:** Single-feature (பண்பு — தகவல் நெடுவரிசை) monitoring where data is roughly bell-shaped. Server response times, manufacturing tolerances, sensor readings with stable baselines.
...
**Weaknesses:** Univariate only (applies per feature (பண்பு — தகவல் நெடுவரிசை) independently). Cannot detect anomalies that are unusual only when features are considered together (a point might be normal in each feature individually but anomalous in the joint space).
...
```mermaid
flowchart TD
    A[All Data Points] --> B{Random feature (பண்பு — தகவல் நெடுவரிசை) + Random Split}
    B --> C[Left Partition]
    B --> D[Right Partition]
...
2. At each node, pick a random feature (பண்பு — தகவல் நெடுவரிசை) and a random split value between the feature's min and max
...
**Strengths:** No distribution assumptions. Works in high dimension (பரிமாணம் — கோணம்)s. Scales well (sublinear in sample size because each tree uses a subsample). Handles mixed feature (பண்பு — தகவல் நெடுவரிசை) types.

**Weaknesses:** Struggles with anomalies in dense regions (masking effect). Random splitting is less effective when many feature (பண்பு — தகவல் நெடுவரிசை)s are irrelevant.

**Key hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s:**
- `n_estimators`: Number of trees. 100 is usually enough. More trees give more stable scores but slower calculation.
...
**Weaknesses:** Slow on large datasets (O(n^2) for naive how to build). Sensitive to the choice of k. Does not work well in very high dimension (பரிமாணம் — கோணம்)s (curse of dimensionality affects distance calculations).

### Comparison

| Method | Assumptions | Speed | Handles High Dims | Detects Local Anomalies |
|--------|------------|-------|-------------------|------------------------|
| Z-score | Normal distribution | Very fast | Yes (per feature (பண்பு — தகவல் நெடுவரிசை)) | No |
...
2. **feature (பண்பு — தகவல் நெடுவரிசை) engineering.** Raw features plus derived features (rolling statistics, time features, ratios).
...
Simple and vector (வெக்டர் — எண் பட்டியல்)ized. Flags a point if any feature (பண்பு — தகவல் நெடுவரிசை) exceeds the threshold.
...
The from-scratch how to build builds isolation trees that randomly partition the feature (பண்பு — தகவல் நெடுவரிசை) space:
...
class IsolationTree:
    ...
    def fit(self, X, depth=0):
        ...
        if depth >= self.max_depth or n <= 1:
            ...
            return self
        self.is_leaf = False
        self.feature (பண்பு — தகவல் நெடுவரிசை) = np.random.randint(p)
...
2. **Multimodal data.** Three clusters of different sizes and densities. Points between clusters are anomalous. Z-score struggles because the per-feature (பண்பு — தகவல் நெடுவரிசை) ranges are wide.
3. **High-dimension (பரிமாணம் — கோணம்)al data.** 50 features, but anomalies differ in only 5 of them. Tests whether methods can find anomalies in a subset of features.
...
### sklearn Contamination parameter (பாரமீட்டர் — கட்டுப்பாடு)

The `contamination` parameter (பாரமீட்டர் — கட்டுப்பாடு) in sklearn determines the threshold for converting continuous anomaly scores into binary predictions. It does not change the underlying scores.
...
Both produce the same anomaly scores. But `iso_5` flags the top 5% while `iso_10` flags the top 10%. If you do not know the true anomaly rate (you usually do not), set contamination to "auto" and work with the raw scores directly. Set your own threshold based on the cost choice between false positives and false negatives.

### One-Class SVM

Another unsupervised anomaly detector worth knowing. One-Class SVM fits a boundary around normal data in a high-dimension (பரிமாணம் — கோணம்)al feature (பண்பு — தகவல் நெடுவரிசை) space (using the kernel trick).
...
The `nu` parameter (பாரமீட்டர் — கட்டுப்பாடு) approximates the fraction of anomalies. One-Class SVM works well on small to medium datasets but does not scale to very large data (the kernel matrix (மேட்ரிக்ஸ் — எண் மேஜை) grows quadratically).

### Autoencoder Approach (Preview)

Autoencoders are neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)s that learn to compress and reconstruct data. Train on normal data. At test time, anomalies have high reconstruction error because the network learned to reconstruct normal patterns only.

This is covered in Phase 3 (deep learning (ஆழ் கற்றல் — பல்லாய கொட்ட பயிற்சி)), but the principle is the same: model what is normal, flag what deviates.

### Ensemble Anomaly Detection

Just as ensemble methods improve classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) (Lesson 11), combining multiple anomaly detectors improves detection. The simplest approach:
...
4. **feature (பண்பு — தகவல் நெடுவரிசை) engineering.** Raw features are rarely enough. Add rolling statistics, ratios, time-since-last-event, and domain-specific features. A good feature set matters more than the choice of detector.
...
In both cases, the optimal threshold depends on the cost ratio between false positives and false negatives. Plot precision and recall at different thresholds, overlay the cost function (சார்பு — உள்ளீட்டுக்கு வெளியீடு), and pick the minimum-cost point.

### Scaling to Production

For real-time anomaly detection in production:

1. **batch (பத்தி — தொகுப்பு) training, online scoring.** Train the model periodically (daily, weekly) on recent normal data. Score each new observation as it arrives.
2. **feature (பண்பு — தகவல் நெடுவரிசை) calculation must match.** If you trained with rolling statistics over 30 days, you need 30 days of history to compute features for a new observation. Buffer the required history.
...
2. **Multivariate anomalies.** Create 2D data where each feature (பண்பு — தகவல் நெடுவரிசை) individually looks normal, but the combination is anomalous (e.g., points far from the main cluster diagonal). Show that Z-score per feature misses these but Isolation Forest catches them.
...
4. **Streaming anomaly detection.** Modify the Z-score detector to work in a streaming setting: update the running mean and variance as new points arrive (Welford's online algorithm (அல்கோரிதம் — வழிமுறை)). Compare to batch (பத்தி — தொகுப்பு) Z-score on the same data.
...
| Contamination | "Expected fraction of anomalies" | A hyperparameter (பாரமீட்டர் — கட்டுப்பாடு) telling the detector what proportion of the data it should flag as anomalous |
...
- [Goldstein and Uchida, A Comparative Evaluation of Unsupervised Anomaly Detection algorithm (அல்கோரிதம் — வழிமுறை)s (2016)](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0152173) -- empirical comparison of 10 methods on real datasets
