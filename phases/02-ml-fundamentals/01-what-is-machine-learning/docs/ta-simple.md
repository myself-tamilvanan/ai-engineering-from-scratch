...
- Distinguish between classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) and regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) tasks and select the appropriate loss function (சேதத் சூத்திரம் — எவ்வளவு தவறு என்பதை அளக்கும் சூத்திரம்) (சார்பு — உள்ளீட்டுக்கு வெளியீடு) for each
...
flowchart LR
    ...
    subgraph ML["Machine Learning"]
        direction LR
        D2[Data] --> P2[Learning algorithm (அல்கோரிதம் — வழிமுறை)]
...
Machine learning: you provide data and expected outputs. The algorithm (அல்கோரிதம் — வழிமுறை) discovers the rules.

The "model" that comes out of training IS the rules, encoded as numbers (weights, parameter (பாரமீட்டர் — கட்டுப்பாடு)s). It generalizes from examples it has seen to make predictions on data it has never seen.
...
flowchart TD
    ...
    SL --> C[classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)]
    SL --> R[regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)]

    UL --> CL[Clustering]
    UL --> DR[dimension (பரிமாணம் — கோணம்)ality Reduction]

    RL --> PO[Policy optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)]
...
- "Here are house feature (பண்பு — தகவல் நெடுவரிசை)s and prices. Learn to predict the price."
...
- "Here are 1,000 dimension (பரிமாணம் — கோணம்)al data points. Reduce to 2 dimensions while keeping structure."
...
### classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) vs regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)

These are the two main supervised learning tasks.

| Aspect | classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) | regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) |
|--------|---------------|------------|
| Output | Discrete categories | Continuous numbers |
| Example | "Is this email spam?" | "What will the house price be?" |
| Output space | {cat, dog, bird} | Any real number |
| loss function (சேதத் சூத்திரம் — எவ்வளவு தவறு என்பதை அளக்கும் சூத்திரம்) (சார்பு — உள்ளீட்டுக்கு வெளியீடு) | Cross-entropy, accuracy | Mean squared error, MAE |
| Decision | Boundaries between classes | A curve that fits the data |

classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) answers "which category?" regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) answers "how much?"

Some problems is framed either way. Predicting if a stock goes up or down is classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்). Predicting the exact price is regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்).

### The ML Workflow

Every machine learning project follows the same pipeline, regardless of the algorithm (அல்கோரிதம் — வழிமுறை).

```mermaid
flowchart LR
    A[Collect Data] --> B[Clean & Explore]
    B --> C[feature (பண்பு — தகவல் நெடுவரிசை) Engineering]
    C --> D[Split Data]
    D --> E[Train Model]
    E --> F[Evaluate]
    F -->|Not good enough| C
...
**feature (பண்பு — தகவல் நெடுவரிசை) Engineering**: Transform raw data into features the model can use. Turn dates into day-of-week. Normalize numerical columns. Encode categorical variables. Good features matter more than fancy algorithm (அல்கோரிதம் — வழிமுறை)s.

**Split Data**: Divide into training, validation, and test sets. The model trains on training data, you tune hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s on validation data, and you report final performance on test data.

**Train Model**: Feed training data into an algorithm (அல்கோரிதம் — வழிமுறை). The algorithm adjusts internal parameter (பாரமீட்டர் — கட்டுப்பாடு)s to minimize a loss function (சேதத் சூத்திரம் — எவ்வளவு தவறு என்பதை அளக்கும் சூத்திரம்) (சார்பு — உள்ளீட்டுக்கு வெளியீடு).

**Evaluate**: Measure performance on validation/test data. If performance is not acceptable, go back and try different feature (பண்பு — தகவல் நெடுவரிசை)s, algorithm (அல்கோரிதம் — வழிமுறை)s, or hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s.
...
flowchart LR
    ...
    M -->|Tune hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s| VA
...
| Validation | Tune hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s, compare models | After each training run | 10-20% |
...
- Reduce model complexity (fewer parameter (பாரமீட்டர் — கட்டுப்பாடு)s, simpler architecture)
...
- Add more feature (பண்பு — தகவல் நெடுவரிசை)s
- Reduce regularization
- Train longer

### The Bias-Variance choice
...
There is no single algorithm (அல்கோரிதம் — வழிமுறை) that works best for every problem. An algorithm that performs well on one class of problems will perform poorly on another. This is why data scientists try multiple algorithms and compare results.

In practice, the choice depends on:
- How much data you have
- How many feature (பண்பு — தகவல் நெடுவரிசை)s there are
...
- **Rules are simple and well-defined.** Tax calculation, sorting algorithm (அல்கோரிதம் — வழிமுறை)s, unit conversions. If you can write the logic in a few if-statements, a model adds complexity for no benefit.
...
- **You cannot explain the decision and explainability is required.** Regulated industries (lending, insurance, criminal justice) sometimes require that every decision be fully explainable. Some ML models are interpretable (linear regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்), small decision trees). Most are not.
...
The code in `code/ml_intro.py` implements a nearest centroid classifier from scratch, the simplest possible ML algorithm (அல்கோரிதம் — வழிமுறை). It shows the core idea: learn from data, then predict on new data.
...
That is the entire algorithm (அல்கோரிதம் — வழிமுறை). Fit computes two means. Predict computes distances. No gradient (சாய்வு — மாற்றத்தின் दिशा) descent, no iteration (மறுயுதல் — மீண்டும் செய்தல்), no hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s.

### Step 2: Train on Synthetic Data

We generate a 2D classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) dataset with two classes that overlap slightly. The centroid classifier draws a linear decision boundary between the class centers.
...
The nearest centroid classifier is trivially simple. It has no hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s, no iteration (மறுயுதல் — மீண்டும் செய்தல்), no gradient (சாய்வு — மாற்றத்தின் दिशा) descent. Yet it captures the basic ML pattern:
...
Every ML algorithm (அல்கோரிதம் — வழிமுறை), from logistic regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) to transformers, follows this same three-step pattern. The representation gets more complex, but the workflow stays the same.
...
- Classes have multiple clusters (e.g., the digit "1" is written in several different ways)
- The decision boundary is nonlinear (e.g., one class wraps around another)
- feature (பண்பு — தகவல் நெடுவரிசை)s have very different scales (distance is dominated by the largest-scale feature)

These limitations motivate every other algorithm (அல்கோரிதம் — வழிமுறை) you will learn. K-nearest neighbors handles multiple clusters. Decision trees handle nonlinear boundaries. feature (பண்பு — தகவல் நெடுவரிசை) scaling fixes the scale problem. Each lesson builds on the limitations of the previous one.
...
from sklearn.datasets import make_classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)
from sklearn.model_selection import train_test_split

X, y = make_classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)(
    n_samples=500, n_feature (பண்பு — தகவல் நெடுவரிசை)s=2, n_redundant=0,
...
This lesson produces `outputs/prompt-ml-problem-framer.md` -- a prompt that turns vague business problems into concrete ML tasks. Give it a problem description ("we want to reduce churn" or "predict demand for next quarter") and it identifies the learning type, defines the prediction target, lists candidate feature (பண்பு — தகவல் நெடுவரிசை)s, picks a success metric, establishes a baseline, and flags pitfalls like data leakage or class imbalance. Use it at the start of any ML project to avoid building the wrong thing.

## Key Terms

| Term | What people say | What it actually means |
|------|----------------|----------------------|
| Model | "The AI" | A mathematical function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) with learnable parameter (பாரமீட்டர் — கட்டுப்பாடு)s that maps inputs to outputs |
| Training | "Teaching the AI" | Running an optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) algorithm (அல்கோரிதம் — வழிமுறை) to adjust model parameters so predictions match known outputs |
| feature (பண்பு — தகவல் நெடுவரிசை) | "An input column" | A measurable property of the data that the model uses to make predictions |
...
| loss function (சேதத் சூத்திரம் — எவ்வளவு தவறு என்பதை அளக்கும் சூத்திரம்) | "How wrong the model is" | A function that measures the gap between predicted and actual outputs, which training tries to minimize |
...
1. Take any dataset (e.g., Iris, Titanic). Split it 70/15/15 into train/validation/test. Explain why you should not tune hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s on the test set.
2. List three real-world problems. For each one, identify whether it is classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்), regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்), or clustering, and whether it is supervised or unsupervised.
