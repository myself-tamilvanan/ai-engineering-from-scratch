...
- Implement lag feature (பண்பு — தகவல் நெடுவரிசை)s and rolling statistics to convert a time series into a supervised learning problem
...
You reach for your standard ML toolkit: random train/test split, cross-validation, feature (பண்பு — தகவல் நெடுவரிசை) matrix (மேட்ரிக்ஸ் — எண் மேஜை) in, prediction out. Every step is wrong.

Time series breaks the assumptions that standard ML relies on. Samples are not independent -- today's temperature depends on yesterday's. Random splits leak future information into the past. feature (பண்பு — தகவல் நெடுவரிசை)s that look great in backtest fail in production because they rely on patterns that shift over time.
...
This lesson covers the fundamentals: what makes time data different, how to evaluate models honestly, and how to turn a time series into feature (பண்பு — தகவல் நெடுவரிசை)s that standard ML models can consume.
...
These violations are not minor. They change how you build feature (பண்பு — தகவல் நெடுவரிசை)s, how you evaluate models, and which algorithm (அல்கோரிதம் — வழிமுறை)s work.
...
Autocorrelation measures how much a value at time t correlates with the value at time t-k (k steps in the past). The autocorrelation function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) (ACF) plots this correlation for each lag k.
...
- How many lag feature (பண்பு — தகவல் நெடுவரிசை)s to create. Use lags up to where ACF becomes negligible.

**PACF (Partial Autocorrelation function (சார்பு — உள்ளீட்டுக்கு வெளியீடு))** removes indirect correlations. If today correlates with 3 days ago only because both correlate with yesterday, PACF at lag 3 will be zero while ACF at lag 3 will not.

### Lag feature (பண்பு — தகவல் நெடுவரிசை)s: Turning Time Series into Supervised Learning

Standard ML models need a feature (பண்பு — தகவல் நெடுவரிசை) matrix (மேட்ரிக்ஸ் — எண் மேஜை) X and a target y. Time series gives you a single column of values. The bridge is lag features.

Take the series [10, 12, 14, 13, 15] and create lag-1 and lag-2 feature (பண்பு — தகவல் நெடுவரிசை)s:
...
Now you have a standard regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) problem. Any ML model (linear regression, random forest, gradient (சாய்வு — மாற்றத்தின் दिशा) boosting) can predict the target from the lags.

extra feature (பண்பு — தகவல் நெடுவரிசை)s you can engineer:
...
**How many lags?** Use the autocorrelation function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). If ACF is big up to lag 10, use at least 10 lags. If there is weekly seasonality, include lag 7 (and possibly 14). More lags give the model more history but also more feature (பண்பு — தகவல் நெடுவரிசை)s to fit, increasing the risk of overfitting.

**The target alignment trap.** When creating lag feature (பண்பு — தகவல் நெடுவரிசை)s, the target must be the value at time t, and all features must use values at time t-1 or earlier. If you accidentally include the value at time t as a feature, you have a perfect predictor -- and a completely useless model. This is the most common bug in time series feature engineering.
...
We will not implement ARIMA from scratch -- it requires numerical optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) that is beyond the scope of this lesson. The key insight is understanding what each component does so you can interpret ARIMA results and know when to use it.

### When to Use What

| Approach | Best For | Handles Seasonality | Handles External feature (பண்பு — தகவல் நெடுவரிசை)s |
|----------|---------|-------------------|------------------------|
| Lag features + ML | Tabular with many external features | With calendar features | Yes |
| ARIMA | Single univariate series, short-term | SARIMA variant | No (ARIMAX for limited) |
| Exponential smoothing | Simple trend + seasonality | Yes (Holt-Winters) | No |
| Prophet | Business forecasting, holidays | Yes (Fourier terms) | Limited |
| neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)s (LSTM, Transformer) | Long sequences, many series | Learned | Yes |

For most practical problems, lag feature (பண்பு — தகவல் நெடுவரிசை)s + gradient (சாய்வு — மாற்றத்தின் दिशा) boosting is the strongest starting point. It handles external features naturally, does not require stationarity, and is easy to debug.
...
**Multi-output:** Train one model that outputs all horizons simultaneously. Shares information across horizons but requires a model that supports multiple outputs (or a custom loss function (சேதத் சூத்திரம் — எவ்வளவு தவறு என்பதை அளக்கும் சூத்திரம்) (சார்பு — உள்ளீட்டுக்கு வெளியீடு)).
...
| Using future feature (பண்பு — தகவல் நெடுவரிசை)s | Feature at time t included by mistake | Audit every feature for temporal alignment |
...
### Lag feature (பண்பு — தகவல் நெடுவரிசை) Creator

```python
def make_lag_feature (பண்பு — தகவல் நெடுவரிசை)s(series, n_lags):
    n = len(series)
...
This converts a 1D series into a feature (பண்பு — தகவல் நெடுவரிசை) matrix (மேட்ரிக்ஸ் — எண் மேஜை) where each row has the last `n_lags` values as features, and the current value as the target.
...
A pure AR model is just linear regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) on lag feature (பண்பு — தகவல் நெடுவரிசை)s:
...
class SimpleAR:
    ...
    def fit(self, series):
        X, y = make_lag_feature (பண்பு — தகவல் நெடுவரிசை)s(series, self.n_lags)
...
This is in simple idea identical to linear regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) from Lesson 02, but applied to time-lagged versions of the same variable.
...
With sklearn, you use lag feature (பண்பு — தகவல் நெடுவரிசை)s directly with any regressor:

```python
from sklearn.linear_model import Ridge
from sklearn.ensemble import gradient (சாய்வு — மாற்றத்தின் दिशा)BoostingRegressor

X, y = make_lag_feature (பண்பு — தகவல் நெடுவரிசை)s(series, n_lags=10)
...
The code in `time_series.py` shows both approaches and compares them using walk-forward validation.
...
Time series forecasting uses regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) metrics, but with time-aware context:
...
### Rolling feature (பண்பு — தகவல் நெடுவரிசை)s

The code shows adding rolling statistics (mean, std, min, max over windows of 7 and 14 days) to lag feature (பண்பு — தகவல் நெடுவரிசை)s. These give the model information about recent trends and volatility that lag features alone do not capture.
...
- `code/time_series.py` -- lag feature (பண்பு — தகவல் நெடுவரிசை)s, walk-forward validation, AR model, stationarity checks
...
If your fancy ML model loses to the seasonal naive baseline, you have a bug. Most commonly: future leakage in feature (பண்பு — தகவல் நெடுவரிசை)s, wrong evaluation method, or the series is truly random and unpredictable.
...
2. **Difference first, model second.** If the series has a clear trend, difference it before creating lag feature (பண்பு — தகவல் நெடுவரிசை)s. Tree-based models can handle trends, but linear models cannot, and differencing never hurts.
...
5. **Beware of regime changes.** A model trained on pre-pandemic data will not predict post-pandemic behavior. Include indicators of known regime changes as feature (பண்பு — தகவல் நெடுவரிசை)s, or use a sliding window that forgets old data.
...
2. **Lag selection.** Compute ACF on a seasonal series (period=7). Which lags have the highest autocorrelation? Create lag feature (பண்பு — தகவல் நெடுவரிசை)s using only those lags (not consecutive lags). Does accuracy improve compared to using lags 1 through 7?

3. **Walk-forward vs random split.** Train a Ridge regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) on lag feature (பண்பு — தகவல் நெடுவரிசை)s. Evaluate with random 80/20 split and with walk-forward validation. How much does the random split overestimate performance?

4. **feature (பண்பு — தகவல் நெடுவரிசை) engineering.** Add rolling mean (window=7), rolling std (window=7), and day-of-week features to the lag features. Compare accuracy with and without these extras using walk-forward validation.
...
| Autocorrelation (ACF) | "How a series correlates with itself" | The correlation between a time series and a lagged copy of itself, as a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) of the lag |
...
| Lag feature (பண்பு — தகவல் நெடுவரிசை)s | "Past values as inputs" | Using y[t-1], y[t-2], ..., y[t-k] as features to predict y[t] |
...
- [statsmodels ARIMA docs](https://www.statsmodels.org/stable/generated/statsmodels.tsa.arima.model.ARIMA.html) -- ARIMA how to build with diagnostics
