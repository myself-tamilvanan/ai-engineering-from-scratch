...
**Type:** Build
**Language:** Python
**Prerequisites:** Phase 2, Lesson 12 (Hyperparameter (பாரமீட்டர் — கட்டுப்பாடு) Tuning)
**Time:** ~120 minutes
...
- Construct a ColumnTransformer that applies different preprocessing to numeric and categorical feature (பண்பு — தகவல் நெடுவரிசை)s
...
You have a notebook that loads data, fills missing values with the median, scales feature (பண்பு — தகவல் நெடுவரிசை)s, trains a model, and prints accuracy. It works. You ship it.

A month later, someone retrains the model and gets different results. The median was computed on the full dataset including test data (data leakage). The scaling parameter (பாரமீட்டர் — கட்டுப்பாடு)s were not saved, so inference uses different statistics. The feature (பண்பு — தகவல் நெடுவரிசை) engineering code was copy-pasted between training and serving, and the copies diverged. A categorical column gained a new value in production that the encoder has never seen.
...
flowchart LR
    ...
    B --> C[Scale Numeric feature (பண்பு — தகவல் நெடுவரிசை)s]
...
- The entire object is serialized and deployed as one artifact
...
full_pipeline = Pipeline([
    ...
    ("model", gradient (சாய்வு — மாற்றத்தின் दिशा)BoostingClassifier()),
])
```

The `handle_unknown="ignore"` in OneHotEncoder is critical for production. When a new category appears (a city the model has never seen), it produces a zero vector (வெக்டர் — எண் பட்டியல்) instead of crashing.

### Experiment Tracking

A pipeline makes training reproducible, but you also need to track what happened across experiments: which hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s were used, which dataset version, what the metrics were, which code was running.
...
Every run is recorded with parameter (பாரமீட்டர் — கட்டுப்பாடு)s, metrics, artifacts, and the full model. You can compare runs, reproduce any experiment, and deploy any model version.

**Weights & Biases (wandb)** provides the same function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)ality with a hosted dashboard:
...
4. **Config files:** All hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s in a config, not hardcoded
...
```mermaid
flowchart TD
    A[Jupyter Notebook] --> B[Extract function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s]
    B --> C[Build Pipeline object]
    C --> D[Add config file for hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s]
...
1. **Notebook exploration:** Quick experiments, visualizations, feature (பண்பு — தகவல் நெடுவரிசை) ideas
2. **Extract function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s:** Move preprocessing, feature engineering, evaluation into modules
3. **Build Pipeline:** Chain transformations into a sklearn Pipeline or custom class
4. **Config management:** Move all hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s into a YAML/JSON config
...
| feature (பண்பு — தகவல் நெடுவரிசை) engineering outside pipeline | Different transforms at train vs serve | Put all transforms in the Pipeline |
...
The code shows how cross-validation with a pipeline prevents data leakage: the scaler is fit separately on each fold's training data.
...
4. Add a custom transformer to the pipeline that creates polynomial feature (பண்பு — தகவல் நெடுவரிசை)s (degree 2) for the two most important numeric columns. Where should it go in the pipeline?

5. Set up MLflow tracking for the pipeline. Run 5 experiments with different hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s. Use the MLflow UI (`mlflow ui`) to compare runs and pick the best model.
...
| Experiment tracking | "Logging your runs" | Recording parameter (பாரமீட்டர் — கட்டுப்பாடு)s, metrics, artifacts, and code versions for every training run |
