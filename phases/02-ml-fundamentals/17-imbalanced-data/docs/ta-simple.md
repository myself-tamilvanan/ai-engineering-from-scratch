...
- Build a complete imbalanced data pipeline that combines SMOTE, class weights, and threshold optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)
...
This happens everywhere real classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) matters. Disease diagnosis: 1% positive rate. Network intrusion: 0.01% attacks. Manufacturing defects: 0.5% defective. Spam filtering: 20% spam. Churn prediction: 5% churners. The more consequential the minority class, the rarer it tends to be.
...
SMOTE creates new synthetic minority samples that are plausible but not copies. The algorithm (அல்கோரிதம் — வழிமுறை):
...
This interpolates between real minority points, creating samples in the same region of feature (பண்பு — தகவல் நெடுவரிசை) space without just copying existing data.
...
In logistic regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்), this modifies the loss function (சேதத் சூத்திரம் — எவ்வளவு தவறு என்பதை அளக்கும் சூத்திரம்) (சார்பு — உள்ளீட்டுக்கு வெளியீடு):
...
Generalization of class weights. Instead of uniform costs, assign specific misclassification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) costs:
...
flowchart TD
    ...
    M -->|No| N[Raise threshold or add feature (பண்பு — தகவல் நெடுவரிசை)s]
    M -->|Yes| O[Ship it]
```
...
### Step 4: Logistic regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) with class weights
...
def logistic_regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)_weighted(X, y, weights, lr=0.01, epoch (யுக் — ஒரு முழு சுழற்சி)s=200):
    ...
    for _ in range(epoch (யுக் — ஒரு முழு சுழற்சி)s):
        ...
        gradient (சாய்வு — மாற்றத்தின் दिशा)_w = (X.T @ weighted_error) / n_samples
...
### Step 6: Evaluation function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s

```python
def confusion_matrix (மேட்ரிக்ஸ் — எண் மேஜை)_values(y_true, y_pred):
    tp = np.sum((y_pred == 1) & (y_true == 1))
...
def compute_metrics(y_true, y_pred):
    tp, tn, fp, fn = confusion_matrix (மேட்ரிக்ஸ் — எண் மேஜை)_values(y_true, y_pred)
...
w_base, b_base = logistic_regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)_weighted(
    X_train, y_train, np.ones(len(y_train)), lr=0.1, epoch (யுக் — ஒரு முழு சுழற்சி)s=300
)
...
w_over, b_over = logistic_regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)_weighted(
    X_over, y_over, np.ones(len(y_over)), lr=0.1, epoch (யுக் — ஒரு முழு சுழற்சி)s=300
)
...
w_sm, b_sm = logistic_regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)_weighted(
    X_smote, y_smote, np.ones(len(y_smote)), lr=0.1, epoch (யுக் — ஒரு முழு சுழற்சி)s=300
)
preds_smote = (sigmoid(X_test @ w_sm + b_sm) >= 0.5).astype(int)

# Class weights
sample_weights = compute_class_weights(y_train)
w_cw, b_cw = logistic_regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)_weighted(
    X_train, y_train, sample_weights, lr=0.1, epoch (யுக் — ஒரு முழு சுழற்சி)s=300
)
...
from sklearn.metrics import classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)_report, f1_score
...
model_weighted = Logisticregression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)(class_weight="balanced")
model_weighted.fit(X_train, y_train)
print(classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)_report(y_test, model_weighted.predict(X_test)))

smote = SMOTE(random_state=42)
X_resampled, y_resampled = smote.fit_resample(X_train, y_train)
model_smote = Logisticregression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)()
model_smote.fit(X_resampled, y_resampled)
print(classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)_report(y_test, model_smote.predict(X_test)))

pipeline = Pipeline([
    ("smote", SMOTE()),
    ("model", Logisticregression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)(class_weight="balanced")),
])
pipeline.fit(X_train, y_train)
print(classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)_report(y_test, pipeline.predict(X_test)))
```
...
- `outputs/skill-imbalanced-data.md` -- a decision checklist for handling imbalanced classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) problems

## Exercises

1. **Borderline-SMOTE**: modify the SMOTE how to build to only generate synthetic samples for minority points that are near the decision boundary (those whose k-nearest neighbors include majority class samples). Compare results with standard SMOTE on a dataset where classes overlap.

2. **Cost matrix (மேட்ரிக்ஸ் — எண் மேஜை) optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)**: implement cost-sensitive learning where the cost matrix is a parameter (பாரமீட்டர் — கட்டுப்பாடு). Create a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) that takes a cost matrix and returns optimal predictions that minimize expected cost. Test with different cost ratios (1:10, 1:100, 1:1000) and plot how the precision-recall choice changes.

3. **Threshold calibration**: implement Platt scaling (fit a logistic regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) on the model's raw outputs to produce calibrated probabilities). Compare the precision-recall curve before and after calibration. Show that calibration does not change the ranking (AUC stays the same) but makes the probabilities more meaningful.
...
| Class weights | "Making errors on rare classes more expensive" | Multiplying the loss function (சேதத் சூத்திரம் — எவ்வளவு தவறு என்பதை அளக்கும் சூத்திரம்) (சார்பு — உள்ளீட்டுக்கு வெளியீடு) by class-specific weights so the model penalizes minority misclassification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) more heavily |
...
| Precision-recall choice | "You cannot have both" | Lowering the threshold catches more positives (higher recall) but also flags more false positives (lower precision), and vice versa |
...
- [Learning from Imbalanced Data (He & Garcia, 2009)](https://ieeexplore.ieee.org/document/5128907) -- comprehensive survey covering sampling, cost-sensitive, and algorithm (அல்கோரிதம் — வழிமுறை)ic approaches
