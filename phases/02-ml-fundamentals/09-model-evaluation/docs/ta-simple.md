...
- Compute precision, recall, F1, AUC-ROC, and regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) metrics (MSE, RMSE, MAE, R-squared) from scratch
...
flowchart LR
    ...
    B --> E[Fit Model]
    E --> C
    C --> F[Tune Hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s]
    F --> E
    F --> G[Final Model]
    G --> D
...
- **Validation set**: used to tune hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s and select between models. The model never trains on this data, but your decisions are influenced by it.
...
### classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) Metrics

**Confusion matrix (மேட்ரிக்ஸ் — எண் மேஜை)**: the foundation. For binary classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்):
...
From this matrix (மேட்ரிக்ஸ் — எண் மேஜை), all other metrics follow:
...
- **R-squared** = 1 - SS_res / SS_tot, where SS_res = sum((y_true - y_pred)^2) and SS_tot = sum((y_true - y_mean)^2). Fraction of variance explained by the model. R^2 = 1.0 is perfect. R^2 = 0.0 means the model is no better than always predicting the mean. R^2 is negative if the model is worse than the mean.

### Learning Curves

Plot training and validation scores as a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) of training set size:
...
Plot training and validation scores as a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) of a hyperparameter (பாரமீட்டர் — கட்டுப்பாடு):
...
The optimal hyperparameter (பாரமீட்டர் — கட்டுப்பாடு) value is where the validation score peaks.

### Common Evaluation Mistakes

**Data leakage**: information from the test set leaks into training. Examples: fitting a scaler on the full dataset before splitting, including future data in time series prediction, using a feature (பண்பு — தகவல் நெடுவரிசை) that is derived from the target. Always split first, then preprocess.
...
### Step 3: Confusion matrix (மேட்ரிக்ஸ் — எண் மேஜை) and classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) metrics

```python
def confusion_matrix (மேட்ரிக்ஸ் — எண் மேஜை)(y_true, y_pred):
...
def accuracy(y_true, y_pred):
    tp, tn, fp, fn = confusion_matrix (மேட்ரிக்ஸ் — எண் மேஜை)(y_true, y_pred)
    total = tp + tn + fp + fn
...
### Step 4: regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) metrics
...
class SimpleLogistic:
    def __init__(self, lr=0.1, epoch (யுக் — ஒரு முழு சுழற்சி)s=100):
        self.lr = lr
        self.epochs = epochs
        self.weights = None
        self.bias = 0.0
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.epoch (யுக் — ஒரு முழு சுழற்சி)s):
...
class SimpleLinearregression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்):
    def __init__(self, lr=0.001, epoch (யுக் — ஒரு முழு சுழற்சி)s=200):
        self.lr = lr
        self.epochs = epochs
        self.weights = None
        self.bias = 0.0
...
def make_classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)_data(n=300, seed=42):
    random.seed(seed)
    X = []
    y = []
    for _ in range(n):
...
def make_regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)_data(n=200, seed=42):
    random.seed(seed)
    X = []
    y = []
    for _ in range(n):
...
if __name__ == "__main__":
    X_clf, y_clf = make_classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)_data(300)
    ...
    model = SimpleLogistic(lr=0.1, epoch (யுக் — ஒரு முழு சுழற்சி)s=200)
    model.fit(X_train, y_train)

    print("\n=== classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) Metrics ===")
    y_pred = [model.predict(x) for x in X_test]
    tp, tn, fp, fn = confusion_matrix (மேட்ரிக்ஸ் — எண் மேஜை)(y_test, y_pred)
    ...
    cv_scores = cross_validate(
        X_clf, y_clf,
        model_fn=lambda: SimpleLogistic(lr=0.1, epoch (யுக் — ஒரு முழு சுழற்சி)s=200),
        k=5,
        metric_fn=accuracy,
    )
    ...
    model_imb = SimpleLogistic(lr=0.5, epoch (யுக் — ஒரு முழு சுழற்சி)s=500)
    ...
    reg_model = SimpleLinearregression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)(lr=0.01, epoch (யுக் — ஒரு முழு சுழற்சி)s=500)
    ...
    model_a_scores = cross_validate(
        X_clf, y_clf,
        model_fn=lambda: SimpleLogistic(lr=0.1, epoch (யுக் — ஒரு முழு சுழற்சி)s=100),
...
from sklearn.metrics import (
    ...
    roc_auc_score, confusion_matrix (மேட்ரிக்ஸ் — எண் மேஜை), mean_squared_error, r2_score,
)
from sklearn.linear_model import Logisticregression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)

model = Logisticregression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)()
...
- `outputs/skill-evaluation.md` - a skill covering evaluation strategy for classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) and regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) models
...
2. Build a nested cross-validation loop: the outer loop evaluates model performance, the inner loop tunes hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s. Use it to compare two models fairly without leaking validation data into the evaluation.
...
| Cross-validation | "Testing on different subsets" | Systematically rotating which portion of data helps for validation, averaging results across all rotations |
...
- [scikit-learn Model Selection Guide](https://scikit-learn.org/stable/model_selection.html) - comprehensive reference on cross-validation, metrics, and hyperparameter (பாரமீட்டர் — கட்டுப்பாடு) tuning
- [Beyond Accuracy: Precision and Recall (Google ML Crash Course)](https://developers.google.com/machine-learning/crash-course/classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)/precision-and-recall) - clear explanation with interactive examples
