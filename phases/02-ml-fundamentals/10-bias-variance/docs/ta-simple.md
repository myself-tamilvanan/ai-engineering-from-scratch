...
# Bias-Variance choice

> Every model error comes from one of three sources: bias, variance, or noise. You can only control the first two.

**Type:** Learn
**Language:** Python
**Prerequisites:** Phase 2, Lessons 01-09 (ML basics, regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்), classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்), evaluation)
**Time:** ~75 minutes
...
- Implement experiments that visualize the bias-variance choice across models of increasing complexity

## The Problem

You trained a model. It has some error on test data. Where does that error come from?

If your model is too simple (linear regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) on a curved dataset), it will consistently miss the true pattern. That is bias. If your model is too complex (degree-20 polynomial on 15 data points), it will fit the training data perfectly but give wildly different predictions on new data. That is variance.

You cannot minimize both at the same time for a fixed model capacity. Push bias down and variance goes up. Push variance down and bias goes up. Understanding this choice is the single most useful diagnostic skill in machine learning. It tells you whether to make your model more complex or less complex, whether to get more data or engineer better feature (பண்பு — தகவல் நெடுவரிசை)s, whether to regularize more or less.
...
- `f(x)` is the true function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)
...
- **L2 (Ridge):** Shrinks all weights toward zero. Keeps all feature (பண்பு — தகவல் நெடுவரிசை)s but reduces their influence.
...
The regularization strength (lambda, dropout rate, number of epoch (யுக் — ஒரு முழு சுழற்சி)s) directly controls where you sit on the bias-variance curve. More regularization means more bias, less variance.

### Double Descent: The Modern Perspective

Classical theory says: after the sweet spot, more complexity always hurts. But research since 2019 has shown something unexpected. If you keep increasing model capacity far past the interpolation threshold (where the model has enough parameter (பாரமீட்டர் — கட்டுப்பாடு)s to perfectly fit training data), test error can decrease again.
...
This "double descent" phenomenon explains why massively overparameter (பாரமீட்டர் — கட்டுப்பாடு)ized neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)s (with far more parameters than training examples) still generalize well. The classical bias-variance choice is not wrong, but it is incomplete for the modern regime.

Key observations about double descent:
- It happens in linear models, decision trees, and neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)s
- More data can actually hurt in the interpolation region (sample-wise double descent)
- More training epoch (யுக் — ஒரு முழு சுழற்சி)s can cause it too (epoch-wise double descent)
- Regularization smooths out the peak but does not eliminate it

Why does this happen? At the interpolation threshold, the model has just enough capacity to fit all training points. It is forced into a very specific solution that threads through every point, and small perturbations in the data cause large changes in the fit. This is where variance peaks. Past the threshold, the model has many possible solutions that fit the data perfectly. The learning algorithm (அல்கோரிதம் — வழிமுறை) (e.g., gradient (சாய்வு — மாற்றத்தின் दिशा) descent with implicit regularization) tends to pick the simplest one among them. This implicit bias toward simple solutions is why overparameter (பாரமீட்டர் — கட்டுப்பாடு)ized models generalize.

| Regime | parameter (பாரமீட்டர் — கட்டுப்பாடு)s vs Samples | Behavior |
|--------|----------------------|----------|
| Underparameterized | p << n | Classical choice applies |
| Interpolation threshold | p ~ n | Variance peaks, test error spikes |
| Overparameterized | p >> n | Implicit regularization kicks in, test error drops |

For practical purposes: if you are using neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)s or large tree ensembles, do not stop at the interpolation threshold. Either stay well below it (with explicit regularization) or go well past it. The worst place to be is right at the threshold.
...
flowchart TD
    ...
    E --> H[More feature (பண்பு — தகவல் நெடுவரிசை)s / Complex model / Less regularization]
    F --> I[Deploy]
```

| Symptom | Diagnosis | Fix |
|---------|-----------|-----|
| High train error, high test error | Bias | More feature (பண்பு — தகவல் நெடுவரிசை)s, complex model, less regularization |
...
- Add polynomial or interaction feature (பண்பு — தகவல் நெடுவரிசை)s
...
- feature (பண்பு — தகவல் நெடுவரிசை) selection (remove noisy features)
...
Why it works mathematically: if you average N independent predictions, each with variance sigma^2, the variance of the average is sigma^2 / N. The models are not truly independent (they all see similar data), so the reduction is less than 1/N, but it is still large.

**Boosting** reduces bias by building models sequentially, where each new model focuses on the errors of the ensemble so far. gradient (சாய்வு — மாற்றத்தின் दिशा) boosting and AdaBoost are the main examples. Boosting can overfit if you add too many models, so you need early stopping or regularization.
...
Learning curves plot training and validation error as a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) of training set size. They are the most practical diagnostic tool you have. Unlike a single train/test comparison, learning curves show you the trajectory of your model and tell you whether more data will help.
...
|----------|---------------|-----------------|-----|---------------|------------|
| High bias | High | High | Small | Model cannot capture the pattern | More feature (பண்பு — தகவல் நெடுவரிசை)s, complex model, less regularization |
...
**Approach 1: Vary training set size, fixed model.** Hold the model and hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s constant. Train on increasingly large subsets of the training data. Measure training error and validation error at each size. This is the standard learning curve.

**Approach 2: Vary model complexity, fixed data.** Hold the data constant. Sweep a complexity parameter (பாரமீட்டர் — கட்டுப்பாடு) (polynomial degree, tree depth, number of layers). Measure training error and validation error at each complexity. This is a validation curve and shows the bias-variance choice directly.
...
### Step 1: Generate Synthetic Data from a Known function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)

We use `f(x) = sin(1.5x) + 0.5x` with Gaussian noise. Knowing the true function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) lets us compute exact bias and variance.

```python
def true_function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)(x):
    return np.sin(1.5 * x) + 0.5 * x

def generate_data(n_samples=30, noise_std=0.5, x_range=(-3, 3), seed=None):
    rng = np.random.RandomState(seed)
    x = rng.uniform(x_range[0], x_range[1], n_samples)
    y = true_function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)(x) + rng.normal(0, noise_std, n_samples)
    return x, y
```
...
The code also includes `demo_regularization_sweep()`, which fixes a high-degree polynomial (degree 15) and sweeps Ridge regularization strength from 0.001 to 100. This shows the bias-variance choice from a different angle: instead of varying model complexity, we vary the constraint strength.
...
At low alpha, the degree-15 polynomial is nearly unconstrained. Variance dominates because the model chases noise in each bootstrap sample. At high alpha, the penalty is so strong that the model effectively becomes a near-constant function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). Bias dominates. The optimal alpha sits between these extremes.

This is the same U-curve from varying polynomial degree, but controlled by a continuous knob instead of a discrete one. In practice, regularization is the preferred way to control the choice because it allows fine-grained control without changing the feature (பண்பு — தகவல் நெடுவரிசை) set.
...
from sklearn.preprocessing import Polynomialfeature (பண்பு — தகவல் நெடுவரிசை)s
...
for d in degrees:
    pipe = make_pipeline(Polynomialfeature (பண்பு — தகவல் நெடுவரிசை)s(d), Ridge(alpha=0.01))
...
This gives you the bias-variance choice curve directly. Where the validation score is worst relative to train score, variance dominates. Where both are bad, bias dominates.
...
pipe = make_pipeline(Polynomialfeature (பண்பு — தகவல் நெடுவரிசை)s(5), Ridge(alpha=0.01))
...
for alpha in alphas:
    pipe = make_pipeline(Polynomialfeature (பண்பு — தகவல் நெடுவரிசை)s(10), Ridge(alpha=alpha))
    scores = cross_val_score(pipe, X, y, cv=5, scoring="neg_mean_squared_error")
    print(f"alpha={alpha:>7.3f}  MSE={-scores.mean():.4f} +/- {scores.std():.4f}")
```

This sweeps regularization strength for a fixed model complexity. You will see the same bias-variance choice: low alpha means high variance, high alpha means high bias.
...
4. Generate a validation curve sweeping your main complexity parameter (பாரமீட்டர் — கட்டுப்பாடு). Find the sweet spot.
...
3. Add L2 regularization (Ridge regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)) to the experiment. For a fixed high-degree polynomial (degree 15), sweep lambda from 0 to 100. Plot bias^2 and variance as function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s of lambda.

4. Modify the true function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) from a polynomial to `sin(x)`. How does the bias-variance decomposition change? Is there still a clear optimal degree?
...
| Double descent | "More parameter (பாரமீட்டர் — கட்டுப்பாடு)s can help" | Test error decreases again when model capacity far exceeds the interpolation threshold. |
| Model complexity | "How flexible the model is" | The capacity of a model to fit arbitrary patterns. Controlled by architecture, feature (பண்பு — தகவல் நெடுவரிசை)s, or regularization. |
...
- [Nakkiran et al., Deep Double Descent (2019)](https://arxiv.org/abs/1912.02292) -- epoch (யுக் — ஒரு முழு சுழற்சி)-wise and sample-wise double descent
- [Scott Fortmann-Roe: Understanding the Bias-Variance choice](http://scott.fortmann-roe.com/docs/BiasVariance.html) -- clear visual explanation
