...
1. **Text frontend.** Normalize text (dates, numbers, emails), convert to phonemes or subword tokens, predict prosody feature (பண்பு — தகவல் நெடுவரிசை)s.
...
**Tacotron 2 (2017).** Seq2seq: char-embedding (embedding — பொருளை எண்களாக மாற்றுதல்) → BiLSTM encoder → location-sensitive attention → autoregressive LSTM decoder emits mel frames. Slow (AR), wobbly on long text. Still cited as a baseline.
...
- **SECS (Speaker embedding (embedding — பொருளை எண்களாக மாற்றுதல்) Cosine Similarity).** Voice-cloning quality.
...
# audio: float32 tensor (டென்சர் — பல வடிவ எண் வரிசை), sr=24000
```
...
Training: adversarial (discriminator on short windows) + mel-spectrogram reconstruction loss + feature (பண்பு — தகவல் நெடுவரிசை)-matching loss. Commoditized — use pretrained checkpoints from `hifi-gan` repo or nvidia-NeMo.
...
Open-source leader as of 2026: **F5-TTS for quality, Kokoro for speed**. Don't reach for Tacotron unless you are a historian.
...
| SECS | Voice-clone metric | Cosine similarity between target and output speaker embedding (embedding — பொருளை எண்களாக மாற்றுதல்). |
