...
> batch (பத்தி — தொகுப்பு) pipelines process a file. Real-time pipelines process the next 20 milliseconds before the next 20 arrive. Every conversational AI, broadcast studio, and telephony bot lives and dies by this latency budget.
...
is_speech = vad(torch.tensor (டென்சர் — பல வடிவ எண் வரிசை)(frame), 16000).item() > 0.5
```
