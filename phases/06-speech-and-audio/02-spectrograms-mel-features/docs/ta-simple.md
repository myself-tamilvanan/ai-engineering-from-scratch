...
# Spectrograms, Mel Scale & Audio feature (பண்பு — தகவல் நெடுவரிசை)s
...
Mel spectrograms push further. Humans perceive pitch logarithmically: 100 Hz vs 200 Hz sounds "the same distance apart" as 1000 Hz vs 2000 Hz. The mel scale warps the frequency axis to match. A mel-scaled spectrogram is the single most important feature (பண்பு — தகவல் நெடுவரிசை) in speech ML from 2010 through 2026.

## The Concept

![Waveform to STFT to mel spectrogram to MFCC ladder](../assets/mel-feature (பண்பு — தகவல் நெடுவரிசை)s.svg)

**STFT (Short-Time Fourier Transform).** Slice the waveform into overlapping frames (typical: 25 ms window, 10 ms hop = 400 samples / 160 samples at 16 kHz). Multiply each frame by a window function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) (Hann is the default. Hamming slightly different choice). FFT each frame. Stack the magnitude spectra into a matrix (மேட்ரிக்ஸ் — எண் மேஜை) of shape `(n_frames, n_freq_bins)`. That is your spectrogram.
...
**Mel filterbank.** A set of triangular filters spaced equally on the mel scale. Each filter is a weighted sum of adjacent FFT bins. Multiplying the STFT magnitude by the filterbank matrix (மேட்ரிக்ஸ் — எண் மேஜை) gives the mel spectrogram in one matmul.
...
**MFCCs.** Take the log-mel spectrogram, apply a DCT (type II), keep the first 13 coefficients. Decorrelates the feature (பண்பு — தகவல் நெடுவரிசை)s and compresses further. Dominant feature until about 2015 when CNNs/Transformers on raw log-mels caught up. Still used in speaker recognition (x-vector (வெக்டர் — எண் பட்டியல்)s, ECAPA).
...
Production uses `torch.stft` or `librosa.stft` (FFT-backed, vector (வெக்டர் — எண் பட்டியல்)ized). The loop here is pedagogical. it runs on short clips in `code/main.py`.
...
80 mels covering 0–8 kHz with `n_fft=400` gives an `(80, 201)` matrix (மேட்ரிக்ஸ் — எண் மேஜை). Multiply the `(n_frames, 201)` STFT magnitude by the transpose to get `(n_frames, 80)` mel spectrogram.
...
Apply DCT to each log-mel frame, keep the first 13 coefficients. That is your MFCC matrix (மேட்ரிக்ஸ் — எண் மேஜை). The first coefficient is usually dropped (it encodes overall energy).

## Use It

The 2026 stack:

| Task | feature (பண்பு — தகவல் நெடுவரிசை)s |
|------|----------|
...
| Audio classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) (AST, PANNs, BEATs) | 128 log-mels, 10 ms hop |
| Speaker embedding (embedding — பொருளை எண்களாக மாற்றுதல்) (ECAPA-TDNN, WavLM) | 80 log-mels or raw-waveform SSL |
...
- **Mel count mismatch.** Training with 80 mels, inference with 128 mels. Silent failure. Log the feature (பண்பு — தகவல் நெடுவரிசை) shape at both ends.
...
Save as `outputs/skill-feature (பண்பு — தகவல் நெடுவரிசை)-extractor.md`. The skill picks feature type, mel count, frame/hop, and normalization for a given model target.
...
| STFT | Spectrogram generator | Framed + windowed FFT. yields time × frequency matrix (மேட்ரிக்ஸ் — எண் மேஜை). |
...
| MFCC | Old-school feature (பண்பு — தகவல் நெடுவரிசை) | DCT of log-mel. 13 coeffs, decorrelated. |
...
- [OpenAI — Whisper source, log_mel_spectrogram](https://github.com/openai/whisper/blob/main/whisper/audio.py) — read the reference how to build.
- [librosa feature (பண்பு — தகவல் நெடுவரிசை) extraction docs](https://librosa.org/doc/main/feature.html) — reference for `mfcc`, `melspectrogram`, and hop/window.
