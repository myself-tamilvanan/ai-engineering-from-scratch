...
Every voice agent built from Lessons 11 + 12 has a basic latency floor around 300-500 ms: VAD fires, STT processes, LLM reasons, TTS generates. Each stage has its own minimum latency. You can tune and parallelize, but the pipeline shape caps you.
...
**The transformer.** A 7B-parameter (பாரமீட்டர் — கட்டுப்பாடு) Temporal Transformer processes both streams and a text "inner monologue" stream. At each 80 ms step, it:
...
Same architecture, trained on translation pairs. Source audio in, target-language audio out, continuously. Hibiki-Zero (Feb 2026) eliminates the need for word-level aligned training data — uses sentence-level data + GRPO reinforcement learning for latency optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்).

Four language pairs supported initially. is adapted to a new language with ≈1000 hours.
...
Sesame CSM (2025) uses a similar idea — a Llama-3 backbone with a Mimi codec head. But CSM is single-directional (takes context + text, produces speech) quite than full-duplex. It's the best "voice presence" TTS on the market. not quite the same as Moshi's full-duplex capability.
