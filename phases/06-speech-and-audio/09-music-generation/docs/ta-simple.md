...
3. **Conditional / controllable.** Extend an existing clip, regenerate a bridge, swap genre, stem-separate, or inpaint. Udio's inpainting + stem separation is the 2026 feature (பண்பு — தகவல் நெடுவரிசை) to match.
...
Meta's **MusicGen** (2023, MIT) and many derivatives: condition on text/melody embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s, autoregressively predict EnCodec tokens (32 kHz, 4 codebooks), decode with EnCodec. 300M - 3.3B params. Strong baseline. struggles past 30 seconds.
...
- **FAD (Fréchet Audio Distance).** embedding (embedding — பொருளை எண்களாக மாற்றுதல்)-level distance between generated vs real audio distribution using VGGish or PANNs feature (பண்பு — தகவல் நெடுவரிசை)s. Lower is better. MusicGen small: 4.5 FAD on MusicCaps. SOTA ~3.0.
...
| ACE-Step XL (Apr 2026) | 4B | &gt. 2 min | yes | Apache-2.0 |
| YuE | 7B | &gt. 2 min | yes, multilingual | Apache-2.0 |
...
Computes VGGish-embedding (embedding — பொருளை எண்களாக மாற்றுதல்) distance. Useful for genre-level regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) tests. not a substitute for human listeners.
...
|------|-----------------|-----------------------|
| FAD | Audio FID | Fréchet distance between embedding (embedding — பொருளை எண்களாக மாற்றுதல்) distributions of real vs generated. |
| Chromagram | Melody as pitches | 12-dim per-frame vector (வெக்டர் — எண் பட்டியல்). input to melody conditioning. |
