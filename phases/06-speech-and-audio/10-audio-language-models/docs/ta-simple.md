...
- **Long-audio retrieval.** "Where in this 90-minute lecture did the instructor explain gradient (சாய்வு — மாற்றத்தின் दिशा) descent?"
...
2. **Projector.** Linear or MLP bridging audio-encoder feature (பண்பு — தகவல் நெடுவரிசை)s into the LLM's token embedding (embedding — பொருளை எண்களாக மாற்றுதல்) space.
...
- Real-time streaming reasoning (most are offline batch (பத்தி — தொகுப்பு) inference).
...
inputs = processor.apply_chat_template(messages, tokenize=True, return_tensor (டென்சர் — பல வடிவ எண் வரிசை)s="pt")
...
class AudioProjector(nn.Module):
    ...
    def forward(self, audio_feature (பண்பு — தகவல் நெடுவரிசை)s):
...
1. **Easy.** Run `code/main.py` to see a toy projector pattern + fake LALM routing of (audio-embedding (embedding — பொருளை எண்களாக மாற்றுதல்), text-tokens) → output tokens.
...
| Projector | Adapter | Small MLP mapping audio feature (பண்பு — தகவல் நெடுவரிசை)s into LLM embedding (embedding — பொருளை எண்களாக மாற்றுதல்) space. |
