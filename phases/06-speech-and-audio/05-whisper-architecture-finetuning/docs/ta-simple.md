...
**Log-mel normalization.** `(log_mel - mean) / std` where the stats come from Whisper's own training corpus. You *must* use Whisper's preprocessing (`whisper.audio.log_mel_spectrogram`), not `librosa.feature (பண்பு — தகவல் நெடுவரிசை).melspectrogram`.
...
3. parameter (பாரமீட்டர் — கட்டுப்பாடு)-efficient: LoRA on `q_proj`, `k_proj`, `v_proj` of attention layers reduces GPU memory 4× with <0.3 WER cost.
...
segments = model.transcribe("1hour.mp3", batch (பத்தி — தொகுப்பு)_size=16, chunk_size=30)
```
...
# model.print_trainable_parameter (பாரமீட்டர் — கட்டுப்பாடு)s()  -> ~3M trainable / 809M total
```
...
with torch.inference_mode():
    out = model.generate(
        input_feature (பண்பு — தகவல் நெடுவரிசை)s=features,
...
3. **Hard.** Using HF `datasets`, pick a language Whisper struggles with (e.g., Urdu), fine-tune Medium with LoRA for 2 epoch (யுக் — ஒரு முழு சுழற்சி)s on 2 hours, and report WER delta.
...
| Turbo | The fast variant | 4-decoder layers, 8× faster, <1% WER regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்). |
