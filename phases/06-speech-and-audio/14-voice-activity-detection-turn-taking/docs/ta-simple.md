...
**Tier 2: Silero VAD** (2020-2026, MIT). 1M parameter (பாரமீட்டர் — கட்டுப்பாடு)s. Trained on 6000+ languages. Runs in ~1 ms per 30 ms chunk on a single CPU thread. 87.7% TPR at 5% FPR. The open-source default.
...
### Key parameter (பாரமீட்டர் — கட்டுப்பாடு)s and their defaults

- **Threshold.** Silero outputs a probability. classify speech at &gt. 0.5 (default) or &gt. 0.3 (sensitive). Lower threshold = fewer first-word clips, more false positives.
...
vad = load_silero_vad()
audio = torch.tensor (டென்சர் — பல வடிவ எண் வரிசை)(waveform_16k, dtype=torch.float32)
...
3. **Hard.** Build a mini turn-detector: Silero VAD + a 3-layer MLP on the last 10 words' embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s (use sentence-transformers). Train on a hand-labeled turn-end dataset. Beat Silero-only by 10% F1.
