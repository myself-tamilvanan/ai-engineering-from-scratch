...
> GPT predicts the next word. BERT predicts a missing word. One sentence of difference — and half a decade of everything embedding (embedding — பொருளை எண்களாக மாற்றுதல்)-shaped.
...
In 2018 every NLP task — sentiment, NER, QA, entailment — trained its own model from scratch on its own labeled data. There was no pre-trained "understand English" checkpoint you could fine-tune. ELMo (2018) showed you could pre-train contextual embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s with a bidirectional LSTM. it helped but did not generalize.

BERT (Devlin et al. 2018) asked: what if we took a transformer encoder, trained it on every sentence on the internet, and forced it to predict missing words from context on both sides? Then you fine-tune one head on your downstream task. parameter (பாரமீட்டர் — கட்டுப்பாடு) speed was a revelation.
...
In 2026 encoder-only models are still the right tool for classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்), retrieval, and structured extraction — they run 5–10× faster per token than decoders and their embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s are the backbone of every modern retrieval stack. ModernBERT (Dec 2024) pushed the architecture to 8K context with Flash Attention + RoPE + GeGLU.
...
|------|---------------------------|
| Retrieval / semantic search embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s | Bidirectional context = better embedding quality per token |
| classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) (sentiment, intent, toxicity) | One forward pass. no generation extra work |
...
See `code/main.py`. The function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) `create_mlm_batch (பத்தி — தொகுப்பு)` takes a list of token IDs, a vocab size, and a mask probability. Returns input IDs (with masks applied) and labels (only at masked positions, -100 elsewhere — PyTorch's ignore index convention).

```python
def create_mlm_batch (பத்தி — தொகுப்பு)(tokens, vocab_size, mask_prob=0.15, rng=None):
...
Train a 2-layer encoder + MLM head on a vocabulary of 20 words, 200 sentences. No gradient (சாய்வு — மாற்றத்தின் दिशा) — we do forward-pass sanity checks. Full training needs PyTorch.
...
Replace the MLM head with a classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) head on a toy sentiment dataset. Only the head trains. the encoder is frozen. This is the pattern every BERT application follows.
...
inputs = tok(text, return_tensor (டென்சர் — பல வடிவ எண் வரிசை)s="pt")
out = model(**inputs).last_hidden_state   # (1, N, 768)
```

**embedding (embedding — பொருளை எண்களாக மாற்றுதல்) models are fine-tuned BERT.** `sentence-transformers` models like `all-MiniLM-L6-v2` are BERTs trained with contrastive loss. The encoder is the same. The loss changed.

**Cross-encoder rerankers are also fine-tuned BERT.** Pair-classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) on `[CLS] query [SEP] doc [SEP]`. The bidirectional attention between query and doc is exactly what gives cross-encoders their quality edge over biencoders.
...
See `outputs/skill-bert-finetuner.md`. The skill scopes a BERT fine-tune (backbone choice, head spec, data, eval, stopping) for a new classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) or extraction task.
...
| `[CLS]` | "The pooler token" | A special token prepended to every sequence. its final embedding (embedding — பொருளை எண்களாக மாற்றுதல்) helps as the sentence-level representation. |
...
- [Clark et al. (2020). ELECTRA: Pre-training Text Encoders as Discriminators quite Than Generators](https://arxiv.org/abs/2003.10555) — replaced-token detection beats MLM at matched compute.
