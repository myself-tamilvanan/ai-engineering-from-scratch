...
> The 2020 Kaplan paper said: bigger model, lower loss. The 2022 Hoffmann paper said: you were under-training. Compute goes into two buckets — parameter (பாரமீட்டர் — கட்டுப்பாடு)s and tokens — and the split is not obvious.
...
1. **How many parameter (பாரமீட்டர் — கட்டுப்பாடு)s (N)?** Bigger model, higher capacity.
...
Before 2022, the answer was "push N hard." GPT-3 (2020) was 175B parameter (பாரமீட்டர் — கட்டுப்பாடு)s trained on ~300B tokens. A ratio of about 1.7 tokens per parameter. The Kaplan scaling laws backed this up.

Hoffmann et al. (2022), training a small family of models called Chinchilla, found something different: optimal ratio is closer to **20 tokens per parameter (பாரமீட்டர் — கட்டுப்பாடு)**. GPT-3 was 10× undertrained. Chinchilla (70B params, 1.4T tokens) beat GPT-3 (175B, 300B tokens) on every benchmark at 2.5× less inference cost.

2026 is Chinchilla's world — with one important twist. Llama 3 8B was trained on 15 trillion tokens, a ratio of 1,875 tokens per parameter (பாரமீட்டர் — கட்டுப்பாடு). Ninety-four times past Chinchilla-optimal. Inference cost matters more than training cost for models that will be used at scale, so over-training (past Chinchilla) for a smaller deployable footprint is the 2026 default.
...
- `N` = parameter (பாரமீட்டர் — கட்டுப்பாடு)s (non-embedding (embedding — பொருளை எண்களாக மாற்றுதல்)).
- `D` = training tokens.
...
Compute-optimal: 20 tokens per parameter (பாரமீட்டர் — கட்டுப்பாடு).
...
DeepMind's 2024 paper ("Over-training is the new optimal") formalized this. For inference-dominated workloads, the right ratio is closer to 100–500 tokens per parameter (பாரமீட்டர் — கட்டுப்பாடு) depending on serving volume.
...
2. **Medium.** Implement the Hoffmann loss-as-function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)-of-compute curve. Plot loss vs `log10(C)` for the compute-optimal frontier. Identify when the law predicts we'd need `>10^28` FLOPs for the next 0.1 reduction in cross-entropy.
...
|------|-----------------|-----------------------|
| parameter (பாரமீட்டர் — கட்டுப்பாடு)s (N) | "Model size" | Non-embedding (embedding — பொருளை எண்களாக மாற்றுதல்) weight count. determines capacity. |
...
| Effective compute | "Training-speed multiplier" | Better data / optimizer / architecture multiplies how far a FLOP goes. |
...
- [Jordan et al. (2024). Muon: An optimizer for hidden layers in neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)s](https://kellerjordan.github.io/posts/muon/) — 2× compute multiplier.
