...
A single attention layer is a feature (பண்பு — தகவல் நெடுவரிசை) extractor, not a model. One matmul per layer is not enough capacity for language. You need depth — and depth breaks without the right plumbing.
...
1. **embedding (embedding — பொருளை எண்களாக மாற்றுதல்) + positional signal.** Tokens → vector (வெக்டர் — எண் பட்டியல்)s. Position injected via RoPE (modern) or sinusoidal (classic).
...
4. **Residual connection.** `x + sublayer(x)`. Without this, gradient (சாய்வு — மாற்றத்தின் दिशा)s vanish past ~6 layers.
...
Watch a vector (வெக்டர் — எண் பட்டியல்) flow through one block: attention mixes across positions, the residual carries it forward, the FFN transforms it, and norm keeps the stream stable.
...
### parameter (பாரமீட்டர் — கட்டுப்பாடு) count
...
At `d = 4096, r = 2.6, layers = 32` (roughly Llama 3 8B), total: `32 · (4·4096² + 3·2.6·4096²) ≈ 32 · (16 + 32) M = ~1.5B parameter (பாரமீட்டர் — கட்டுப்பாடு)s per layer × 32 ≈ 7B` (plus embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s and head). Matches published counts.

## Build It

### Step 1: the building blocks

Using the tiny `matrix (மேட்ரிக்ஸ் — எண் மேஜை)` class from Lesson 03 (copied to this file for independence):
...
Replace LayerNorm and ReLU-FFN with RMSNorm and SwiGLU. Confirm shapes still match. This is the 2026 modernization with one function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) substitution.
...
| Need | Pick | Example |
|------|------|---------|
| classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்), embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s, QA over text | Encoder-only | BERT, DeBERTa, ModernBERT |
...
See `outputs/skill-transformer-block-reviewer.md`. The skill reviews a new transformer block how to build against the 2026 defaults and flags missing pieces (pre-norm, RoPE, RMSNorm, GQA, FFN expansion ratio).

## Exercises

1. **Easy.** Count the parameter (பாரமீட்டர் — கட்டுப்பாடு)s in your encoder_block at `d_model=512, n_heads=8, ffn_expansion=4, swiglu=True`. Validate by implementing the block and using `sum(p.numel() for p in block.parameters())`.
...
| Residual | "Skip connection" | `x + f(x)` output. enables gradient (சாய்வு — மாற்றத்தின் दिशा) flow through deep stacks. |
