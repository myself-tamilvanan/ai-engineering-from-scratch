...
Dosovitskiy et al. (2020) — "An Image is Worth 16x16 Words" — showed you can drop the convolutions entirely. Slice an image into fixed-size patches, linearly project each patch into an embedding (embedding — பொருளை எண்களாக மாற்றுதல்), feed the sequence to a vanilla transformer encoder. At sufficient scale (ImageNet-21k pretraining or bigger), ViT matches or beats ResNet-based models.
...
```
image (224, 224, 3) → 14 × 14 grid of 16x16x3 patches → 196 vector (வெக்டர் — எண் பட்டியல்)s of length 768
```
...
### Step 2 — linear embedding (embedding — பொருளை எண்களாக மாற்றுதல்)

A single learned matrix (மேட்ரிக்ஸ் — எண் மேஜை) projects each flat patch to `d_model`. Equivalent to a convolution of kernel size `P` and stride `P`. In PyTorch this is literally `nn.Conv2d(C, d_model, kernel_size=P, stride=P)` — a 2-line how to build.

### Step 3 — prepend `[CLS]` token, add positional embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s

- Prepend a learnable `[CLS]` token. Its final hidden state is the image representation used for classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்).
- Add learnable positional embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s (ViT-original) or sinusoidal 2D (later variants).
...
For classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்): take `[CLS]` hidden state → linear → softmax. For DINOv2 or SAM, discard `[CLS]`, use the patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s directly.
...
| DINOv2 | 2023 | Self-supervised (no labels). Best general vision feature (பண்பு — தகவல் நெடுவரிசை)s. |
...
See `code/main.py`. Pure-stdlib patchify + linear embedding (embedding — பொருளை எண்களாக மாற்றுதல்) + sanity checks. No training — ViT at any realistic scale needs PyTorch and hours of GPU time.

### Step 1: fake image

A 24 × 24 RGB image as a list of rows of `(R, G, B)` tuples. We use 6×6 patches → 16 patches, 108-d embedding (embedding — பொருளை எண்களாக மாற்றுதல்) vector (வெக்டர் — எண் பட்டியல்) each.
...
Multiply each flat patch by a random `(patch_flat_size, d_model)` matrix (மேட்ரிக்ஸ் — எண் மேஜை). Verify output shape is `(N_patches + 1, d_model)` after prepending `[CLS]`.

### Step 4: count parameter (பாரமீட்டர் — கட்டுப்பாடு)s for a realistic ViT
...
inputs = processor(img, return_tensor (டென்சர் — பல வடிவ எண் வரிசை)s="pt")
out = model(**inputs).last_hidden_state   # (1, 197, 768): [CLS] + 196 patches
cls_emb = out[:, 0]                       # image representation
```

**DINOv2 embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s are the 2026 default for image feature (பண்பு — தகவல் நெடுவரிசை)s.** Freeze the backbone, train a tiny head. Works for classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்), retrieval, detection, captioning. Meta's DINOv2 checkpoints outperform CLIP on every non-text vision task.
...
1. **Easy.** Run `code/main.py`. Verify the number of patches equals `(H/P) * (W/P)` and the flat patch dimension (பரிமாணம் — கோணம்) equals `P*P*C`.
2. **Medium.** Implement 2D sinusoidal positional embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s — two independent sinusoidal codes for `row` and `col` of each patch, concatenated. Feed them into a tiny PyTorch ViT and compare accuracy vs learnable positional embeddings on CIFAR-10.
...
|------|-----------------|-----------------------|
| Patch | "The vision-transformer token" | Flat vector (வெக்டர் — எண் பட்டியல்) of pixel values for a `P × P × C` region of the image. |
| Patchify | "Chop + flatten" | Slice image into non-overlapping patches, flatten each to a vector. |
| `[CLS]` token | "The image summary" | Prepended learnable token. its final embedding (embedding — பொருளை எண்களாக மாற்றுதல்) is the image representation. |
| Inductive bias | "What the model assumes" | ViT has fewer priors than CNNs. needs more data to make up the gap. |
| DINOv2 | "Self-supervised ViT" | Trained without labels using image augmentation + momentum teacher. Best general image feature (பண்பு — தகவல் நெடுவரிசை)s in 2026. |
...
- [Oquab et al. (2023). DINOv2: Learning Robust Visual feature (பண்பு — தகவல் நெடுவரிசை)s without Supervision](https://arxiv.org/abs/2304.07193) — DINOv2.
