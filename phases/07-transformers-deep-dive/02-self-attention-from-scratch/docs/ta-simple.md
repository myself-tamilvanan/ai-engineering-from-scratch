...
**Type:** Build
**Languages:** Python
**Prerequisites:** Phase 3 (deep learning (ஆழ் கற்றல் — பல்லாய கொட்ட பயிற்சி) Core), Phase 5 Lesson 10 (Sequence-to-Sequence)
**Time:** ~90 minutes
...
- Trace how the attention matrix (மேட்ரிக்ஸ் — எண் மேஜை) captures token relationships and explain why scaling by sqrt(d_k) prevents softmax saturation
...
Every token generates three vector (வெக்டர் — எண் பட்டியல்)s:
...
Each token embedding (embedding — பொருளை எண்களாக மாற்றுதல்) gets projected through three learned weight matrices:

```
Input embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s (sequence of n tokens, each d-dimension (பரிமாணம் — கோணம்)al):
...
### The Attention matrix (மேட்ரிக்ஸ் — எண் மேஜை)

Once you have Q, K, V for all tokens, attention scores form a matrix (மேட்ரிக்ஸ் — எண் மேஜை):
...
Watch one query at a time sweep the keys: each row scores every token, softmax turns the scores into weights, and the context vector (வெக்டர் — எண் பட்டியல்) is the weighted blend of values.

```figure
attention-matrix (மேட்ரிக்ஸ் — எண் மேஜை)
```

### Why Scale?

The dot products grow with dimension (பரிமாணம் — கோணம்) dk. If dk = 64, dot products is in the range of tens, pushing softmax into regions where gradient (சாய்வு — மாற்றத்தின் दिशा)s vanish. The fix: divide by sqrt(dk).

```
Scaled scores = (Q @ K^T) / sqrt(dk)
```

This keeps values in a range where softmax produces useful gradient (சாய்வு — மாற்றத்தின் दिशा)s.
...
The final output for each token is a weighted sum of all value vector (வெக்டர் — எண் பட்டியல்)s:
...
The core function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). Takes Q, K, V matrices and returns the attention output plus the weight matrix (மேட்ரிக்ஸ் — எண் மேஜை).
...
Create fake embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s for a sentence and watch the attention weights.
...
mha = nn.MultiheadAttention(embed_dim=d_model, num_heads=n_heads, batch (பத்தி — தொகுப்பு)_first=True)
...
The key difference: multi-head attention runs multiple attention function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s in parallel, each with its own Q, K, V projections of size dk = d_model / n_heads, then concatenates results. This lets the model attend to different relationship types simultaneously.
...
1. Modify `scaled_dot_product_attention` to accept an optional mask matrix (மேட்ரிக்ஸ் — எண் மேஜை) that sets certain positions to negative infinity before softmax (this is how causal/decoder masking works)
...
|------|----------------|----------------------|
| Query (Q) | "The question vector (வெக்டர் — எண் பட்டியல்)" | A learned projection of the input that represents what information this token is looking for |
...
| Scaled dot-product attention | "The attention formula" | softmax(QK^T / sqrt(dk)) @ V - scaling prevents softmax saturation in high dimension (பரிமாணம் — கோணம்)s |
...
| Multi-head attention | "Parallel attention" | Running multiple attention function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s with different projections, then concatenating results for richer representations |
...
- [The Annotated Transformer (Harvard NLP)](https://nlp.seas.harvard.edu/annotated-transformer/) - line-by-line PyTorch how to build with explanations
