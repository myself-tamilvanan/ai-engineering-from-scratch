...
# KV Cache, Flash Attention & Inference optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)
...
On top of that, attention itself moves a lot of data. Standard attention materializes an N×N score matrix (மேட்ரிக்ஸ் — எண் மேஜை), N×d softmax output, N×d final output — too many reads and writes to HBM. For N≥2K, attention becomes memory-bound before it becomes FLOP-bound. Classic attention kernels underuse modern GPUs by 4–10×.

Two optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)s, both from Dao et al., pushed frontier inference from "slow" to "fast":

1. **KV cache.** Store the K and V vector (வெக்டர் — எண் பட்டியல்)s of every prefix token. Each new token's attention is one query against the cached keys. Inference reduces from `O(N²)` to `O(N)` per generation step.
2. **Flash Attention.** Tile the attention calculation so the full N×N matrix (மேட்ரிக்ஸ் — எண் மேஜை) never hits HBM. All of softmax + matmul happens in SRAM. 2–4× wall-clock speedup on A100. 5–10× on H100 with FP8.

By 2026 both are universal. Every production inference stack (vLLM, tensor (டென்சர் — பல வடிவ எண் வரிசை)RT-LLM, SGLang, llama.cpp) assumes them. Every frontier model ships with Flash Attention enabled.
...
That 10 GB is why Llama 3 70B at 128K context needs most of a 40 GB A100 just for KV cache at batch (பத்தி — தொகுப்பு) size 1.

**GQA is the KV-cache win.** MHA with 64 heads would be 32 GB. MLA compresses even further.

Drag the dimension (பரிமாணம் — கோணம்)s and watch the cache size move. Push the sequence length or batch (பத்தி — தொகுப்பு) up and see how fast it blows past a single GPU:
...
- **Lookahead decoding.** Jacobi iteration (மறுயுதல் — மீண்டும் செய்தல்). no draft model needed. Niche but free.

### Continuous batch (பத்தி — தொகுப்பு)ing

Classic batch (பத்தி — தொகுப்பு)ed inference: wait for the slowest sequence to finish, then start a new batch. Wastes GPU when short responses finish early.

Continuous batch (பத்தி — தொகுப்பு)ing (first shipped in Orca, now in vLLM, tensor (டென்சர் — பல வடிவ எண் வரிசை)RT-LLM, SGLang): swap new requests into the batch as soon as old ones finish. 5–10× throughput gain for typical chat workloads.

### PagedAttention — KV cache as virtual memory

vLLM's headline feature (பண்பு — தகவல் நெடுவரிசை). KV cache is allocated in 16-token blocks. a page table maps logical positions to physical blocks. Lets you share KV across parallel samples (beam search, parallel sampling), hot-swap prefixes for prompt caching, and defragment memory. 4× throughput improvement over naive contiguous allocation.
...
3. A tiled softmax that simulates Flash Attention's running-max algorithm (அல்கோரிதம் — வழிமுறை).
...
Simple: keep growing per-token K, V vector (வெக்டர் — எண் பட்டியல்)s in per-layer, per-head lists.
...
vllm serve meta-llama/Llama-3.1-70B-Instruct \
    --tensor (டென்சர் — பல வடிவ எண் வரிசை)-parallel-size 4 \
    --max-model-len 32768 \
...
See `outputs/skill-inference-optimizer.md`. The skill picks attention how to build, KV cache strategy, quantization, and speculative decoding for a new inference deployment.
...
| Continuous batch (பத்தி — தொகுப்பு)ing | "No-wait batching" | Swap finished sequences out, new ones in, without draining the batch. |
...
- [Li et al. (2024). EAGLE: Speculative Sampling Requires Rethinking feature (பண்பு — தகவல் நெடுவரிசை) Uncertainty](https://arxiv.org/abs/2401.15077) — EAGLE-1/2 paper for the integrated-draft approach the lesson cites.
