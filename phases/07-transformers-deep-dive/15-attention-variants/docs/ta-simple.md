...
Three classes of variants change the topology of the attention matrix (மேட்ரிக்ஸ் — எண் மேஜை) itself:
...
Each query at position `i` attends only to positions in `[i - W, i]` (causal SWA) or `[i - W/2, i + W/2]` (bidirectional). Tokens outside the window get `-inf` in the score matrix (மேட்ரிக்ஸ் — எண் மேஜை).
...
For `N = 8192` and `W = 1024`, the score matrix (மேட்ரிக்ஸ் — எண் மேஜை) has 1024 × 8192 non-zero rows in expectation — an 8× reduction.
...
Sparse attention is a kernel-engineering story. The math is simple (mask the score matrix (மேட்ரிக்ஸ் — எண் மேஜை)). the win comes from never loading the zero entries into SRAM. FlashAttention-3 and the 2026 FlexAttention API make custom sparse patterns first-class in PyTorch.
...
where `λ` is a learned scalar (scalar — ஒரு எண்) (usually 0.5–0.8). A1 captures real content weights. A2 captures the sink. Subtraction cancels the sink, reallocates weight to relevant tokens.
...
One parameter (பாரமீட்டர் — கட்டுப்பாடு) — `window`. For `window >= n`, you recover full causal attention. For `window = 1`, each token attends only to itself.
...
Dense local window plus every `stride`-th token back to the start of the sequence. Receptive field grows in log steps with extra layers.
...
FlexAttention in PyTorch 2.5+ accepts a mask function (சார்பு — உள்ளீட்டுக்கு வெளியீடு):
...
mask = create_block_mask(swa_pattern, B=batch (பத்தி — தொகுப்பு), H=heads, Q_LEN=n, KV_LEN=n)
out = flex_attention(q, k, v, block_mask=mask)
```

This compiles to a custom Triton kernel. Within 10% of FlashAttention-3 speed for common patterns, and the mask function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) is a Python callable.
...
3. **Hard.** Implement a Gemma-3-style 5:1 layer mix (5 SWA, 1 global) in the capstone model. Compare loss, memory, and generation quality against pure-SWA and pure-global baselines at matched parameter (பாரமீட்டர் — கட்டுப்பாடு)s.
...
| FlexAttention | "Mask-as-Python" | PyTorch 2.5+ API that compiles arbitrary mask function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s into FlashAttention-shape kernels. |
