...
The trick has to preserve the distribution. Speculative sampling, introduced by Leviathan et al. (2023) and by Chen et al. concurrently, guarantees that the output sequence is **identically distributed** to what the big model would have produced on its own. No quality choice. Just faster.
...
4. **Lookahead decoding (Fu 2024).** Jacobi iteration (மறுயுதல் — மீண்டும் செய்தல்). no draft model required at all. Self-speculation. Niche but dependency-free.

Every production inference stack in 2026 ships speculative decoding by default. vLLM, tensor (டென்சர் — பல வடிவ எண் வரிசை)RT-LLM, SGLang, and llama.cpp all support at least vanilla + EAGLE-2.

## The Concept

### The core algorithm (அல்கோரிதம் — வழிமுறை)
...
Pros: no second model. Cons: adds trainable parameter (பாரமீட்டர் — கட்டுப்பாடு)s. needs a supervised fine-tuning stage (~1B tokens). acceptance rate is a bit lower than vanilla speculative with a good draft.

### EAGLE — better draft by reusing hidden states

EAGLE-1/2/3 (Li et al., 2024–2025) makes the draft model a tiny transformer (usually 1 layer) that ingests the verifier's last-layer hidden states. Because the draft sees the verifier's feature (பண்பு — தகவல் நெடுவரிசை) representation, its predictions correlate strongly with the verifier's output distribution. Acceptance rates climb from ~0.6 (vanilla) to 0.85+.
...
Production implementations (vLLM's `--speculative-model`, tensor (டென்சர் — பல வடிவ எண் வரிசை)RT-LLM's LookaheadDecoder) handle this with scratch KV buffers. Write first, commit on acceptance. It's not in simple idea hard, but it is fiddly.

## Build It

See `code/main.py`. We implement the core speculative-sampling algorithm (அல்கோரிதம் — வழிமுறை) (rejection step + residual distribution) with:
...
vllm serve meta-llama/Llama-3.1-70B-Instruct \
    ...
    --speculative-draft-tensor (டென்சர் — பல வடிவ எண் வரிசை)-parallel-size 1 \
...
tensor (டென்சர் — பல வடிவ எண் வரிசை)RT-LLM has the fastest Medusa path as of mid-2026. `faster-whisper` wraps speculative decoding for Whisper-large with a small draft.
...
- Single-sequence generation of 1–5 tokens. extra work dominates.
- Wildly creative / high-temperature sampling (α drops).
- Memory-constrained deployments (draft model adds VRAM).

## Ship It

See `outputs/skill-spec-decode-picker.md`. The skill picks a speculative decoding strategy (vanilla / Medusa / EAGLE / lookahead) and tuning parameter (பாரமீட்டர் — கட்டுப்பாடு)s (N, draft temperature) for a new inference workload.
...
2. **Medium.** Plot speedup (tokens per big-model forward) as a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) of `N` for `α = 0.5, 0.7, 0.85`. Identify the optimal `N` for each α. (Hint: expected tokens per verify call = `(1 - α^{N+1}) / (1 - α)`.)
...
4. **Hard.** Implement rollback: start with a 10-token prefix KV cache, feed 5 draft tokens, simulate a rejection at position 3. Verify your cache reads correctly match "prefix + first 2 accepted drafts" at the next iteration (மறுயுதல் — மீண்டும் செய்தல்).
...
| Lookahead decoding | "Jacobi iteration (மறுயுதல் — மீண்டும் செய்தல்)" | Self-speculation using a fixed-point iteration. no draft model. |
...
- [Leviathan, Kalman, Matias (2023). Fast Inference from Transformers via Speculative Decoding](https://arxiv.org/abs/2211.17192) — the core algorithm (அல்கோரிதம் — வழிமுறை) and the equivalence theorem.
...
- [Li et al. (2024). EAGLE: Speculative Sampling Requires Rethinking feature (பண்பு — தகவல் நெடுவரிசை) Uncertainty](https://arxiv.org/abs/2401.15077) — EAGLE-1. hidden-state-conditioned draft.
...
- [SafeAILab / EAGLE reference how to build](https://github.com/SafeAILab/EAGLE) — the reference code for EAGLE-1/2/3.
