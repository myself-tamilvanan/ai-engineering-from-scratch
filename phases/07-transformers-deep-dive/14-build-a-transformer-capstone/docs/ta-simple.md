...
This is the "nanoGPT" of the course. It is not original — Karpathy's 2023 nanoGPT tutorial is the reference how to build every student writes at least once. We lift the shape and retool it around what we've covered.
...
input tokens (B, N)
   │
   ▼
token embedding (embedding — பொருளை எண்களாக மாற்றுதல்) + positional embedding  ◀── Lesson 04 (RoPE option)
   │
   ▼
┌──── block × L ────────────────────┐
│  RMSNorm                          │  ◀── Lesson 05
...
- `GPTConfig` — one place to configure all hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s.
- `MultiHeadAttention` — causal, batch (பத்தி — தொகுப்பு)ed, with optional Flash-style pathway (PyTorch's `scaled_dot_product_attention`).
- `SwiGLUFFN` — modern FFN.
- `Block` — pre-norm, residual-wrapped attention + FFN.
- `GPT` — embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s, stacked blocks, LM head, generate().
- Training loop with AdamW, cosine LR, gradient (சாய்வு — மாற்றத்தின் दिशा) clipping.
- Char-level tokenizer on Shakespeare text.

### What we don't ship

- RoPE — implemented in simple idea in Lesson 04. Here we use learned positional embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s for simplicity. The exercises ask you to swap in RoPE.
...
See `code/main.py`. The block is textbook from Lesson 05 — pre-norm, RMSNorm, SwiGLU, causal MHA. parameter (பாரமீட்டர் — கட்டுப்பாடு) count for 4/4/128: ~800K.

### Step 3: training loop

Get a random batch (பத்தி — தொகுப்பு) of length-256 token windows. Forward. Shift-by-one cross-entropy. Backward. AdamW step. Log. Repeat.

```python
for step in range(max_steps):
    x, y = get_batch (பத்தி — தொகுப்பு)("train")
    logits = model(x)
    loss = F.cross_entropy(logits.view(-1, vocab_size), y.view(-1))
    loss.backward()
    torch.nn.utils.clip_grad_norm_(model.parameter (பாரமீட்டர் — கட்டுப்பாடு)s(), 1.0)
    opt.step()
    opt.zero_grad()
```
...
Not Shakespeare. But Shakespeare-shaped. A clear win for ~800K parameter (பாரமீட்டர் — கட்டுப்பாடு)s and 6 minutes on a laptop.
...
This ends up as a 125M-parameter (பாரமீட்டர் — கட்டுப்பாடு) GPT that generates fluent English. Not a frontier model. But the same code path — just bigger — is what Karpathy, EleutherAI, and the Allen Institute use to train research checkpoints in 2026.

## Ship It

See `outputs/skill-transformer-review.md`. The skill reviews a transformer-from-scratch how to build for correctness across all 13 prior lessons.
...
2. **Medium.** Replace learned positional embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s with RoPE. Apply the rotation to Q and K inside `MultiHeadAttention`. Train and verify val loss is at least as low.
...
5. **Hard.** Replace the single FFN per block with a 4-expert MoE. Router + top-2 routing. See how val loss changes at matched active parameter (பாரமீட்டர் — கட்டுப்பாடு)s.
...
| Tied embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s | "Share input/output matrix (மேட்ரிக்ஸ் — எண் மேஜை)" | LM head weight = transpose of token embedding matrix. saves parameter (பாரமீட்டர் — கட்டுப்பாடு)s, improves quality. |
| bf16 autocast | "Training precision trick" | Run forward/back in bf16, keep optimizer state in fp32. standard since 2021. |
| gradient (சாய்வு — மாற்றத்தின் दिशा) clipping | "Stops spikes" | Cap global grad norm at 1.0. prevents training blowups. |
...
- [The Annotated Transformer (Harvard NLP)](https://nlp.seas.harvard.edu/annotated-transformer/) — the classic annotated how to build.
