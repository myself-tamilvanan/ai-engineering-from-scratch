...
- **Serializable state.** All agent state has to be persistable. function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) closures with live database connections do not survive.
...
Fibers (Python `asyncio`, Go goroutines, Rust `tokio`) cooperatively yield on I/O. The same 10,000 calls fit comfortably in process. At LLM-agent scale, async is not an optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) — it is the architecture.

Exception: CPU-bound post-processing (embedding (embedding — பொருளை எண்களாக மாற்றுதல்), tokenizer tricks) still wants threads or processes. Separate your I/O layer from your CPU layer.
...
4. Read LangGraph's runtime doc (linked below). Identify which feature (பண்பு — தகவல் நெடுவரிசை)s of the runtime would take the longest to replicate in a hand-rolled FastAPI + Postgres version. Is that a reason to adopt, or can you defer?
...
- [matrix (மேட்ரிக்ஸ் — எண் மேஜை)](https://arxiv.org/abs/2511.21686) — decentralized framework with message queues as the coordination substrate
