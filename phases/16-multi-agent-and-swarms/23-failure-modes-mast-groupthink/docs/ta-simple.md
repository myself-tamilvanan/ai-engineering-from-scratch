...
The most dangerous failure category is silent correctness failure. A system that fails loudly (crash, exception, alert) is monitored. A system that produces plausible-but-wrong outputs cannot be detected by exception logs. This is why verification gaps are the most expensive category per-failure even though they are only 21.30% by count.

Invest in:
- Sample-based human review.
- Golden-dataset regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) tests.
...
- **Golden datasets.** Small, high-quality, hand-audited. regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)-test against them weekly.
...
1. Run `code/main.py`. Confirm the circuit breaker caps the retry storm. Vary the failure threshold and observe the choice.
