...
Every agent sees every message. A selector function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) is invoked at each turn to pick who speaks next.
...
**Custom.** A Python function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) with whatever logic you want. Typical: LLM-selected with fallback rules (e.g., "always give the verifier the turn after the coder").
...
- **Strict determinism.** The LLM selector is inconsistent. Same prompt, different runs, different next speakers.
...
- **Hot speakers.** One agent dominates the conversation because the selector favors its specialties. Introduce speaker balance as a selector feature (பண்பு — தகவல் நெடுவரிசை).
...
- Group chat: all agents are peers. selector is a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) over the shared pool.
...
|------|----------------|------------------------|
| GroupChat | "Agents in one chat room" | Shared message pool + selector function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). AutoGen / AG2 primitive. |
...
- [AutoGen group chat docs](https://microsoft.github.io/autogen/stable/user-guide/core-user-guide/design-patterns/group-chat.html) — the reference how to build
