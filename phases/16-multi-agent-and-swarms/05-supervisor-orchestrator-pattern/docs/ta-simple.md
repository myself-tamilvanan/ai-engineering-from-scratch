...
1. Run `code/main.py`, then modify the lead to spawn 5 workers instead of 3. Observe the wall-clock effect. At what worker count does spawn extra work exceed parallel savings in this demo?
...
3. Add a conflict-detection step to the lead's synthesis: if two workers return contradictory answers, the lead notes the disagreement quite than picking one. How do you detect contradiction without calling an LLM?
