...
**Opaque lifecycle.** A2A does not prescribe *how* the remote agent solves the task. The client sees state transitions and artifacts. the how to build is free to use any framework.
...
- **Small teams.** Spec extra work is real. internal-only agents may not need the formality.
...
5. Compare A2A (Agent Card discovery) to MCP (server-side capability listing via `listTools`). What is the choice between self-describing agents and capability-probing?
