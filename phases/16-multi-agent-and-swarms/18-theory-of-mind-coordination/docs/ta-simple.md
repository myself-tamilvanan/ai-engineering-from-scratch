...
2. **Goal-directed complementarity.** Do agents' actions complement each other (different subtasks) quite than duplicate?
...
- **Ground-truth-dependent tasks.** ToM is about beliefs. if correctness depends on facts, ToM is a distraction.

### The coordination you can actually measure

Three practical signals a team's coordination is real quite than prompt-dressed:
...
- **Statistical test.** Is the difference between system and control big at `p < 0.05` on your metric?
