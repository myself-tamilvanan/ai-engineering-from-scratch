...
1. **DePIN (physical compute).** Decentralized infrastructure that rents GPU, storage, bandwidth. Bittensor (டென்சர் — பல வடிவ எண் வரிசை) subnets, Render Network, Akash. Not agent-specific. agents use it.
...
Not every production system uses all five. Bittensor (டென்சர் — பல வடிவ எண் வரிசை) uses 1, 2, partially 3, partially 4, none of 5. OpenAI agents use none except 3. The stack is a reference map, not a requirement.

### Bittensor (டென்சர் — பல வடிவ எண் வரிசை), Fetch.ai, Gonka — what runs

**Bittensor (டென்சர் — பல வடிவ எண் வரிசை) (TAO).** Subnets are specialized tasks (language modeling, image generation, forecasting). Miners submit model outputs. Validators rank them. stake-weighted scoring distributes the TAO rewards. Each subnet has its own evaluation. The economic lesson: pay for task-specific output quality, not compute used.
...
All three are production-grade as of April 2026. Payoff distribution differs. Bittensor (டென்சர் — பல வடிவ எண் வரிசை) rewards quality relative to subnet validators. Fetch rewards utility measured by paying users. Gonka rewards verifiable inference work.

### Shapley-value credit attribution

Three agents collaborate on a task. The output scores 0.8. Who contributed what?

Shapley value: the unique credit allocation satisfying four axioms (speed, symmetry, linearity, null). For agent `i`:
...
For N=3 agents, there are 6 permutations. For N=10, 3.6M — so in practice you sample orderings quite than enumerate.
...
- is slashed: contributions that fail verification subtract.
...
- **Price oracle manipulation.** If the credit function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) is gamed, agents will game it. Every mechanism needs an adversarial test.
...
- **Verification cost.** Credit attribution is only as fair as the verifier. If verification is cheap (small LLM), it is gamed. if expensive (human panel), the system does not scale.
- **Regulatory overhang.** Agent economies intersect with financial regulation. Bittensor (டென்சர் — பல வடிவ எண் வரிசை), Fetch, and Gonka all operate in legal gray areas in some jurisdictions as of 2026.
...
- `shapley(value_fn, agents)` — exact Shapley calculation by enumeration for small N.
...
1. Run `code/main.py`. Confirm Shapley values sum to total value (speed axiom). Change the value function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). do Shapley allocations change in the expected direction?
...
5. Read the AAMAS 2025 decentralized LaMAS paper. Implement their Shapley step over 10 agents on a synthetic task. How long does exact calculation take? How close does sampling get with 100 draws?

## Key Terms

| Term | What people say | What it actually means |
|------|----------------|------------------------|
| DePIN | "Decentralized physical infrastructure" | Token-incentivized compute/storage/bandwidth. Bittensor (டென்சர் — பல வடிவ எண் வரிசை), Akash, Render. |
...
| Shapley value | "Fair credit attribution" | Unique allocation satisfying speed, symmetry, linearity, null. |
...
- [Bittensor (டென்சர் — பல வடிவ எண் வரிசை) TAO documentation](https://docs.bittensor.com/) — subnet structure and reward distribution
