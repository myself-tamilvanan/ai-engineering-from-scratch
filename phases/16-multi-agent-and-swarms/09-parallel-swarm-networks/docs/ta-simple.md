...
> Contrast with supervisor: no central decider. Agents read a shared event bus, pick up work asynchronously, write results back. LangGraph explicitly supports "Swarm Architecture" for decentralized, dynamic environments. matrix (மேட்ரிக்ஸ் — எண் மேஜை) (arXiv:2511.21686) represents both control and data flow as serialized messages passed through distributed queues to eliminate the orchestrator bottleneck. The choice is explicit: determinism and traceability for grows big. Swarm fits tasks with many independent sub-problems. it does not fit tasks that need a single coherent plan.
...
### matrix (மேட்ரிக்ஸ் — எண் மேஜை) (arXiv:2511.21686)

matrix (மேட்ரிக்ஸ் — எண் மேஜை) is the 2025 paper that takes swarm to its natural conclusion: both control flow and data flow are serialized messages on distributed queues. No central coordinator. Fault tolerance comes from message durability. grows big is the message broker's problem, not the system's.

Contribution: a programming model where multi-agent coordination is "what message topic does this agent subscribe to?" quite than "which agent does the supervisor pick next?" This makes the system look like a pub/sub event mesh.

### LangGraph's Swarm Architecture

LangGraph 2025 docs explicitly describe "Swarm Architecture" as one of the multi-agent patterns: agents are nodes, but edges form a directed graph with cycles and any node is activated from the pool. A worker picks from available work by condition, not by supervisor assignment.
...
4. Read the matrix (மேட்ரிக்ஸ் — எண் மேஜை) paper (arXiv:2511.21686) abstract and Section 3. Identify one specific choice Matrix accepts (grows big gain) and one it gives up (traceability, determinism).
...
| matrix (மேட்ரிக்ஸ் — எண் மேஜை) framework | "Full message-passing swarm" | Both data and control flow are serialized messages on distributed queues. |
...
- [matrix (மேட்ரிக்ஸ் — எண் மேஜை) — A Decentralized Framework for Multi-Agent Systems](https://arxiv.org/abs/2511.21686) — full message-passing swarm
