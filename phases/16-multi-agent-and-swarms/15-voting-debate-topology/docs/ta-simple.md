...
Replace N samples with N *different* agents. Different base models (Claude, GPT, Llama), different prompts, different tool access. The benefit: uncorrelated errors. The cost: different agents cost different amounts. coordinating them adds extra work.
...
The 4-agent ceiling is empirical, not basic. It reflects 2026 LLM context capacity: each agent's context fills with peers' outputs, and marginal value of adding agent N+1 drops once everyone can see everyone.
...
A 2024-2026 pattern in the practical literature: swapping one of your N agents for a different base model gives a bigger accuracy bump than increasing N by 1. The simple idea is monoculture — each new independent-error source is worth more than an extra correlated sample.
...
- The question has ground truth (fact, math, code behavior). Vote convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) is meaningful.
...
4. Read the AgentVerse paper (ICLR 2024). Identify which emergent behavior your how to build exhibits most strongly. Can you elicit the opposite behavior by a prompt change?
...
- [MARBLE repo](https://github.com/ulab-uiuc/MARBLE) — reference benchmark how to build
