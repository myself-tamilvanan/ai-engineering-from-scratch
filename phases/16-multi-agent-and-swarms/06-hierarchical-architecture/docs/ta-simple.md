...
3. **Consensus loops.** Two sub-managers disagree. top manager asks them to reconcile. they re-delegate down. workers re-run. sub-managers return slightly different answers. loop. CrewAI's `Process.hierarchical` guards against this with step limits, but the limit itself is now a hyperparameter (பாரமீட்டர் — கட்டுப்பாடு).
