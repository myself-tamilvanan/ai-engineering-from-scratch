...
how to build: the role prompt includes "when you need specific information you were not given, ask the relevant role by name before producing output."
...
- A critic is an LLM reviewing an artifact for quality. Subjective. is fooled by plausible prose.
...
`code/main.py` implements a 4-role pipeline building a simple Python function (சார்பு — உள்ளீட்டுக்கு வெளியீடு):
...
1. Run `code/main.py` and observe how the verifier catches the bug the critic missed. Add a static-analysis check (count occurrences of `return`) as an extra verifier. What does it catch that the runtime test misses?
...
| Communicative dehallucination | "Ask before inventing" | ChatDev pattern: executor asks planner when a detail is missing quite than making one up. |
| Critic | "LLM reviewer" | Subjective, opinionated reviewer. Catches taste issues. is fooled by plausible prose. |
