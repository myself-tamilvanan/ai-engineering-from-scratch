...
triage_agent = Agent(
    ...
    function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s=[transfer_to_refunds, transfer_to_sales, transfer_to_support],
)
```
...
- GroupChat: a selector (function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) or LLM) picks the next speaker from outside.
...
| Handoff filter | "Gate on transfer" | SDK feature (பண்பு — தகவல் நெடுவரிசை) to inspect and modify context at the handoff boundary. |
...
- [OpenAI Swarm repo](https://github.com/openai/swarm) — original how to build, kept as conceptual reference
