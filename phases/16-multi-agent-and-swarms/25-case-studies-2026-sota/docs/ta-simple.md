...
ChatDev's contribution: **communicative dehallucination**. Agents request specifics before answering — a designer agent asks the programmer what language is intended before sketching UI, quite than guessing. The paper reports this reduces hallucination in multi-agent pipelines measurably.
...
- **Cost is the central engineering constraint.** Token cost per task, wall-clock per interaction, rainbow-deploy extra work. Multi-agent wins on accuracy but loses on cost — and that trade is the business decision.
