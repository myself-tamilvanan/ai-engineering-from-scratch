...
**WBFT** (arXiv:2505.05103) — Weighted BFT with Hierarchical Structure Clustering. Vote weights are assigned by response quality plus a trust score learned from history. Cluster agents into Core and Edge. Core agents must achieve consensus first, Edge agents follow. Mitigation for: grows big (Core consensus is small and fast) and partially for monoculture (Core is chosen for diversity).

### Empirical: "Can AI Agents Agree?" (arXiv:2603.01213)

The paper measures scalar (scalar — ஒரு எண்) agreement (LLM agents agreeing on a single numeric value) across multiple frontier models. The finding is uncomfortable:

- Even with no adversaries, LLM agents disagree on scalar (scalar — ஒரு எண்) questions at rates above 30% on many benchmarks.
...
The semantic clustering step is the LLM-specific twist. Two answers "the study reports 4.2%" and "4.2% improvement" are the same cluster. A naive string-equality check would miss this. In production, use a cheap embedding (embedding — பொருளை எண்களாக மாற்றுதல்) model or explicit canonicalization.

### Threshold tuning

The `threshold` parameter (பாரமீட்டர் — கட்டுப்பாடு) decides when to accept and when to retry. Too low: you accept weak majorities. Too high: you never accept anything. Empirical range: 0.5-0.67 for `n=5-7` agents, higher for smaller `n`. Below a threshold, escalate to a human or to a different agent ensemble.
...
- **Adversarial multi-round.** If agents can observe prior rounds and mimic (Du 2023 debate), they start agreeing with each other regardless of truth. Bound the rounds (2-3 usually).
...
3. Swap the semantic clustering from string canonicalization to embedding (embedding — பொருளை எண்களாக மாற்றுதல்)-similarity (use any open-source embedding model). What happens to the sycophancy attack?
...
5. Read "Can AI Agents Agree?" (arXiv:2603.01213). Reproduce a simplified scalar (scalar — ஒரு எண்)-agreement experiment: three agents, one scalar question, the deceptive-persona prompt. Does CPWBFT or DecentLLMs catch it?
...
- [Can AI Agents Agree?](https://arxiv.org/abs/2603.01213) — scalar (scalar — ஒரு எண்)-agreement fragility and deceptive-persona attack
