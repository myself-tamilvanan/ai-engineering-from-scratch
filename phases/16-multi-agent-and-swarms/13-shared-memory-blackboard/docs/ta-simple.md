...
**Blackboard with subscription.** Agents declare interest in topics. the substrate routes only relevant messages. CA-MCP (arXiv:2601.11595) and the matrix (மேட்ரிக்ஸ் — எண் மேஜை) decentralized framework (arXiv:2511.21686) use this. Scales further, but requires upfront schema design to make subscriptions meaningful.
...
The blackboard pattern predates LLM agents by four decades. Hayes-Roth (1985, "A Blackboard Architecture for Control") described specialist Knowledge Sources that observe a global blackboard, contribute partial solutions, and trigger other sources. The 2026 blackboard (CA-MCP, matrix (மேட்ரிக்ஸ் — எண் மேஜை)) is the same pattern with LLM agents as Knowledge Sources and JSON blobs as partial solutions. The old literature has documented solutions to write contention, opportunistic control, and consistency that modern systems rediscover.

### Projection vs full view

A pure blackboard gives every subscriber the same projection (topic-scoped). A more aggressive design is **per-agent projection**: each agent gets a view customized to its role. LangGraph's state reducers are the canonical 2026 how to build — the reducer function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) folds global state into a role-specific slice.
...
The most load-bearing mitigation is the read-only verifier. how to build rules:
...
- [matrix (மேட்ரிக்ஸ் — எண் மேஜை) — decentralized multi-agent framework](https://arxiv.org/abs/2511.21686) — message-queue-based blackboard without a central orchestrator
