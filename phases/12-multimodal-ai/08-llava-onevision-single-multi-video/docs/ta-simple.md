...
- Explain why a single model beats specialists at the same parameter (பாரமீட்டர் — கட்டுப்பாடு) count when curriculum is done right.
...
The allocation maintains roughly constant total tokens. The LLM never sees a batch (பத்தி — தொகுப்பு) that blows its context. The encoder produces different geometry per scenario, but the LLM consumes the same budget.
...
2. OneVision SFT (stage OV). Mix single-image + multi-image + video (uniformly sampled frames). Train on the unified token budget. This teaches the model to handle heterogeneous batch (பத்தி — தொகுப்பு) shapes. No weight reset — continues from stage SI.
3. Task transfer (stage TT). Continue with a target task mix, usually heavier on multi-image or video depending on product. Optional fine-tune for deployment.
...
Single-image training builds the perceptual base. Patch tokens carry fine-grained visual feature (பண்பு — தகவல் நெடுவரிசை)s. the LLM learns to integrate them with text. Multi-image and video introduce structural challenges (which image is which, what happened first) that are hard to learn without a strong perceptual base.

If you train all scenarios from scratch together, the model underfits perception (limited single-image data per batch (பத்தி — தொகுப்பு)) and overfits structure (lots of multi-image / video data). Result: a model that follows cross-image reasoning patterns but is visually shallow.
...
The choice of pooling factor per scenario is itself a hyperparameter (பாரமீட்டர் — கட்டுப்பாடு). Less pooling = more tokens = richer representation. More pooling = fewer tokens = more frames / images fit.
...
| Token budget | "How many tokens per sample" | Total visual tokens the LLM sees per training / inference sample, usually 3000-4000 |
