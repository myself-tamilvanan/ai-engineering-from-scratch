...
1. Multiple retrieval heads (each modality needs embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s in a compatible space).
...
1. Shared embedding (embedding — பொருளை எண்களாக மாற்றுதல்) space. CLIP and CLAP produce text + image / text + audio embeddings in a shared space. Cosine similarity across modalities works directly. Limited to CLIP-trained pairs.
...
1. Decompose query. "quiet" → audio/review keyword. "vegan brunch" → menu item. "natural light" → image feature (பண்பு — தகவல் நெடுவரிசை).
