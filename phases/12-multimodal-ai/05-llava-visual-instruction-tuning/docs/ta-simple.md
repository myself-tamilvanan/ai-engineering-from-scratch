...
- Build a 2-layer MLP projector that maps ViT patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s (dim 1024) to an LLM's embedding dim (dim 4096).
...
In a single epoch (யுக் — ஒரு முழு சுழற்சி) at batch (பத்தி — தொகுப்பு) 128 this is done in a few hours. The projector learns to map ViT-space to LLM-space. No task-specific supervision.
...
- LLM is swapped (LLaVA-Llama2, LLaVA-Mistral, LLaVA-Llama3) by retraining just the projector.
...
### parameter (பாரமீட்டர் — கட்டுப்பாடு) economy
...
1. Compute the trainable-parameter (பாரமீட்டர் — கட்டுப்பாடு) count for the 2-layer MLP projector at `1024 → 4096 → 4096`. With GELU and bias, what fraction of LLaVA-13B does it represent?
