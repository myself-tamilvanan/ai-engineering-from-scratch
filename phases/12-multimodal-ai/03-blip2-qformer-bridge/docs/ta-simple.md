...
> CLIP aligns image and text but cannot generate captions, answer questions, or hold a conversation. BLIP-2 (Salesforce, 2023) solved that with a small trainable bridge: 32 learnable query vector (வெக்டர் — எண் பட்டியல்)s attend over a frozen ViT's feature (பண்பு — தகவல் நெடுவரிசை)s via cross-attention, then slot directly into a frozen LLM's input stream. 188M parameter (பாரமீட்டர் — கட்டுப்பாடு)s of bridge connected an 11B LLM to a ViT-g/14. Every adapter-based VLM through 2026 — MiniGPT-4, InstructBLIP, LLaVA's cousins — is a descendant. This lesson reads the Q-Former's architecture, explains its two-stage training, and builds a toy version that feeds visual tokens into a frozen text decoder.
...
- Implement a cross-attention block where a fixed set of learnable queries attend to external image feature (பண்பு — தகவல் நெடுவரிசை)s.
...
You have a frozen ViT that produces 256 patch tokens of dim 1408 per image. You have a frozen 7B LLM that expects token embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s of dim 4096. The obvious bridge — a linear layer from 1408 to 4096 — works, but feeding all 256 patch tokens into the LLM's context costs 256 extra tokens per image. Over a batch (பத்தி — தொகுப்பு) of 32 images that is 8192 tokens consumed by the visual modality alone.

The BLIP-2 question: can you compress the 256-token image representation into far fewer tokens (say 32) while preserving enough information for the LLM to caption, answer questions, and reason about the image? And can you train this bridge without touching the frozen backbones, keeping the training cost at just the bridge's parameter (பாரமீட்டர் — கட்டுப்பாடு)s?

The answer: a Q-Former. 32 learnable "query" vector (வெக்டர் — எண் பட்டியல்)s that cross-attend to the ViT's patch tokens, producing a 32-token visual summary that the LLM consumes. 188M parameter (பாரமீட்டர் — கட்டுப்பாடு)s total. Trained with contrastive, matching, and generative objectives before ever touching the LLM.

## The Concept

### Learnable queries

The Q-Former's core trick: instead of letting the LLM's text tokens attend to image patches, introduce a new set of 32 learnable query vector (வெக்டர் — எண் பட்டியல்)s `Q` and let *them* attend to image patches. The queries are parameter (பாரமீட்டர் — கட்டுப்பாடு)s of the model — they are learned during training and the same 32 queries are used for every image.
...
1. Query path: 32 query vector (வெக்டர் — எண் பட்டியல்)s flow through self-attention (among themselves), then cross-attention over the frozen ViT's patch tokens, then FFN.
...
Stage 2: generative learning. Attach a frozen LLM (OPT-2.7B or Flan-T5-XL, etc.). Project the 32 query outputs to the LLM's embedding (embedding — பொருளை எண்களாக மாற்றுதல்) dim via a small linear layer. Prepend them to the text prompt. Train only the linear projection and the Q-Former on LM loss over the concatenated prompt + image + caption sequence.
...
### parameter (பாரமீட்டர் — கட்டுப்பாடு) economics

BLIP-2 with ViT-g/14 (1.1B, frozen) + OPT-6.7B (6.7B, frozen) + Q-Former (188M, trained) = 8B total, 188M trained. The Q-Former alone is ~2.4% of the full stack's parameter (பாரமீட்டர் — கட்டுப்பாடு)s. Training cost reflects this: days on a handful of A100s vs weeks for end-to-end.
...
InstructBLIP (2023) extends the Q-Former with an extra input: the instruction text itself. At cross-attention time, the queries now have access to both the image patches and the instruction. The queries can specialize per-instruction ("count the cars", "describe the mood") quite than learning a single fixed summary. Benchmark gains on held-out tasks.

### MiniGPT-4 and the projector-only approach

MiniGPT-4 kept the Q-Former but trained only the output linear projection while freezing everything else. Cheap, but cost is quality — the queries were BLIP-2's, not yours. Good for rapid iteration (மறுயுதல் — மீண்டும் செய்தல்), not the best architecture.

### Why LLaVA went simpler

LLaVA (2023, Lesson 12.05) replaced the Q-Former with a plain 2-layer MLP that projects every ViT patch token into LLM space — 576 tokens per image for a 24x24 grid, all fed to the LLM. Worse compression but lets the LLM attend over raw patches. At the time this was controversial. by late 2023 it was dominant because visual instruction data (LLaVA-Instruct-150k) proved that the MLP could be trained to preserve enough signal. The choice: LLaVA's context fills faster, but it scales naturally to multi-image and video.
...
All math in pure Python (nested loops over vector (வெக்டர் — எண் பட்டியல்)s). Toy but correct shape. The attention-weight matrix (மேட்ரிக்ஸ் — எண் மேஜை) is printed so you can see which patches each query pulled from.

## Ship It

This lesson produces `outputs/skill-modality-bridge-picker.md`. Given a target VLM configuration (vision encoder token count, LLM context budget, deployment constraints, quality target), it recommends Q-Former vs MLP vs Perceiver resampler with a short justification and a parameter (பாரமீட்டர் — கட்டுப்பாடு)-count estimate for each bridge.

## Exercises

1. Implement the cross-attention block in PyTorch. Verify that with 32 queries and 256 keys/values, the attention-weight matrix (மேட்ரிக்ஸ் — எண் மேஜை) is 32 x 256 and each row sums to 1 after softmax.
...
3. Compare parameter (பாரமீட்டர் — கட்டுப்பாடு) counts: Q-Former (12 layers, 768 hidden) vs a 2-layer MLP projector (1408 → 4096, two layers). At what LLM scale does the 188M Q-Former cost pay back in training speed?

4. Read Section 3.2 of the BLIP-2 paper (arXiv:2301.12597) on how the Q-Former is initialized. Explain why initializing from BERT-base (not random) accelerates convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்).
...
|------|----------------|------------------------|
| Q-Former | "Querying transformer" | Small transformer with 32 learnable query vector (வெக்டர் — எண் பட்டியல்)s that cross-attend to frozen ViT feature (பண்பு — தகவல் நெடுவரிசை)s |
| Learnable queries | "Soft prompt for vision" | A fixed set of parameter (பாரமீட்டர் — கட்டுப்பாடு)s that serve as the query side of cross-attention. learned per model, shared across all inputs |
...
| Projection head | "Linear to LLM dim" | Final linear layer mapping Q-Former output to the LLM's embedding (embedding — பொருளை எண்களாக மாற்றுதல்) dimension (பரிமாணம் — கோணம்) |
| Perceiver resampler | "Flamingo's version" | Similar learnable-query cross-attention, used by Flamingo at every layer quite than as a single bridge |
