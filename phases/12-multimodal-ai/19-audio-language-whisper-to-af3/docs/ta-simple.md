...
Speech recognition was solved by Whisper. OCR-of-audio is a commodity. But "commodity" stops at transcription. If the model cannot reason over what it heard — timing, speakers, emotion, music structure, environmental sounds — transcription alone cannot drive product feature (பண்பு — தகவல் நெடுவரிசை)s.
...
### Log-Mel spectrogram: the input feature (பண்பு — தகவல் நெடுவரிசை)

Every audio encoder starts with the same feature (பண்பு — தகவல் நெடுவரிசை): a log-Mel spectrogram.

1. Resample to 16 kHz.
2. Short-time Fourier transform with 25ms windows, 10ms hop.
3. Take magnitude of the FFT result.
4. Apply Mel filter banks (usually 80 filters log-spaced 0-8000 Hz) to warp to perceptual frequency.
...
Whisper's encoder is a 12-layer ViT-style transformer processing the log-Mel spectrogram as a sequence of time frames. Output: one hidden-state vector (வெக்டர் — எண் பட்டியல்) per time frame.
...
BEATs (Chen et al., 2022) is a self-supervised transformer trained on AudioSet. Captures music and environmental sounds better than Whisper at the same parameter (பாரமீட்டர் — கட்டுப்பாடு) count.

AF-Whisper (Audio Flamingo 3's hybrid): concat Whisper + BEATs feature (பண்பு — தகவல் நெடுவரிசை)s as the audio input. Whisper carries linguistic signal, BEATs carries acoustic signal.
...
- "Is this real or generated audio?" — deepfake detection needs acoustic feature (பண்பு — தகவல் நெடுவரிசை)s.
...
- Covers classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்), temporal reasoning, causal reasoning, open-ended QA.
...
- Implements log-Mel spectrogram calculation in stdlib: windowing, naive DFT, Mel filter-bank.
...
This lesson produces `outputs/skill-audio-llm-pipeline-picker.md`. Given an audio task (transcription, music tagging, emotion inference, multi-speaker diarization, environment classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)), it picks cascaded, end-to-end AF3, or a hybrid.

## Exercises

1. Compute the log-Mel spectrogram dimension (பரிமாணம் — கோணம்) for a 30-second clip at 16kHz, 25ms window, 10ms hop, 80 Mel bins. How does this change at 48kHz?

2. Why does Whisper underperform on music? What audio feature (பண்பு — தகவல் நெடுவரிசை)s does BEATs capture that Whisper does not?
...
|------|-----------------|------------------------|
| Log-Mel spectrogram | "Mel feature (பண்பு — தகவல் நெடுவரிசை)s" | 2D (time, frequency) array of log-magnitude values after Mel filter banks |
