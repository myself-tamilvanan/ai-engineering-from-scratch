...
- `alpha` is a learnable scalar (scalar — ஒரு எண்) initialized to zero.
...
The model sees the completion pattern and outputs "bird" (or whatever image3 shows). No gradient (சாய்வு — மாற்றத்தின் दिशा) steps. The frozen LLM's in-context learning capability carries through the gated cross-attention — this is the punchline of the paper and why it matters.
...
- Gemini's interleaved input: in simple idea inherits Flamingo's interleaved-format flexibility, though the exact mechanism is proprietary.
...
| parameter (பாரமீட்டர் — கட்டுப்பாடு) count | 188M trained | ~10B trained (cross-attn layers) |
...
1. Compute Flamingo-9B's visual parameter (பாரமீட்டர் — கட்டுப்பாடு) count: 9B LLM + 1.4B gated cross-attention layers + 64M resampler. What fraction of total params is trained?

2. Implement the gated residual `y = tanh(alpha) * cross + x` in PyTorch. Show experimentally that with `alpha=0`, `y==x` exactly at init.

3. Read OpenFlamingo Section 3.2 (arXiv:2308.01390) on how they handle multiple images in a batch (பத்தி — தொகுப்பு) when each prompt has a different image count. Describe the padding strategy.

4. Why does Flamingo's cross-attention mask let a text token attend to *only the most recent* preceding image quite than all preceding images? Read the Flamingo paper Section 2.4 and explain the choice.
...
| Frozen LLM | "No LLM gradient (சாய்வு — மாற்றத்தின் दिशा)s" | The text LLM's weights do not update. only resampler + cross-attn layers train |
