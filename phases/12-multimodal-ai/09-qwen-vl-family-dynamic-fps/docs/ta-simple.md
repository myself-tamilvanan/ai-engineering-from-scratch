...
- M-RoPE (Multimodal RoPE). Each token carries a 3D position (t, h, w) instead of 1D. For images t=0, for video t = frame_index. RoPE rotates query/key vector (வெக்டர் — எண் பட்டியல்)s by a frequency per axis. No positional embedding (embedding — பொருளை எண்களாக மாற்றுதல்) table.
...
Qwen3-VL is an incremental upgrade that consolidates quite than reinvents: larger LLM backbone (Qwen3-72B), expanded training data, improved OCR, stronger reasoning via the Qwen3 "thinking mode." The ViT and M-RoPE stay. The paper focuses on data and training improvements over architecture.

The lineage takeaway: by 2025 the Qwen-VL architecture had stabilized. extra generations scale compute and data, not primitives.

### M-RoPE mathematically

Classical RoPE rotates a query `q` of dimension (பரிமாணம் — கோணம்) `d` by position `m` using paired coordinates:
...
Qwen2.5-VL trains this logic implicitly. at inference the user controls via `fps` parameter (பாரமீட்டர் — கட்டுப்பாடு). A 60-second action sequence at 4 FPS with 81 tokens per frame = 19440 tokens, manageable in a 32k context.
...
- M-RoPE position calculation for a packed sequence mixing text, image patches, and video frames.
...
|------|-----------------|------------------------|
| M-RoPE | "Multimodal RoPE" | 3D rotary position embedding (embedding — பொருளை எண்களாக மாற்றுதல்) with temporal, height, and width bands in the hidden dim |
