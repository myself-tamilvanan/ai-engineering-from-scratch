...
The tokenizer is a vector (வெக்டர் — எண் பட்டியல்)-quantized variational autoencoder. The architecture:

- Encoder: CNN + ViT that maps image to a spatial feature (பண்பு — தகவல் நெடுவரிசை) map, say 32x32 features of dim 256.
- Codebook: a learned vocabulary of K vector (வெக்டர் — எண் பட்டியல்)s (Chameleon uses 8192), also dim 256.
...
Chameleon's vocabulary combines text tokens, image tokens, and modality separators. Each token has a single ID. The input embedding (embedding — பொருளை எண்களாக மாற்றுதல்) layer maps every ID to a D-dim hidden vector (வெக்டர் — எண் பட்டியல்). The output projection maps hidden back to vocab logits. Softmax picks the next token, whatever modality.
...
- Dropout placement. Dropout after every residual-add, not just after attention and MLP. More regularization required when gradient (சாய்வு — மாற்றத்தின் दिशा)s from image tokens can dominate.
...
| Codebook size | "K entries" | Number of discrete vector (வெக்டர் — எண் பட்டியல்)s the VQ-VAE can quantize to. trades compression for fidelity |
