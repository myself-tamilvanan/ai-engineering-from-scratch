...
3. Chunk → bi-encoder embedding (embedding — பொருளை எண்களாக மாற்றுதல்) (one vector (வெக்டர் — எண் பட்டியல்)).
...
ColBERT (Khattab & Zaharia, arXiv:2004.12832) is a text retrieval method. Instead of one vector (வெக்டர் — எண் பட்டியல்) per document, it produces one vector per token. At query time:

- Query tokens get their own embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s (N_q vector (வெக்டர் — எண் பட்டியல்)s).
- Document tokens get embeddings (N_d vectors, usually cached).
...
Pros: strong recall, handles term-level semantics. Cons: N_d vector (வெக்டர் — எண் பட்டியல்)s per document, storage expensive.

### ColPali

ColPali (Faysse et al., arXiv:2407.01449) applies the ColBERT pattern to images.

- Each page is encoded by PaliGemma (ViT + language) into patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s: N_p vector (வெக்டர் — எண் பட்டியல்)s per page.
...
At document-ingestion time: embed every page with PaliGemma, store all patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s. At query time: embed the query tokens, compute MaxSim against all stored page embeddings, return top-k pages.

Pros: end-to-end beats text-RAG by 20-40% on visually rich documents. Each patch-vector (வெக்டர் — எண் பட்டியல்) captures local layout and content.

Cons: N_p patches × 4-byte floats × D-dim vector (வெக்டர் — எண் பட்டியல்)s per page = storage grows fast. Mitigated by PQ / OPQ quantization.
...
VisRAG (Yu et al., arXiv:2410.10594) is a different variant: instead of MaxSim on patches, pool each page into a single vector (வெக்டர் — எண் பட்டியல்) with a VLM then bi-encoder retrieve. Faster indexing + smaller storage, weaker recall.
...
1. Ingest: PDF → page images → PaliGemma encoding → store all patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s.
...
A 50-page financial report with 729 patches per page and 128-dim embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s:
...
- Toy patch encoder: maps a "page" (small grid of feature (பண்பு — தகவல் நெடுவரிசை) vector (வெக்டர் — எண் பட்டியல்)s) to an array of patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s.
...
|------|-----------------|------------------------|
| Late interaction | "ColBERT-style" | Retrieval using per-token or per-patch embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s + MaxSim, not a single doc vector (வெக்டர் — எண் பட்டியல்) |
...
| Patch embedding | "Page feature (பண்பு — தகவல் நெடுவரிசை)" | One vector per image patch from a VLM encoder, cached per page |
