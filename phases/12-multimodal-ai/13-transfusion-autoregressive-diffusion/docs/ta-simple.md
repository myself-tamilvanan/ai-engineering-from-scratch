...
Transfusion asks: can we have both? Keep images continuous, still train one model, use two losses stitched into one gradient (சாய்வு — மாற்றத்தின் दिशा) step.
...
- Image patches (continuous, 16x16 pixel blocks projected into hidden dim via linear embedding (embedding — பொருளை எண்களாக மாற்றுதல்) — same as a ViT encoder's input).
...
The gradient (சாய்வு — மாற்றத்தின் दिशा) flows through the shared transformer body. Both losses improve the shared weights simultaneously.
...
This lesson produces `outputs/skill-two-loss-trainer-designer.md`. Given a new multimodal training task (text + image, text + audio, text + video), it designs the two-loss schedule (loss weights, mask shape, shared vs modality-specific blocks) and flags how to build risks.
...
3. MMDiT has modality-specific QKV weights. What parameter (பாரமீட்டர் — கட்டுப்பாடு) count extra work does this add vs Transfusion's fully-shared transformer? At 7B params, is it worth it?
...
|------|-----------------|------------------------|
| Two-loss training | "NTP + diffusion" | A single transformer optimizes both cross-entropy on text tokens and MSE on continuous image patches in the same gradient (சாய்வு — மாற்றத்தின் दिशा) step |
...
| Continuous image representation | "No VQ" | Image patches as real-valued vector (வெக்டர் — எண் பட்டியல்)s, not integer codebook indices |
| Velocity prediction | "v-parameter (பாரமீட்டர் — கட்டுப்பாடு)ization" | Network output is the velocity field between noise and data, not the noise itself |
