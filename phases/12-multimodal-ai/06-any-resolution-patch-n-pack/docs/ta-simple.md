...
- Pack patches from a batch (பத்தி — தொகுப்பு) of variable-resolution images into one sequence and build the block-diagonal attention mask.
...
Transformers expect a sequence. A batch (பத்தி — தொகுப்பு) is a stack of sequences the same length. If your images are 224x224, you get 196 patch tokens every time, padding not required, job done. Train on 224, infer on 224, never think about resolution again.
...
The 2024-2025 answer: let the transformer eat patches at the image's native resolution, and figure out how to pack a heterogeneous batch (பத்தி — தொகுப்பு) into one sequence without wasted compute.
...
1. For each image in the batch (பத்தி — தொகுப்பு), compute its native patch grid at a chosen patch size (say 14).
...
5. Carry per-patch position information (2D RoPE or fractional position embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s).

A batch (பத்தி — தொகுப்பு) of three images at 336x336 (576 tokens), 224x224 (256 tokens), and 448x336 (768 tokens) becomes one 1600-token sequence with a 1600x1600 block-diagonal mask. No padding. No wasted compute. The transformer handles arbitrary aspect ratios.

NaViT also introduced fractional patch dropping during training — drop 50% of patches at random across the batch (பத்தி — தொகுப்பு) — which both regularizes and speeds training. SigLIP 2 inherited this.
...
Qwen2-VL introduced Multimodal Rotary Position embedding (embedding — பொருளை எண்களாக மாற்றுதல்). Instead of NaViT's fractional positions or AnyRes's tile-and-thumbnail, each patch carries a 3D position (temporal, height, width). The query/key rotations handle arbitrary H, W, and temporal length.
...
Unlike AnyRes, M-RoPE is O(H x W / P^2) tokens at native resolution — no multiplicative tile extra work. Unlike NaViT, it still expects a single image per forward. batch (பத்தி — தொகுப்பு)ing across resolutions still needs patch-n'-pack on top.
...
For a semantic task (classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்), retrieval), 256 tokens. For OCR or chart understanding, 1024 tokens. No retraining.
...
This is one line in PyTorch with `torch.block_diag` or an explicit gather. FlashAttention's variable-length path (`cu_seqlens`) skips the mask entirely and attends within sequences using the cumulative-length tensor (டென்சர் — பல வடிவ எண் வரிசை) directly — ~10x faster than a dense mask for typical batch (பத்தி — தொகுப்பு)es.
...
The 2026 production rule: pick a per-task max-pixels cap, encode at native aspect ratio up to that cap, pack the batch (பத்தி — தொகுப்பு), and skip padding. Qwen2.5-VL exposes `min_pixels` and `max_pixels` for exactly this knob.

## Use It

`code/main.py` implements patch-n'-pack for a heterogeneous batch (பத்தி — தொகுப்பு) of images with integer pixel coordinates. It:
...
- Prints a token budget table for a mixed batch (பத்தி — தொகுப்பு) (receipt, chart, screenshot, photo).
...
2. Build the block-diagonal mask for a batch (பத்தி — தொகுப்பு) of four images with lengths 256, 576, 729, 1024. Verify the attention matrix (மேட்ரிக்ஸ் — எண் மேஜை) is 2585x2585 and has exactly `256^2 + 576^2 + 729^2 + 1024^2` non-zero entries.
...
|------|-----------------|------------------------|
| Patch-n'-pack | "NaViT-style packing" | Concatenate variable-length patch sequences from different images into one batch (பத்தி — தொகுப்பு) dimension (பரிமாணம் — கோணம்) |
...
| cu_seqlens | "FlashAttention packing" | Cumulative-length tensor (டென்சர் — பல வடிவ எண் வரிசை) the FlashAttention varlen path uses instead of a dense block-diagonal mask |
