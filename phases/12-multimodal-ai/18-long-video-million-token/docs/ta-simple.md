...
Engineering: a custom attention how to build with memory hierarchy (local + global + sparse) plus MoE expert routing for long-context speed. Not published in full detail. Not open-source.
...
Token compression trades off recall at specific timestamps for grows big. The model knows generally what happened but sometimes misses exact frames.
