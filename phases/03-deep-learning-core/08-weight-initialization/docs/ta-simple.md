...
**Type:** Build
**Languages:** Python
**Prerequisites:** Lesson 03.04 (Activation function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s), Lesson 03.07 (Regularization)
**Time:** ~90 minutes
...
- Match the correct initialization strategy to the activation function (சார்பு — உள்ளீட்டுக்கு வெளியீடு): Xavier for sigmoid/tanh, Kaiming for ReLU/GELU

## The Problem

Initialize all weights to zero. Nothing learns. Every neuron computes the same function (சார்பு — உள்ளீட்டுக்கு வெளியீடு), receives the same gradient (சாய்வு — மாற்றத்தின் दिशा), and updates identically. After 10,000 epoch (யுக் — ஒரு முழு சுழற்சி)s, your 512-neuron hidden layer is still 512 copies of the same neuron. You paid for 512 parameter (பாரமீட்டர் — கட்டுப்பாடு)s and got 1.

Initialize them too large. Activations explode through the network. By layer 10, values hit 1e15. By layer 20, they overflow to infinity. gradient (சாய்வு — மாற்றத்தின் दिशा)s follow the same trajectory in reverse.
...
Weight initialization is the most underrated decision in deep learning (ஆழ் கற்றல் — பல்லாய கொட்ட பயிற்சி). Architecture gets papers. Optimizers get blog posts. Initialization gets a footnote. But get it wrong and nothing else matters -- your network is dead before training begins.

## The Concept

### The Symmetry Problem

Every neuron in a layer has the same structure: multiply inputs by weights, add bias, apply activation. If all weights start at the same value (zero is the extreme case), every neuron computes the same output. During backpropagation (பின்னாக பரப்புதல் — தவறிலிருந்து கற்றுக்கொள்ளுதல்), every neuron receives the same gradient (சாய்வு — மாற்றத்தின் दिशा). During the update step, every neuron changes by the same amount.

You're stuck. The network has hundreds of parameter (பாரமீட்டர் — கட்டுப்பாடு)s, but they all move in lockstep. This is called symmetry, and random initialization is the brute-force way to break it. Each neuron starts at a different point in weight space, so each learns a different feature (பண்பு — தகவல் நெடுவரிசை).
...
Llama 3 (405B parameter (பாரமீட்டர் — கட்டுப்பாடு)s, 126 layers) uses a similar scheme. Without this scaling, the residual stream would grow unbounded through 126 layers of attention and feedforward blocks.
...
Four ways to initialize a weight matrix (மேட்ரிக்ஸ் — எண் மேஜை). Each returns a list of lists (a 2D matrix) with fan_in columns and fan_out rows.
...
### Step 2: Activation function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s
...
def symmetry_demo():
    ...
    print(f"  Effective parameter (பாரமீட்டர் — கட்டுப்பாடு)s: 1 (not {len(weights) * len(weights[0])})")
```
...
PyTorch provides these as built-in function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s:
...
For transformers, HuggingFace models usually handle initialization in their `_init_weights` method. GPT-2's how to build scales residual projections by 1/sqrt(N). If you're building a transformer from scratch, you need to add this yourself.
...
3. Create an "init health check" function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) that takes a network's layer dimension (பரிமாணம் — கோணம்)s and activation type, then recommends the correct initialization and warns if the current init will cause problems.
...
5. Implement orthogonal initialization (generate a random matrix (மேட்ரிக்ஸ் — எண் மேஜை), compute its SVD, use the orthogonal matrix U). Compare to Kaiming for ReLU networks at 50 layers.
...
| Symmetry breaking | "Make neurons different" | Using random initialization to ensure neurons learn distinct feature (பண்பு — தகவல் நெடுவரிசை)s instead of computing identical function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s |
...
| Fan-out | "Number of outputs from a neuron" | The number of outgoing connections, relevant for maintaining gradient (சாய்வு — மாற்றத்தின் दिशा) variance during backpropagation (பின்னாக பரப்புதல் — தவறிலிருந்து கற்றுக்கொள்ளுதல்) |
...
- Glorot & Bengio, "Understanding the difficulty of training deep feedforward neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)s" (2010) -- the original Xavier initialization paper with variance analysis
