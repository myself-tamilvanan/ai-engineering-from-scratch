...
> The learning rate is the single most important hyperparameter (பாரமீட்டர் — கட்டுப்பாடு). Not the architecture. Not the dataset size. Not the activation function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). The learning rate. If you tune nothing else, tune this.
...
- Compare convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) speed across all five schedules on the same task and select the appropriate one for a given training budget

## The Problem

Set the learning rate to 0.1. Training diverges -- loss jumps to infinity in 3 steps. Set it to 0.0001. Training crawls -- after 100 epoch (யுக் — ஒரு முழு சுழற்சி)s, the model has barely moved from random. Set it to 0.01. Training works for 50 epochs, then the loss oscillates around a minimum it can never reach because the steps are too large.
...
Every major model published in the last three years uses a learning rate schedule. Llama 3 used peak lr=3e-4 with 2000 warmup steps and cosine decay to 3e-5. GPT-3 used lr=6e-4 with warmup over 375 million tokens. These are not arbitrary choices. They are the result of extensive hyperparameter (பாரமீட்டர் — கட்டுப்பாடு) sweeps that cost millions of dollars.

You need to understand schedules because the defaults will not work for your problem. When you fine-tune a pretrained model, the right schedule is different than training from scratch. When you increase batch (பத்தி — தொகுப்பு) size, the warmup period needs to change. When training breaks at step 10,000, you need to know whether it's a schedule problem or something else.
...
The old-school approach from the ResNet era. Cut the learning rate by a factor (usually 10x) at fixed epoch (யுக் — ஒரு முழு சுழற்சி)s.

```
lr(t) = lr_0 * gamma^(floor(epoch (யுக் — ஒரு முழு சுழற்சி) / step_size))
```

Where gamma = 0.1 and step_size = 30 means: lr drops by 10x every 30 epoch (யுக் — ஒரு முழு சுழற்சி)s. ResNet-50 used this -- lr=0.1, drop by 10x at epochs 30, 60, and 90.
...
This is the default for most modern training runs. No hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s to tune beyond lr_max and lr_min. The cosine shape matches the empirical observation that most learning happens in the middle of training -- you want reasonable step sizes during that critical period.

### Warmup: Why You Start Small

Adam and other adaptive optimizers maintain running estimates of gradient (சாய்வு — மாற்றத்தின் दिशा) mean and variance. At step 0, these estimates are initialized to zero. The first few gradient updates are based on garbage statistics. If your learning rate is large during this period, the model takes huge, poorly-directed steps.
...
The theory: a high learning rate acts as regularization by adding noise to the optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) trajectory. The model explores more of the loss landscape during the ramp-up phase, finding better basins. The ramp-down phase then refines within the best basin found.
...
1cycle often trains faster than cosine annealing for a fixed compute budget. The choice: you must know the total number of steps in advance.
...
flowchart TD
    ...
    Budget -->|"Small (hours)"| OneCycle["1cycle Policy<br/>(fastest convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்))"]
...
### Step 1: Schedule function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s

Each function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) takes the current step and returns the learning rate at that step.
...
def train_with_schedule(schedule_fn, schedule_name, data, epoch (யுக் — ஒரு முழு சுழற்சி)s=300, base_lr=0.05, **kwargs):
    random.seed(0)
    hidden_size = 8
    ...
    step = 0
    epoch (யுக் — ஒரு முழு சுழற்சி)_losses = []
    ...
    for epoch (யுக் — ஒரு முழு சுழற்சி) in range(epochs):
        ...
        epoch (யுக் — ஒரு முழு சுழற்சி)_losses.append(avg_loss)

    return epoch (யுக் — ஒரு முழு சுழற்சி)_losses
```

### Step 4: Compare All Schedules

Train the same network with each schedule and compare final loss and convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) behavior.
...
def compare_schedules(data):
    ...
    for name, schedule_fn, extra_kwargs in configs:
        losses = train_with_schedule(schedule_fn, name, data, epoch (யுக் — ஒரு முழு சுழற்சி)s=300, base_lr=0.05, **extra_kwargs)
...
def lr_sensitivity(data):
    ...
    print("\nLR Sensitivity (constant schedule, 100 epoch (யுக் — ஒரு முழு சுழற்சி)s):")
    print(f"  {'LR':>10} {'Start Loss':>12} {'End Loss':>12} {'Status':>15}")
    print("  " + "-" * 52)

    for lr in learning_rates:
        losses = train_with_schedule(constant_schedule, f"lr={lr}", data, epoch (யுக் — ஒரு முழு சுழற்சி)s=100, base_lr=lr)
        start = losses[0]
        end = losses[-1]
...
optimizer = optim.Adam(model.parameter (பாரமீட்டர் — கட்டுப்பாடு)s(), lr=3e-4)
...
The HuggingFace function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) is what most Llama and GPT fine-tuning scripts use. When in doubt, use warmup + cosine with warmup = 3-5% of total steps. It works for almost everything.

## Ship It

This lesson produces:
- `outputs/prompt-lr-schedule-advisor.md` -- a prompt that recommends the right learning rate schedule and hyperparameter (பாரமீட்டர் — கட்டுப்பாடு)s for your training setup
...
|------|----------------|----------------------|
| Learning rate | "How fast the model learns" | The scalar (scalar — ஒரு எண்) that multiplies the gradient (சாய்வு — மாற்றத்தின் दिशा) to determine the parameter (பாரமீட்டர் — கட்டுப்பாடு) update size |
| Schedule | "Change the LR over time" | A function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) that maps training step to learning rate, designed to optimize convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) |
...
| Step decay | "Drop LR at milestones" | Multiplying the LR by a factor (usually 0.1) at fixed epoch (யுக் — ஒரு முழு சுழற்சி) intervals |
...
| Peak learning rate | "The maximum LR" | The highest LR reached during training, usually after warmup |

## Further Reading

- Loshchilov & Hutter, "SGDR: Stochastic gradient (சாய்வு — மாற்றத்தின் दिशा) Descent with Warm Restarts" (2017) -- introduced cosine annealing and warm restarts
- Smith, "Super-convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்): Very Fast Training of neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)s Using Large Learning Rates" (2018) -- the 1cycle policy paper
- Touvron et al., "Llama 2: Open Foundation and Fine-Tuned Chat Models" (2023) -- documents the warmup + cosine schedule used at scale
- Goyal et al., "Accurate, Large Minibatch (பத்தி — தொகுப்பு) SGD: Training ImageNet in 1 Hour" (2017) -- linear scaling rule and warmup for large batch training
