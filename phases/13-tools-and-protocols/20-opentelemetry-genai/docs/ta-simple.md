...
- `gen_ai.operation.name` — `"chat"`, `"text_completion"`, `"embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s"`, `"execute_tool"`, `"invoke_agent"`.
...
Token-level events is added as span events:
