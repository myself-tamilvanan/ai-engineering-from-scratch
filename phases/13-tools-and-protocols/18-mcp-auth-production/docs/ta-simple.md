...
- Read an IdP capability matrix (மேட்ரிக்ஸ் — எண் மேஜை) and refuse to deploy when the IdP cannot satisfy MCP's auth profile.
...
- On 401/403, return `WWW-Authenticate: Bearer` carrying `error=...`, the `resource_metadata="<PRM-URL>"` parameter (பாரமீட்டர் — கட்டுப்பாடு) (the URL of the metadata document, *not* the bare resource), and `scope="..."` on `insufficient_scope` (403). Note: the parameter is `resource_metadata`, a discovery pointer — there is no `resource` parameter in the challenge.
...
### IdP capability matrix (மேட்ரிக்ஸ் — எண் மேஜை)

Not every IdP supports the full MCP profile. The matrix (மேட்ரிக்ஸ் — எண் மேஜை) below documents factual capability statements as of the 2025-11-25 spec. It is a *deployment gate*, not a recommendation.
...
2. On the authorization response, the client compares the returned `iss` parameter (பாரமீட்டர் — கட்டுப்பாடு) against that recorded issuer (simple string comparison, no normalization) before sending the code anywhere.
...
- **Mix-up via missing `iss` check.** A client that does not validate the RFC 9207 `iss` authorization-response parameter (பாரமீட்டர் — கட்டுப்பாடு) against the issuer it recorded before redirecting is steered into redeeming an honest AS's code at an attacker's token endpoint. This is a client-side failure. the resource server cannot compensate for it.
...
| Resource indicator | "Audience parameter (பாரமீட்டர் — கட்டுப்பாடு)" | RFC 8707 `resource` parameter pinning the token to one server |
...
- [RFC 9207 — OAuth 2.0 Authorization Server Issuer Identification](https://datatracker.ietf.org/doc/html/rfc9207) — the `iss` parameter (பாரமீட்டர் — கட்டுப்பாடு) that defends against mix-up attacks
