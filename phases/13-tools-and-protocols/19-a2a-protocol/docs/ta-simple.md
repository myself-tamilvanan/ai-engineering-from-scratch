...
Artifacts is streamed as chunks. The caller accumulates.
...
Rationale: A2A enables competitors to collaborate without revealing internals. A2A is "call this customer-service agent" without the caller learning how that agent implements the service.
...
| dimension (பரிமாணம் — கோணம்) | MCP | A2A |
|-----------|-----|-----|
...
5. Read the A2A v1.0 announcement and identify the one feature (பண்பு — தகவல் நெடுவரிசை) that is not yet implemented by any framework as of April 2026. (Hint: it relates to multi-hop task delegation.)
