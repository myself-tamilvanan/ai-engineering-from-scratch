...
**Prerequisites:** Phase 13 · 02 (function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) calling), Phase 13 · 17 (gateways)
**Time:** ~45 minutes
...
3. **Latency.** A live chat UI needs fast time-to-first-token. A batch (பத்தி — தொகுப்பு) summarizer does not. Route by latency SLA.
...
Identical-or-near-identical prompts hit a cache instead of the provider. Savings on repeated agent loops is 30 to 60 percent. Keys are embedding (embedding — பொருளை எண்களாக மாற்றுதல்)-based. near-identical prompts share a cache slot.
...
5. Read LiteLLM, OpenRouter, and Portkey docs side by side. Name the one feature (பண்பு — தகவல் நெடுவரிசை) each ships that the other two do not.
...
| Semantic caching | "Prompt-embedding (embedding — பொருளை எண்களாக மாற்றுதல்) cache" | Key is embedding of the prompt. near-duplicates share a cache hit |
