...
> Real agent work takes minutes to hours: CI runs, deep-research synthesis, batch (பத்தி — தொகுப்பு) exports. Synchronous tool calls drop connections, time out, or block the UI. SEP-1686, merged in 2025-11-25, adds a Tasks primitive: any request is augmented to become a task, and the result is fetched later or streamed via state notifications. Drift-risk note: Tasks are experimental through H1 2026. SDK surface is still being designed around the spec.
...
None are good. SEP-1686 adds a fourth: task augmentation. Any request (usually `tools/call`) is tagged as a task. The server returns a task id immediately. The client polls `tasks/status` and fetches `tasks/result` when done. Server-side state survives restarts.
...
Clients that stream quite than poll get better UX. Polling is always supported as the minimal surface.
...
- [FastMCP — Tasks](https://gofastmcp.com/servers/tasks) — SDK-level task how to build patterns
