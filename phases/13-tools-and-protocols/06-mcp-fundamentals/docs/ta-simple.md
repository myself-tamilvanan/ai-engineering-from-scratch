...
**Prerequisites:** Phase 13 · 01 through 05 (the tool interface and function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) calling)
**Time:** ~45 minutes
...
This is what prevents ecosystem drift. A client that does not support sampling is still a valid MCP client. a server that does not call `sampling` is still a valid MCP server. They just do not use that feature (பண்பு — தகவல் நெடுவரிசை) together.
...
4. Sketch on paper the primitive a hypothetical "cron job" feature (பண்பு — தகவல் நெடுவரிசை) would belong to. (Hint: the server wants the client to invoke it at a scheduled time. None of the six primitives fit today.) MCP's 2026 roadmap has a draft SEP for this.
...
| `initialize` handshake | "Capability negotiation" | First message pair. servers and clients declare feature (பண்பு — தகவல் நெடுவரிசை)s they support |
