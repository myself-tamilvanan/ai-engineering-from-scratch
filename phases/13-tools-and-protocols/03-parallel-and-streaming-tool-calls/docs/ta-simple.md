...
**Prerequisites:** Phase 13 · 02 (function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) calling deep dive)
**Time:** ~75 minutes
...
- Run a three-city weather benchmark that shows sequential vs parallel latency.
...
- **Gemini.** Always parallel-capable. `tool_config.function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)_calling_config.mode = "AUTO"` lets the model decide.
...
- **Gemini.** `id` on each `function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)Response` (Gemini 3 and up. Gemini 2 matched by name which broke for same-name parallel calls).
...
- **OpenAI.** Each chunk is `choices[0].delta.tool_calls[i].function (சார்பு — உள்ளீட்டுக்கு வெளியீடு).arguments` (partial string). The chunk carries `index` (position in the call list). You accumulate per-index, read `id` when it first appears, and parse JSON when `finish_reason = "tool_calls"`.
...
Real-world warning: parallel calls stress downstream APIs. A 10-way fan-out to a rate-limited service will fail. Phase 13 · 17 covers gateway-level backpressure. retry semantics are planned for a future phase.

### Streaming fan-out wall-clock

If the model itself streams, you can start executing as soon as one call's arguments are complete, quite than waiting for all calls to finalize. This is an optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) OpenAI documents but not all SDKs expose. The harness in this lesson does it: as soon as the simulated stream yields a complete argument object, the host kicks off that call.
...
1. Run `code/main.py` and vary the simulated latencies. Confirm that the parallel-to-sequential ratio is approximately `max/sum` (real runs deviate slightly from the ideal because of thread scheduling, serialization, and harness extra work). At what latency distribution does parallel stop mattering?
...
5. Read OpenAI's parallel-function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)-calling section and Anthropic's `disable_parallel_tool_use` docs. Identify the one real-world tool type where Anthropic recommends disabling parallelism. (Hint: consequential mutations on the same resource.)
...
| `streamfunction (சார்பு — உள்ளீட்டுக்கு வெளியீடு)CallArguments` | "Gemini 3 feature (பண்பு — தகவல் நெடுவரிசை)" | Streamed argument chunks with unique id per call |
| Completion-order reply | "Don't wait for all" | Reply with results as they arrive, keyed by id |

## Further Reading

- [OpenAI — Parallel function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) calling](https://platform.openai.com/docs/guides/function-calling#parallel-function-calling) — default behavior and the opt-out flag
- [Anthropic — Tool use: implementing tool use](https://docs.anthropic.com/en/docs/agents-and-tools/tool-use/implementing-tool-use) — `disable_parallel_tool_use` and result batch (பத்தி — தொகுப்பு)ing
