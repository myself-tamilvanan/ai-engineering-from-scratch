...
- **Scope escalation.** A notes server granted `notes:read` can later need `notes:write` for a specific action. Instead of re-doing the whole flow, step-up (SEP-835) asks for the extra scope.
...
In MCP's profile, resource and authorization servers is the same host but SHOULD be distinguished by URLs.
...
`resource` parameter (பாரமீட்டர் — கட்டுப்பாடு) in the token request pins the token's intended audience. The issued token contains `aud: "https://notes.example.com"`. Another MCP server receiving this token checks `aud` and rejects it.
...
Client sees the insufficient_scope error, prompts the user with a consent dialog for the extra scope, performs a mini OAuth flow for it, retries the request with the new token.
...
| Resource indicator | "Token audience" | RFC 8707 `resource` parameter (பாரமீட்டர் — கட்டுப்பாடு) pinning token to one server |
