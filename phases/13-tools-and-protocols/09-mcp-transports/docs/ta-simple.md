...
- **POST /mcp.** Client sends a JSON-RPC message. Server replies with either a single JSON response, or an SSE stream of one-or-more responses (useful for batch (பத்தி — தொகுப்பு)ed responses and notifications related to that request).
...
The 2025-11-25 spec requires servers to reject requests whose `Origin` is not on an allowlist. The allowlist usually contains the MCP client host (`https://claude.ai`, `vscode-webview://*`) and localhost variants for local UIs.
...
4. Session is revoked by the server. client sees 404 on subsequent requests and must re-initialize.
