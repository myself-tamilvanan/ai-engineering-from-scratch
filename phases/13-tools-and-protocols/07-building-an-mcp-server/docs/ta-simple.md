...
- Emit structured error responses per the JSON-RPC 2.0 spec and MCP's extra codes.
- Graduate a stdlib how to build to FastMCP (Python SDK) or the TypeScript SDK without rewriting tool logic.
...
Declare only what you support. The client relies on the capability set to gate feature (பண்பு — தகவல் நெடுவரிசை)s.
...
Resources are read-only by design. `resources/list` returns a manifest. `resources/read` returns the content. URIs is `file://...`, `http://...`, or a custom scheme like `notes://`.
...
| Dispatcher | "Method router" | Map of JSON-RPC method name to handler function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) |
...
| Capability declaration | "feature (பண்பு — தகவல் நெடுவரிசை) toggle" | Per-primitive flags declared in `initialize` |

## Further Reading

- [Model Context Protocol — Python SDK](https://github.com/modelcontextprotocol/python-sdk) — the reference Python how to build
- [Model Context Protocol — TypeScript SDK](https://github.com/modelcontextprotocol/typescript-sdk) — parallel TS how to build
