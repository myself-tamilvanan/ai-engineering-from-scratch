...
}, "https://host.example.com"). // host to iframe  (pin to iframe origin)
iframe.contentWindow.postMessage({
  jsonrpc: "2.0",
  id: 1,
  result: { content: [...] }
}, "https://iframe.example.com"). // receiver on both sides
window.addEventListener("message", (event) => {
  if (event.origin !== "https://expected-peer.example.com") return. // safe to process event.data
}). ```
...
5. Read the SEP-1724 spec and identify one capability in the MCP Apps SDK that this toy how to build does not use. (Hint: component-level state sync.)
...
- [MCP ext-apps — GitHub](https://github.com/modelcontextprotocol/ext-apps) — reference how to build and SDK
