...
Constrained decoding works on small models. A 3B-parameter (பாரமீட்டர் — கட்டுப்பாடு) open model with grammar enforcement out-performs a 70B-parameter model with raw prompting on structured tasks. This is the main reason structured outputs matter for production: it decouples reliability from model size.
