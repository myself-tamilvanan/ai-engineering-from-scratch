...
2. Group by label: feature (பண்பு — தகவல் நெடுவரிசை), fix, chore, docs.
...
- Progressive disclosure demoed via a `read_subresource` function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) that pulls referenced files on demand.
...
5. Read the Anthropic Agent Skills blog post. Identify one feature (பண்பு — தகவல் நெடுவரிசை) in the Claude Agent SDK that this lesson's loader does not cover. (Hint: agent sub-invocation.)
