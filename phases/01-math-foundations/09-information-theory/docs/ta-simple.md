...
> Information theory measures surprise. loss function (சேதத் சூத்திரம் — எவ்வளவு தவறு என்பதை அளக்கும் சூத்திரம்) (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s are built on it.
...
- Calculate mutual information between feature (பண்பு — தகவல் நெடுவரிசை)s and a target to rank feature importance
- Explain perplexity as the effective vocabulary size a language model chooses from

## The Problem

You call `CrossEntropyLoss()` in every classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) model you train. You see "perplexity" in every language model paper. You read about KL divergence in VAEs, distillation, and RLHF. These are not disconnected concepts. They are all the same idea wearing different hats.

Information theory gives you the language to reason about uncertainty, compression, and prediction. Claude Shannon invented it in 1948 to solve communication problems. Turns out, training a neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு) is a communication problem: the model is trying to transmit the correct label through a noisy channel of learned weights.
...
### Cross-Entropy (The loss function (சேதத் சூத்திரம் — எவ்வளவு தவறு என்பதை அளக்கும் சூத்திரம்) (சார்பு — உள்ளீட்டுக்கு வெளியீடு) You Use Every Day)
...
In classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்), P is a one-hot vector (வெக்டர் — எண் பட்டியல்) (the true class has probability 1, everything else 0). This simplifies cross-entropy to:

```
H(P, Q) = -log(q(true_class))
```

That is the entire cross-entropy loss formula for classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்). Maximize the predicted probability of the correct class.
...
In feature (பண்பு — தகவல் நெடுவரிசை) selection, high mutual information between a feature and the target means the feature is useful. Low mutual information means it is noise.
...
In machine learning, conditional entropy appears in decision trees. At each split, the algorithm (அல்கோரிதம் — வழிமுறை) picks the feature (பண்பு — தகவல் நெடுவரிசை) X that minimizes H(Y|X) -- the feature that removes the most uncertainty about the label Y.
...
**Mutual information for feature (பண்பு — தகவல் நெடுவரிசை) selection.** In ML, you want features that are informative about the target. Mutual information gives you a principled way to rank features:

1. For each feature (பண்பு — தகவல் நெடுவரிசை) X_i, compute I(X_i. Y) where Y is the target variable.
2. Rank features by MI score.
3. Keep the top k features.

This works for any relationship between feature (பண்பு — தகவல் நெடுவரிசை) and target -- linear, nonlinear, monotonic, or not. Correlation only catches linear relationships. MI catches everything.
...
Standard classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) uses hard targets: [0, 0, 1, 0]. The true class gets probability 1, everything else gets 0. Label smoothing replaces these with soft targets:
...
### Why Cross-Entropy Is THE classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) Loss
...
**gradient (சாய்வு — மாற்றத்தின் दिशा) view.** The gradient of cross-entropy with respect to the logits is simply (predicted - true). Clean, stable, and fast to compute. This is why it pairs perfectly with softmax.
...
1 nat = 1/ln(2) bits = 1.4427 bits. PyTorch and tensor (டென்சர் — பல வடிவ எண் வரிசை)Flow use natural log (nats) by default.
...
### Step 3: Cross-entropy as classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) loss
...
2. A model outputs logits [5.0, 2.0, 0.5] for a sample with true class 1. Compute the cross-entropy loss by hand, then verify with your `cross_entropy_loss` function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). What logits would give zero loss?
...
4. Build a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) that computes perplexity for a sequence of token predictions. Given a list of (true_token_index, predicted_logits) pairs, return the perplexity of the sequence.
...
| Cross-entropy | "The loss function (சேதத் சூத்திரம் — எவ்வளவு தவறு என்பதை அளக்கும் சூத்திரம்) (சார்பு — உள்ளீட்டுக்கு வெளியீடு)" | Average surprise when using model distribution Q to encode events from true distribution P. |
...
| Softmax | "Turn logits into probabilities" | Exponentiate and normalize. Maps any real-valued vector (வெக்டர் — எண் பட்டியல்) to a valid probability distribution. |
...
| Nats | "ML's unit" | Information measured with natural log. Used by PyTorch and tensor (டென்சர் — பல வடிவ எண் வரிசை)Flow by default. |
