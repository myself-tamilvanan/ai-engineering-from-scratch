...
**The golden rule:** correlation does not imply causation. Ice cream sales and drowning deaths are correlated because both increase in summer. Your model's accuracy and the number of parameter (பாரமீட்டர் — கட்டுப்பாடு)s are correlated, but adding parameters does not automatically improve accuracy (see: overfitting).

### Covariance matrix (மேட்ரிக்ஸ் — எண் மேஜை)
...
For d feature (பண்பு — தகவல் நெடுவரிசை)s, the covariance matrix (மேட்ரிக்ஸ் — எண் மேஜை) C is a d x d matrix where C[i][j] = Cov(feature_i, feature_j). The diagonal entries C[i][i] are the variances of each feature.
...
**Connection to PCA.** PCA eigendecomposes the covariance matrix (மேட்ரிக்ஸ் — எண் மேஜை). The eigenvector (வெக்டர் — எண் பட்டியல்)s are the principal components (directions of maximum variance). The eigenvalues tell you how much variance each component captures. This is exactly what Lesson 10 covered, but now you see why the covariance matrix is the right thing to decompose: it encodes all pairwise linear relationships in your data.

**Connection to correlation.** The correlation matrix (மேட்ரிக்ஸ் — எண் மேஜை) is the covariance matrix of standardized variables (each divided by its standard deviation). Correlation normalizes covariance so all values fall in [-1, 1].
...
If p-value < alpha (usually 0.05):
    Reject H0. The result is "statistically big."
If p-value >= alpha:
    Fail to reject H0. You do not have enough evidence.
    This does NOT mean H0 is true.
```

**Confidence intervals** give a range of plausible values for a parameter (பாரமீட்டர் — கட்டுப்பாடு):
...
A result is statistically big but practically meaningless. With enough data, even a trivial difference becomes statistically big.
...
Statistically big? Yes.
Practically big? A 0.03% improvement is not worth the
...
When you test many hypotheses, some will be "big" by chance. If you test 20 things at alpha = 0.05, you expect 1 false positive even when nothing is real.
...
In ML, this matters when you compare a model across multiple metrics, test many hyperparameter (பாரமீட்டர் — கட்டுப்பாடு) configurations, or evaluate on multiple datasets.
...
4. Repeat B times (usually B = 1000 to 10000)
...
2. For each bootstrap iteration (மறுயுதல் — மீண்டும் செய்தல்):
   a. Resample test indices with replacement
   b. Compute metric_A and metric_B on the resampled set
   c. Store diff = metric_B - metric_A
3. 95% CI for the difference:
   [2.5th percentile of diffs, 97.5th percentile of diffs]
4. If the CI does not contain 0, the difference is big
```
...
In ML experiments, you usually have small n (5 or 10 cross-validation folds), so non-parametric tests like Wilcoxon signed-rank are often more appropriate than t-tests.
...
3. Mini-batch (பத்தி — தொகுப்பு) gradient (சாய்வு — மாற்றத்தின் दिशा) descent works because the average gradient
...
2. **Correlation function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s** (Pearson and Spearman, with the covariance matrix (மேட்ரிக்ஸ் — எண் மேஜை))
...
6. **Statistical vs practical significance demo** (showing that large n makes everything "big")
...
| Covariance matrix (மேட்ரிக்ஸ் — எண் மேஜை) | Matrix of pairwise covariances between all feature (பண்பு — தகவல் நெடுவரிசை)s. |
...
| Confidence interval | Range of plausible values for a parameter (பாரமீட்டர் — கட்டுப்பாடு) at a given confidence level. |
