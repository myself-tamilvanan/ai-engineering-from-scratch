...
This matters for ML because frequency-domain thinking appears everywhere. Convolutional neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)s perform convolution, which is multiplication in the frequency domain. Transformer positional encodings use frequency decomposition to represent position. Audio models (speech recognition, music generation) operate on spectrograms -- frequency representations of sound. Time series models look for periodic patterns. Understanding the Fourier transform gives you the vocabulary to work with all of these.
...
The Cooley-Tukey algorithm (அல்கோரிதம் — வழிமுறை) (the most common FFT) works by divide and conquer:
...
Each dimension (பரிமாணம் — கோணம்) pair (2i, 2i+1) oscillates at a different frequency. The frequencies are geometrically spaced from high (dimension 0,1) to low (last dimensions). This gives each position a unique pattern across all frequency bands -- similar to how Fourier coefficients uniquely identify a signal.
...
- **Relative position:** The encoding of position p+k is expressed as a linear function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) of the encoding at position p. The model can learn to attend to relative positions.
...
This is why analog-to-digital converters include anti-aliasing filters that remove frequencies above Nyquist before sampling. In ML, aliasing appears when downsampling feature (பண்பு — தகவல் நெடுவரிசை) maps without proper low-pass filtering -- some architectures address this with anti-aliased pooling layers.
...
True frequency resolution depends only on the observation time T = N / fs. To resolve two frequencies separated by delta_f, you need at least T = 1 / delta_f seconds of data. No amount of zero-padding changes this basic limit.
...
The spectrogram matrix (மேட்ரிக்ஸ் — எண் மேஜை) has shape (n_frequencies, n_time_frames). Each column is the power spectrum at one time window. This is what audio ML models consume as input.
...
2. **FFT vs DFT verification.** Generate a random signal of length 64. Compute both DFT (O(N^2)) and FFT. Verify that all coefficients match to within 1e-10. Time both function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s on signals of length 256, 512, 1024, and 2048. Plot the ratio of DFT time to FFT time.
...
| FFT (Fast Fourier Transform) | An O(N log N) algorithm (அல்கோரிதம் — வழிமுறை) to compute the DFT. The Cooley-Tukey algorithm splits even/odd indices recursively |
...
| Window function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) | A tapering function (Hann, Hamming, Blackman) applied before DFT to reduce spectral leakage |
| Twiddle factor | The complex exponential e^(-2*pi*i*k/N) used to combine sub-DFTs in the FFT butterfly calculation |
| Convolution theorem | Convolution in time domain equals pointwise multiplication in frequency domain. basic to signal processing and CNNs |
...
- [Cooley & Tukey: An algorithm (அல்கோரிதம் — வழிமுறை) for the Machine Calculation of Complex Fourier Series (1965)](https://www.ams.org/journals/mcom/1965-19-090/S0025-5718-1965-0178586-1/) - the original FFT paper that changed computing
