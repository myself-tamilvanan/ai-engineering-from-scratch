...
- Build a Naive Bayes text classifier from scratch with Laplace smoothing and log-space calculation
...
Naive Bayes extends this to multiple feature (பண்பு — தகவல் நெடுவரிசை)s by assuming all features are conditionally independent given the class:

```
P(class | feature (பண்பு — தகவல் நெடுவரிசை)_1, feature_2, ..., feature_n)
...
```
score(class) = P(class) * product of P(feature (பண்பு — தகவல் நெடுவரிசை)_i | class)
```
...
How do you get P(feature (பண்பு — தகவல் நெடுவரிசை)|class) from training data? Count.

```
P("free"|spam) = (number of spam emails containing "free") / (total spam emails)
```

This is MLE: choose the parameter (பாரமீட்டர் — கட்டுப்பாடு) values that make the observed data most likely. You are maximizing the likelihood function (சார்பு — உள்ளீட்டுக்கு வெளியீடு), which for discrete counts reduces to relative frequency.
...
MLE asks: what parameter (பாரமீட்டர் — கட்டுப்பாடு)s maximize P(data|parameters)?

MAP asks: what parameter (பாரமீட்டர் — கட்டுப்பாடு)s maximize P(parameters|data)?

By Bayes' theorem:

```
P(parameter (பாரமீட்டர் — கட்டுப்பாடு)s|data) proportional to P(data|parameters) * P(parameters)
```

MAP adds a prior over the parameter (பாரமீட்டர் — கட்டுப்பாடு)s themselves. If you believe parameters should be small, you encode that as a prior that penalizes large values. This is identical to L2 regularization in ML. The "ridge" penalty in ridge regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) is literally a Gaussian prior on the weights.
...
Frequentists treat parameter (பாரமீட்டர் — கட்டுப்பாடு)s as fixed unknowns. They ask: "If I repeated this experiment many times, what would happen?"

Bayesians treat parameter (பாரமீட்டர் — கட்டுப்பாடு)s as distributions. They ask: "Given what I have observed, what do I believe about the parameters?"
...
| Uncertainty | Confidence intervals (about procedure) | Credible intervals (about parameter (பாரமீட்டர் — கட்டுப்பாடு)) |
| Small data | Can overfit | Prior acts as regularization |
| calculation | Usually faster | Often requires sampling (MCMC) |
...
**Priors are regularization.** A Gaussian prior on weights is L2 regularization. A Laplace prior is L1. Every time you add a regularization term, you are making a Bayesian statement about what parameter (பாரமீட்டர் — கட்டுப்பாடு) values you expect.
...
### Step 1: Bayes theorem function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)
...
from sklearn.metrics import classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)_report
...
X_test = vector (வெக்டர் — எண் பட்டியல்)izer.transform(test_messages)
predictions = clf.predict(X_test)
for msg, pred in zip(test_messages, predictions):
    print(f"  '{msg}' -> {pred}")
```

Same algorithm (அல்கோரிதம் — வழிமுறை). Countvector (வெக்டர் — எண் பட்டியல்)izer handles tokenization and vocabulary building. MultinomialNB handles smoothing and log-probabilities internally. Your from-scratch version does the same thing in 40 lines.

## Ship It

The NaiveBayes class built here shows the full pipeline: tokenization, probability estimation with Laplace smoothing, log-space prediction. The code in `code/bayes.py` runs end-to-end with no dependencies beyond Python's standard library.
...
The Beta distribution is the most common conjugate prior in practice. Beta(a, b) represents your belief about a probability parameter (பாரமீட்டர் — கட்டுப்பாடு). The mean is a/(a+b). The larger a+b, the more concentrated (confident) the distribution.

Special cases of the Beta prior:
- Beta(1, 1) = uniform. You have no opinion about the parameter (பாரமீட்டர் — கட்டுப்பாடு).
...
The order of observations does not matter. Beta(1,1) updated with all 12 heads and 8 tails at once gives Beta(13, 9) -- the same result. Sequential updating and batch (பத்தி — தொகுப்பு) updating are mathematically equivalent. But sequential updating lets you make decisions at each step without storing raw data.
...
3. **Add feature (பண்பு — தகவல் நெடுவரிசை)s.** Extend the NaiveBayes class to also use message length (short/long) as a feature alongside word counts. Estimate P(short|spam) and P(short|ham) from the training data and fold it into the prediction score.
...
| Naive Bayes | "That simple text classifier" | A classifier that assumes feature (பண்பு — தகவல் நெடுவரிசை)s are independent given the class. Works well despite the false assumption. |
| Laplace smoothing | "Add-one smoothing" | Adding a small count to every feature to prevent zero probabilities from unseen data. |
| MLE | "Just use the frequencies" | Choose parameter (பாரமீட்டர் — கட்டுப்பாடு)s that maximize P(data\|parameters). No prior. Can overfit with small data. |
...
- [Stanford CS229: Generative Learning algorithm (அல்கோரிதம் — வழிமுறை)s](https://cs229.stanford.edu/notes2022fall/cs229-notes2.pdf) - naive Bayes and its connection to discriminative models
