...
2. Markov chains -- structured randomness with a transition matrix (மேட்ரிக்ஸ் — எண் மேஜை)
3. Langevin dynamics -- gradient (சாய்வு — மாற்றத்தின் दिशा) descent with noise
...
This sqrt(n) scaling shows up everywhere in ML. SGD noise scales as 1/sqrt(batch (பத்தி — தொகுப்பு)_size). embedding (embedding — பொருளை எண்களாக மாற்றுதல்) dimension (பரிமாணம் — கோணம்)s scale as sqrt(d). The square root is the signature of independent random additions.
...
This is the Markov property. It means you can describe the entire dynamics with a transition matrix (மேட்ரிக்ஸ் — எண் மேஜை) P:
...
Start in any state. After many transitions, the distribution of states converges to the stationary distribution pi, where pi * P = pi. This is the left eigenvector (வெக்டர் — எண் பட்டியல்) of P with eigenvalue 1.
...
1. **Power method**: multiply any initial distribution by P repeatedly. After enough iteration (மறுயுதல் — மீண்டும் செய்தல்)s, it converges.
2. **Eigenvalue method**: find the left eigenvector (வெக்டர் — எண் பட்டியல்) of P with eigenvalue 1. This is the eigenvector of P^T with eigenvalue 1.

Both approaches require the chain to satisfy convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) conditions.

**convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) conditions.** A Markov chain converges to a unique stationary distribution if it is:
...
Brownian motion is continuous but nowhere differentiable -- it jiggles at every scale. The path has fractal dimension (பரிமாணம் — கோணம்) 2 in the plane.
...
gradient (சாய்வு — மாற்றத்தின் दिशा) descent finds the minimum of a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). Langevin dynamics finds the probability distribution proportional to exp(-U(x)/T), where U is an energy function and T is temperature.

```
x_{t+1} = x_t - dt * gradient (சாய்வு — மாற்றத்தின் दिशा)(U(x_t)) + sqrt(2 * T * dt) * z_t
```

Two forces act on the particle:
1. **gradient (சாய்வு — மாற்றத்தின் दिशा) force** (-dt * gradient(U)): pushes toward low energy (like gradient descent)
2. **Random force** (sqrt(2*T*dt) * z): pushes in random directions (exploration)

At temperature T = 0, this is pure gradient (சாய்வு — மாற்றத்தின் दिशा) descent. At high temperature, it is nearly a random walk. At the right temperature, the particle explores the energy landscape and spends more time in low-energy regions.
...
The reverse process -- going from noise back to data -- is also a Markov chain, but its transition probabilities are learned by a neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு). The network learns to predict the noise that was added at each step, then subtracts it.
...
The chain is guaranteed to converge to p(x) under mild conditions. But convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) is slow if the proposal is too small (random walk) or too large (high rejection). Tuning the proposal is the art of MCMC.
...
- **Multiple chains**: run several chains from different starting points. If they converge to the same distribution, you have evidence of convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்).
- **Acceptance rate**: for Gaussian proposals in d dimension (பரிமாணம் — கோணம்)s, the optimal acceptance rate is about 23% (Roberts & Rosenthal, 2001). Too high means the chain barely moves. Too low means it rejects everything.

### Stochastic Processes in AI

| Process | AI Application |
|---------|---------------|
| Random walk | Exploration in RL, Node2Vec embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s |
...
```python
class MarkovChain:
    def __init__(self, transition_matrix (மேட்ரிக்ஸ் — எண் மேஜை), state_names=None):
    ...
    def stationary_distribution(self):
        eigenvalues, eigenvector (வெக்டர் — எண் பட்டியல்)s = np.linalg.eig(self.P.T)
...
The stationary distribution is the left eigenvector (வெக்டர் — எண் பட்டியல்) of P with eigenvalue 1. We find it by computing eigenvectors of P^T (transposing turns left eigenvectors into right eigenvectors).
...
The gradient (சாய்வு — மாற்றத்தின் दिशा) pushes x toward low energy. The noise prevents it from getting stuck. At equilibrium, the distribution of samples is proportional to exp(-U(x)/temperature).
...
The algorithm (அல்கோரிதம் — வழிமுறை) proposes a new point, checks if it has higher probability (or accepts with probability proportional to the ratio), and repeats. The acceptance rate should be around 23-50% for good mixing.

## Use It

In practice, you use established libraries for these algorithm (அல்கோரிதம் — வழிமுறை)s. But understanding the mechanics matters for debugging and tuning.
...
Multiply the initial distribution by P repeatedly. After enough iteration (மறுயுதல் — மீண்டும் செய்தல்)s, it converges to the stationary distribution regardless of where you started. This is the power method for finding the dominant left eigenvector (வெக்டர் — எண் பட்டியல்).
...
- **Gymnasium (RL):** The environment step function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) defines a Markov decision process

### Verifying Markov chain convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்)
...
|---------|------------------|
| Random walk | Node2Vec graph embedding (embedding — பொருளை எண்களாக மாற்றுதல்)s, exploration in RL |
| Markov chain | Token generation in LLMs, MCMC sampling |
| Brownian motion | Forward diffusion process in DDPM, SDE-based models |
| Langevin dynamics | Score-based generative models, stochastic gradient (சாய்வு — மாற்றத்தின் दिशा) Langevin dynamics (SGLD) |
| Stationary distribution | MCMC convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) target, PageRank |
...
where beta_t is a noise schedule. After T steps, x_T is approximately N(0, I). The reverse process is parameter (பாரமீட்டர் — கட்டுப்பாடு)ized by a neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு) that predicts the noise:
...
SGLD (Stochastic gradient (சாய்வு — மாற்றத்தின் दिशा) Langevin Dynamics) combines mini-batch (பத்தி — தொகுப்பு) gradient descent with Langevin noise. Instead of computing the full gradient, you use a stochastic estimate and add calibrated noise. As learning rate decays, SGLD transitions from optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) to sampling -- you get approximate Bayesian posterior samples for free. This is one of the simplest ways to get uncertainty estimates from a neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு).
...
2. **Build a text generator using a Markov chain.** Train on a small corpus: for each word, count transitions to the next word. Build the transition matrix (மேட்ரிக்ஸ் — எண் மேஜை). Generate new sentences by sampling from the chain.

3. **Implement simulated annealing** using Metropolis-Hastings. Start at high temperature (accept almost everything) and gradually cool down (accept only improvements). Use it to find the minimum of a function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) with many local minima.
...
| Transition matrix (மேட்ரிக்ஸ் — எண் மேஜை) | "The probability table" | P[i][j] = probability of moving from state i to state j |
...
| Langevin dynamics | "gradient (சாய்வு — மாற்றத்தின் दिशा) descent with noise" | Update rule that combines deterministic gradient and random perturbation |
| MCMC | "Walking toward the target" | Constructing a Markov chain whose stationary distribution is the one you want |
| Metropolis-Hastings | "Propose and accept/reject" | MCMC algorithm (அல்கோரிதம் — வழிமுறை) that uses acceptance ratios to ensure convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) |
| Temperature | "The randomness knob" | parameter (பாரமீட்டர் — கட்டுப்பாடு) controlling the choice between exploration and exploitation |
...
- **Song & Ermon (2019)** -- "Generative Modeling by Estimating gradient (சாய்வு — மாற்றத்தின் दिशा)s of the Data Distribution." Score-based approach using Langevin dynamics for sampling.
- **Roberts & Rosenthal (2004)** -- "General state space Markov chains and MCMC algorithm (அல்கோரிதம் — வழிமுறை)s." The theory behind when and why MCMC works.
- **Norris (1997)** -- "Markov Chains." The standard textbook. Covers convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்), stationary distributions, and hitting times.
