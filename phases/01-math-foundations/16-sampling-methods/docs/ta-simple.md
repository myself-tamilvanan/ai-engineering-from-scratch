...
- Explain the reparameter (பாரமீட்டர் — கட்டுப்பாடு)ization trick and why it enables backpropagation (பின்னாக பரப்புதல் — தவறிலிருந்து கற்றுக்கொள்ளுதல்) through sampling in VAEs
- Run Metropolis-Hastings MCMC to sample from an unnormalized target distribution

## The Problem

A language model finishes processing your prompt and produces a vector (வெக்டர் — எண் பட்டியல்) of 50,000 logits. One for every token in its vocabulary. Now it has to pick one. How?
...
Sampling is not limited to text generation. Reinforcement learning estimates policy gradient (சாய்வு — மாற்றத்தின் दिशा)s by sampling trajectories. VAEs learn latent representations by sampling from learned distributions and backpropagating through the randomness. Diffusion models generate images by sampling noise and iteratively denoising. Monte Carlo methods estimate integrals that have no closed-form solution. MCMC algorithm (அல்கோரிதம் — வழிமுறை)s explore high-dimension (பரிமாணம் — கோணம்)al posterior distributions that are impossible to enumerate.
...
Sampling appears in four basic roles across AI and machine learning:

**Generation.** Language models, diffusion models, and GANs all produce output by sampling. The sampling algorithm (அல்கோரிதம் — வழிமுறை) directly controls creativity, coherence, and diversity. Temperature, top-k, and nucleus sampling are the knobs that engineers turn daily.

**Training.** Stochastic gradient (சாய்வு — மாற்றத்தின் दिशा) descent samples mini-batch (பத்தி — தொகுப்பு)es. Dropout samples neurons to deactivate. Data augmentation samples random transformations. Importance sampling reweights samples to reduce gradient variance in reinforcement learning (PPO, TRPO).

**Estimation.** Many quantities in ML have no closed-form solution. The expected loss over a data distribution, the partition function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) of an energy-based model, the evidence in Bayesian inference. Monte Carlo estimation approximates all of these by averaging over samples.

**Exploration.** MCMC algorithm (அல்கோரிதம் — வழிமுறை)s explore posterior distributions in Bayesian inference. Evolutionary strategies sample parameter (பாரமீட்டர் — கட்டுப்பாடு) perturbations. Thompson sampling balances exploration and exploitation in bandits.
...
The cumulative distribution function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) (CDF) maps values to probabilities:
...
The tighter the bound M, the higher the acceptance rate. In low dimension (பரிமாணம் — கோணம்)s (1-3), rejection sampling works well. In high dimensions, the acceptance rate drops exponentially because most of the proposal volume gets rejected. This is the curse of dimensionality for rejection sampling.
...
This is critical in reinforcement learning. In PPO (Proximal Policy optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)), you collect trajectories under an old policy pi_old but want to optimize a new policy pi_new. The importance weight is pi_new(a|s) / pi_old(a|s). PPO clips these weights to prevent the new policy from diverging too far from the old one.
...
Monte Carlo estimation approximates integrals by averaging random samples. The law of large numbers guarantees convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்).
...
Error: O(1 / sqrt(N))   regardless of dimension (பரிமாணம் — கோணம்)
```

The error rate is dimension (பரிமாணம் — கோணம்)-independent. This is why Monte Carlo methods dominate in high dimensions where grid-based integration is impossible.
...
Metropolis-Hastings algorithm (அல்கோரிதம் — வழிமுறை):
  1. Start at some x_0
  2. For t = 1, 2, ..., T:
     a. Propose x' ~ q(x'|x_t)
...
For symmetric proposals (q(x'|x) = q(x|x')), the ratio simplifies to p(x')/p(x). This is the original Metropolis algorithm (அல்கோரிதம் — வழிமுறை).
...
- The optimal acceptance rate for a Gaussian proposal in high dimension (பரிமாணம் — கோணம்)s is approximately 0.234

### Gibbs Sampling

Gibbs sampling is a special case of MCMC for multivariate distributions. Instead of proposing a move in all dimension (பரிமாணம் — கோணம்)s at once, it updates one variable at a time from its conditional distribution.

```
Target: p(x_1, x_2, ..., x_d)

algorithm (அல்கோரிதம் — வழிமுறை):
  For each iteration (மறுயுதல் — மீண்டும் செய்தல்) t:
...
T < 1.0: sharpens the distribution (more confident, less many different)
T > 1.0: flattens the distribution (less confident, more many different)
```
...
Top-k and top-p is combined. Apply top-k first, then top-p on the remaining set.

### Reparameter (பாரமீட்டர் — கட்டுப்பாடு)ization Trick (Used in VAEs)
...
Standard sampling (not differentiable):
  ...
  The randomness blocks gradient (சாய்வு — மாற்றத்தின் दिशा) flow.
  d/d_mu [sample from N(mu, sigma^2)] = ???
```

The reparameter (பாரமீட்டர் — கட்டுப்பாடு)ization trick separates the randomness from the parameters:

```
Reparameter (பாரமீட்டர் — கட்டுப்பாடு)ized sampling:
  epsilon ~ N(0, 1)          (fixed random noise, no parameters)
  z = mu + sigma * epsilon   (deterministic function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) of parameters)

  Now z is a deterministic, differentiable function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) of mu and sigma.
  d(z)/d(mu) = 1
  d(z)/d(sigma) = epsilon

  gradient (சாய்வு — மாற்றத்தின் दिशा)s flow through mu and sigma.
```

This works because N(mu, sigma^2) has the same distribution as mu + sigma * N(0, 1). The key insight: move the randomness to a parameter (பாரமீட்டர் — கட்டுப்பாடு)-free source (epsilon), then express the sample as a differentiable transformation of the parameters.
...
Without the reparameter (பாரமீட்டர் — கட்டுப்பாடு)ization trick, VAEs cannot be trained with standard backpropagation (பின்னாக பரப்புதல் — தவறிலிருந்து கற்றுக்கொள்ளுதல்). This single insight made VAEs practical.

### Gumbel-Softmax (Differentiable Categorical Sampling)

The reparameter (பாரமீட்டர் — கட்டுப்பாடு)ization trick works for continuous distributions (Gaussian). For discrete categorical distributions, we need a different approach. Gumbel-Softmax provides a differentiable approximation to categorical sampling.
...
tau (temperature) controls the approximation:
  tau -> 0:  approaches a one-hot vector (வெக்டர் — எண் பட்டியல்) (hard categorical)
  tau -> inf: approaches uniform (1/k, 1/k, ..., 1/k)
  tau = 1.0: soft approximation
```

Gumbel-Softmax produces a continuous relaxation of a discrete sample. The output is a probability vector (வெக்டர் — எண் பட்டியல்) (soft one-hot) instead of a hard one-hot. gradient (சாய்வு — மாற்றத்தின் दिशा)s flow through the softmax. During the forward pass in training, you can use the "straight-through" estimator: use the hard argmax for the forward pass but the soft Gumbel-Softmax gradients for the backward pass.
...
For piecewise-constant function (சார்பு — உள்ளீட்டுக்கு வெளியீடு)s, stratified sampling is exact.
```
...
- Each denoising step uses the reparameter (பாரமீட்டர் — கட்டுப்பாடு)ization trick (sample noise, apply deterministic transform)
...
### Step 9: Reparameter (பாரமீட்டர் — கட்டுப்பாடு)ization trick
...
def reparam_gradient (சாய்வு — மாற்றத்தின் दिशा)(mu, sigma, epsilon):
    dz_dmu = 1.0
    dz_dsigma = epsilon
    return dz_dmu, dz_dsigma
```

Demonstrate that gradient (சாய்வு — மாற்றத்தின் दिशा)s flow through the reparameter (பாரமீட்டர் — கட்டுப்பாடு)ized sample but not through direct sampling.
...
Show how decreasing temperature makes the output approach a one-hot vector (வெக்டர் — எண் பட்டியல்).
...
| Monte Carlo | "Average random samples" | Approximate integrals as sample averages. Error O(1/sqrt(N)) regardless of dimension (பரிமாணம் — கோணம்) |
| MCMC | "Random walk that converges" | Construct a Markov chain whose stationary distribution is the target. Metropolis-Hastings is the foundational algorithm (அல்கோரிதம் — வழிமுறை) |
| Metropolis-Hastings | "Accept uphill, sometimes downhill" | Propose moves, accept based on density ratio. Detailed balance ensures convergence (ஒருங்கிணைப்பு — ஒரு பதிலில் சேர்ந்துவருதல்) to target distribution |
...
| Temperature | "Confidence knob" | Divides logits by T before softmax. T<1 sharpens (more confident), T>1 flattens (more many different) |
...
| Reparameter (பாரமீட்டர் — கட்டுப்பாடு)ization trick | "Move randomness outside" | Write z = mu + sigma * epsilon where epsilon ~ N(0,1). Makes sampling differentiable. Essential for VAE training |
...
- [Holbrook (2023): The Metropolis-Hastings algorithm (அல்கோரிதம் — வழிமுறை)](https://arxiv.org/abs/2304.07010) - detailed tutorial on MCMC foundations
- [Jang, Gu, Poole (2017): Categorical Reparameter (பாரமீட்டர் — கட்டுப்பாடு)ization with Gumbel-Softmax](https://arxiv.org/abs/1611.01144) - original Gumbel-Softmax paper
