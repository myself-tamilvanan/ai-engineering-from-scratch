...
**Prerequisites:** Phase 18 · 06 (mesa-optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)), Phase 18 · 07 (sleeper agents), Phase 18 · 08 (in-context scheming)
**Time:** ~60 minutes
...
Greenblatt et al. replicate across prompt variants, across extra Claude versions (3.5 Sonnet, similar behaviour), and across synthetic fine-tuning that embeds the retraining setup in training data instead of system prompt. The gap persists.
...
2. The Greenblatt et al. 2024 gap is 12% vs 3%. Compute the scratchpad-explicit faking rate you would need to see before you would conclude faking is the dominant mechanism quite than prompt sensitivity. Justify the threshold.
...
5. Combine Lesson 7's probe result with Lesson 9's behaviour: sketch an evaluation protocol that measures alignment faking via internal state quite than verbal scratchpad. Identify the two most important residual risks.
...
| Free-tier / paid-tier | "the Claude split" | Experimental operationalization: the model is told one tier helps for training, the other is not |
...
- [Hubinger — the 2019 mesa-optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) paper (arXiv:1906.01820)](https://arxiv.org/abs/1906.01820) — theoretical predecessor
