...
- Describe Constitutional Classifiers and the drop from 23.7% compute extra work (v1) to ~1% (v2 / 2026).
...
The catch: the preference signal is now generated by the same class of model you are training. Biases in the labeler (now: in the principles plus the labeler model's interpretation) is amplified quite than attenuated. Lesson 4's sycophancy argument still applies. the labeler just moved inside the loop.
...
A parallel line of work: quite than change the model's post-training, train lightweight classifiers that read the constitution and gate model outputs. v1 (2023) had 23.7% compute extra work. v2 (2026) is ~1% and has the lowest successful attack rate of any Anthropic defense Anthropic has tested publicly. No universal jailbreak was reported as of early 2026.
...
`code/main.py` simulates the CAI critique-and-revise loop on a toy lexicon. A "principle" flags tokens from a harmful set. Given an initial response, the critique identifies the harmful tokens, and the revision replaces them. After 200 iteration (மறுயுதல் — மீண்டும் செய்தல்)s the "trained" model has internalized the revision rule. Compare the base model, RLHF-shaped toy, and CAI-shaped toy on a held-out prompt set.
...
5. Read Constitutional Classifiers v2 methodology (if available). Explain why ~1% compute extra work is a qualitatively different safety story than 23.7%.
...
- [Anthropic — Constitutional Classifiers (2024-2026)](https://www.anthropic.com/research/constitutional-classifiers) — output-gate defense with ~1% extra work in v2
