...
`code/main.py` simulates the four protocols on a toy backdoor task. U produces outputs. a fraction are backdoored (red-team policy). T approves or edits. H is called on a budget. You can sweep the budget and the collusion parameter (பாரமீட்டர் — கட்டுப்பாடு) to reproduce the Pareto frontier in Greenblatt et al. 2024 Figure 2 (shape, not numbers).

## Ship It

This lesson produces `outputs/skill-control-protocol-audit.md`. Given a deployment protocol description, it identifies the U / T / H roles, names the collusion vector (வெக்டர் — எண் பட்டியல்)s, and estimates the protocol's position on the usefulness-safety frontier under red-team pressure.
