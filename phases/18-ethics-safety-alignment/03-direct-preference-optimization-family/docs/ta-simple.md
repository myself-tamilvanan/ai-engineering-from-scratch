...
# The Direct Preference optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) Family

> Rafailov et al. (2023) showed RLHF's optimum has a closed form in terms of the preference data, so you can skip the explicit reward model and optimize the policy directly. That insight spawned a family — IPO, KTO, SimPO, ORPO, BPO — each fixing a failure mode of DPO. In 2026, direct alignment algorithm (அல்கோரிதம் — வழிமுறை)s ship more frontier post-training runs than PPO. But the over-optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) curve from Lesson 2 still applies: DAAs do not escape Goodhart, they just move where it bites.
...
Substitute this into the Bradley-Terry preference likelihood and the partition function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) `Z(x)` cancels because it depends only on `x`. What remains is a loss in the policy parameter (பாரமீட்டர் — கட்டுப்பாடு)s alone — no reward model needed. That is DPO.
...
Identity Preference optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) replaces the log-sigmoid with an identity mapping on the preference probability. The loss becomes a squared-error on a bounded target:
...
Kahneman-Tversky optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) drops pairwise structure entirely. Given a single labeled output and a binary "desirable" or "undesirable" signal, it maps to a prospect-theory utility:
...
Simple Preference optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) aligns the training signal with generation. Remove the reference policy entirely and normalize log-likelihood by length:
...
Odds-Ratio Preference optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) adds a preference term to the standard SFT negative log-likelihood:
...
Rafailov et al. "Scaling Laws for Reward Model Overoptimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) in Direct Alignment algorithm (அல்கோரிதம் — வழிமுறை)s" (NeurIPS 2024) trained policies with DPO, IPO, SLiC on multiple datasets across KL budgets. The gold-reward-vs-KL curves have the same Gao et al. peak-and-collapse shape. The implicit reward queries out-of-distribution samples during training. KL regularization does not stabilize this.
...
4. Rafailov et al. (NeurIPS 2024) claim DAAs over-optimize. Reproduce a single-point version: plot chosen-minus-rejected KL divergence and observe over-optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) in DPO at large beta.

5. Read the BPO paper abstract (OpenReview b97EwMUWu7). Write down the one-line correction BPO adds to DPO. Confirm against the how to build in `code/main.py`.

## Key Terms

| Term | What people say | What it actually means |
|------|-----------------|------------------------|
| DPO | "RLHF without a reward model" | Loss derived from the closed-form RLHF optimum. policy parameter (பாரமீட்டர் — கட்டுப்பாடு)s only |
...
| DAA | "direct alignment algorithm (அல்கோரிதம் — வழிமுறை)" | Any preference-loss method that skips an explicit RM |

## Further Reading

- [Rafailov et al. — Direct Preference optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) (NeurIPS 2023, arXiv:2305.18290)](https://arxiv.org/abs/2305.18290)
