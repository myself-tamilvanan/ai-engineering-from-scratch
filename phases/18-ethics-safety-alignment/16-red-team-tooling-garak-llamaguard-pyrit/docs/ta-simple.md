...
Llama Guard 3 is a Llama-3.1-8B model fine-tuned for input/output classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்) over the MLCommons AILuminate 14 categories:
...
Put Llama Guard on both sides of the model. Run Garak nightly for regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்). Run PyRIT for pre-release campaigns. This is the 2026 default configuration for most production deployments.
...
`code/main.py` builds a toy Llama Guard-style classifier (keyword + semantic feature (பண்பு — தகவல் நெடுவரிசை)s over 14 categories), a toy Garak harness (probe-detector loop), and a PyRIT-style multi-turn converter chain. You can run the three tools against a mock target and observe the different coverage signatures.

## Ship It

This lesson produces `outputs/skill-red-team-stack.md`. Given a deployment description, it names which of the three tools are appropriate, what to configure in each, and what regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) cadence to run.
