...
- Describe two mechanistic-interpretability approaches to bias (gender neurons, SAE feature (பண்பு — தகவல் நெடுவரிசை)s, attention-head manipulation).

## The Problem

The previous lessons cover deliberate harm (jailbreaks, scheming) and safety governance. Bias is harm that emerges without intent — from training data distributions, from prompt framing, from accumulated design choices. Measuring and reducing it is a distinct methodological challenge from adversarial strength.
...
These are not the same. A model is "representationally unbiased" (produces many different portrayals) while being "allocationally biased" (makes unequal recommendations). Evaluations need to measure both.

### Three evaluation-metric categories (Gallegos et al. 2024)

- **embedding (embedding — பொருளை எண்களாக மாற்றுதல்)-based.** WEAT-style tests on pre-RLHF embeddings. Measures statistical associations between identity terms and attribute terms. Limited: measures the representation, not the behaviour.
...
- **Clinical racial bias via SAEs (Ahsan & Wallace 2025).** Sparse autoencoder feature (பண்பு — தகவல் நெடுவரிசை)s decompose the internal representation into interpretable dimension (பரிமாணம் — கோணம்)s. race-correlated features is identified and suppressed.
...
`code/main.py` builds a toy embedding (embedding — பொருளை எண்களாக மாற்றுதல்)-based bias probe: measure WEAT-style distance between identity terms and attribute terms in a simple co-occurrence embedding. You can inject a bias and observe the metric fire. apply a simple debiasing operation and observe partial recovery.

## Ship It

This lesson produces `outputs/skill-bias-eval.md`. Given a model card or fairness claim, it audits the evaluation across the three metric categories (embedding (embedding — பொருளை எண்களாக மாற்றுதல்), probability, generated-text), the intersectionality coverage, and the mechanism of any debiasing intervention.
...
| WEAT | "the embedding (embedding — பொருளை எண்களாக மாற்றுதல்) test" | Word Embedding Association Test. co-occurrence-based bias probe |
...
| SAE feature (பண்பு — தகவல் நெடுவரிசை) | "interpretable dimension (பரிமாணம் — கோணம்)" | Sparse-autoencoder-identified feature. useful for mechanistic bias analysis |
