...
- Orchestrate multi-service AI applications (inference server + vector (வெக்டர் — எண் பட்டியல்) database) using Docker Compose
...
2. **Model weights are large.** A 7B parameter (பாரமீட்டர் — கட்டுப்பாடு) model is 14 GB in fp16. You do not want to re-download it every time you rebuild. Docker volumes let you mount a models directory from the host.

3. **Multi-service architectures are common.** A real AI application is not just a Python script. It is an inference server, a vector (வெக்டர் — எண் பட்டியல்) database for RAG, maybe a web frontend. Docker Compose orchestrates all of these with one command.
...
RUN python -m pip install --no-cache-dir \
    ...
    datasets \
    accelerate \
    safetensor (டென்சர் — பல வடிவ எண் வரிசை)s
...
A real RAG application needs an inference server and a vector (வெக்டர் — எண் பட்டியல்) database. Docker Compose runs both with one command.
...
Now your AI dev container can reach the vector (வெக்டர் — எண் பட்டியல்) database at `http://qdrant:6333` by service name. Docker Compose creates a shared network automatically.
...
- Use `docker compose up` to start your dev environment and vector (வெக்டர் — எண் பட்டியல்) database together
