...
- Verify GPU access with CUDA/MPS and run a test tensor (டென்சர் — பல வடிவ எண் வரிசை) operation
...
a = np.array([1, 2, 3])
print(f"vector (வெக்டர் — எண் பட்டியல்): {a}, dot product with itself: {np.dot(a, a)}")
```
