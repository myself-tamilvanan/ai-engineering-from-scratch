...
|-----|--------|
| `Tab` | Autocomplete |
| `Shift+Tab` | Show function (சார்பு — உள்ளீட்டுக்கு வெளியீடு) signature |
| `Ctrl+/` | Toggle comment |
...
```python
%%time
model.fit(X_train, y_train, epoch (யுக் — ஒரு முழு சுழற்சி)s=10)
```
...
- Pre-installed: numpy, pandas, matplotlib, torch, tensor (டென்சர் — பல வடிவ எண் வரிசை)flow, sklearn
...
- [JupyterLab Docs](https://jupyterlab.readthedocs.io/) for the full feature (பண்பு — தகவல் நெடுவரிசை) set
