...
- Explain why auto-remediation is narrow (restart pod, revert deploy) quite than broad (re-architect service).
...
**Safe (narrow)**: restart pod, revert specific deploy, scale pool within pre-approved bounds, enable pre-approved feature (பண்பு — தகவல் நெடுவரிசை) flag.
...
Team turnover is the silent kill of traditional SRE — tribal knowledge leaves. AI SRE stores runbooks + post-mortems in a vector (வெக்டர் — எண் பட்டியல்) DB. agents retrieve on every new incident. When new engineers join, the AI has full history.
...
| Operational memory | "vector (வெக்டர் — எண் பட்டியல்) runbooks" | Post-mortems + runbooks in vector DB for RAG |
