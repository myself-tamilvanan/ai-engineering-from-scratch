...
- Enumerate the six core gateway feature (பண்பு — தகவல் நெடுவரிசை)s (routing, fallback, retries, rate limits, secrets, observability, guardrails).
...
Reinventing this at the app layer couples every service to every provider. A gateway layer consolidates it into one process with one API (usually OpenAI-compatible) that fans out to providers.

## The Concept

### Six core feature (பண்பு — தகவல் நெடுவரிசை)s
...
- 20-40 ms per-request latency extra work.
...
Data residency is the forcing function (சார்பு — உள்ளீட்டுக்கு வெளியீடு). Healthcare and finance default self-host (LiteLLM or Portkey OSS or Kong). Consumer products default managed (Cloudflare AI Gateway) or middle-tier (Portkey managed). Hybrid: self-hosted for regulated tenant, managed for others.

### Latency budget

- LiteLLM: 5-15 ms extra work typical.
- Portkey: 20-40 ms extra work.
- Kong: 3-8 ms extra work.
- Cloudflare/Vercel: 1-3 ms extra work (edge advantage).
...
- Portkey: 20-40 ms extra work. Apache 2.0 since March 2026.
- Kong: 228% faster than Portkey, 859% faster than LiteLLM.
- Kong pricing: $100/model/month, 5 max on Plus tier.
- Cloudflare/Vercel: 1-3 ms extra work at the edge.
