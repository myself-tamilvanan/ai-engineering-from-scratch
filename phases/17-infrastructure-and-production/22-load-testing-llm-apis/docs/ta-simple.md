...
- Build a realistic prompt distribution using mean + stddev of input tokens quite than fixed length.
...
All known load testers let you configure one prompt. In a loop test of 10,000 iteration (மறுயுதல் — மீண்டும் செய்தல்)s the exact same prompt sends each time. Server sees the same prefix every time — prefix cache hits approach 100%, throughput looks great.

Fix: sample from a prompt distribution. LLMPerf uses `--mean-input-tokens 500 --stddev-input-tokens 150` — many different lengths, many different content.

### Four load patterns

1. **Steady-state** — constant RPS for 30-60 min. Catches: baseline performance regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)s.
...
- 30-50 iteration (மறுயுதல் — மீண்டும் செய்தல்)s each at baseline RPS.
...
- Typical CI gate: 30-50 iteration (மறுயுதல் — மீண்டும் செய்தல்)s per PR.
- Four patterns: steady, ramp, spike, soak.

## Use It

`code/main.py` simulates a load test with realistic prompt distribution, measures effective TPOT, and shows the uniform-prompt trap.
...
| GIL trap | "Python client extra work" | Tokenization backlog inflates reported latency |
