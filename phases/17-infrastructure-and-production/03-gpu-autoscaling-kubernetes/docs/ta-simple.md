...
**Topology awareness** — know which GPUs share NVLink, which sit on the same rack, which have InfiniBand between them. Place pods accordingly. A DeepSeek-V3 67B tensor (டென்சர் — பல வடிவ எண் வரிசை)-parallel workload must stay on one NVLink domain. KAI Scheduler respects that.
