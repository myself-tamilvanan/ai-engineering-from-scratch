...
Before calling the LLM at all, hash the prompt, embed it, and look for a similar cached request (cosine similarity above threshold, usually 0.95+). On hit, return the cached response. On miss, call LLM and cache the result.

Open-source: Redis vector (வெக்டர் — எண் பட்டியல்) Similarity, GPTCache, Qdrant. Commercial: Portkey Cache, Helicone Cache.
...
Fix: batch (பத்தி — தொகுப்பு) with sequential-first — make request 1 alone, then fire 2-10 once 1's cache has populated. Adds 300 ms to the first tool call. saves 5-10x the bill.
...
### Stack batch (பத்தி — தொகுப்பு) + cache for overnight workloads

batch (பத்தி — தொகுப்பு) APIs (Phase 17 · 15) give 50% discount at 24-hour turnaround. Cached input on top gets you ~10x on top of that. Overnight classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்), labeling, and report generation workloads can drop to ~10% of synchronous-uncached cost by stacking.
...
5. You batch (பத்தி — தொகுப்பு) 10 parallel sub-queries per user question. Rewrite for cache-friendliness without adding end-to-end latency.
...
| L1 semantic cache | "embedding (embedding — பொருளை எண்களாக மாற்றுதல்) cache" | App-level hash-and-embed before calling LLM |
...
| RadixAttention | "intra-replica cache" | SGLang's prefix-cache how to build |
