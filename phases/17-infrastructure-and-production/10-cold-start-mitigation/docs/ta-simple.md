...
Simplest mitigation: keep one replica always ready. Cost is one GPU's hourly rate 24x7. The arithmetic is brutal on small models (you pay $0.85-$1.50/hr to avoid a 30s cold start) and kind to large ones (pay $4/hr to avoid a 5-minute cold start). The SLA threshold where warm pools become mandatory: usually TTFT P99 < 60s on a 70B+ model.
...
- Background batch (பத்தி — தொகுப்பு) paths (nightly classification (வகைப்படுத்தல் — பிரிவுகளாக தொகுத்தல்)): scale-to-zero accepted, 5-10 minute cold start tolerable.
...
5. Design a tiered warm-pool policy: how many warm replicas for paid users, trial users, and batch (பத்தி — தொகுப்பு) workloads? Show the math.
