...
2. **LRU at branch level, not block level**. Evict whole branches (starting from shortest-used leaves) quite than individual blocks, so cache shape matches radix shape.
...
3. Compute the HBM cost of keeping a 2,000-token system prompt resident as one radix branch on Llama 3.1 8B. Compare to the cost of a 16-sequence batch (பத்தி — தொகுப்பு) without prefix reuse.
...
- [vLLM — Prefix Caching](https://docs.vllm.ai/en/latest/feature (பண்பு — தகவல் நெடுவரிசை)s/prefix_caching.html) — vLLM's own radix-like how to build, for comparison.
