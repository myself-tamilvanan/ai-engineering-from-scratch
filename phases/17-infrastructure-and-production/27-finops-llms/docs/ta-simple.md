...
- Explain why traditional FinOps (tags + tiers) breaks on LLM spend and name the three new attribution dimension (பரிமாணம் — கோணம்)s.
...
- Which product feature (பண்பு — தகவல் நெடுவரிசை) drove it.
...
### Three attribution dimension (பரிமாணம் — கோணம்)s

**Per-user** (`user_id`): who is costing what. Drives seat pricing, expansion conversations, identifies power users.

**Per-task** (`task_id` + `route`): which product surface costs what. Drives feature (பண்பு — தகவல் நெடுவரிசை) prioritization, kill-expensive-features decisions.
...
Bucketing all four together makes optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) blind. Break them out in your attribution schema.
...
- **Model-based allocation**: regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்) to infer cost driver. For legacy data without tags.
...
Tie cost to a product outcome. Otherwise optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) is unanchored.
...
trace_id: abc123
  ...
  cost_usd: 0.0135
  cached_input: true
  batch (பத்தி — தொகுப்பு): false
```

Emit on every call. Store in data lake. Aggregate per dimension (பரிமாணம் — கோணம்). Phase 17 · 13 observability stack is where this lives.

### The compounded-savings stack

Stack: cache + batch (பத்தி — தொகுப்பு) + route + gateway. With all four:
- Cache L2 (Phase 17 · 14): ~10x cheaper input.
- Batch (Phase 17 · 15): 50% off.
- Route to cheap model (Phase 17 · 16): 60% cost reduction.
- Gateway speed (Phase 17 · 19): redundancy + retries.

Best-case stacked: ~5-10% of naive baseline. Most teams have 2-3 levers engaged. few stack all four.

### Numbers you should remember

- Attribution dimension (பரிமாணம் — கோணம்)s: per-user, per-task, per-tenant.
- Four token layers: prompt, tool, memory, response.
- Kill switch: spend z-score > 4.
- Unit metric: cost per resolved query, not $/M tokens.
- Stacked optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)s: ~5-10% of baseline possible.

## Use It

`code/main.py` simulates a multi-tenant LLM service with the three-tier enforcement ladder. Injects an abusive tenant and shows the kill switch firing.
...
| Per-task attribution | "feature (பண்பு — தகவல் நெடுவரிசை) cost" | `task_id` + `route` identify product surface |
...
| Stacked optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்) | "cache+batch (பத்தி — தொகுப்பு)+route+gateway" | Compounding savings to ~5-10% baseline |
