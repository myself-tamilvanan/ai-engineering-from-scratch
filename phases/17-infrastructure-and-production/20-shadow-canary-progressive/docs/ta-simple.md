...
Catches: cost blow-ups, length regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)s, obvious refusal changes, hard errors. Does NOT catch: quality delta users would perceive. Shadow is a smoke test, not a quality test.
...
- GPU FP non-associativity (floating-point reduction order varies by batch (பத்தி — தொகுப்பு)).
...
A 20% better model is 3x more expensive per call. Cost/request is one of the five gates. Shipping a "better" model that breaks unit economics is a rollback case.

### Rollback is the weapon

- Policy flag (feature (பண்பு — தகவல் நெடுவரிசை) flag system): flip percentage in config. takes seconds.
...
**feature (பண்பு — தகவல் நெடுவரிசை) flags** — LaunchDarkly, Flagsmith, Unleash. Policy-level flip, no redeploy.
...
`code/main.py` simulates a canary rollout with injected regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்)s. Reports which stage the rollout halts at and which gate triggered.
...
1. Run `code/main.py`. Inject a 25% cost regression (பின்னடைவு — எண் தொடர்பு கண்டுபிடித்தல்). At which stage does the canary halt?
...
- [TianPan — Releasing AI feature (பண்பு — தகவல் நெடுவரிசை)s Without Breaking Production](https://tianpan.co/blog/2026-04-09-llm-gradual-rollout-shadow-canary-ab-testing)
...
- [APXML — Advanced LLM Deployment Patterns](https://apxml.com/courses/mlops-for-large-models-llmops/chapter-4-llm-deployment-serving-optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)/advanced-llm-deployment-patterns)
