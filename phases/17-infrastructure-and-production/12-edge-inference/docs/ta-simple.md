...
> The core edge constraint is memory bandwidth, not compute. Mobile DRAM sits at 50-90 GB/s. datacenter HBM3 clears 2-3 TB/s — a 30-50x gap. Decode is memory-bound so the gap is decisive. In 2026 the landscape splits four ways. Apple M4/A18 Neural Engine peaks at 38 TOPS with unified memory (no CPU↔NPU copy). Qualcomm Snapdragon X Elite / 8 Gen 4 Hexagon hits 45 TOPS. WebGPU + WebLLM runs Llama 3.1 8B (Q4) at ~41 tok/s on M3 Max (roughly 70-80% of native). 17.6k GitHub stars, OpenAI-compatible API, ~70-75% mobile coverage. NVIDIA Jetson Orin Nano Super (8GB) fits Llama 3.2 3B / Phi-3. AGX Orin runs gpt-oss-20b via vLLM at ~40 tok/s. Jetson T4000 (JetPack 7.1) is 2x AGX Orin. tensor (டென்சர் — பல வடிவ எண் வரிசை)RT Edge-LLM supports EAGLE-3, NVFP4, chunked prefill — shown at CES 2026 by Bosch, ThunderSoft, MediaTek.
...
- Up to 38 TOPS. Unified memory (CPU and ANE share the same pool) — no copy extra work.
...
- QNN (Qualcomm neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு)) SDK and AI Hub provide conversion from PyTorch/ONNX.
...
- tensor (டென்சர் — பல வடிவ எண் வரிசை)RT Edge-LLM (2026) supports EAGLE-3 speculative decoding, NVFP4 weights, chunked prefill — the datacenter optimization (உகந்தாக்கம் — சிறந்ததாக்குதல்)s ported to edge.
...
Llama 3.1's 128K context is a datacenter feature (பண்பு — தகவல் நெடுவரிசை). On a phone with 8 GB RAM, 4 GB model + 2 GB KV cache for 32K tokens + OS extra work = OOM. Edge deployments keep context at 4K-8K unless aggressive KV quantization (Q4 KV) is accepted.
...
| TRT Edge-LLM | "edge tensor (டென்சர் — பல வடிவ எண் வரிசை)RT" | 2026 edge port of TensorRT-LLM. EAGLE-3 + NVFP4 |
| Unified memory | "shared pool" | CPU and NPU see same RAM. no copy extra work |
...
| QNN | "Qualcomm stack" | Qualcomm neural network (வலைப்பின்னல் — மூளை போன்ற எண் அமைப்பு) SDK |
...
- [NVIDIA tensor (டென்சர் — பல வடிவ எண் வரிசை)RT Edge-LLM](https://developer.nvidia.com/blog/accelerating-llm-and-vlm-inference-for-automotive-and-robotics-with-nvidia-tensorrt-edge-llm/) — 2026 edge port announcement.
