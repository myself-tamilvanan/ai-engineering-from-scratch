...
- Name the Blackwell-specific feature (பண்பு — தகவல் நெடுவரிசை)s TRT-LLM exploits (day-0 FP4, MTP, disaggregated serving, all-to-all primitives).
...
You cannot replicate this outside NVIDIA's stack. That is the choice — portability for economics. Understanding which stack choices give which share of the gap is the point of this lesson.
...
NVFP4 (2025-2026) applies to weights and activations. Microscaling: each block of weights has its own scale factor so small blocks can span different dynamic ranges without per-tensor (டென்சர் — பல வடிவ எண் வரிசை) scale loss. For activations, FP4 holds up because activations are small-range within a layer.
...
- **NVFP4 + MXFP8 microscaling**: hardware-accelerated scale-factor handling on Blackwell tensor (டென்சர் — பல வடிவ எண் வரிசை) Cores.
...
For a $100M+ annual inference bill, running on Hopper + vLLM leaves 7-10x on the table. Migrate cost-dominant workloads to Blackwell + TRT-LLM + Dynamo. Keep experimentation tier on H100 + vLLM for model iteration (மறுயுதல் — மீண்டும் செய்தல்) speed. Validate quality on each NVFP4-converted model before production.
...
1. Run `code/main.py`. On a 120B MoE with 30% active parameter (பாரமீட்டர் — கட்டுப்பாடு)s, compute the memory-bandwidth-limited decode throughput on H100 BF16, H100 FP8, and B200 NVFP4/FP8. Where does the biggest jump come from?
...
| MXFP8 | "MX eight" | Microscaling FP8 variant. hardware-accelerated on Blackwell tensor (டென்சர் — பல வடிவ எண் வரிசை) Cores |
...
- [tensor (டென்சர் — பல வடிவ எண் வரிசை)RT-LLM Overview](https://nvidia.github.io/TensorRT-LLM/overview.html) — official engine documentation.
